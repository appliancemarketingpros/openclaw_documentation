---
title: Skills(macOS)
source_url: https://docs.openclaw.ai/ko/platforms/mac/skills
...
## 데이터 소스
  ...
  * `skills.status`(gateway)는 모든 Skills와 함께 적격성 및 누락된 요구 사항을 반환합니다. (번들된 Skills에 대한 allowlist 차단 포함)
...
## 설치 작업
  ...
  * 모든 설치 옵션이 `download`이면 gateway가 모든 다운로드 선택지를 표시합니다.
  * 그렇지 않으면 gateway는 현재 설치 환경 설정과 호스트 바이너리를 사용해 선호 설치 프로그램 하나를 선택합니다. `skills.install.preferBrew`가 활성화되어 있고 `brew`가 있으면 Homebrew를 우선하고, 그다음 `uv`, 그다음 `skills.install.nodeManager`에 구성된 node 관리자, 이후 `go` 또는 `download` 같은 대체 수단을 사용합니다.
