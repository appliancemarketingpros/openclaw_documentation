---
title: پروتکل پل
source_url: https://docs.openclaw.ai/fa/gateway/bridge-protocol
...
Nodeها می‌توانند رویدادهای `exec.finished` یا `exec.denied` را منتشر کنند تا فعالیت system.run را نمایان کنند. این‌ها در Gateway به رویدادهای سیستم نگاشت می‌شوند. (Nodeهای قدیمی ممکن است همچنان `exec.started` را منتشر کنند.)
...
## استفادهٔ تاریخی از tailnet
  ...
  * پل را به یک IP مربوط به tailnet متصل کنید: `bridge.bind: "tailnet"` در `~/.openclaw/openclaw.json` (فقط تاریخی؛ `bridge.*` دیگر معتبر نیست).
