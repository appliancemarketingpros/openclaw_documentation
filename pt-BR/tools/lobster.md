---
title: Lagosta
source_url: https://docs.openclaw.ai/pt-BR/tools/lobster
...
O OpenClaw executa fluxos de trabalho Lobster **in-process** usando um executor incorporado. Nenhum subprocesso de CLI externo é iniciado; o mecanismo de fluxo de trabalho executa dentro do processo do Gateway e retorna um envelope JSON diretamente. Se o pipeline pausar para aprovação, a ferramenta retorna um `resumeToken` para que você possa continuar depois.
...
Para fluxos de trabalho que precisam de uma **etapa LLM estruturada** , habilite a ferramenta opcional do plugin `llm-task` e chame-a a partir do Lobster. Isso mantém o fluxo de trabalho determinístico, ao mesmo tempo em que permite classificar/resumir/rascunhar com um modelo.
