...
source_url: https://docs.openclaw.ai/it/gateway/openresponses-http-api
...
Sotto il cofano, le richieste vengono eseguite come una normale esecuzione agente del Gateway (lo stesso percorso di codice di `openclaw agent`), quindi routing/permessi/configurazione corrispondono al tuo Gateway.
...
Matrice di autenticazione:
  ...
  * `gateway.auth.mode="token"` o `"password"` \+ `Authorization: Bearer ...`
    ...
    * ripristina il set completo di ambiti operatore predefinito: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
Se l'agente decide di chiamare uno strumento, la risposta restituisce un elemento di output `function_call`. Poi invii una richiesta di follow-up con `function_call_output` per continuare il turno.
...
Tipi MIME consentiti (attuali): `image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/heic`, `image/heif`. Dimensione massima (attuale): 10MB.
...
Tipi MIME consentiti (attuali): `text/plain`, `text/markdown`, `text/html`, `text/csv`, `application/json`, `application/pdf`.

Dimensione massima (attuale): 5MB.

Comportamento attuale:

  * Il contenuto del file viene decodificato e aggiunto al **prompt di sistema** , non al messaggio utente, quindi rimane effimero (non persistito nella cronologia della sessione).
  * Il testo del file decodificato viene avvolto come **contenuto esterno non attendibile** prima di essere aggiunto, quindi i byte del file sono trattati come dati, non come istruzioni attendibili.
  ...
  * Questo percorso di input file omette intenzionalmente il lungo banner `SECURITY NOTICE:` per preservare il budget del prompt; i marcatori di confine e i metadati restano comunque presenti.
  * I PDF vengono prima analizzati per estrarre testo. Se viene trovato poco testo, le prime pagine vengono rasterizzate in immagini e passate al modello, e il blocco file iniettato usa il segnaposto `[PDF content rendered to images]`.


L'analisi dei PDF è fornita dal Plugin `document-extract` in bundle, che usa la build legacy di `pdfjs-dist` compatibile con Node (senza worker). La build moderna di PDF.js si aspetta worker del browser/globali DOM, quindi non viene usata nel Gateway.
...
Nota di sicurezza:
  ...
  * Per gateway esposti a Internet, applica controlli di uscita di rete oltre alle protezioni a livello applicazione. Vedi [Sicurezza](</it/gateway/security>).
...
`usage` viene popolato quando il provider sottostante riporta i conteggi dei token. OpenClaw normalizza gli alias comuni in stile OpenAI prima che quei contatori raggiungano le superfici downstream di stato/sessione, inclusi `input_tokens` / `output_tokens` e `prompt_tokens` / `completion_tokens`.
