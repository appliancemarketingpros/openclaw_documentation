...
source_url: https://docs.openclaw.ai/ru/tools/exec
...
Агенты в песочнице могут требовать одобрение для каждого запроса перед запуском `exec` на Gateway или хосте Node. См. [Одобрения exec](</ru/tools/exec-approvals>) для политики, списка разрешений и UI-потока.
