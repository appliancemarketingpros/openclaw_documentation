...
Write prompt text for the person who will approve the action:
  ...
  * Keep `title` short and action-focused. The Gateway accepts up to 80 characters.
  * Keep `description` specific and bounded. The Gateway accepts up to 256 characters.
  ...
  * Use `severity: "critical"` only for actions where the wrong decision could cause production damage or data loss.
  * Use `allowedDecisions: ["allow-once", "deny"]` when persistent trust is unsafe for that action.
...
See [Advanced exec approvals](</tools/exec-approvals-advanced#plugin-approval-forwarding>) for the full forwarding model, same-chat approval behavior, native channel delivery, and channel-specific approver rules.
...
See [Codex harness runtime](</plugins/codex-harness-runtime#native-permissions-and-mcp-elicitations>) for the Codex-specific behavior and fallback rules.
