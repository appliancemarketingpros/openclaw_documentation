---
title: PixVerse
source_url: https://docs.openclaw.ai/zh-TW/providers/pixverse
...
精靈會先詢問要使用國際端點 (`https://app-api.pixverse.ai/openapi/v2`) 或 CN 端點 (`https://app-api.pixverseai.cn/openapi/v2`)，再將 `region` 和 `baseUrl` 寫入提供者設定。

* ### 將 PixVerse 設為預設影片提供者
...
OpenClaw 預設使用國際 PixVerse API。當你的金鑰屬於特定 PixVerse 平台區域時，請手動設定 `models.providers.pixverse.region`，或使用 `openclaw onboard --auth-choice pixverse-api-key` 在設定精靈中選擇：
...
只有在透過可信任的相容代理路由時，才設定 `models.providers.pixverse.baseUrl`。 `baseUrl` 的優先順序高於 `region`。
...
PixVerse 會從生成請求回傳 `video_id`。OpenClaw 會輪詢 `/openapi/v2/video/result/{video_id}`，直到任務成功、失敗或逾時。

## 相關

[**影片生成** 共用工具參數、提供者選擇與非同步行為。 ](</zh-TW/tools/video-generation>) [**設定參考** 代理預設設定，包括影片生成模型。 ](</zh-TW/gateway/config-agents#agent-defaults>)
