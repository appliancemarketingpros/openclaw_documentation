...
source_url: https://docs.openclaw.ai/ko/platforms/mac/child-process
...
## 기본 동작(launchd)
  ...
  * 앱은 `ai.openclaw.gateway` 레이블이 지정된 사용자별 LaunchAgent를 설치합니다 (`--profile`/`OPENCLAW_PROFILE`을 사용하는 경우 `ai.openclaw.<profile>`, 레거시 `com.openclaw.*`도 지원).
  * 로컬 모드가 활성화되면 앱은 LaunchAgent가 로드되어 있는지 확인하고 필요한 경우 Gateway를 시작합니다.
