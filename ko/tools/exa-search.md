---
title: Exa 검색
source_url: https://docs.openclaw.ai/ko/tools/exa-search
scraped_at: 2026-05-25
---

OpenClaw는 `web_search` 제공자로 [Exa AI](<https://exa.ai/>)를 지원합니다. Exa는 내장 콘텐츠 추출(하이라이트, 텍스트, 요약)과 함께 신경망, 키워드, 하이브리드 검색 모드를 제공합니다.
...
Gateway 환경에 `EXA_API_KEY`를 설정하거나 다음으로 구성합니다.
...
**환경 대안:** Gateway 환경에 `EXA_API_KEY`를 설정합니다. Gateway 설치의 경우 `~/.openclaw/.env`에 넣습니다.

## 기본 URL 재정의

Exa 검색 요청이 호환 프록시나 대체 Exa 엔드포인트를 거쳐야 할 때 `plugins.entries.exa.config.webSearch.baseUrl`을 설정합니다. OpenClaw는 기본 호스트 앞에 `https://`를 붙여 정규화하고, 경로가 이미 해당 위치에서 끝나지 않는 한 `/search`를 추가합니다. 해석된 엔드포인트는 검색 캐시 키에 포함되므로 서로 다른 Exa 엔드포인트의 결과는 공유되지 않습니다.
...
## 참고
  ...
  * 사용 가능한 경우 결과는 Exa API 응답의 `highlightScores` 및 `summary` 필드를 보존합니다.
  * 결과 설명은 하이라이트, 요약, 전체 텍스트 순으로, 사용 가능한 항목에서 결정됩니다.
  * `freshness`와 `date_after`/`date_before`는 함께 사용할 수 없습니다. 하나의 시간 필터 모드를 사용하세요.
