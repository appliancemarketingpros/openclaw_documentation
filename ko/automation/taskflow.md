---
title: 작업 흐름
source_url: https://docs.openclaw.ai/ko/automation/taskflow
...
각 플로는 자체 상태를 영구 저장하고 리비전을 추적하므로 Gateway 재시작 후에도 진행 상황이 유지됩니다. 리비전 추적은 여러 소스가 동일한 플로를 동시에 진행하려 할 때 충돌 감지를 가능하게 합니다. 플로 레지스트리는 주기적 체크포인트와 종료 체크포인트를 포함한 제한된 write-ahead-log 유지 관리를 갖춘 SQLite를 사용하므로, 장시간 실행되는 Gateway가 제한 없는 `registry.sqlite-wal` 사이드카 파일을 유지하지 않습니다.
