---
title: दूरस्थ पहुँच
source_url: https://docs.openclaw.ai/hi/gateway/remote
...
जब gateway केवल loopback हो, URL को `ws://127.0.0.1:18789` पर रखें और पहले SSH tunnel खोलें। macOS app के SSH tunnel transport में, discovered gateway hostnames `gateway.remote.sshTarget` में होते हैं; `gateway.remote.url` local tunnel URL बना रहता है। यदि वे ports अलग हों, तो `gateway.remote.remotePort` को SSH host पर gateway port पर set करें।
...
Short version: **Gateway को केवल loopback रखें** जब तक आपको यकीन न हो कि bind की आवश्यकता है।
  ...
  * **Tailscale Serve** `gateway.auth.allowTailscale: true` होने पर identity headers के माध्यम से Control UI/WebSocket traffic authenticate कर सकता है; HTTP API endpoints उस Tailscale header auth का उपयोग नहीं करते और इसके बजाय gateway के normal HTTP auth mode का पालन करते हैं। यह tokenless flow मानता है कि gateway host trusted है। यदि आप हर जगह shared-secret auth चाहते हैं तो इसे `false` पर set करें।
  * **Trusted-proxy** auth default रूप से non-loopback identity-aware proxy setups की अपेक्षा करता है। Same-host loopback reverse proxies को explicit `gateway.auth.trustedProxy.allowLoopback = true` चाहिए।
