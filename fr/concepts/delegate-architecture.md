...
source_url: https://docs.openclaw.ai/fr/concepts/delegate-architecture
...
Si vous accordez `sessions_history`, n’oubliez pas qu’il s’agit d’une vue de rappel bornée et filtrée pour la sécurité. OpenClaw expurge le texte ressemblant à des identifiants d’accès ou jetons, tronque les contenus longs, supprime les balises de raisonnement / l’échafaudage `<relevant-memories>` / les charges utiles XML d’appels d’outils en texte brut (y compris `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>` et les blocs d’appels d’outils tronqués) / l’échafaudage dégradé d’appels d’outils / les jetons de contrôle de modèle ASCII/pleine chasse divulgués / le XML d’appel d’outil MiniMax mal formé dans le rappel de l’assistant, et peut remplacer les lignes surdimensionnées par `[sessions_history omitted: message too large]` au lieu de renvoyer un vidage brut de transcription.
