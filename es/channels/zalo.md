---
title: Zalo
source_url: https://docs.openclaw.ai/es/channels/zalo
scraped_at: 2026-05-25
---

Estado: experimental. Se admiten los mensajes directos. La sección Capacidades siguiente refleja el comportamiento actual de los bots de Marketplace.
...
Zalo es una aplicación de mensajería centrada en Vietnam; su Bot API permite que el Gateway ejecute un bot para conversaciones 1:1. Es una buena opción para soporte o notificaciones cuando quieres enrutamiento determinista de vuelta a Zalo.

Esta página refleja el comportamiento actual de OpenClaw para **bots de Zalo Bot Creator / Marketplace**. Los **bots de Zalo Official Account (OA)** son una superficie de producto diferente de Zalo y pueden comportarse de otra manera.
  ...
  * La sección Capacidades siguiente muestra la compatibilidad actual de los bots de Marketplace.
...
Si más adelante migras a una superficie de bot de Zalo donde los grupos estén disponibles, puedes añadir explícitamente configuración específica de grupos como `groupPolicy` y `groupAllowFrom`. Para el comportamiento actual de bots de Marketplace, consulta Capacidades.
...
Para una instantánea rápida de compatibilidad, consulta Capacidades. Las notas siguientes añaden detalles cuando el comportamiento necesita contexto adicional.
  ...
  * **Vistas previas de enlaces / tarjetas de enlace enriquecidas** : Consulta el estado de bots de Marketplace en Capacidades; no activaban una respuesta de forma fiable.
  * **Mensajes de imagen** : Consulta el estado de bots de Marketplace en Capacidades; el manejo de imágenes entrantes no era fiable (indicador de escritura sin respuesta final).
  * **Stickers** : Consulta el estado de bots de Marketplace en Capacidades.
  * **Notas de voz / archivos de audio / video / adjuntos de archivo genéricos** : Consulta el estado de bots de Marketplace en Capacidades.
...
Opciones del proveedor:
  ...
  * `channels.zalo.groupPolicy`: `open | allowlist | disabled` (predeterminado: allowlist). Presente en la configuración; consulta Capacidades y Control de acceso (grupos) para el comportamiento actual de bots de Marketplace.
...
Opciones de varias cuentas:
  ...
  * `channels.zalo.accounts.<id>.groupPolicy`: política de grupo por cuenta. Presente en la configuración; consulta Capacidades y Control de acceso (grupos) para el comportamiento actual de bots de Marketplace.
