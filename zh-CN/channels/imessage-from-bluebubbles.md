...
source_url: https://docs.openclaw.ai/zh-CN/channels/imessage-from-bluebubbles
...
`channels status --probe` 只会探测已配置且已启用的账号。除非你有意同时运行两个渠道监视器，否则不要在 BlueBubbles 和 iMessage 同时启用的情况下重启 Gateway 网关。如果你不会立即切换，在重启 Gateway 网关前将 `channels.imessage.enabled` 设回 `false`。在启用 OpenClaw 流量前，使用 开始之前 中的直接 `imsg` 命令验证 Mac。
