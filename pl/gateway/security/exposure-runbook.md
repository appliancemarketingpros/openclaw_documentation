...
source_url: https://docs.openclaw.ai/pl/gateway/security/exposure-runbook
scraped_at: 2026-06-29
---

Gateway & OpsGateway

Ten runbook przekształca szersze wytyczne z sekcji [Bezpieczeństwo](</pl/gateway/security>) w listę kontrolną operatora dotyczącą dostępu zdalnego i ekspozycji komunikatorów.
...
Unikaj bezpośredniego publicznego przekierowania portów do Gateway. Jeśli potrzebujesz dostępu publicznego, umieść przed nim proxy świadome tożsamości i spraw, aby proxy było jedyną ścieżką sieciową do Gateway.
...
Jeśli więcej niż jedna osoba może wysyłać wiadomości do bota, traktuj to jako współdzielone delegowane uprawnienie do narzędzi, a nie jako izolację hosta per użytkownik.
...
Najpierw rozwiąż krytyczne ustalenia. Ostrzeżenia mogą być akceptowalne tylko wtedy, gdy są zamierzone i udokumentowane dla danego wdrożenia.
...
Następnie poszerzaj po jednym zabezpieczeniu naraz. Na przykład dodaj konkretną listę dozwolonych dla kanału przed włączeniem narzędzi zdolnych do zapisu albo włącz reverse proxy przed akceptowaniem zdalnego ruchu Control UI.

Ścisła baza `exec.security: "deny"` blokuje wszystkie wywołania exec, w tym nieszkodliwą diagnostykę. Jeśli diagnostyka lub polecenia niskiego ryzyka są wymagane, złagodź to dopiero po wybraniu konkretnych nadawców, agentów, poleceń i trybu zatwierdzania zgodnych z Twoim modelem zagrożeń.
...
Parowanie zatwierdza nadawcę do uruchamiania bota. Nie czyni tego nadawcy oddzielną granicą bezpieczeństwa hosta.
...
Uruchom `openclaw security audit --deep` po zmianach proxy. Ustalenia dotyczące zaufanego proxy są celowo wysokiej jakości sygnałem, ponieważ proxy staje się granicą uwierzytelniania.
...
Jeśli zdalni użytkownicy nie są w pełni zaufani, izolacja musi pochodzić z oddzielnych wdrożeń, a nie tylko z promptów lub etykiet sesji.
