---
title: Signal
source_url: https://docs.openclaw.ai/id/channels/signal
...
Jika Anda menggunakan build JVM (`signal-cli-${VERSION}.tar.gz`), instal JRE 25+ terlebih dahulu. Selalu perbarui `signal-cli`; catatan upstream menyebutkan bahwa rilis lama dapat rusak saat API server Signal berubah.
...
Opsi penyedia:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (default: auto). Lihat Mode container.
