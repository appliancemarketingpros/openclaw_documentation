...
source_url: https://docs.openclaw.ai/uk/tools/thinking
...
## Що це робить
  ...
  * Примітки щодо провайдерів: 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) підтримує лише бінарне thinking (`on`/`off`). Будь-який рівень не `off` вважається `on` (зіставляється з `low`).
