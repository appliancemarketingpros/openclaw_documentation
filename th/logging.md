---
title: การบันทึกล็อก
source_url: https://docs.openclaw.ai/th/logging
...
แต่ละไฟล์จะหมุนเวียนเมื่อมีขนาดถึง `logging.maxFileBytes` (ค่าเริ่มต้น: 100 MB) OpenClaw จะเก็บไฟล์เก็บถาวรที่มีหมายเลขไว้สูงสุดห้าไฟล์ข้างไฟล์ที่ใช้งานอยู่ เช่น `openclaw-YYYY-MM-DD.1.log` และเขียนต่อไปยังไฟล์บันทึกที่ใช้งานอยู่ใหม่แทนการ ระงับข้อมูลวินิจฉัย
...
เมื่อคุณส่ง `--url` อย่างชัดเจน CLI จะไม่ปรับใช้ข้อมูลประจำตัวจากการกำหนดค่า หรือสภาพแวดล้อมโดยอัตโนมัติ ให้ใส่ `--token` เองหาก Gateway เป้าหมาย ต้องใช้การตรวจสอบสิทธิ์
...
หาก Gateway แบบ local loopback โดยนัยขอการจับคู่ ปิดระหว่างการเชื่อมต่อ หรือหมดเวลาก่อนที่ `logs.tail` จะตอบ `openclaw logs` จะย้อนกลับไปใช้ ไฟล์บันทึก Gateway ที่กำหนดค่าไว้โดยอัตโนมัติ เป้าหมาย `--url` ที่ระบุอย่างชัดเจนจะไม่ใช้ กลไกย้อนกลับนี้
...
แท็บ **บันทึก** ของ Control UI จะติดตามไฟล์เดียวกันโดยใช้ `logs.tail` ดู [Control UI](</th/web/control-ui>) สำหรับวิธีเปิดใช้งาน
...
`--verbose` มีผลเฉพาะเอาต์พุตคอนโซลและความละเอียดของบันทึก WS เท่านั้น ไม่ได้เปลี่ยน ระดับบันทึกไฟล์
...
บันทึกไฟล์เป็น JSONL เมื่อการเรียกบันทึกมีบริบทเทรซการวินิจฉัยที่ถูกต้อง OpenClaw จะเขียนฟิลด์เทรซเป็นคีย์ JSON ระดับบนสุด (`traceId`, `spanId`, `parentSpanId`, `traceFlags`) เพื่อให้ตัวประมวลผลบันทึกภายนอกสามารถเชื่อมโยงบรรทัด กับ OTEL spans และการส่งต่อ `traceparent` ของผู้ให้บริการได้
...
พื้นผิวที่อยู่ติดกันสองส่วน:
  ...
  * **แฟล็กการวินิจฉัย** — แฟล็กบันทึกดีบักแบบเจาะจงที่ส่งบันทึกเพิ่มเติมไปยัง `logging.file` โดยไม่เพิ่ม `logging.level` แฟล็กไม่คำนึงถึงตัวพิมพ์ใหญ่เล็ก และรองรับไวลด์การ์ด (`telegram.*`, `*`) กำหนดค่าใต้ `diagnostics.flags` หรือผ่านการแทนที่ด้วยตัวแปรสภาพแวดล้อม `OPENCLAW_DIAGNOSTICS=...` คู่มือฉบับเต็ม: [แฟล็กการวินิจฉัย](</th/diagnostics/flags>)
