---
title: Slack
source_url: https://docs.openclaw.ai/zh-TW/channels/slack
scraped_at: 2026-05-25
---

可透過 Slack 應用程式整合，在 DM 和頻道中達到生產環境可用。預設模式為 Socket Mode；也支援 HTTP Request URLs。

[**Pairing** Slack DM 預設使用配對模式。 ](</zh-TW/channels/pairing>) [**Slash commands** 原生命令行為與命令目錄。 ](</zh-TW/tools/slash-commands>) [**Channel troubleshooting** 跨頻道診斷與修復手冊。 ](</zh-TW/channels/troubleshooting>)
...
## 權杖模型
  ...
  * `botToken`、`appToken`、`signingSecret` 和 `userToken` 可接受明文 字串或 SecretRef 物件。
  * 設定權杖會覆寫 env 後援。
...
當 `enabled` 未設定或為 `"auto"`，且至少解析出一位核准者時，Slack 會自動啟用原生 exec 核准。設定 `enabled: false` 可明確停用 Slack 作為原生核准用戶端。 設定 `enabled: true` 可在解析出核准者時強制開啟原生核准。
...
如果 `openclaw channels status --probe --json` 顯示 `botTokenStatus` 或 `appTokenStatus: "configured_unavailable"`，表示 Slack 帳戶已 設定，但目前執行階段無法解析由 SecretRef 支援的 值。
...
[**Pairing** 將 Slack 使用者配對到 Gateway。 ](</zh-TW/channels/pairing>) [**Groups** 頻道與群組 DM 行為。 ](</zh-TW/channels/groups>) [**Channel routing** 將入站訊息路由至 agent。 ](</zh-TW/channels/channel-routing>) [**Security** 威脅模型與強化。 ](</zh-TW/gateway/security>) [**Configuration** 設定版面配置與優先順序。 ](</zh-TW/gateway/configuration>) [**Slash commands** 命令目錄與行為。 ](</zh-TW/tools/slash-commands>)
