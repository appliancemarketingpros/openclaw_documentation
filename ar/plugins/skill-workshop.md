...
source_url: https://docs.openclaw.ai/ar/plugins/skill-workshop
scraped_at: 2026-05-25
---

ورشة المهارات **تجريبية**. تكون معطّلة افتراضيًا، وقد تتغير استدلالات الالتقاط ومطالبات المراجع بين الإصدارات، ويجب استخدام الكتابات التلقائية فقط في مساحات العمل الموثوقة بعد مراجعة مخرجات وضع الانتظار أولًا.

ورشة المهارات هي ذاكرة إجرائية لمهارات مساحة العمل. تتيح للوكيل تحويل سير العمل القابل لإعادة الاستخدام، وتصحيحات المستخدم، والإصلاحات المكتسبة بصعوبة، والمزالق المتكررة إلى ملفات `SKILL.md` ضمن:
...
الـ Plugin المضمّن **تجريبي** و**معطّل افتراضيًا** ما لم يتم تفعيله صراحةً في `plugins.entries.skill-workshop`.

لا يضبط بيان الـ Plugin القيمة `enabledByDefault: true`. ينطبق الإعداد الافتراضي `enabled: true` داخل مخطط إعدادات الـ Plugin فقط بعد أن يكون إدخال الـ Plugin قد اختير وحُمّل بالفعل.
...
ما يزال `approvalPolicy: "auto"` يستخدم مسار الفحص والحجر نفسه. ولا يطبّق المقترحات التي تحتوي على نتائج حرجة.
...
يمكن للنموذج استدعاء `skill_workshop` مباشرةً عندما يرى إجراءً قابلًا لإعادة الاستخدام أو عندما يطلب المستخدم منه حفظ مهارة أو تحديثها.

هذا هو المسار الأكثر صراحة ويعمل حتى مع `autoCapture: false`.

### الالتقاط بالاستدلال

عند تفعيل `autoCapture` وكان `reviewMode` هو `heuristic` أو `hybrid`، يفحص الـ Plugin الجولات الناجحة بحثًا عن عبارات تصحيح صريحة من المستخدم:
...
ينشئ الاستدلال مقترحًا من أحدث تعليمة مستخدم مطابقة. ويستخدم تلميحات الموضوع لاختيار أسماء المهارات لسير العمل الشائعة:
...
الالتقاط بالاستدلال ضيق عمدًا. فهو مخصص للتصحيحات الواضحة وملاحظات العمليات القابلة للتكرار، وليس لتلخيص نصوص المحادثات عمومًا.

### مراجع LLM

عند تفعيل `autoCapture` وكان `reviewMode` هو `llm` أو `hybrid`، يشغّل الـ Plugin مراجعًا مضمّنًا مضغوطًا بعد الوصول إلى العتبات.
...
تُزال التكرارات من المقترحات المعلّقة والمعزولة حسب اسم المهارة وحمولة التغيير. يحتفظ المخزن بأحدث المقترحات المعلّقة/المعزولة حتى `maxPending`.
...
استخدم هذا عندما يبدو أن الالتقاط التلقائي لا يفعل شيئًا وتذكر السجلات `skill-workshop: quarantined <skill>`.
...
طلب كتابة فورية في الوضع التلقائي (apply: true) jsonCopy code
[code]
...
مع `approvalPolicy: "pending"`، يظل `apply: true` يضع المقترح في الطابور. راجعه، ثم استخدم إجراء `apply` بعد الموافقة.

فرض التعليق ضمن سياسة تلقائية (apply: false) jsonCopy code
[code]
...
الإلحاق بقسم مسمّى jsonCopy code
[code]
...
استبدال النص المطابق تمامًا jsonCopy code
[code]
...
تكون ملفات الدعم محددة بنطاق مساحة العمل، ومتحققا من مساراتها، ومحدودة بالبايت بواسطة `maxSkillBytes`، ومفحوصة، وتكتب ذريًا.
...
كل الكتابات ذرية وتحدّث لقطة Skills في الذاكرة فورًا، بحيث يمكن أن تصبح Skill الجديدة أو المحدثة مرئية دون إعادة تشغيل Gateway.
...
للتعافي من مقترح معزول، أنشئ مقترحًا آمنًا جديدًا مع إزالة المحتوى غير الآمن. لا تعدّل JSON المخزن يدويًا.

## إرشادات المطالبة

عند التفعيل، يحقن Skill Workshop قسم مطالبة قصيرًا يخبر الوكيل باستخدام `skill_workshop` للذاكرة الإجرائية الدائمة.
...
تستخدم مراجعة LLM تشغيلًا مضمّنًا على نموذج الوكيل النشط/الافتراضي. وهي مبنية على عتبات كي لا تعمل افتراضيًا في كل دورة.
...
إذا فشلت المراجعة أو انتهت مهلتها أو أعادت JSON غير صالح، يسجل Plugin رسالة تحذير/تصحيح ويتخطى دورة المراجعة تلك.
...
سيناريو المراجع منفصل عمدًا لأنه يفعّل `reviewMode: "llm"` ويمرّن دورة المراجع المضمّن.
...
استخدم وضع الانتظار أولًا. انتقل إلى الوضع التلقائي فقط بعد مراجعة نوع Skills التي يقترحها الوكيل في مساحة العمل تلك.
