---
title: Агент
source_url: https://docs.openclaw.ai/ru/cli/agent
...
Запустить ход агента через Gateway (используйте `--local` для встроенного режима). Используйте `--agent <id>`, чтобы напрямую выбрать настроенного агента.
