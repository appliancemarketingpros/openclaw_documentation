---
title: Gateway 런북
source_url: https://docs.openclaw.ai/ko/gateway
scraped_at: 2026-05-25
---

이 페이지는 Gateway 서비스의 1일 차 시작과 2일 차 운영에 사용하세요.

[**Deep troubleshooting** 정확한 명령 단계와 로그 시그니처를 포함한 증상 우선 진단입니다. ](</ko/gateway/troubleshooting>) [**Configuration** 작업 중심 설정 가이드 + 전체 구성 참조입니다. ](</ko/gateway/configuration>) [**Secrets management** SecretRef 계약, 런타임 스냅샷 동작, 마이그레이션/다시 로드 작업입니다. ](</ko/gateway/secrets>) [**Secrets plan contract** 정확한 `secrets apply` 대상/경로 규칙과 ref 전용 auth-profile 동작입니다. ](</ko/gateway/secrets-plan-contract>)
...
## 프로토콜 빠른 참조(운영자 관점)
  ...
  * 일반 이벤트에는 `connect.challenge`, `agent`, `chat`, `session.message`, `session.tool`, `sessions.changed`, `presence`, `tick`, `health`, `heartbeat`, 페어링/승인 수명 주기 이벤트, `shutdown`이 포함됩니다.
