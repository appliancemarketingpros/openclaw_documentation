...
source_url: https://docs.openclaw.ai/ko/plugins/oc-path
...
`onStartup: false`는 이 Plugin을 Gateway 핫 경로에서 제외합니다. `onCommands: ["path"]`는 처음 `openclaw path …`를 실행할 때 CLI가 Plugin을 지연 로드하도록 하므로, 이 동사를 사용하지 않는 설치 환경은 비용을 지불하지 않습니다.
