...
source_url: https://docs.openclaw.ai/it/reference/token-use
...
Altre superfici:
  ...
  * **CLI:** `openclaw status --usage` e `openclaw channels list` mostrano finestre quota provider normalizzate (`X% left`, non costi per risposta). Provider correnti con finestra d’uso: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax, Xiaomi e [z.ai](<http://z.ai>).
...
Configuralo in [Configurazione Gateway](</it/gateway/configuration>) e vedi i dettagli del comportamento in [Potatura sessione](</it/concepts/session-pruning>).
