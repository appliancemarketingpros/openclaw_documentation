...
Sandbox-level settings (`mode`, `scope`, `workspaceAccess`) are configured under `agents.defaults.sandbox` as with any backend. See [Sandboxing](</gateway/sandboxing>) for the full matrix.
...
[Sandboxing](</gateway/sandboxing>)[Sandbox vs Tool Policy vs Elevated](</gateway/sandbox-vs-tool-policy-vs-elevated>)
