...
[**Exec approvals** シェル実行の許可/拒否ポリシー。 ](</ja-JP/tools/exec-approvals>) [**Thinking levels** 推論エフォートレベルとプロバイダーポリシーの相互作用。 ](</ja-JP/tools/thinking>) [**Sub-agents** 暴走動作を制限するために分離されたエージェントを生成する。 ](</ja-JP/tools/subagents>) [**Configuration reference** 完全な `tools.loopDetection` スキーマとマージセマンティクス。 ](</ja-JP/gateway/configuration-reference>)
