...
source_url: https://docs.openclaw.ai/ko/channels/nextcloud-talk
...
Nextcloud Talk은 현재 OpenClaw 릴리스에서 번들 Plugin으로 제공되므로 일반 패키지 빌드에서는 별도 설치가 필요하지 않습니다.

Nextcloud Talk을 제외한 이전 빌드나 사용자 지정 설치를 사용하는 경우, npm 패키지를 직접 설치하세요.
...
현재 공식 릴리스 태그를 따르려면 기본 패키지를 사용하세요. 재현 가능한 설치가 필요한 경우에만 정확한 버전을 고정하세요.
