...
source_url: https://docs.openclaw.ai/it/help/faq
...
Esegue un probe live dello stato del gateway, inclusi i probe dei canali quando supportati (richiede un gateway raggiungibile). Vedi [Integrità](</it/gateway/health>).
...
Le domande e risposte sul primo avvio — installazione, onboarding, route di autenticazione, sottoscrizioni, errori iniziali — si trovano nelle [FAQ sul primo avvio](</it/help/faq-first-run>).
...
Documentazione: [Gateway](</it/gateway>), [Canali](</it/channels>), [Multi-agente](</it/concepts/multi-agent>), [Memoria](</it/concepts/memory>).
...
OpenClaw può aiutare con generazione di lead, outreach, annunci e blog per un SaaS?
...
Come posso personalizzare le Skills senza lasciare il repository sporco?
...
Il bot si blocca mentre svolge lavori pesanti. Come posso scaricarli altrove?
...
Come funzionano le sessioni dei subagenti vincolate ai thread su Discord?
...
Un subagente ha terminato, ma l'aggiornamento di completamento è andato nel posto sbagliato o non è mai stato pubblicato. Cosa dovrei controllare?
...
Cron o i promemoria non partono. Cosa dovrei controllare?
...
Perché un'esecuzione Cron isolata ha cambiato modello o ha riprovato una volta?
...
Come installo le Skills su Linux?
...
OpenClaw può eseguire attività in base a una pianificazione o in modo continuo in background?
...
Documentazione: [job Cron](</it/automation/cron-jobs>), [automazione](</it/automation>), [Heartbeat](</it/gateway/heartbeat>).

Posso eseguire Skills disponibili solo su Apple macOS da Linux?
...
Come uso il mio Chrome già autenticato con OpenClaw?
...
Esiste una documentazione dedicata all'isolamento in sandbox?
...
Posso mantenere personali i DM ma rendere pubblici/in sandbox i gruppi con un solo agente?
...
Come associo una cartella host alla sandbox?
...
OpenClaw esegue anche un **flush di memoria silenzioso pre-Compaction** per ricordare al modello di scrivere note durevoli prima della Compaction automatica. Questo viene eseguito solo quando il workspace è scrivibile (le sandbox in sola lettura lo saltano). Vedi [memoria](</it/concepts/memory>).
...
La ricerca semantica nella memoria richiede una chiave API OpenAI?
...
Dove archivia i suoi dati OpenClaw?
...
Il tuo **workspace** ([AGENTS.md](<http://AGENTS.md>), file di memoria, Skills, ecc.) è separato e configurato tramite `agents.defaults.workspace` (predefinito: `~/.openclaw/workspace`).

Dove devono trovarsi AGENTS.md / SOUL.md / USER.md / MEMORY.md?
...
Suggerimento: se vuoi un comportamento o una preferenza durevole, chiedi al bot di **scriverlo in[AGENTS.md](<http://AGENTS.md>) o [MEMORY.md](<http://MEMORY.md>)** invece di fare affidamento sulla cronologia della chat.
...
Modalità remota: dov'è l'archivio delle sessioni?
...
Che formato ha la configurazione? Dove si trova?
...
Ho impostato gateway.bind: "lan" (o "tailnet") e ora non c
...
Come disabilito gli slogan divertenti della CLI?
...
Il browser OpenClaw può funzionare in modalità headless?
...
Come uso Brave per controllare il browser?

Imposta `browser.executablePath` sul binario Brave (o su qualsiasi browser basato su Chromium) e riavvia il Gateway. Consulta gli esempi completi di configurazione in [Browser](</it/tools/browser#use-brave-or-another-chromium-based-browser>).
...
Promemoria di sicurezza: abbinare un nodo macOS consente `system.run` su quella macchina. Abbina solo dispositivi di cui ti fidi e consulta [Sicurezza](</it/gateway/security>).
...
Tailscale è connesso ma non ricevo risposte. Cosa faccio ora?
...
**Bridge CLI (generico):** esegui uno script che chiama l'altro Gateway con `openclaw agent --message ... --deliver`, indirizzandolo a una chat in cui l'altro bot è in ascolto. Se un bot è su un VPS remoto, punta la tua CLI a quel Gateway remoto tramite SSH/Tailscale (vedi [Accesso remoto](</it/gateway/remote>)).
...
Configurazione minima ragionevole per una prima installazione json5Copy code
[code]
...
Come configuro Tailscale su una VPS e mi connetto dal mio Mac?
...
Dovrei installarlo su un secondo laptop o aggiungere semplicemente un Node?
...
Come carica OpenClaw le variabili d'ambiente?
...
Ho avviato il Gateway tramite il servizio e le mie variabili d'ambiente sono scomparse. Che faccio?
...
I token Copilot vengono letti da `COPILOT_GITHUB_TOKEN` (anche `GH_TOKEN` / `GITHUB_TOKEN`). Vedi [/concepts/model-providers](</it/concepts/model-providers>) e [/environment](</it/help/environment>).
...
Come reimposto completamente OpenClaw mantenendolo installato?
...
Ricevo errori "context too large": come reimposto o compatto?
...
Come ottengo il JID di un gruppo WhatsApp?
...
Documentazione: [Instradamento Multi-Agent](</it/concepts/multi-agent>), [Slack](</it/channels/slack>), [Browser](</it/tools/browser>), [Nodi](</it/nodes>).

## Modelli, failover e profili di autenticazione

Le domande e risposte sui modelli — impostazioni predefinite, selezione, alias, cambio, failover, profili di autenticazione — si trovano nelle [FAQ sui modelli](</it/help/faq-models>).
...
Che cosa significa "another gateway instance is already listening"?
...
Come eseguo OpenClaw in modalità remota (il client si connette a un Gateway altrove)?
...
La Control UI dice "unauthorized" (o continua a riconnettersi). E adesso?
...
Ho impostato gateway.bind tailnet ma non riesce ad associarsi e nulla resta in ascolto
...
I profili aggiungono anche un suffisso ai nomi dei servizi (`ai.openclaw.<profile>`; legacy `com.openclaw.*`, `openclaw-gateway-<profile>.service`, `OpenClaw Gateway (<profile>)`). Guida completa: [Gateway multipli](</it/gateway/multiple-gateways>).

Che cosa significa "invalid handshake" / codice 1008?
...
"Disconnesso dal gateway: nessun motivo" - cosa fare ora?
...
Come posso arrestare completamente e poi avviare il Gateway?
...
Spiegato semplice: openclaw gateway restart rispetto a openclaw gateway
...
WhatsApp: invierà messaggi ai miei contatti? Come funziona il pairing?
...
Come posso impedire che i messaggi di sistema interni vengano mostrati in chat?
...
Documentazione: [Pensiero e verbose](</it/tools/thinking>), [Sicurezza](</it/gateway/security#reasoning-and-verbose-output-in-groups>).

Come posso arrestare/annullare un'attività in esecuzione?
...
Come posso inviare un messaggio Discord da Telegram? ("Messaggistica tra contesti negata")
