---
title: दस्तावेज़ हब
source_url: https://docs.openclaw.ai/hi/start/hubs
scraped_at: 2026-06-29
---

HelpCommunity and meta

हर पेज खोजने के लिए इन हब का उपयोग करें, जिनमें गहन व्याख्याएं और वे संदर्भ दस्तावेज़ भी शामिल हैं जो बाएं नेविगेशन में दिखाई नहीं देते।

## यहां से शुरू करें

  * [अनुक्रमणिका](</hi>)
  * [शुरू करना](</hi/start/getting-started>)
  * [ऑनबोर्डिंग](</hi/start/onboarding>)
  * [ऑनबोर्डिंग (CLI)](</hi/start/wizard>)
  * [सेटअप](</hi/start/setup>)
  * [डैशबोर्ड (स्थानीय Gateway)](<http://127.0.0.1:18789/>)
  * [सहायता](</hi/help>)
  * [दस्तावेज़ निर्देशिका](</hi/start/docs-directory>)
  * [कॉन्फ़िगरेशन](</hi/gateway/configuration>)
  * [कॉन्फ़िगरेशन उदाहरण](</hi/gateway/configuration-examples>)
  * [OpenClaw सहायक](</hi/start/openclaw>)
  * [शोकेस](</hi/start/showcase>)
  * [कथा](</hi/start/lore>)


## इंस्टॉलेशन + अपडेट

  * [Docker](</hi/install/docker>)
  * [Nix](</hi/install/nix>)
  * [अपडेट करना / रोलबैक](</hi/install/updating>)
  * [Bun वर्कफ़्लो (प्रयोगात्मक)](</hi/install/bun>)


## मुख्य अवधारणाएं

  * [आर्किटेक्चर](</hi/concepts/architecture>)
  * [सुविधाएं](</hi/concepts/features>)
  * [नेटवर्क हब](</hi/network>)
  * [एजेंट रनटाइम](</hi/concepts/agent>)
  * [एजेंट वर्कस्पेस](</hi/concepts/agent-workspace>)
  * [मेमोरी](</hi/concepts/memory>)
  * [एजेंट लूप](</hi/concepts/agent-loop>)
  * [स्ट्रीमिंग + चंकिंग](</hi/concepts/streaming>)
  * [मल्टी-एजेंट रूटिंग](</hi/concepts/multi-agent>)
  * [Compaction](</hi/concepts/compaction>)
  * [सेशन](</hi/concepts/session>)
  * [सेशन प्रूनिंग](</hi/concepts/session-pruning>)
  * [सेशन टूल्स](</hi/concepts/session-tool>)
  * [क्यू](</hi/concepts/queue>)
  * [स्लैश कमांड](</hi/tools/slash-commands>)
  * [RPC अडैप्टर](</hi/reference/rpc>)
  * [TypeBox स्कीमा](</hi/concepts/typebox>)
  * [टाइमज़ोन हैंडलिंग](</hi/concepts/timezone>)
  * [प्रेज़ेंस](</hi/concepts/presence>)
  * [डिस्कवरी + ट्रांसपोर्ट](</hi/gateway/discovery>)
  * [Bonjour](</hi/gateway/bonjour>)
  * [चैनल रूटिंग](</hi/channels/channel-routing>)
  * [समूह](</hi/channels/groups>)
  * [समूह संदेश](</hi/channels/group-messages>)
  * [मॉडल फ़ेलओवर](</hi/concepts/model-failover>)
  * [OAuth](</hi/concepts/oauth>)


## प्रदाता + प्रवेश

  * [चैट चैनल हब](</hi/channels>)
  * [मॉडल प्रदाता हब](</hi/providers/models>)
  * [WhatsApp](</hi/channels/whatsapp>)
  * [Telegram](</hi/channels/telegram>)
  * [Slack](</hi/channels/slack>)
  * [Discord](</hi/channels/discord>)
  * [Mattermost](</hi/channels/mattermost>)
  * [Signal](</hi/channels/signal>)
  * [QQ Bot](</hi/channels/qqbot>)
  * [iMessage](</hi/channels/imessage>)
  * [लोकेशन पार्सिंग](</hi/channels/location>)
  * [WebChat](</hi/web/webchat>)
  * [Webhooks](</hi/automation/cron-jobs#webhooks>)
  * [Gmail Pub/Sub](</hi/automation/cron-jobs#gmail-pubsub-integration>)


## Gateway + संचालन

  * [Gateway रनबुक](</hi/gateway>)
  * [नेटवर्क मॉडल](</hi/network#core-model>)
  * [Gateway पेयरिंग](</hi/gateway/pairing>)
  * [Gateway लॉक](</hi/gateway/gateway-lock>)
  * [बैकग्राउंड प्रोसेस](</hi/gateway/background-process>)
  * [स्वास्थ्य](</hi/gateway/health>)
  * [Heartbeat](</hi/gateway/heartbeat>)
  * [Doctor](</hi/gateway/doctor>)
  * [लॉगिंग](</hi/gateway/logging>)
  * [सैंडबॉक्सिंग](</hi/gateway/sandboxing>)
  * [डैशबोर्ड](</hi/web/dashboard>)
  * [कंट्रोल UI](</hi/web/control-ui>)
  * [रिमोट एक्सेस](</hi/gateway/remote>)
  * [रिमोट Gateway README](</hi/gateway/remote-gateway-readme>)
  * [Tailscale](</hi/gateway/tailscale>)
  * [सुरक्षा](</hi/gateway/security>)
  * [समस्या निवारण](</hi/gateway/troubleshooting>)


## टूल्स + ऑटोमेशन

  * [टूल्स सतह](</hi/tools>)
  * [OpenProse](</hi/prose>)
  * [CLI संदर्भ](</hi/cli>)
  * [Exec टूल](</hi/tools/exec>)
  * [PDF टूल](</hi/tools/pdf>)
  * [उन्नत मोड](</hi/tools/elevated>)
  * [Cron जॉब्स](</hi/automation/cron-jobs>)
  * [ऑटोमेशन](</hi/automation>)
  * [सोच + वर्बोज़](</hi/tools/thinking>)
  * [मॉडल](</hi/concepts/models>)
  * [सब-एजेंट](</hi/tools/subagents>)
  * [एजेंट सेंड CLI](</hi/tools/agent-send>)
  * [टर्मिनल UI](</hi/web/tui>)
  * [ब्राउज़र नियंत्रण](</hi/tools/browser>)
  * [ब्राउज़र (Linux समस्या निवारण)](</hi/tools/browser-linux-troubleshooting>)
  * [पोल](</hi/cli/message>)


## Nodes, मीडिया, आवाज़

  * [Nodes अवलोकन](</hi/nodes>)
  * [कैमरा](</hi/nodes/camera>)
  * [छवियां](</hi/nodes/images>)
  * [ऑडियो](</hi/nodes/audio>)
  * [लोकेशन कमांड](</hi/nodes/location-command>)
  * [वॉइस वेक](</hi/nodes/voicewake>)
  * [टॉक मोड](</hi/nodes/talk>)


## प्लेटफ़ॉर्म

  * [प्लेटफ़ॉर्म अवलोकन](</hi/platforms>)
  * [macOS](</hi/platforms/macos>)
  * [iOS](</hi/platforms/ios>)
  * [Android](</hi/platforms/android>)
  * [Windows हब](</hi/platforms/windows>)
  * [Linux](</hi/platforms/linux>)
  * [वेब सतहें](</hi/web>)


## macOS companion ऐप (उन्नत)

  * [macOS डेवलपर सेटअप](</hi/platforms/mac/dev-setup>)
  * [macOS मेनू बार](</hi/platforms/mac/menu-bar>)
  * [macOS वॉइस वेक](</hi/platforms/mac/voicewake>)
  * [macOS वॉइस ओवरले](</hi/platforms/mac/voice-overlay>)
  * [macOS WebChat](</hi/platforms/mac/webchat>)
  * [macOS Canvas](</hi/platforms/mac/canvas>)
  * [macOS चाइल्ड प्रोसेस](</hi/platforms/mac/child-process>)
  * [macOS स्वास्थ्य](</hi/platforms/mac/health>)
  * [macOS आइकन](</hi/platforms/mac/icon>)
  * [macOS लॉगिंग](</hi/platforms/mac/logging>)
  * [macOS अनुमतियां](</hi/platforms/mac/permissions>)
  * [macOS रिमोट](</hi/platforms/mac/remote>)
  * [macOS साइनिंग](</hi/platforms/mac/signing>)
  * [macOS Gateway (launchd)](</hi/platforms/mac/bundled-gateway>)
  * [macOS XPC](</hi/platforms/mac/xpc>)
  * [macOS Skills](</hi/platforms/mac/skills>)
  * [macOS Peekaboo](</hi/platforms/mac/peekaboo>)


## Plugins

  * [Plugins अवलोकन](</hi/tools/plugin>)
  * [Plugins बनाना](</hi/plugins/building-plugins>)
  * [Plugin हुक](</hi/plugins/hooks>)
  * [Plugin मैनिफ़ेस्ट](</hi/plugins/manifest>)
  * [एजेंट टूल्स](</hi/plugins/building-plugins#registering-agent-tools>)
  * [Plugin बंडल](</hi/plugins/bundles>)
  * [ClawHub](</hi/clawhub>)
  * [क्षमता कुकबुक](</hi/plugins/adding-capabilities>)
  * [वॉइस कॉल Plugin](</hi/plugins/voice-call>)
  * [Zalo उपयोगकर्ता Plugin](</hi/plugins/zalouser>)


## वर्कस्पेस + टेम्पलेट

  * [Skills](</hi/tools/skills>)
  * [ClawHub](</hi/clawhub>)
  * [Skills कॉन्फ़िगरेशन](</hi/tools/skills-config>)
  * [डिफ़ॉल्ट AGENTS](</hi/reference/AGENTS.default>)
  * [टेम्पलेट: AGENTS](</hi/reference/templates/AGENTS>)
  * [टेम्पलेट: BOOTSTRAP](</hi/reference/templates/BOOTSTRAP>)
  * [टेम्पलेट: HEARTBEAT](</hi/reference/templates/HEARTBEAT>)
  * [टेम्पलेट: IDENTITY](</hi/reference/templates/IDENTITY>)
  * [टेम्पलेट: SOUL](</hi/reference/templates/SOUL>)
  * [टेम्पलेट: TOOLS](</hi/reference/templates/TOOLS>)
  * [टेम्पलेट: USER](</hi/reference/templates/USER>)


## प्रोजेक्ट

  * [श्रेय](</hi/reference/credits>)


## परीक्षण + रिलीज़

  * [परीक्षण](</hi/reference/test>)
  * [रिलीज़ नीति](</hi/reference/RELEASING>)
  * [डिवाइस मॉडल](</hi/reference/device-models>)


## संबंधित

  * [शुरू करना](</hi/start/getting-started>)


Was this useful?YesNo

Open issue