---
title: Podman
source_url: https://docs.openclaw.ai/zh-TW/install/podman
...
在 macOS 上，Podman machine 可能會讓瀏覽器對 Gateway 而言看起來不是本機。 如果控制 UI 在啟動後回報裝置驗證錯誤，請使用 Podman 與 Tailscale 中的 Tailscale 指引。
...
預設情況下，這些是主機目錄，而不是匿名容器狀態，因此 `openclaw.json`、每個 agent 的 `auth-profiles.json`、channel/provider 狀態、 sessions 和工作區會在容器替換後保留下來。 Podman 設定也會為已發布 Gateway 連接埠上的 `127.0.0.1` 和 `localhost` 種子設定 `gateway.controlUi.allowedOrigins`，讓本機 dashboard 可搭配容器的非 loopback 綁定運作。
...
## 實用命令
  ...
  * **透過主機 CLI 取得健康狀態/狀態：** `openclaw gateway status --deep`（RPC probe + 額外 服務掃描）
