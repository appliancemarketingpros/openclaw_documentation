---
title: Signal
source_url: https://docs.openclaw.ai/fa/channels/signal
...
اگر از ساخت JVM (`signal-cli-${VERSION}.tar.gz`) استفاده می‌کنید، ابتدا JRE 25+ را نصب کنید. `signal-cli` را به‌روز نگه دارید؛ بالادست اشاره می‌کند که نسخه‌های قدیمی با تغییر APIهای سرور Signal ممکن است از کار بیفتند.
...
گزینه‌های ارائه‌دهنده:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (پیش‌فرض: auto). حالت کانتینر را ببینید.
