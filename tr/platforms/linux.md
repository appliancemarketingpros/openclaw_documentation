...
source_url: https://docs.openclaw.ai/tr/platforms/linux
scraped_at: 2026-05-25
---

Gateway, Linux üzerinde tamamen desteklenir. **Node önerilen çalışma zamanıdır**. Bun, Gateway için önerilmez (WhatsApp/Telegram hataları).
...
OpenClaw varsayılan olarak bir systemd **kullanıcı** hizmeti kurar. Paylaşılan veya her zaman açık sunucular için bir **sistem** hizmeti kullanın. `openclaw gateway install` ve `openclaw onboard --install-daemon` sizin için zaten geçerli kanonik birimi oluşturur; yalnızca özel bir sistem/hizmet yöneticisi kurulumu gerektiğinde elle yazın. Tam hizmet rehberi [Gateway çalışma kitabı](</tr/gateway>) içindedir.
...
Uygun Linux alt süreç oluşturma işlemleri için OpenClaw, alt süreci, alt sürecin kendi `oom_score_adj` değerini `1000` seviyesine yükselten ve ardından gerçek komutu `exec` eden kısa bir `/bin/sh` sarmalayıcısı üzerinden başlatır. Bu ayrıcalıksız bir işlemdir çünkü alt süreç yalnızca kendi OOM sonlandırılma olasılığını artırır.
...
Sarmalayıcı yalnızca Linux içindir ve `/bin/sh` kullanılamadığında atlanır. Alt süreç env değeri `OPENCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`, `no` veya `off` olarak ayarlanmışsa da atlanır.
