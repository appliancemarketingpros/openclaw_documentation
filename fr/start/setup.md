---
title: Configuration
source_url: https://docs.openclaw.ai/fr/start/setup
...
Si vous voulez une configuration « 100 % adaptée à moi » _et_ des mises à jour faciles, gardez votre personnalisation dans :
...
Utilisez ceci pour déboguer l’authentification ou décider quoi sauvegarder :
  ...
  * **Import OAuth hérité** : `~/.openclaw/credentials/oauth.json` Plus de détails : [Sécurité](</fr/gateway/security#credential-storage-map>).
...
Pour des serveurs toujours actifs ou multi-utilisateurs, envisagez plutôt un service **système** qu’un service utilisateur (pas besoin de persistance). Consultez le [runbook Gateway](</fr/gateway>) pour les notes systemd.
