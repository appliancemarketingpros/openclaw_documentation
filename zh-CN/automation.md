---
title: 自动化
source_url: https://docs.openclaw.ai/zh-CN/automation
...
后台任务台账会跟踪所有脱离主流程的工作：ACP 运行、子智能体启动、隔离的 cron 执行和 CLI 操作。任务是记录，而不是调度器。使用 `openclaw tasks list` 和 `openclaw tasks audit` 查看它们。
...
内部钩子是由智能体生命周期事件（`/new`、`/reset`、`/stop`）、会话压缩、Gateway 网关启动和消息流触发的事件驱动脚本。它们会从目录中自动发现，并可用 `openclaw hooks` 管理。对于进程内工具调用拦截，请使用[插件钩子](</zh-CN/plugins/hooks>)。
...
## 相关内容
  ...
  * [推断式跟进承诺](</zh-CN/concepts/commitments>) — 类似记忆的跟进回访
