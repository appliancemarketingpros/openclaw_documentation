...
source_url: https://docs.openclaw.ai/pl/channels/imessage-from-bluebubbles
...
`channels status --probe` sprawdza sondą tylko skonfigurowane, włączone konta. Nie restartuj Gateway z włączonymi jednocześnie BlueBubbles i iMessage, chyba że celowo chcesz uruchomić oba monitory kanałów. Jeśli nie przełączasz ruchu natychmiast, przed restartem Gateway ustaw `channels.imessage.enabled` z powrotem na `false`. Użyj bezpośrednich poleceń `imsg` z sekcji Zanim zaczniesz, aby zweryfikować Maca przed włączeniem ruchu OpenClaw.
