...
source_url: https://docs.openclaw.ai/fr/channels
scraped_at: 2026-05-25
---

OpenClaw peut vous parler sur n’importe quelle application de chat que vous utilisez déjà. Chaque canal se connecte via le Gateway. Le texte est pris en charge partout ; les médias et les réactions varient selon le canal.

## Notes de livraison

  * Les réponses Telegram qui contiennent une syntaxe d’image Markdown, telle que `![alt](url)`, sont converties en réponses multimédias sur le chemin sortant final lorsque c’est possible.
  * Les DM Slack à plusieurs personnes sont acheminés comme des discussions de groupe ; la politique de groupe, le comportement des mentions et les règles de session de groupe s’appliquent donc aux conversations MPIM.
  * La configuration de WhatsApp s’installe à la demande : l’onboarding peut afficher le flux de configuration avant que le package Plugin soit installé, et le Gateway charge le runtime WhatsApp uniquement lorsque le canal est réellement actif.
...
## Notes
  ...
  * La configuration la plus rapide est généralement **Telegram** (simple jeton de bot). WhatsApp nécessite un appairage par QR code et stocke davantage d’état sur le disque.
