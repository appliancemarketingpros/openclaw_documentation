---
title: Discord
source_url: https://docs.openclaw.ai/zh-TW/channels/discord
scraped_at: 2026-05-25
---

準備好透過官方 Discord gateway 使用私訊與 guild 頻道。

[**配對** Discord 私訊預設使用配對模式。 ](</zh-TW/channels/pairing>) [**Slash commands** 原生指令行為與指令目錄。 ](</zh-TW/tools/slash-commands>) [**頻道疑難排解** 跨頻道診斷與修復流程。 ](</zh-TW/channels/troubleshooting>)
...
這是一般文字頻道的基準集合。如果你計畫在 Discord threads 中發文，包括會建立或延續 thread 的論壇或媒體頻道工作流程，也請啟用 **Send Messages in Threads** 。 複製底部產生的 URL，貼到瀏覽器中，選取你的伺服器，然後點擊 **Continue** 進行連線。你現在應該能在 Discord 伺服器中看到你的 bot。
...
如果 OpenClaw 已作為背景服務執行，請透過 OpenClaw Mac app 重新啟動，或停止並重新啟動 `openclaw gateway run` 程序。 對於受管理的服務安裝，請從存在 `DISCORD_BOT_TOKEN` 的 shell 執行 `openclaw gateway install`，或將變數儲存在 `~/.openclaw/.env`，如此服務才能在重新啟動後解析 env SecretRef。 如果你的主機被 Discord 的啟動應用程式查詢封鎖或受到速率限制，請從 Developer Portal 設定 Discord application/client ID，讓啟動可略過該 REST 呼叫。預設帳號使用 `channels.discord.applicationId`，如果執行多個 Discord bots，則使用 `channels.discord.accounts.<accountId>.applicationId`。
...
預設情況下，長期記憶（[MEMORY.md](<http://MEMORY.md>)）只會載入私訊工作階段。Guild 頻道不會自動載入 [MEMORY.md](<http://MEMORY.md>)。
...
## Runtime model
  ...
  * 傳送到 Discord 的純文字 cron/heartbeat 公告會使用最終的 助理可見答案一次。當代理發出多個可傳遞承載時，媒體和結構化元件承載仍會保持 多訊息形式。
...
`requireMention` 依伺服器／頻道設定（`channels.discord.guilds...`）。 `ignoreOtherMentions` 可選擇性丟棄提及其他使用者／角色但未提及機器人的訊息（不含 @everyone/@here）。
...
注意：`off` 會停用隱含回覆串接。明確的 `[[reply_to_*]]` 標籤仍會被採用。 `first` 一律會把隱含的原生回覆參照附加到該回合的第一則外送 Discord 訊息。 `batched` 只會在傳入回合是由多則訊息防抖批次組成時，附加 Discord 的隱含原生回覆參照。當你希望原生回覆主要用於模糊的爆發式聊天，而非每個單訊息回合時，這會很有用。
...
Discord 也會呈現其他聊天頻道使用的共用核准按鈕。原生 Discord 轉接器主要新增核准者 DM 路由與頻道扇出。 當這些按鈕存在時，它們是主要的核准使用者體驗；OpenClaw 只有在工具結果指出聊天核准不可用，或手動核准是唯一途徑時， 才應包含手動 `/approve` 命令。 如果 Discord 原生核准執行階段未啟用，OpenClaw 會保留 本機決定性的 `/approve <id> <decision>` 提示可見。如果 執行階段已啟用，但原生卡片無法傳送至任何目標， OpenClaw 會在同一聊天中傳送備援通知，其中包含待處理核准的確切 `/approve` 命令。
...
[**配對** 將 Discord 使用者與 gateway 配對。 ](</zh-TW/channels/pairing>) [**群組** 群組聊天與允許清單行為。 ](</zh-TW/channels/groups>) [**Channel 路由** 將傳入訊息路由到 agent。 ](</zh-TW/channels/channel-routing>) [**安全性** 威脅模型與強化。 ](</zh-TW/gateway/security>) [**多 agent 路由** 將 guild 和 channel 對應到 agent。 ](</zh-TW/concepts/multi-agent>) [**Slash commands** 原生命令行為。 ](</zh-TW/tools/slash-commands>)
