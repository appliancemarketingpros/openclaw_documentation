---
title: Podman
source_url: https://docs.openclaw.ai/tr/install/podman
...
Varsayılan olarak bunlar anonim kapsayıcı durumu değil, ana makine dizinleridir; bu nedenle `openclaw.json`, ajan başına `auth-profiles.json`, kanal/sağlayıcı durumu, oturumlar ve çalışma alanı kapsayıcı değişiminden sonra korunur. Podman kurulumu ayrıca yerel panonun kapsayıcının loopback olmayan bağlamasıyla çalışması için yayımlanan Gateway bağlantı noktasında `127.0.0.1` ve `localhost` için `gateway.controlUi.allowedOrigins` değerlerini hazırlar.
...
## Yararlı komutlar
  ...
  * **Ana makine CLI üzerinden sağlık/durum:** `openclaw gateway status --deep` (RPC yoklaması + ek servis taraması)
