---
title: Strumento PDF
source_url: https://docs.openclaw.ai/it/tools/pdf
...
Note sulla disponibilità:
  ...
  * La catena di fallback è consapevole dell'autenticazione. Un `provider/model` configurato conta solo se OpenClaw può effettivamente autenticare quel provider per l'agente.
...
La modalità nativa viene usata per i provider `anthropic` e `google`. Lo strumento invia i byte PDF grezzi direttamente alle API del provider.

Limiti della modalità nativa:

  * `pages` non è supportato. Se impostato, lo strumento restituisce un errore.
  * L'input multi-PDF è supportato; ogni PDF viene inviato come blocco documento nativo / parte PDF inline prima del prompt.
...
Dettagli del fallback:
  ...
  * Il fallback di estrazione usa il Plugin `document-extract` incluso. Il Plugin possiede `pdfjs-dist`; `@napi-rs/canvas` viene usato solo quando è disponibile il fallback di rendering immagini.
