---
title: 概览
source_url: https://docs.openclaw.ai/zh-CN/tools
...
如果你需要... | 首先使用 | 然后阅读  
---|---|---  
让智能体使用现有能力执行操作 | 内置工具 | 工具类别  
控制智能体可以调用的内容 | 工具策略 | [工具和自定义提供商](</zh-CN/gateway/config-tools>)  
教智能体一个工作流 | Skills | [Skills](</zh-CN/tools/skills>) 和 [创建技能](</zh-CN/tools/creating-skills>)  
添加新的集成或运行时表面 | 插件 | [插件](</zh-CN/tools/plugin>) 和 [构建插件](</zh-CN/plugins/building-plugins>)  
...
工具是智能体可以调用的类型化函数，例如 `exec`、`browser`、 `web_search`、`message` 或 `image_generate`。当智能体需要读取数据、更改文件、发送消息、调用提供商或操作另一个系统时，请使用工具。可见工具会作为结构化函数定义发送给模型。
