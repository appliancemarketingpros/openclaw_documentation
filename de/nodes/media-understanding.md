...
source_url: https://docs.openclaw.ai/de/nodes/media-understanding
...
Primäre/Fallback-Referenzen von `agents.defaults.imageModel` (nur Bild). `provider/model`-Referenzen bevorzugen. Einfache Referenzen werden nur dann aus konfigurierten bildfähigen Provider-Modelleinträgen qualifiziert, wenn die Übereinstimmung eindeutig ist.
...
Gebündelte Fallback-Reihenfolge:
  ...
  * Bild: OpenAI → Anthropic → Google → MiniMax → MiniMax Portal → [Z.AI](<http://Z.AI>)
