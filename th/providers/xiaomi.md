---
title: Xiaomi MiMo
source_url: https://docs.openclaw.ai/th/providers/xiaomi
...
Plugin `xiaomi` ที่มาพร้อมในตัวยังลงทะเบียน Xiaomi MiMo เป็นผู้ให้บริการเสียงพูดสำหรับ `messages.tts` ด้วย โดยจะเรียกสัญญา TTS แบบ chat-completions ของ Xiaomi พร้อมข้อความเป็น ข้อความ `assistant` และคำแนะนำสไตล์ที่ไม่บังคับเป็นข้อความ `user`
...
เสียงในตัวที่รองรับประกอบด้วย `mimo_default`, `default_zh`, `default_en`, `Mia`, `Chloe`, `Milo` และ `Dean` รองรับ `mimo-v2-tts` สำหรับบัญชี MiMo TTS รุ่นเก่า; ค่าเริ่มต้นใช้โมเดล TTS MiMo-V2.5 ปัจจุบัน สำหรับเป้าหมาย ข้อความเสียง เช่น Feishu และ Telegram, OpenClaw จะแปลงเอาต์พุตของ Xiaomi เป็น Opus 48kHz ด้วย `ffmpeg` ก่อนส่งมอบ
...
[**Model selection** การเลือกผู้ให้บริการ การอ้างอิงโมเดล และพฤติกรรมการเปลี่ยนไปใช้ตัวสำรอง ](</th/concepts/model-providers>) [**Configuration reference** เอกสารอ้างอิงการกำหนดค่า OpenClaw ฉบับเต็ม ](</th/gateway/configuration-reference>) [**Xiaomi MiMo console** แดชบอร์ด Xiaomi MiMo และการจัดการ API key ](<https://platform.xiaomimimo.com>)
