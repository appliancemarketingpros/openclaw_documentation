---
title: บรรทัด
source_url: https://docs.openclaw.ai/th/channels/line
scraped_at: 2026-05-25
---

LINE เชื่อมต่อกับ OpenClaw ผ่าน LINE Messaging API โดย Plugin ทำงานเป็นตัวรับ Webhook บน Gateway และใช้ channel access token + channel secret ของคุณสำหรับ การยืนยันตัวตน

สถานะ: Plugin ที่ดาวน์โหลดได้ รองรับข้อความส่วนตัว แชทกลุ่ม สื่อ ตำแหน่งที่ตั้ง ข้อความ Flex ข้อความ template และ quick replies ไม่รองรับ reactions และ threads
...
## ตั้งค่า
  ...
  1. สร้างบัญชี LINE Developers และเปิด Console: <https://developers.line.biz/console/>
...
Gateway ตอบกลับการตรวจสอบ Webhook ของ LINE (GET) และเหตุการณ์ขาเข้า (POST) หากคุณต้องใช้ path แบบกำหนดเอง ให้ตั้งค่า `channels.line.webhookPath` หรือ `channels.line.accounts.<id>.webhookPath` แล้วอัปเดต URL ให้ตรงกัน
...
ข้อความส่วนตัวมีค่าเริ่มต้นเป็นการจับคู่ ผู้ส่งที่ไม่รู้จักจะได้รับรหัสจับคู่ และ ข้อความของพวกเขาจะถูกละเว้นจนกว่าจะได้รับการอนุมัติ
...
## พฤติกรรมของข้อความ
  ...
  * สื่อขาเข้าจะถูกบันทึกไว้ใต้ `~/.openclaw/media/inbound/` ก่อนส่งต่อ ไปยัง agent ให้ตรงกับ media store ร่วมที่ใช้โดย Plugin channel แบบ bundled อื่นๆ
...
## การแก้ไขปัญหา
  ...
  * **การตรวจสอบ Webhook ล้มเหลว:** ตรวจสอบให้แน่ใจว่า URL ของ Webhook เป็น HTTPS และ `channelSecret` ตรงกับ LINE console
  * **ไม่มีเหตุการณ์ขาเข้า:** ยืนยันว่า path ของ Webhook ตรงกับ `channels.line.webhookPath` และ Gateway สามารถเข้าถึงได้จาก LINE
  * **ข้อผิดพลาดในการดาวน์โหลดสื่อ:** เพิ่ม `channels.line.mediaMaxMb` หากสื่อเกิน ขีดจำกัดเริ่มต้น
