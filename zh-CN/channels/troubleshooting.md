---
title: 渠道故障排除
source_url: https://docs.openclaw.ai/zh-CN/channels/troubleshooting
...
随机断开连接/重新登录循环 | `openclaw channels status --probe` \+ 日志 | 即使当前已连接，最近的重连也会被标记；观察日志，重启 Gateway 网关，如果仍持续抖动则重新关联。  
回复延迟数秒/数分钟才到达 | `openclaw doctor --fix` | 当已验证的陈旧本地 TUI 客户端正在拖慢 Gateway 网关事件循环时，Doctor 会停止它们。  
...
机器人在线但群组保持静默 | 验证提及要求和机器人隐私模式 | 禁用隐私模式以允许群组可见性，或提及机器人。  
发送失败并出现网络错误 | 检查日志中的 Telegram API 调用失败 | 修复到 `api.telegram.org` 的 DNS/IPv6/代理路由。  
启动报告 `getMe returned 401` | 检查已配置的令牌来源 | 重新复制或重新生成 BotFather 令牌，并更新 `botToken`、`tokenFile` 或默认账户的 `TELEGRAM_BOT_TOKEN`。  
轮询停滞或重连缓慢 | 用 `openclaw logs --follow` 查看轮询诊断 | 升级；如果重启是假阳性，请调优 `pollingStallThresholdMs`。持续停滞仍指向代理/DNS/IPv6 问题。  
...
症状 | 最快检查 | 修复  
---|---|---  
机器人在线但没有服务器回复 | `openclaw channels status --probe` | 允许服务器/频道，并验证消息内容意图。  
群组消息被忽略 | 检查日志中因提及门控而丢弃的记录 | 提及机器人，或设置服务器/频道 `requireMention: false`。  
有输入状态/令牌使用但没有 Discord 消息 | 会话日志显示助手文本且 `didSendViaMessagingTool: false` | 模型私下回答了，而不是调用消息工具。使用可靠调用工具的模型，或设置 `messages.groupChat.visibleReplies: "automatic"` 以自动发布。  
...
症状 | 最快检查 | 修复  
---|---|---  
Socket mode 已连接但没有响应 | `openclaw channels status --probe` | 验证 app token + bot token 和所需 scopes；在基于 SecretRef 的设置中留意 `botTokenStatus` / `appTokenStatus = configured_unavailable`。  
...
在 macOS 上可以发送但无法接收 | 检查 macOS 对 Messages 自动化的隐私权限 | 重新授予 TCC 权限，并重启渠道进程。  
...
症状 | 最快检查 | 修复  
---|---|---  
机器人回复“gone to Mars” | 验证配置中的 `appId` 和 `clientSecret` | 设置凭据，或重启 Gateway 网关。  
...
主动消息没有到达 | 检查 QQ 平台交互要求 | 如果近期没有交互，QQ 可能会阻止机器人发起的消息。  
...
备份恢复处于等待状态/已损坏 | `openclaw matrix verify backup status` | 运行 `openclaw matrix verify backup restore`，或使用恢复密钥重新运行。  
