...
source_url: https://docs.openclaw.ai/it/tools/thinking
...
## Che cosa fa
  ...
  * Note sui provider: 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) supporta solo il pensiero binario (`on`/`off`). Qualsiasi livello diverso da `off` viene trattato come `on` (mappato a `low`).
