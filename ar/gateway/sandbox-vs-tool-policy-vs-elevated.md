...
source_url: https://docs.openclaw.ai/ar/gateway/sandbox-vs-tool-policy-vs-elevated
...
قواعد عامة:
  ...
  * يغيّر `/exec` افتراضيات الجلسة فقط للمرسلين المصرح لهم؛ ولا يمنح وصولًا إلى الأدوات. تقبل مفاتيح أدوات المزوّد إما `provider` (مثل `google-antigravity`) أو `provider/model` (مثل `openai/gpt-5.4`).
...
المجموعات المتاحة:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` للوكلاء المخصصين للقراءة فقط، احظر `group:runtime` وكذلك أدوات نظام الملفات المعدّلة، ما لم تفرض سياسة نظام ملفات وضع الحماية أو حد مضيف منفصل قيد القراءة فقط.
