---
title: خرچنگ دریایی
source_url: https://docs.openclaw.ai/fa/tools/lobster
...
OpenClaw گردش‌کارهای Lobster را با استفاده از یک اجراکنندهٔ توکار **درون‌فرایندی** اجرا می‌کند. هیچ زیرفرایند CLI خارجی ایجاد نمی‌شود؛ موتور گردش‌کار داخل فرایند Gateway اجرا می‌شود و یک پوشش JSON را مستقیماً برمی‌گرداند. اگر پایپ‌لاین برای تأیید مکث کند، ابزار یک `resumeToken` برمی‌گرداند تا بتوانید بعداً ادامه دهید.
...
برای گردش‌کارهایی که به یک **مرحلهٔ LLM ساخت‌یافته** نیاز دارند، ابزار Plugin اختیاری `llm-task` را فعال کنید و آن را از Lobster فراخوانی کنید. این کار گردش‌کار را قطعی نگه می‌دارد و همچنان اجازه می‌دهد با یک مدل طبقه‌بندی/خلاصه‌سازی/پیش‌نویس‌سازی کنید.
