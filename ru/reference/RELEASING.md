...
source_url: https://docs.openclaw.ai/ru/reference/RELEASING
...
Изменение dist-tag находится в репозитории реестра релизов, потому что для него все еще требуется `NPM_TOKEN`, тогда как исходный репозиторий сохраняет публикацию только через OIDC.

Так и прямой путь публикации, и путь продвижения сначала beta остаются задокументированными и видимыми оператору.

Если сопровождающему нужно откатиться к локальной npm-аутентификации, запускайте любые команды 1Password CLI (`op`) только внутри выделенной сессии tmux. Не вызывайте `op` напрямую из основной оболочки агента; удержание его внутри tmux делает prompts, alerts и обработку OTP наблюдаемыми и предотвращает повторяющиеся host alerts.
...
Сопровождающие используют закрытую релизную документацию в [`openclaw/maintainers/release/README.md`](<https://github.com/openclaw/maintainers/blob/main/release/README.md>) для фактического runbook.
