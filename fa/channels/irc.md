---
title: IRC
source_url: https://docs.openclaw.ai/fa/channels/irc
scraped_at: 2026-05-25
---

وقتی OpenClaw را در کانال‌های کلاسیک (`#room`) و پیام‌های مستقیم می‌خواهید، از IRC استفاده کنید. IRC به‌صورت یک Plugin بسته‌بندی‌شده عرضه می‌شود، اما در پیکربندی اصلی زیر `channels.irc` تنظیم می‌شود.
...
ورودی‌های فهرست مجاز باید از هویت‌های پایدار فرستنده (`nick!user@host`) استفاده کنند. تطبیق nick تنها قابل‌تغییر است و فقط وقتی فعال می‌شود که `channels.irc.dangerouslyAllowNameMatching: true` باشد.
...
اگر `allowFrom: ["*"]` را در یک کانال عمومی مجاز کنید، هر کسی می‌تواند به بات prompt بدهد. برای کاهش ریسک، ابزارها را برای آن کانال محدود کنید.
...
یادداشت‌ها:
  ...
  * کلیدهای `toolsBySender` باید برای مقادیر هویت فرستندهٔ IRC از `id:` استفاده کنند: `id:eigen` یا `id:eigen!~eigen@174.127.248.171` برای تطبیق قوی‌تر.
