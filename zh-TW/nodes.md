---
title: Node
source_url: https://docs.openclaw.ai/zh-TW/nodes
...
舊版傳輸：[Bridge 通訊協定](</zh-TW/gateway/bridge-protocol>)（TCP JSONL； 對目前 Node 僅作歷史用途）。

macOS 也可以在 **Node 模式** 中執行：選單列 app 會連接到 Gateway 的 WS 伺服器，並將其本機 canvas/camera 命令作為 Node 暴露（因此 `openclaw nodes …` 可對這台 Mac 運作）。在遠端 gateway 模式中，瀏覽器 自動化由 CLI Node 主機（`openclaw node run` 或已安裝的 Node 服務）處理，而不是由原生 app Node 處理。
...
**WS Node 使用裝置配對。** Node 會在 `connect` 期間呈現裝置身分；Gateway 會為 `role: node` 建立裝置配對請求。請透過 devices CLI（或 UI）核准。
...
如果 Node 以變更後的驗證詳細資料（role/scopes/public key）重試，先前的 待處理請求會被取代，並建立新的 `requestId`。核准前請重新執行 `openclaw devices list`。

注意事項：

  * 當裝置配對 role 包含 `node` 時，`nodes status` 會將 Node 標記為**已配對** 。
  * 裝置配對記錄是持久的已核准 role 合約。Token 輪替會保留在該合約內；它不能將已配對的 Node 升級為 配對核准從未授予的不同 role。
  * `node.pair.*`（CLI：`openclaw nodes pending/approve/reject/remove/rename`）是另一個由 gateway 擁有的 Node 配對儲存區；它**不會** 管制 WS `connect` 握手。
  * `openclaw nodes remove --node <id|name|ip>` 會從該 另一個由 gateway 擁有的 Node 配對儲存區刪除過期項目。
  * 核准範圍會依照待處理請求宣告的命令： 
...
當你的 Gateway 在一台機器上執行，而你想在另一台機器上執行命令時，請使用 **Node 主機** 。模型仍會與 **gateway** 通訊；當選取 `host=node` 時，gateway 會將 `exec` 呼叫轉送到 **Node 主機** 。
...
核准注意事項：
  ...
  * 對於直接的 shell/runtime 檔案執行，OpenClaw 也會盡力綁定一個具體的本機 檔案運算元，並在該檔案於執行前變更時拒絕執行。
  * 如果 OpenClaw 無法為 interpreter/runtime 命令精確識別一個具體的本機檔案， 以核准為基礎的執行會遭拒，而不是假裝具備完整 runtime 涵蓋範圍。請使用沙盒、 獨立主機，或明確信任的 allowlist/完整工作流程，以支援更廣泛的 interpreter 語意。
...
如果 Gateway 綁定到 loopback（`gateway.bind=loopback`，本機模式預設）， 遠端 Node 主機無法直接連線。請建立 SSH tunnel，並將 Node 主機指向 tunnel 的本機端。
...
如果 Node 以變更後的驗證詳細資料重試，請重新執行 `openclaw devices list` 並核准目前的 `requestId`。
...
設定後，任何帶有 `host=node` 的 `exec` 呼叫都會在 Node 主機上執行（受 Node allowlist/核准約束）。
...
Windows 和 macOS 配套 Node 預設允許安全的已宣告命令，例如 `canvas.*`、`camera.list`、`location.get` 和 `screen.snapshot`。 宣告 `talk` capability 或宣告 `talk.*` 命令的受信任 Node 也預設允許已宣告的 push-to-talk 命令（`talk.ptt.start`、`talk.ptt.stop`、 `talk.ptt.cancel`、`talk.ptt.once`），不受平台標籤影響。 危險或涉及高度隱私的命令，例如 `camera.snap`、`camera.clip` 和 `screen.record`，仍需要透過 `gateway.nodes.allowCommands` 明確 opt-in。`gateway.nodes.denyCommands` 永遠優先於 預設值和額外 allowlist 項目。

Plugin 擁有的 Node 命令可以新增 Gateway node-invoke 政策。該政策 會在 allowlist 檢查後、轉送到 Node 前執行，因此原始 `node.invoke`、CLI helper 和專用 agent 工具都共用相同的 Plugin 權限邊界。危險的 Plugin Node 命令仍需要明確的 `gateway.nodes.allowCommands` opt-in。

Node 變更其宣告的命令清單後，請拒絕舊的裝置配對 並核准新請求，使 gateway 儲存更新後的命令快照。
...
注意事項：
  ...
  * 在 macOS 節點模式中，`system.run` 會由 macOS 應用程式中的 exec 核准閘控（Settings → Exec approvals）。 ask/allowlist/full 的行為與無頭節點主機相同；遭拒絕的提示會傳回 `SYSTEM_RUN_DENIED`。
  * 在無頭節點主機上，`system.run` 會由 exec 核准（`~/.openclaw/exec-approvals.json`）閘控。


## Exec 節點繫結

當有多個節點可用時，你可以將 exec 繫結到特定節點。 這會設定 `exec host=node` 的預設節點（也可依每個代理覆寫）。
...
注意事項：
  ...
  * 在 macOS 上，無頭節點主機預設會在本機執行 `system.run`。設定 `OPENCLAW_NODE_EXEC_HOST=app` 可將 `system.run` 路由到配套應用程式的 exec 主機；新增 `OPENCLAW_NODE_EXEC_FALLBACK=0` 可要求使用應用程式主機，並在其無法使用時封閉失敗。
