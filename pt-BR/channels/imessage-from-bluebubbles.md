...
source_url: https://docs.openclaw.ai/pt-BR/channels/imessage-from-bluebubbles
...
`channels status --probe` testa apenas contas configuradas e habilitadas. Não reinicie o Gateway com BlueBubbles e iMessage habilitados ao mesmo tempo, a menos que você queira intencionalmente que ambos os monitores de canal estejam em execução. Se você não for fazer a transição imediatamente, defina `channels.imessage.enabled` de volta como `false` antes de reiniciar o Gateway. Use os comandos diretos `imsg` em Antes de começar para validar o Mac antes de habilitar tráfego do OpenClaw.
