...
`openclaw security audit` emits structured findings keyed by `checkId`. This page is the reference catalog for those IDs. For the high-level threat model and hardening guidance, see [Security](</gateway/security>). High-signal `checkId` values you will most likely see in real deployments (not exhaustive):
...
[Secure file operations](</gateway/security/secure-file-operations>)[Operator scopes](</gateway/operator-scopes>)
