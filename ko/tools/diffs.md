---
title: 차이점
source_url: https://docs.openclaw.ai/ko/tools/diffs
scraped_at: 2026-05-25
---

`diffs`는 짧은 내장 시스템 지침과 변경 내용을 에이전트를 위한 읽기 전용 diff 아티팩트로 변환하는 동반 Skills를 제공하는 선택적 Plugin 도구입니다.
...
활성화되면 Plugin은 간결한 사용 지침을 시스템 프롬프트 공간 앞에 추가하고, 에이전트가 더 자세한 지침이 필요할 때 사용할 수 있는 상세 Skills도 노출합니다.
...
캔버스 우선 플로우: 에이전트는 `mode: "view"`로 `diffs`를 호출하고 `canvas present`로 `details.viewerUrl`을 엽니다.
...
결합: 에이전트는 `mode: "both"`로 `diffs`를 호출하여 한 번의 호출로 두 아티팩트를 모두 가져옵니다.

## 내장 시스템 지침 비활성화

`diffs` 도구는 활성화한 상태로 유지하되 내장 시스템 프롬프트 지침을 비활성화하려면 `plugins.entries.diffs.hooks.allowPromptInjection`을 `false`로 설정합니다.
...
이렇게 하면 diffs Plugin의 `before_prompt_build` 훅이 차단되지만 Plugin, 도구, 동반 Skills는 계속 사용할 수 있습니다.

지침과 도구를 모두 비활성화하려면 대신 Plugin을 비활성화합니다.

## 일반적인 에이전트 워크플로

* ### diffs 호출

에이전트가 입력으로 `diffs` 도구를 호출합니다.
...
이전 및 이후 모드에 표시할 파일 이름입니다.

이전 및 이후 모드의 언어 재정의 힌트입니다. 알 수 없는 값은 일반 텍스트로 대체됩니다.
...
뷰어 및 독립 실행형 파일 출력의 아티팩트 TTL(초)입니다. 최대값은 21600입니다.

뷰어 URL 원본 재정의입니다. Plugin `viewerBaseUrl`을 재정의합니다. `http` 또는 `https`여야 하며 query/hash가 없어야 합니다.
...
검증 및 제한
  ...
  * `patch`를 `before` 또는 `after`와 함께 사용하면 거부됩니다.
  * 렌더링된 파일 안전 제한(PNG와 PDF에 적용): 
...
`"both"` | 뷰어 필드와 파일 필드를 함께 반환합니다. 파일 렌더링이 실패하면 뷰어는 `fileError` 및 `imageError` 별칭과 함께 계속 반환됩니다.  
  
## 접힌 변경되지 않은 섹션

  * 뷰어는 `N unmodified lines`와 같은 행을 표시할 수 있습니다.
  * 해당 행의 확장 컨트롤은 조건부이며 모든 입력 종류에서 보장되지는 않습니다.
  * 확장 컨트롤은 렌더링된 diff에 확장 가능한 컨텍스트 데이터가 있을 때 나타나며, 이는 이전 및 이후 입력에서 일반적입니다.
  * 많은 통합 패치 입력의 경우 생략된 컨텍스트 본문을 파싱된 패치 hunk에서 사용할 수 없으므로, 확장 컨트롤 없이 행이 나타날 수 있습니다. 이는 예상된 동작입니다.
...
명시적 도구 매개변수는 이러한 기본값을 재정의합니다.

### 영구 뷰어 URL 설정

도구 호출에서 `baseUrl`을 전달하지 않을 때 반환되는 뷰어 링크에 대한 Plugin 소유 fallback입니다. `http` 또는 `https`여야 하며 query/hash가 없어야 합니다.
...
## 아티팩트 수명 주기 및 저장소
  ...
  * 만료된 아티팩트는 삭제됩니다.
  * 메타데이터가 없는 경우 fallback 정리는 24시간보다 오래된 오래된 폴더를 제거합니다.
...
뷰어 문서는 해당 자산을 뷰어 URL을 기준으로 해석하므로, 선택적 `baseUrl` 경로 접두사는 두 자산 요청에도 보존됩니다.

URL 구성 동작:

  * 도구 호출 `baseUrl`이 제공되면 엄격한 검증 후 사용됩니다.
  * 그렇지 않고 Plugin `viewerBaseUrl`이 구성되어 있으면 이를 사용합니다.
  * 둘 중 어느 재정의도 없으면 뷰어 URL은 local loopback `127.0.0.1`을 기본값으로 사용합니다.
...
입력 검증 오류
  ...
  * `Invalid baseUrl: ...` — 선택적 경로가 있는 `http(s)` 출처를 사용하고, 쿼리/해시는 사용하지 마세요.
  * `{field} exceeds maximum size (...)` — 페이로드 크기를 줄이세요.
  * 큰 패치 거부 — 패치 파일 수나 전체 줄 수를 줄이세요.
...
뷰어 접근성
  ...
  * 외부 뷰어 액세스를 의도하는 경우에만 `security.allowRemoteViewer`를 활성화하세요.

수정되지 않은 줄 행에 펼치기 버튼이 없음

패치 입력에서 패치가 펼칠 수 있는 컨텍스트를 포함하지 않을 때 발생할 수 있습니다. 이는 예상된 동작이며 뷰어 실패를 의미하지 않습니다.
...
## 운영 지침
  ...
  * 캔버스에서 로컬 대화형 검토에는 `mode: "view"`를 선호하세요.
