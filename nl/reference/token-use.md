...
source_url: https://docs.openclaw.ai/nl/reference/token-use
...
Andere oppervlakken:
  ...
  * **CLI:** `openclaw status --usage` en `openclaw channels list` tonen genormaliseerde quotavensters van providers (`X% left`, geen kosten per antwoord). Huidige providers met gebruiksvenster: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax, Xiaomi en [z.ai](<http://z.ai>).
...
Configureer dit in [Gateway-configuratie](</nl/gateway/configuration>) en bekijk de gedragsdetails in [Sessiesnoei](</nl/concepts/session-pruning>).
