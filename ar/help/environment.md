...
source_url: https://docs.openclaw.ai/ar/help/environment
...
يقرأ OpenClaw متغيرات البيئة `OPENCLAW_*` فقط. البادئات القديمة `CLAWDBOT_*` و`MOLTBOT_*` من الإصدارات السابقة تُتجاهل بصمت.

إذا ظل أي منها معيّنًا في عملية Gateway عند بدء التشغيل، يصدر OpenClaw تحذير إهمال واحدًا من Node (`OPENCLAW_LEGACY_ENV_VARS`) يسرد البادئات المكتشفة والعدد الإجمالي. أعد تسمية كل قيمة باستبدال البادئة القديمة بـ `OPENCLAW_` (مثل `CLAWDBOT_GATEWAY_TOKEN` → `OPENCLAW_GATEWAY_TOKEN`)؛ الأسماء القديمة لا يكون لها أي أثر.
