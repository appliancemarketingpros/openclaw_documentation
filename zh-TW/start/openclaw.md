---
title: 個人助理設定
source_url: https://docs.openclaw.ai/zh-TW/start/openclaw
...
完整工作區配置與備份指南：[代理工作區](</zh-TW/concepts/agent-workspace>) 記憶工作流程：[記憶](</zh-TW/concepts/memory>)
...
預設情況下，OpenClaw 每 30 分鐘執行一次 Heartbeat，提示為： `Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.` 設定 `agents.defaults.heartbeat.every: "0m"` 可停用。
