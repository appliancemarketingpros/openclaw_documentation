---
title: 频道轮次内核
source_url: https://docs.openclaw.ai/zh-CN/plugins/sdk-channel-turn
...
当提供 `log` 回调时，每个阶段都会发出结构化日志事件。请参阅 可观测性。
...
当渠道已经解析路由、构建了 `FinalizedMsgContext`， 并且只需要共享的记录、回复流水线、分发和最终处理 顺序时使用。这是简单内置入站路径的首选形态，否则这些路径 会重复 `createChannelMessageReplyPipeline(...)` 和 `runPrepared(...)` 样板代码。
