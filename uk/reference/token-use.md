...
source_url: https://docs.openclaw.ai/uk/reference/token-use
...
Інші поверхні:
  ...
  * **CLI:** `openclaw status --usage` і `openclaw channels list` показують нормалізовані вікна квот провайдера (`X% left`, а не вартість за відповідь). Поточні провайдери вікон використання: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax, Xiaomi і [z.ai](<http://z.ai>).
...
Налаштуйте це в [конфігурації Gateway](</uk/gateway/configuration>) і перегляньте подробиці поведінки в [Обрізання сесій](</uk/concepts/session-pruning>).
