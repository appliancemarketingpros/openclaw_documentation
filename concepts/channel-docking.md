...
Channel docking is call forwarding for one OpenClaw session. It keeps the same conversation context, but changes where future replies for that session are delivered.
...
Use docking when a task starts in one chat app but the next replies should land somewhere else. Common flow:
...
[Session management](</concepts/session>)[Session pruning](</concepts/session-pruning>)
