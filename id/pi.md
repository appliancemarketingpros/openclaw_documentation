...
source_url: https://docs.openclaw.ai/id/pi
...
Runtime tindakan pesan khusus kanal sekarang berada di direktori ekstensi milik Plugin alih-alih di bawah `src/agents/tools`, misalnya:
...
Compaction otomatis terpicu saat konteks melampaui batas. Tanda-tanda umum pelampauan mencakup `request_too_large`, `context length exceeded`, `input exceeds the maximum number of tokens`, `input token count exceeds the maximum number of input tokens`, `input is too long for the model`, dan `ollama error: context length exceeded`. `compactEmbeddedPiSessionDirect()` menangani compaction manual:
...
Prompt sistem | [AGENTS.md](<http://AGENTS.md>) \+ prompt | Dinamis per kanal/konteks  
