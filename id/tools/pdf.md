---
title: Alat PDF
source_url: https://docs.openclaw.ai/id/tools/pdf
...
Catatan ketersediaan:
  ...
  * Rantai fallback peka autentikasi. `provider/model` yang dikonfigurasi hanya dihitung jika OpenClaw benar-benar dapat mengautentikasi penyedia tersebut untuk agen.
...
Mode native digunakan untuk penyedia `anthropic` dan `google`. Alat mengirim byte PDF mentah langsung ke API penyedia.

Batasan mode native:

  * `pages` tidak didukung. Jika diatur, alat mengembalikan error.
  * Input multi-PDF didukung; setiap PDF dikirim sebagai blok dokumen native / bagian PDF inline sebelum prompt.
...
Detail fallback:
  ...
  * Fallback ekstraksi menggunakan Plugin `document-extract` bawaan. Plugin memiliki `pdfjs-dist`; `@napi-rs/canvas` hanya digunakan ketika fallback rendering gambar tersedia.
