...
source_url: https://docs.openclaw.ai/pl/tools/thinking
...
## Co robi
  ...
  * Uwagi dotyczące dostawców: 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) obsługuje tylko binarne myślenie (`on`/`off`). Każdy poziom inny niż `off` jest traktowany jako `on` (mapowany na `low`).
