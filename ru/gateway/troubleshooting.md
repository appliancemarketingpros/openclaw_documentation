...
source_url: https://docs.openclaw.ai/ru/gateway/troubleshooting
...
OpenClaw рассматривает каждый корень skill как границу containment. Symlink внутри `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills` или `~/.openclaw/skills` пропускается, когда его реальная цель разрешается за пределами этого корня, если цель не является явно доверенной.
...
Не используйте широкие цели, такие как `~`, `/` или целую синхронизируемую папку проекта. Ограничивайте `allowSymlinkTargets` реальным корнем skill, который содержит доверенные каталоги `SKILL.md`.

Если Skill Workshop apply также должен записывать через эти доверенные symlink-пути workspace skill, включите `skills.workshop.allowSymlinkTargetWrites`. Оставляйте это отключенным для общих корней skill только для чтения.
...
Используйте это, когда upstream LLM provider возвращает общий `403`, например `Your request was blocked`.

Не предполагайте, что это всегда проблема конфигурации OpenClaw. Ответ может приходить от upstream security layer, например CDN, WAF, правила bot-management или reverse proxy перед OpenAI-compatible endpoint.
...
1\. Поведение аутентификации и переопределения URL изменилось bashCopy code
[code]
...
2\. Защитные ограничения привязки и аутентификации стали строже bashCopy code
[code]
...
3\. Состояние сопряжения и идентичности устройства изменилось bashCopy code
[code]
