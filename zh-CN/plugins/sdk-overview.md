---
title: 插件 SDK 概览
source_url: https://docs.openclaw.ai/zh-CN/plugins/sdk-overview
...
内置插件必须为每个目标运行时声明 `contracts.agentToolResultMiddleware`， 例如 `["pi", "codex"]`。外部插件不能注册此中间件；对于不需要模型前工具结果时序的工作，请继续使用普通 OpenClaw 插件钩子。旧的仅限 Pi 的嵌入式扩展工厂注册路径已被移除。
...
端到端编写指南见 [CLI 后端插件](</zh-CN/plugins/cli-backend-plugins>)。
...
[**Entry points** `definePluginEntry` 和 `defineChannelPluginEntry` 选项。 ](</zh-CN/plugins/sdk-entrypoints>) [**Runtime helpers** 完整的 `api.runtime` 命名空间参考。 ](</zh-CN/plugins/sdk-runtime>) [**Setup and config** 打包、清单和配置 schema。 ](</zh-CN/plugins/sdk-setup>) [**Testing** 测试工具和 lint 规则。 ](</zh-CN/plugins/sdk-testing>) [**SDK migration** 从已弃用表面迁移。 ](</zh-CN/plugins/sdk-migration>) [**Plugin internals** 深入的架构和能力模型。 ](</zh-CN/plugins/architecture>)
