---
title: IRC
source_url: https://docs.openclaw.ai/zh-TW/channels/irc
scraped_at: 2026-05-25
---

當你想在傳統頻道（`#room`）和直接訊息中使用 OpenClaw 時，請使用 IRC。 IRC 以內建 Plugin 形式提供，但會在主要設定中的 `channels.irc` 下設定。
...
## 安全預設值
  ...
  * IRC 使用 OpenClaw 操作者管理的轉送 Proxy 路由之外的原始 TCP/TLS Socket。在要求所有輸出流量都必須經過該轉送 Proxy 的部署中，除非直接 IRC 輸出已明確核准，否則請設定 `channels.irc.enabled=false`。
...
IRC 頻道有兩個獨立的「閘門」：
  ...
  1. **頻道存取** （`groupPolicy` \+ `groups`）：機器人是否完全接受來自某個頻道的訊息。
...
設定鍵：
  ...
  * 每個頻道的控制（頻道 + 傳送者 + 提及規則）：`channels.irc.groups["#channel"]`
  * `channels.irc.groupPolicy="open"` 允許未設定的頻道（**預設仍需提及才會觸發** ）


允許清單項目應使用穩定的傳送者身分（`nick!user@host`）。 裸暱稱比對是可變的，且只有在 `channels.irc.dangerouslyAllowNameMatching: true` 時才會啟用。
...
...這表示該傳送者未被允許傳送**群組/頻道** 訊息。請用下列任一方式修正：

  * 設定 `channels.irc.groupAllowFrom`（全域套用於所有頻道），或
...
即使頻道已允許（透過 `groupPolicy` \+ `groups`）且傳送者已允許，OpenClaw 在群組情境中預設仍會使用**提及閘控** 。

這表示除非訊息包含符合機器人的提及模式，否則你可能會看到類似 `drop channel … (missing-mention)` 的記錄。

若要讓機器人在 IRC 頻道中**不需要提及也能回覆** ，請停用該頻道的提及閘控：
...
或若要允許**所有** IRC 頻道（不使用每頻道允許清單），且仍可在沒有提及時回覆：
...
如果你在公開頻道中允許 `allowFrom: ["*"]`，任何人都可以提示機器人。 若要降低風險，請限制該頻道的工具。
...
注意事項：
  ...
  * `toolsBySender` 鍵應使用 `id:` 作為 IRC 傳送者身分值： `id:eigen`，或使用 `id:eigen!~eigen@174.127.248.171` 進行更強比對。
  * 舊版未加前綴的鍵仍會被接受，並且只會以 `id:` 方式比對。
  * 第一個符合的傳送者政策會生效；`"*"` 是萬用字元後備。


若要進一步了解群組存取與提及閘控（以及它們如何互動），請參閱：[/channels/groups](</zh-TW/channels/groups>)。
...
`IRC_HOST` 無法從工作區 `.env` 設定；請參閱[工作區 `.env` 檔案](</zh-TW/gateway/security>)。

## 疑難排解

  * 如果機器人已連線但從不在頻道中回覆，請驗證 `channels.irc.groups`，**並** 確認是否因提及閘控而丟棄訊息（`missing-mention`）。如果你想讓它不需 ping 也能回覆，請為該頻道設定 `requireMention:false`。
