...
source_url: https://docs.openclaw.ai/fa/plugins/hooks
...
به‌جای آن، وقتی یک اسکریپت کوچک `HOOK.md` نصب‌شده توسط اپراتور برای رویدادهای فرمان و Gateway مانند `/new`، `/reset`، `/stop`، `agent:bootstrap`، یا `gateway:startup` می‌خواهید، از [قلاب‌های داخلی](</fa/automation/hooks>) استفاده کنید.
