---
title: Telegram
source_url: https://docs.openclaw.ai/zh-CN/channels/telegram
scraped_at: 2026-05-25
---

通过 grammY 支持可用于生产环境的机器人私信和群组。默认模式是长轮询；webhook 模式是可选的。

[**Pairing** Telegram 的默认私信策略是配对。 ](</zh-CN/channels/pairing>) [**Channel troubleshooting** 跨渠道诊断和修复手册。 ](</zh-CN/channels/troubleshooting>) [**Gateway configuration** 完整的渠道配置模式和示例。 ](</zh-CN/gateway/configuration>)
...
环境变量回退：`TELEGRAM_BOT_TOKEN=...`（仅默认账户）。 Telegram **不** 使用 `openclaw channels login telegram`；在配置/环境变量中配置令牌，然后启动 Gateway 网关。
...
注意：`edit` 和 `topic-create` 当前默认启用，且没有单独的 `channels.telegram.actions.*` 开关。 运行时发送使用活动的配置/密钥快照（启动/重新加载），因此操作路径不会在每次发送时执行临时 SecretRef 重新解析。
...
主题继承：主题条目会继承群组设置，除非被覆盖（`requireMention`、`allowFrom`、`skills`、`systemPrompt`、`enabled`、`groupPolicy`）。 `agentId` 仅限主题，不会从群组默认值继承。
...
[**配对** 将 Telegram 用户配对到 Gateway 网关。 ](</zh-CN/channels/pairing>) [**群组** 群组和主题允许列表行为。 ](</zh-CN/channels/groups>) [**频道路由** 将入站消息路由到智能体。 ](</zh-CN/channels/channel-routing>) [**安全** 威胁模型和加固。 ](</zh-CN/gateway/security>) [**多智能体路由** 将群组和主题映射到智能体。 ](</zh-CN/concepts/multi-agent>) [**故障排除** 跨渠道诊断。 ](</zh-CN/channels/troubleshooting>)
