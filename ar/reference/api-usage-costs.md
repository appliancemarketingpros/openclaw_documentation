...
source_url: https://docs.openclaw.ai/ar/reference/api-usage-costs
scraped_at: 2026-05-25
---

تسرد هذه الوثيقة **الميزات التي يمكنها استدعاء مفاتيح API** وأماكن ظهور تكاليفها. وهي تركّز على ميزات OpenClaw التي يمكنها توليد استخدام لدى المزوّدين أو استدعاءات API مدفوعة.
...
**لقطة تكلفة لكل جلسة**
  ...
  * إذا كانت بيانات تعريف الجلسة الحية محدودة، فيمكن لـ `/status` استعادة عدّادات الرموز/ذاكرة التخزين المؤقت وتسمية نموذج وقت التشغيل النشط من أحدث إدخال استخدام في النص المنسوخ. تظل القيم الحية غير الصفرية الحالية ذات أولوية، ويمكن لإجماليات النص المنسوخ بحجم الموجه أن تفوز عندما تكون الإجماليات المخزّنة مفقودة أو أصغر.
...
**تذييل تكلفة لكل رسالة**
  ...
  * ملاحظة Gemini CLI: عندما يعيد CLI مخرجات JSON، يقرأ OpenClaw الاستخدام من `stats`، ويحوّل `stats.cached` إلى `cacheRead`، ويشتق رموز الإدخال من `stats.input_tokens - stats.cached` عند الحاجة.


ملاحظة Anthropic: أخبرنا موظفو Anthropic أن استخدام Claude CLI بنمط OpenClaw مسموح به مجددًا، لذلك يتعامل OpenClaw مع إعادة استخدام Claude CLI واستخدام `claude -p` على أنهما معتمدان لهذا التكامل ما لم تنشر Anthropic سياسة جديدة. ما زالت Anthropic لا توفّر تقديرًا بالدولار لكل رسالة يمكن لـ OpenClaw عرضه في `/usage full`.

**نوافذ استخدام CLI (حصص المزوّدين)**

  * يعرض `openclaw status --usage` و`openclaw channels list` **نوافذ استخدام** المزوّدين (لقطات للحصة، وليست تكاليف لكل رسالة).
  * يتم توحيد المخرجات البشرية إلى `X% left` عبر المزوّدين.
  * مزوّدو نوافذ الاستخدام الحاليون: Anthropic، وGitHub Copilot، وGemini CLI، وOpenAI Codex، وMiniMax، وXiaomi، [وz.ai](<http://xn--z-uoc.ai>).
  * ملاحظة MiniMax: تعني حقولا `usage_percent` / `usagePercent` الخامان الحصة المتبقية، لذلك يعكسهما OpenClaw قبل العرض. تظل الحقول المعتمدة على العدد ذات أولوية عند وجودها. إذا أعاد المزوّد `model_remains`، يفضّل OpenClaw إدخال نموذج الدردشة، ويشتق تسمية النافذة من الطوابع الزمنية عند الحاجة، و يضمّن اسم النموذج في تسمية الخطة.
  * تأتي مصادقة الاستخدام لتلك النوافذ الحصصية من خطافات خاصة بالمزوّد عند توفرها؛ وإلا فيرجع OpenClaw إلى مطابقة بيانات اعتماد OAuth/مفتاح API من ملفات تعريف المصادقة، أو البيئة، أو الإعدادات.
...
يمكن لـ OpenClaw التقاط بيانات الاعتماد من:
  ...
  * **الإعدادات** (`models.providers.*.apiKey`، و`plugins.entries.*.config.webSearch.apiKey`، و`plugins.entries.firecrawl.config.webFetch.apiKey`، و`memorySearch.*`، و`talk.providers.*.apiKey`).
...
يستخدم كل رد أو استدعاء أداة **مزوّد النموذج الحالي** (OpenAI، Anthropic، إلخ). هذا هو المصدر الأساسي للاستخدام والتكلفة.

ويشمل ذلك أيضًا المزوّدين المستضافين بنمط الاشتراك الذين يظلون يفرضون الرسوم خارج واجهة OpenClaw المحلية، مثل **OpenAI Codex** ، و**Alibaba Cloud Model Studio Coding Plan** ، و**MiniMax Coding Plan** ، و**[Z.AI](<http://Z.AI>) / GLM Coding Plan**، ومسار تسجيل دخول Anthropic Claude في OpenClaw مع تمكين **Extra Usage**.
...
يمكن تلخيص/نسخ الوسائط الواردة قبل تشغيل الرد. يستخدم هذا APIs النماذج/المزوّدين.
  ...
  * الصورة: OpenAI / OpenRouter / Anthropic / DeepInfra / Google / MiniMax / Moonshot / Qwen / [Z.AI](<http://Z.AI>).
...
يمكن لتوليد الصور استنتاج مزوّد افتراضي مدعوم بالمصادقة عندما يكون `agents.defaults.imageGenerationModel` غير معيّن. يتطلب توليد الفيديو حاليًا `agents.defaults.videoGenerationModel` صريحًا مثل `qwen/wan2.6-t2v`.

راجع [توليد الصور](</ar/tools/image-generation>)، و[Qwen Cloud](</ar/providers/qwen>)، و[النماذج](</ar/concepts/models>).
...
**رصيد Brave Search المجاني:** تتضمن كل خطة Brave رصيدًا مجانيًا متجددًا بقيمة $5 شهريًا. تبلغ تكلفة خطة Search $5 لكل 1,000 طلب، لذلك يغطي الرصيد 1,000 طلب شهريًا بلا رسوم. اضبط حد الاستخدام في لوحة تحكم Brave لتجنب الرسوم غير المتوقعة.
...
تستدعي بعض أوامر الحالة **نقاط نهاية استخدام المزوّد** لعرض نوافذ الحصص أو صحة المصادقة. عادةً ما تكون هذه استدعاءات منخفضة الحجم، لكنها ما زالت تصل إلى APIs المزوّدين:
...
يمكن لحماية Compaction تلخيص سجل الجلسة باستخدام **النموذج الحالي** ، مما يستدعي APIs المزوّدين عند تشغيلها.
...
يمكن لـ Skills تخزين `apiKey` في `skills.entries.<name>.apiKey`. إذا استخدمت Skills ذلك المفتاح مع APIs خارجية، فقد تتسبب في تكاليف وفقًا لمزوّد Skills.
