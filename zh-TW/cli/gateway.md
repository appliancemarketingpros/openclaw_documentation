---
title: Gateway
source_url: https://docs.openclaw.ai/zh-TW/cli/gateway
scraped_at: 2026-05-25
---

Gateway 是 OpenClaw 的 WebSocket 伺服器（通道、節點、工作階段、hook）。本頁中的子命令位於 `openclaw gateway …` 底下。

[**Bonjour 探索** 本機 mDNS + 廣域 DNS-SD 設定。 ](</zh-TW/gateway/bonjour>) [**探索概觀** OpenClaw 如何公告並尋找 gateway。 ](</zh-TW/gateway/discovery>) [**設定** 頂層 gateway 設定鍵。 ](</zh-TW/gateway/configuration>)
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tcG9ydCA8cG9ydA " type="number"> WebSocket 連接埠（預設來自設定/env；通常是 `18789`）。

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdG9rZW4gPHRva2Vu " type="string"> token 覆寫（也會為程序設定 `OPENCLAW_GATEWAY_TOKEN`）。

關閉時重設 Tailscale serve/funnel 設定。

允許在設定中沒有 `gateway.mode=local` 時啟動 gateway。僅對臨時/開發 bootstrap 繞過啟動防護；不會寫入或修復設定檔。

如果缺少，建立開發設定 + 工作區（略過 [BOOTSTRAP.md](<http://BOOTSTRAP.md>)）。
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tbGltaXQgPGxpbWl0 " type="number" default="25"> 要包含的近期事件數量上限（最大 `1000`）。

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdHlwZSA8dHlwZQ " type="string"> 依診斷事件類型篩選，例如 `payload.large` 或 `diagnostic.memory.pressure`。

讀取已持久化的穩定性套件組合，而不是呼叫執行中的 Gateway。使用 `--bundle latest`（或只用 `--bundle`）讀取狀態目錄下最新的套件組合，或直接傳入套件組合 JSON 路徑。

寫入可分享的支援診斷 zip，而不是列印穩定性詳細資料。

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tb3V0cHV0IDxwYXRo " type="string"> `--export` 的輸出路徑。
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tc3NoIDx0YXJnZXQ " type="string"> `user@host` 或 `user@host:port`（連接埠預設為 `22`）。
...
當受管理服務必須透過另一個可執行檔啟動時，請使用 `--wrapper`，例如 秘密管理器 shim 或 run-as 輔助工具。wrapper 會接收一般 Gateway 參數，並 負責最終以這些參數 exec `openclaw` 或 Node。
...
你也可以透過環境設定 wrapper。`gateway install` 會驗證該路徑是 可執行檔，將 wrapper 寫入服務 `ProgramArguments`，並將 `OPENCLAW_WRAPPER` 持久化到服務環境，以供之後的強制重新安裝、更新和 doctor 修復使用。
