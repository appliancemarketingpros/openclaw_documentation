...
source_url: https://docs.openclaw.ai/es/reference/wizard
scraped_at: 2026-05-25
---

Esta es la referencia completa para `openclaw onboard`. Para una vista general de alto nivel, consulta [Onboarding (CLI)](</es/start/wizard>).
...
* ### Modelo/Auth
  ...
  * **MiniMax** : la configuración se escribe automáticamente; el valor predeterminado alojado es `MiniMax-M2.7`. La configuración con clave de API usa `minimax/...`, y la configuración con OAuth usa `minimax-portal/...`.
...
Los ejemplos de comandos específicos de proveedor están en [Automatización de CLI](</es/start/wizard-cli-automation#provider-specific-examples>). Usa esta página de referencia para la semántica de flags y el orden de los pasos.
...
El Gateway expone el flujo de onboarding mediante RPC (`wizard.start`, `wizard.next`, `wizard.cancel`, `wizard.status`). Los clientes (app de macOS, Control UI) pueden renderizar pasos sin volver a implementar la lógica de onboarding.
