---
title: 設定範例
source_url: https://docs.openclaw.ai/zh-TW/gateway/configuration-examples
...
除非你能控制網路路徑，否則請維持手動裝置配對。對於專用 實驗室或 tailnet 子網路，你可以選擇使用精確的 CIDR 或 IP， 啟用首次 Node 裝置自動核准：
...
未設定時此功能會保持關閉。它只適用於沒有請求範圍的全新 `role: node` 配對。操作員/瀏覽器用戶端，以及角色、範圍、中繼資料或 公開金鑰升級，仍需要手動核准。
...
對於 Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC，傳送者授權預設會優先使用 ID。 只有在你明確接受該風險時，才啟用每個頻道的 `dangerouslyAllowNameMatching: true`，以直接依可變動的名稱/電子郵件/暱稱比對。
