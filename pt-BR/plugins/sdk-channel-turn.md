...
source_url: https://docs.openclaw.ai/pt-BR/plugins/sdk-channel-turn
...
Use quando o canal já tiver resolvido o roteamento, criado um `FinalizedMsgContext`, e só precisar do registro compartilhado, pipeline de resposta, despacho e ordem de finalização. Este é o formato preferido para caminhos de entrada empacotados simples que, caso contrário, repetiriam boilerplate de `createChannelMessageReplyPipeline(...)` e `runPrepared(...)`.
...
Escolha `runAssembled` em vez de `runPrepared` quando o único comportamento de despacho próprio do canal for a entrega final do payload, mais digitação opcional, opções de resposta, entrega durável ou registro de erros.
