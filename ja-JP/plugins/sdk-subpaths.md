...
source_url: https://docs.openclaw.ai/ja-JP/plugins/sdk-subpaths
...
ランタイムとストレージのサブパス サブパス | 主なエクスポート  
---|---  
...
`plugin-sdk/provider-zai-endpoint` | 非推奨の [Z.AI](<http://Z.AI>) プロバイダー所有エンドポイント検出ファサード。[Z.AI](<http://Z.AI>) プラグインの公開 API を使用してください  
...
Capability とテストのサブパス サブパス | 主なエクスポート  
---|---  
...
現在、予約済みのバンドルヘルパー SDK サブパスはありません。所有者固有の ヘルパーは所有元の Plugin パッケージ内に置かれ、再利用可能なホスト契約は `plugin-sdk/gateway-runtime`、`plugin-sdk/security-runtime`、 `plugin-sdk/plugin-config-runtime` などの汎用 SDK サブパスを使用します。
