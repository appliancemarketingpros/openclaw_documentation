...
`plugins.entries.voice-call.config` の下に設定します (完全な形は下の 設定 を参照)。最低限、`provider`、プロバイダー認証情報、 `fromNumber`、および公開到達可能な Webhook URL が必要です。
...
プロバイダー固有のリアルタイム音声オプションについては、 [Google provider](</ja-JP/providers/google>) と [OpenAI provider](</ja-JP/providers/openai>) を参照してください。
