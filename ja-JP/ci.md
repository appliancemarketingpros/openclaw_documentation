...
ステージマトリクス、正確なワークフロージョブ名、プロファイルの違い、アーティファクト、 およびフォーカスされた再実行ハンドルについては、[フルリリース検証](</ja-JP/reference/full-release-validation>) を参照。
...
変化の速いブランチでピン留めされたコミット証明が必要な場合は、 `gh workflow run ... --ref main -f ref=<sha>` の代わりにヘルパーを使用します。
