---
title: Painel
source_url: https://docs.openclaw.ai/pt-BR/cli/dashboard
...
Observações:
  ...
  * `dashboard` segue `gateway.tls.enabled`: instâncias de Gateway com TLS habilitado imprimem/abrem URLs da UI de Controle com `https://` e se conectam por `wss://`.
  * Se a entrega pela área de transferência/navegador falhar para uma URL do painel autenticada por token, `dashboard` registra uma dica segura de autenticação manual nomeando `OPENCLAW_GATEWAY_TOKEN`, `gateway.auth.token` e a chave de fragmento `token` sem imprimir o valor do token.
