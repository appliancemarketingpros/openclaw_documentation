...
For built-in image generation/editing, prefer `agents.defaults.imageGenerationModel` plus the core `image_generate` tool. `skills.entries.*` is only for custom or third-party skill workflows. Examples:
...
[Creating Skills](</tools/creating-skills>)[Slash Commands](</tools/slash-commands>)
