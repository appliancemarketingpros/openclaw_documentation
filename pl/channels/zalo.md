---
title: Zalo
source_url: https://docs.openclaw.ai/pl/channels/zalo
...
### 1) Utwórz token bota (Zalo Bot Platform)
  ...
  1. Przejdź do <https://bot.zaloplatforms.com> i zaloguj się.
...
Opcje dostawcy:
  ...
  * `channels.zalo.groupPolicy`: `open | allowlist | disabled` (domyślnie: allowlist). Obecne w konfiguracji; bieżące działanie botów Marketplace opisują Możliwości i Kontrola dostępu (grupy).
...
Opcje wielu kont:
  ...
  * `channels.zalo.accounts.<id>.groupPolicy`: polityka grupowa dla konta. Obecne w konfiguracji; bieżące działanie botów Marketplace opisują Możliwości i Kontrola dostępu (grupy).
