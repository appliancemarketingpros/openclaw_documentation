---
title: Proxy réseau
source_url: https://docs.openclaw.ai/fr/security/network-proxy
...
## Limites
  ...
  * Le trafic loopback du plan de contrôle du Gateway utilise par défaut un contournement local direct via `proxy.loopbackMode: "gateway-only"`. OpenClaw met en œuvre ce contournement en enregistrant l’autorité loopback du Gateway actif dans le contrôleur `NO_PROXY` `global-agent` géré. Les opérateurs peuvent définir `proxy.loopbackMode: "proxy"` pour envoyer le trafic loopback du Gateway via le proxy géré, ou `proxy.loopbackMode: "block"` pour refuser les connexions loopback du Gateway. Consultez Mode Loopback du Gateway pour la mise en garde liée au proxy distant.
