---
title: 发布性能梳理
source_url: https://docs.openclaw.ai/zh-CN/reference/release-performance-sweep
...
**稳定版智能体轮次** **冷启动轮次快 5.1 倍**

  * `v2026.4.14`: 9.8s
  * `v2026.5.28`: 1.9s

**已发布包** **17.9MB tarball** 最新稳定版包，低于 3 月 43.3MB 的包大小峰值。 **最新稳定版安装** **361.7MiB 全新安装** `v2026.5.28` 大幅削减了嵌套的 OpenClaw 依赖树，但本地安装审计中仍然保留了一个较小的 259.7MiB 嵌套树。 **依赖图** **300 个已安装包** 最新稳定版发布，在禁用脚本的全新安装中按唯一包名/版本根进行测量。

## 安装占用时间线

**月度高点** **645 个依赖项** `2026.2.26` 是此样本中的月度依赖数量高点。 **引入 shrinkwrap** **1,020.6MB 安装** `2026.5.22` 添加了根 shrinkwrap，并暴露出一个包形态问题：911.8MB 落在嵌套的 `openclaw/node_modules` 下。 **最新稳定版** **361.7MiB 安装** `2026.5.28` 相比 `2026.5.27` 将全新安装大小减少了 52.8%，但仍会安装一个 259.7MiB 的嵌套 OpenClaw 树。 **依赖图** **300 个包根** `2026.5.28` 安装的唯一包名/版本根比 `2026.5.27` 少 71 个。

## 5.28 中的变化

`v2026.5.27` 和 `v2026.5.28` 之间的清理减少了默认安装 图，而不是移除这些能力本身。

**根默认图** 唯一软件包名称/版本根从 **371** 降至 **300** 。软件包 实例从 **372** 降至 **301** 。 **嵌套树** 在同一次本地安装审计中，嵌套的 `openclaw/node_modules` 从 **656.1MiB** 降至 **259.7MiB** 。 **原生可选依赖锥** 全平台 `@napi-rs/canvas` 原生软件包依赖锥不再进入 默认安装。 **供应链表面** 默认软件包更少，意味着默认需要信任的 tarball、维护者、原生二进制文件、 安装时行为和传递更新路径更少。

## 核心数字

不要把四月下旬的异常行用作公开性能基线。 `v2026.4.23` 和 `v2026.4.29` 是有用的回归证据，但较大的 `14x` 风格差值主要描述的是从一条糟糕发布线中恢复的过程。
...
四月较早基线是来自已发布 `clawgrit-reports` mock-provider 运行的 `v2026.4.14`。该运行使用 repeat 3，失败的唯一原因是 未输出诊断时间线；冷轮次、热轮次和 RSS 中位数仍可作为粗略规模参考。 请将其视为叙事背景，而不是发布门禁统计数据。

在五月扫查中，最新发布分支行相较 `v2026.5.2` 有实质变化：
...
`2026.5.12` 是变更日志中可见的插件抽离里程碑： Amazon Bedrock、Bedrock Mantle、Slack、OpenShell 沙箱、Anthropic Vertex、 Matrix 和 WhatsApp 从核心依赖路径中移出，因此它们的依赖 锥会随这些插件安装，而不是随每次核心安装一起安装。

## Kova 智能体轮次摘要

四月稳定线包含两个不同的故事。四月较早版本缓慢， 但仍可辨识。四月下旬变成了回归悬崖。`v2026.5.2` 是 mock-provider 通道首次降至 3-5 秒范围，并在提供的扫查中 开始持续通过的位置。
...
`v2026.5.22` CLI 健康检查峰值在此表中可见，即使 agent-turn 通道仍然通过。调查有针对性的 CLI 或 Gateway 网关回归时， 请保留源探针。

## 安装占用审计

依赖样本每月使用一个稳定版本，另加 `2026.5.22` shrinkwrap 引入事件和最新的 `2026.5.28` 版本。
...
**Before shrinkwrap** `2026.5.20` 没有根 shrinkwrap，也没有大型嵌套 OpenClaw 依赖树。 **Introduced** `2026.5.22` 添加根 shrinkwrap，并在嵌套 `openclaw/node_modules` 下安装 911.8MB。 **Latest stable** `2026.5.28` 保留 shrinkwrap，并且仍在嵌套 `openclaw/node_modules` 下安装 259.7MiB。 **Canvas fanout fixed** `2026.5.28` 在本地全新安装审计中不再安装任何 `@napi-rs/canvas` 包。
...
重要区别：**shrinkwrap 本身不是问题** 。 `v2026.5.28` 仍然发布根 shrinkwrap。问题在于包形态导致 npm 物化了一个大型嵌套 OpenClaw 依赖树，以及全部 12 个 `@napi-rs/canvas` 平台包。嵌套树在 `v2026.5.28` 中更小， 并且 canvas 平台扇出不再出现在本地审计中。
