---
title: Fernsteuerung
source_url: https://docs.openclaw.ai/de/platforms/mac/remote
...
Im SSH-Tunnelmodus werden erkannte LAN-/Tailnet-Hostnamen als `gateway.remote.sshTarget` gespeichert. Die App belässt `gateway.remote.url` auf dem lokalen Tunnel-Endpunkt, zum Beispiel `ws://127.0.0.1:18789`, sodass CLI, Web Chat und der lokale Node-Host-Service alle denselben sicheren loopback-Transport verwenden.

Browser-Automatisierung im Remote-Modus gehört dem CLI-Node-Host, nicht dem nativen macOS-App-Node. Die App startet den installierten Node-Host-Service, wenn möglich; wenn Sie Browser-Steuerung von diesem Mac benötigen, installieren/starten Sie ihn mit `openclaw node install ...` und `openclaw node start` (oder führen Sie `openclaw node run ...` im Vordergrund aus), und zielen Sie dann auf diesen browserfähigen Node.
