...
source_url: https://docs.openclaw.ai/fr/tools/thinking
...
## Ce que cela fait
  ...
  * Notes sur les fournisseurs : 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) ne prend en charge que la réflexion binaire (`on`/`off`). Tout niveau non `off` est traité comme `on` (associé à `low`).
