...
source_url: https://docs.openclaw.ai/ja-JP/channels/nextcloud-talk
...
Nextcloud Talkは現在のOpenClawリリースで同梱Pluginとして提供されるため、 通常のパッケージビルドでは個別のインストールは不要です。

古いビルドを使用している場合、またはNextcloud Talkを除外したカスタムインストールの場合は、 npmパッケージを直接インストールしてください。
