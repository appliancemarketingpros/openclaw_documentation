...
source_url: https://docs.openclaw.ai/zh-TW/plugins/google-meet
...
OpenClaw 的 Google Meet 參與者支援是刻意設計為明確操作的 Plugin：
  ...
  * Agents 透過 `mode` 選擇加入行為：使用 `agent` 進行即時 聆聽/回話，使用 `bidi` 作為直接即時語音備援，或使用 `transcribe` 加入/控制瀏覽器但不啟用回話橋接。
  * 驗證一開始採用個人 Google OAuth，或已登入的 Chrome 設定檔。
  * 不會自動宣布徵得同意。
  * 預設 Chrome 音訊後端是 `BlackHole 2ch`。
  * Chrome 可以在本機執行，或在已配對的節點主機上執行。
  * Twilio 接受撥入號碼加上選用的 PIN 或 DTMF 序列；它 無法直接撥打 Meet URL。
  * CLI 指令是 `googlemeet`；`meet` 保留給更廣泛的 agent 電話會議工作流程。


## 快速開始

安裝本機音訊相依項，並設定即時轉錄 供應商以及一般 OpenClaw TTS。OpenAI 是預設的轉錄 供應商；Google Gemini Live 也可作為獨立的 `bidi` 語音備援，搭配 `realtime.voiceProvider: "google"`：
...
`blackhole-2ch` 會安裝 `BlackHole 2ch` 虛擬音訊裝置。Homebrew 的 安裝程式需要重新啟動，macOS 才會公開該裝置：
...
設定輸出是為了讓 agent 可讀取，並且會感知模式。它會回報 Chrome 設定檔、節點釘選，以及對於即時 Chrome 加入情境，BlackHole/SoX 音訊 橋接和延遲即時簡介檢查。對於僅觀察加入，請使用 `--mode transcribe` 檢查相同 傳輸；該模式會略過即時音訊先決條件， 因為它不會透過橋接聆聽或發話：
...
設定 Twilio 委派時，設定也會回報 `voice-call` Plugin、Twilio 憑證和公開 Webhook 暴露是否就緒。 在要求 agent 加入之前，請將任何 `ok: false` 檢查視為所檢查傳輸和模式的阻擋項。 使用 `openclaw googlemeet setup --json` 取得 腳本或機器可讀輸出。使用 `--transport chrome`、 `--transport chrome-node` 或 `--transport twilio`，在 agent 嘗試之前預先檢查特定 傳輸。
...
這會在 agent 嘗試撥打會議之前，捕捉缺少的 `voice-call` 接線、Twilio 憑證或無法連線的 Webhook 暴露。
...
面向 agent 的 `google_meet` 工具在非 macOS 主機上仍可用於 成品、行事曆、設定、轉錄、Twilio 和 `chrome-node` 流程。本機 Chrome 回話動作會在那裡被封鎖，因為內建 Chrome 音訊路徑 目前依賴 macOS `BlackHole 2ch`。在 Linux 上，請使用 `mode: "transcribe"`、 Twilio 撥入，或使用 macOS `chrome-node` 主機進行 Chrome 回話 參與。
...
對於透過 API 建立的房間，若你希望 房間的免敲門政策明確設定，而不是繼承自 Google 帳戶預設值，請使用 Google Meet `SpaceConfig.accessType`：
...
`OPEN` 允許任何持有 Meet URL 的人不敲門即可加入。`TRUSTED` 允許 主辦機構的受信任使用者、受邀外部使用者和撥入使用者 不敲門即可加入。`RESTRICTED` 將免敲門進入限制為受邀者。這些 設定只適用於官方 Google Meet API 建立路徑，因此必須 設定 OAuth 憑證。

如果你是在此選項可用之前驗證 Google Meet，請在將 `meetings.space.settings` 範圍新增到你的 Google OAuth 同意畫面後，重新執行 `openclaw googlemeet auth login --json`。
...
`googlemeet create` 有兩條路徑：
  ...
  * API 建立：當已設定 Google Meet OAuth 憑證時使用。這是 最具決定性的路徑，且不依賴瀏覽器 UI 狀態。
  * 瀏覽器備援：當缺少 OAuth 憑證時使用。OpenClaw 會使用 釘選的 Chrome 節點，開啟 `https://meet.google.com/new`，等待 Google 重新導向到真正的會議代碼 URL，然後傳回該 URL。此路徑需要 節點上的 OpenClaw Chrome 設定檔已登入 Google。 瀏覽器自動化會處理 Meet 自己的首次執行麥克風提示；該提示 不會被視為 Google 登入失敗。 加入與建立流程也會先嘗試重用現有的 Meet 分頁，再開啟 新分頁。比對會忽略像 `authuser` 這類無害的 URL 查詢字串，因此 agent 重試應該會聚焦已開啟的會議，而不是建立第二個 Chrome 分頁。


指令/工具輸出包含 `source` 欄位（`api` 或 `browser`），因此 agents 可以說明使用了哪條路徑。`create` 預設會加入新會議，並 傳回 `joined: true` 加上加入工作階段。若只要產生 URL，請在 CLI 上使用 `create --no-join`，或將 `"join": false` 傳給工具。

或告訴 agent：「建立一個 Google Meet，以 agent 回話模式加入， 並把連結傳給我。」agent 應呼叫 `google_meet`，並使用 `action: "create"`，然後分享傳回的 `meetingUri`。
...
對於僅觀察/瀏覽器控制加入，請設定 `"mode": "transcribe"`。這不會 啟動雙工即時語音橋接，不需要 BlackHole 或 SoX， 也不會在會議中回話。此模式下的 Chrome 加入也會避免 OpenClaw 的麥克風/攝影機權限授予，並避免 Meet **Use microphone** 路徑。如果 Meet 顯示音訊選擇中介畫面，自動化會嘗試 不使用麥克風的路徑，否則會回報手動動作，而不是開啟 本機麥克風。在轉錄模式中，受管理的 Chrome 傳輸也會安裝 盡力而為的 Meet 字幕觀察器。`googlemeet status --json` 和 `googlemeet doctor` 會顯示 `captioning`、`captionsEnabledAttempted`、 `transcriptLines`、`lastCaptionAt`、`lastCaptionSpeaker`、`lastCaptionText` 以及短的 `recentTranscript` 尾端，讓操作員判斷瀏覽器 是否已加入通話，以及 Meet 字幕是否正在產生文字。 當你需要是/否探測時，請使用 `openclaw googlemeet test-listen <meet-url> --transport chrome-node`： 它會以轉錄模式加入，等待新的字幕或 轉錄移動，並傳回 `listenVerified`、`listenTimedOut`、手動 動作欄位和最新字幕健康狀態。

在即時工作階段期間，`google_meet` 狀態包含瀏覽器和音訊橋接 健康狀態，例如 `inCall`、`manualActionRequired`、`providerConnected`、 `realtimeReady`、`audioInputActive`、`audioOutputActive`、最後輸入/輸出 時間戳記、位元組計數器和橋接已關閉狀態。如果安全的 Meet 頁面提示 出現，瀏覽器自動化會在可行時處理它。登入、主辦者准入和 瀏覽器/OS 權限提示會回報為手動動作，並附上原因與 訊息供 agent 轉述。受管理的 Chrome 工作階段只會在瀏覽器健康狀態回報 `inCall: true` 後 發出簡介或 測試片語；否則狀態會回報 `speechReady: false`，並封鎖語音嘗試，而不是假裝 agent 已在會議中發話。

本機 Chrome 會透過已登入的 OpenClaw 瀏覽器設定檔加入。即時模式 需要 `BlackHole 2ch`，供 OpenClaw 使用的麥克風/喇叭路徑使用。若要 乾淨的雙工音訊，請使用分開的虛擬裝置或 Loopback 風格的圖形；單一 BlackHole 裝置足以進行首次煙霧測試，但可能會產生回音。

### 本機 Gateway + Parallels Chrome

若只是要讓 macOS VM 擁有 Chrome，你**不** 需要在 VM 內執行完整 OpenClaw Gateway 或模型 API 金鑰。 在本機執行 Gateway 和 agent，然後在 VM 中執行節點主機。在 VM 上啟用一次內建 Plugin，讓節點 公告 Chrome 指令：

各處執行內容：

  * Gateway 主機：OpenClaw Gateway、agent 工作區、模型/API 金鑰、即時 供應商，以及 Google Meet Plugin 設定。
  * Parallels macOS VM：OpenClaw CLI/節點主機、Google Chrome、SoX、BlackHole 2ch， 以及已登入 Google 的 Chrome 設定檔。
  * VM 中不需要：Gateway 服務、agent 設定、OpenAI/GPT 金鑰，或模型 供應商設定。
...
如果 `<gateway-host>` 是 LAN IP，且你未使用 TLS，除非你為該受信任的私人網路明確選擇加入，否則節點會拒絕 純文字 WebSocket：
...
`OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` 是程序環境，而不是 `openclaw.json` 設定。當它存在於安裝指令上時，`openclaw node install` 會將它儲存在 LaunchAgent 環境中。
...
確認 Gateway 看得到節點，且節點公告了 `googlemeet.chrome` 和瀏覽器能力/`browser.proxy`：
...
若要進行單指令煙霧測試，建立或重用工作階段、說出已知 片語，並列印工作階段健康狀態：
...
在即時加入期間，OpenClaw 瀏覽器自動化會填入訪客名稱、點擊 Join/Ask to join，並在 Meet 第一次執行時出現「Use microphone」提示時接受該選項。在僅觀察加入或僅用瀏覽器建立會議期間，如果同一提示提供不使用麥克風的選項，它會繼續通過該提示。 如果瀏覽器設定檔尚未登入、Meet 正在等待主持人准入、Chrome 需要麥克風/攝影機權限才能即時加入，或 Meet 卡在自動化無法解決的提示上，join/test-speech 結果會回報 `manualActionRequired: true`，並帶有 `manualActionReason` 和 `manualActionMessage`。Agent 應停止重試加入，回報該確切訊息以及目前的 `browserUrl`/`browserTitle`，並且只在手動瀏覽器動作完成後才重試。
...
常見失敗檢查：
  ...
  * `No connected Google Meet-capable node`：在 VM 中啟動 `openclaw node run`，核准配對，並確認已在 VM 中執行 `openclaw plugins enable google-meet` 和 `openclaw plugins enable browser`。也請確認 Gateway 主機允許這兩個 Node 命令： `gateway.nodes.allowCommands: ["googlemeet.chrome", "browser.proxy"]`。
  ...
  * Chrome 開啟但無法加入：在 VM 內登入瀏覽器設定檔，或保持設定 `chrome.guestName` 以便訪客加入。訪客自動加入會透過 Node 瀏覽器 Proxy 使用 OpenClaw 瀏覽器自動化；請確認 Node 瀏覽器設定指向你想要的設定檔，例如 `browser.defaultProfile: "user"` 或具名的 existing-session 設定檔。
  * 重複的 Meet 分頁：保持啟用 `chrome.reuseExistingTab: true`。OpenClaw 會先啟用相同 Meet URL 的現有分頁，再開啟新分頁；瀏覽器會議建立也會先重用進行中的 `https://meet.google.com/new` 或 Google 帳戶提示分頁，再開啟另一個分頁。
...
Chrome talk-back 預設使用兩個外部工具：
  ...
  * `blackhole-2ch`：macOS 虛擬音訊驅動程式。它會建立 Chrome/Meet 可路由通過的 `BlackHole 2ch` 音訊裝置。


OpenClaw 不會綁定或重新散布任一套件。文件要求使用者透過 Homebrew 將它們安裝為主機相依項。SoX 授權為 `LGPL-2.0-only AND GPL-2.0-only`；BlackHole 為 GPL-3.0。如果你建置的安裝程式或設備會將 BlackHole 與 OpenClaw 綁定，請檢閱 BlackHole 的上游授權條款，或向 Existential Audio 取得獨立授權。

## 傳輸

### Chrome

Chrome 傳輸會透過 OpenClaw 瀏覽器控制開啟 Meet URL，並以已登入的 OpenClaw 瀏覽器設定檔加入。在 macOS 上，Plugin 會在啟動前檢查 `BlackHole 2ch`。如果已設定，它也會在開啟 Chrome 前執行音訊橋接健康命令和啟動命令。當 Chrome/音訊位於 Gateway 主機上時使用 `chrome`；當 Chrome/音訊位於已配對的 Node，例如 Parallels macOS VM 上時使用 `chrome-node`。對於本機 Chrome，使用 `browser.defaultProfile` 選擇設定檔；`chrome.browserProfile` 會傳遞給 `chrome-node` 主機。
...
如果那是你的即時語音供應商，請改用 `realtime.provider: "openai"` 搭配 OpenAI provider Plugin 和 `OPENAI_API_KEY`。
...
`meetings.space.created` 是 Google Meet `spaces.create` 所必需。 `meetings.space.readonly` 讓 OpenClaw 可將 Meet URL/代碼解析為空間。 `meetings.space.settings` 讓 OpenClaw 可在 API 房間建立期間傳遞 `SpaceConfig` 設定，例如 `accessType`。 `meetings.conference.media.readonly` 用於 Meet Media API 預檢和媒體工作；Google 可能會要求加入 Developer Preview，才能實際使用 Media API。 如果你只需要基於瀏覽器的 Chrome 加入，請完全略過 OAuth。
...
這不會載入 Chrome 執行階段，也不需要已連線的 Chrome Node。它會檢查 OAuth 設定是否存在，以及重新整理權杖是否能產生存取權杖。JSON 報告只包含狀態欄位，例如 `ok`、`configured`、 `tokenSource`、`expiresAt` 和檢查訊息；它不會列印存取權杖、重新整理權杖或用戶端密鑰。
...
若也要證明 Google Meet API 已啟用以及 `spaces.create` 權限範圍可用，請執行 會產生副作用的建立檢查：
...
`--create-space` 會建立一個可丟棄的 Meet URL。當你需要確認 Google Cloud 專案已啟用 Meet API，且已授權帳戶具備 `meetings.space.created` 權限範圍時，請使用它。
...
`doctor --oauth --meeting` 和 `resolve-space` 會證明對已授權 Google 帳戶可存取之現有 空間的讀取存取權。這些檢查傳回 `403`，通常表示 Google Meet REST API 已停用、同意授權的重新整理權杖 缺少必要權限範圍，或 Google 帳戶無法存取該 Meet 空間。重新整理權杖錯誤表示請重新執行 `openclaw googlemeet auth login --json` 並儲存新的 `oauth` 區塊。

瀏覽器後援不需要 OAuth 認證資料。在該模式下，Google 驗證來自所選 Node 上已登入的 Chrome 設定檔，而不是 OpenClaw 設定。
...
使用 `--meeting` 時，`artifacts` 和 `attendance` 預設會使用最新的會議記錄。 當你想取得該會議的每個保留記錄時，請傳入 `--all-conference-records`。

Calendar 查詢可以先從 Google Calendar 解析會議 URL，再讀取 Meet 成果：
...
`--today` 會在今天的 `primary` 行事曆中搜尋含有 Google Meet 連結的 Calendar 事件。使用 `--event <query>` 搜尋相符的事件文字，並使用 `--calendar <id>` 指定非主要行事曆。Calendar 查詢需要新的 OAuth 登入，且該登入必須包含 Calendar 事件唯讀權限範圍。 `calendar-events` 會預覽相符的 Meet 事件，並標記 `latest`、`artifacts`、`attendance` 或 `export` 將會選擇的事件。
...
這會呼叫 Google Meet `spaces.endActiveConference`，並需要 OAuth 對已授權帳戶可管理的空間具備 `meetings.space.created` 權限範圍。 OpenClaw 接受 Meet URL、會議代碼或 `spaces/{id}` 輸入，並在結束作用中會議前將其解析為 API 空間資源。 它和 `googlemeet leave` 是分開的：`leave` 會停止 OpenClaw 的本機/工作階段 參與，而 `end-active-conference` 會要求 Google Meet 結束該空間的作用中 會議。
...
當 Google 為該會議公開資料時，`artifacts` 會傳回會議記錄中繼資料，以及參與者、錄影、 逐字稿、結構化逐字稿項目和智慧筆記資源中繼資料。使用 `--no-transcript-entries` 可略過 大型會議的項目查詢。`attendance` 會將參與者展開為 參與者工作階段列，包含首次/最後出現時間、工作階段總時長、 遲到/提前離開標記，並依已登入使用者或顯示名稱合併重複的參與者資源。 傳入 `--no-merge-duplicates` 可讓原始參與者 資源保持分開，傳入 `--late-after-minutes` 可調整遲到偵測，並傳入 `--early-before-minutes` 可調整提前離開偵測。

`export` 會寫入一個包含 `summary.md`、`attendance.csv`、 `transcript.md`、`artifacts.json`、`attendance.json` 和 `manifest.json` 的資料夾。 `manifest.json` 會記錄所選輸入、匯出選項、會議記錄、 輸出檔案、計數、權杖來源、使用過的 Calendar 事件，以及任何 部分擷取警告。傳入 `--zip` 也會在資料夾旁寫入可攜式封存檔。 傳入 `--include-doc-bodies` 可透過 Google Drive `files.export` 匯出連結的逐字稿和 智慧筆記 Google Docs 文字；這需要新的 OAuth 登入，且該登入必須包含 Drive Meet 唯讀權限範圍。若未使用 `--include-doc-bodies`，匯出內容只會包含 Meet 中繼資料和結構化逐字稿 項目。如果 Google 傳回部分成果失敗，例如智慧筆記 清單、逐字稿項目或 Drive 文件本文錯誤，摘要和 manifest 會保留警告，而不是讓整個匯出失敗。 使用 `--dry-run` 可擷取相同的成果/出席資料並列印 manifest JSON，而不建立資料夾或 ZIP。這在寫入 大型匯出前，或當 agent 只需要計數、所選記錄和 警告時很有用。
...
即時 smoke 環境：
  ...
  * `OPENCLAW_GOOGLE_MEET_REFRESH_TOKEN` 或 `GOOGLE_MEET_REFRESH_TOKEN` 提供 重新整理權杖。
  * 選用：`OPENCLAW_GOOGLE_MEET_CLIENT_SECRET`、 `OPENCLAW_GOOGLE_MEET_ACCESS_TOKEN` 和 `OPENCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` 使用相同但不含 `OPENCLAW_` 前綴的後援名稱。


基礎成果/出席即時 smoke 需要 `https://www.googleapis.com/auth/meetings.space.readonly` 和 `https://www.googleapis.com/auth/meetings.conference.media.readonly`。Calendar 查詢需要 `https://www.googleapis.com/auth/calendar.events.readonly`。Drive 文件本文匯出需要 `https://www.googleapis.com/auth/drive.meet.readonly`。
...
該命令會列印新的 `meeting uri`、來源和加入工作階段。使用 OAuth 認證資料時，它會使用官方 Google Meet API。沒有 OAuth 認證資料時，它會 使用已固定 Chrome Node 的已登入瀏覽器設定檔作為後援。Agent 可以 使用 `google_meet` 工具搭配 `action: "create"` 一次完成建立和加入。 若只要建立 URL，請傳入 `"join": false`。
...
如果瀏覽器後援在建立 URL 前遇到 Google 登入或 Meet 權限阻擋，Gateway 方法會傳回失敗回應，而 `google_meet` 工具會傳回結構化詳細資料，而不是純字串：
...
當 agent 看到 `manualActionRequired: true` 時，應回報 `manualActionMessage` 加上瀏覽器 Node/分頁脈絡，並停止開啟新的 Meet 分頁，直到操作員完成瀏覽器步驟。
...
如果電話已建立，但 Meet 名冊從未顯示撥入參與者：
  ...
  * 如果參與者已加入，但你聽不到問候語，請檢查 `openclaw logs --follow` 是否有 DTMF 後的 `voicecall.speak` 要求，以及媒體串流 TTS 播放或 Twilio `OPENCLAW_DOCS_MARKER:calloutOpen:U2F5` 後援。如果通話轉錄仍包含「enter the meeting PIN」，代表電話端尚未加入 Meet 會議室，因此會議參與者不會聽到語音。
