---
title: 日付と時刻
source_url: https://docs.openclaw.ai/ja-JP/date-time
scraped_at: 2026-05-25
---

OpenClaw は既定で、**トランスポートタイムスタンプにホストローカル時刻** を使用し、**system prompt 内でのみユーザーのタイムゾーン** を使用します。 Provider のタイムスタンプは保持されるため、tools はネイティブなセマンティクスを維持します（現在時刻は `session_status` で利用できます）。
...
ユーザーのタイムゾーンが既知の場合、system prompt には専用の **現在の日付と時刻** セクションが含まれ、prompt caching を安定させるために**タイムゾーンのみ** （時計/時刻形式なし）が含まれます。
