...
source_url: https://docs.openclaw.ai/ar/concepts/commitments
...
الالتزامات ليست حقائق دائمة مثل `MEMORY.md`، وليست تذكيرات دقيقة. إنها تقع بين الذاكرة والأتمتة: يتذكر OpenClaw التزامًا مرتبطًا بالمحادثة، ثم يوصله Heartbeat عندما يحين موعده.
...
يحد `commitments.maxPerDay` من عدد المتابعات المستنتجة التي يمكن تسليمها لكل جلسة وكيل خلال يوم متجدد. القيمة الافتراضية هي `3`.
...
يحدث التسليم عبر Heartbeat. عندما يحين موعد التزام، يضيف Heartbeat الالتزام إلى دورة Heartbeat لنطاق الوكيل والقناة نفسه. يمكن للنموذج إرسال تحقق طبيعي واحد أو الرد بـ `HEARTBEAT_OK` لتجاهله. إذا كان Heartbeat معدًا بقيمة `target: "none"`، تبقى الالتزامات المستحقة داخلية ولا ترسل عمليات تحقق خارجية. لا تعيد مطالبات تسليم الالتزامات تشغيل نص المحادثة الأصلي، وتعمل دورات Heartbeat الخاصة بالالتزامات المستحقة من دون أدوات OpenClaw.
...
إذا لم تظهر المتابعات المتوقعة:
  ...
  * تحقق مما إذا كان `commitments.maxPerDay` قد بلغ حده بالفعل لتلك الجلسة الخاصة بالوكيل.
  * تذكر أن التذكيرات الدقيقة يتجاوزها استخراج الالتزامات ويجب أن تظهر ضمن [المهام المجدولة](</ar/automation/cron-jobs>) بدلًا من ذلك.
