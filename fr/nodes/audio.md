...
source_url: https://docs.openclaw.ai/fr/nodes/audio
...
## Ce qui fonctionne
  ...
  * **Compréhension des médias (audio)** : si la compréhension audio est activée (ou détectée automatiquement), OpenClaw : 
    ...
    4. Si elle échoue ou est ignorée (taille/délai d’expiration), il essaie l’entrée suivante.
...
Si vous **ne configurez pas de modèles** et que `tools.media.audio.enabled` n’est **pas** défini sur `false`, OpenClaw effectue la détection automatique dans cet ordre et s’arrête à la première option fonctionnelle :
  ...
  4. **Authentification fournisseur**
     ...
     * Ordre de secours groupé : OpenAI → Groq → xAI → Deepgram → Google → SenseAudio → ElevenLabs → Mistral


Pour désactiver la détection automatique, définissez `tools.media.audio.enabled: false`. Pour personnaliser, définissez `tools.media.audio.models`. Remarque : la détection des binaires est effectuée au mieux sur macOS/Linux/Windows ; assurez-vous que la CLI est dans `PATH` (nous développons `~`), ou définissez un modèle CLI explicite avec un chemin de commande complet.

## Exemples de configuration

### Secours fournisseur + CLI (OpenAI + CLI Whisper)
...
### Fournisseur uniquement avec limitation par portée
...
json5Copy code
[code]
    {  tools: {    media: {      audio: {        enabled: true,        echoTranscript: true, // default is false        echoFormat: '📝 "{transcript}"', // optional, supports {transcript}        models: [{ provider: "openai", model: "gpt-4o-mini-transcribe" }],      },    },  },}
[/code]
...
## Notes et limites
  ...
  * La limite de taille par défaut est de 20 Mo (`tools.media.audio.maxBytes`). Un audio trop volumineux est ignoré pour ce modèle, puis l’entrée suivante est essayée.
  * Les fichiers audio minuscules/vides de moins de 1 024 octets sont ignorés avant la transcription par fournisseur/CLI.
  ...
  * `tools.media.audio.echoTranscript` est désactivé par défaut ; activez-le pour envoyer une confirmation de transcription au chat d’origine avant le traitement par l’agent.
...
Si aucune variable d’environnement proxy n’est définie, la sortie directe est utilisée. Si la configuration du proxy est mal formée, OpenClaw journalise un avertissement et revient à la récupération directe.
...
**Fonctionnement :**
  ...
  1. Si un message vocal n’a pas de corps textuel et que le groupe exige des mentions, OpenClaw effectue une transcription de « prévol ».
...
**Comportement de secours :**
  ...
  * Si la transcription échoue pendant le prévol (délai d’expiration, erreur d’API, etc.), le message est traité selon la détection des mentions uniquement textuelle.
...
**Désactivation par groupe/sujet Telegram :**
  ...
  * La valeur par défaut est `false` (prévol activé lorsque les conditions avec barrière de mention correspondent).
...
## Points d’attention
  ...
  * Assurez-vous que votre CLI quitte avec 0 et imprime du texte brut ; le JSON doit être adapté via `jq -r .text`.
  * Pour `parakeet-mlx`, si vous passez `--output-dir`, OpenClaw lit `<output-dir>/<media-basename>.txt` lorsque `--output-format` vaut `txt` (ou est omis) ; les formats de sortie non `txt` reviennent à l’analyse de stdout.
  * Gardez des délais d’expiration raisonnables (`timeoutSeconds`, 60 s par défaut) pour éviter de bloquer la file de réponses.
  * La transcription de prévol ne traite que la **première** pièce jointe audio pour la détection des mentions. Les audios supplémentaires sont traités pendant la phase principale de compréhension des médias.
