...
source_url: https://docs.openclaw.ai/fa/reference/release-performance-sweep
...
**Stable agent turn** **نوبت سرد ۵.۱ برابر سریع‌تر**

  * `v2026.4.14`: 9.8s
  * `v2026.5.28`: 1.9s

**Published package** **tarball به اندازهٔ ۱۷.۹MB** جدیدترین بستهٔ پایدار، کاهش‌یافته از اوج ۴۳.۳MB اندازهٔ بسته در مارس. **Latest stable install** **نصب تازهٔ ۳۶۱.۷MiB** `v2026.5.28` درخت وابستگی تودرتوی OpenClaw را به‌شدت کاهش می‌دهد، اما در ممیزی نصب محلی هنوز یک درخت تودرتوی کوچک‌تر ۲۵۹.۷MiB باقی می‌ماند. **Dependency graph** **۳۰۰ بستهٔ نصب‌شده** جدیدترین انتشار پایدار، اندازه‌گیری‌شده به‌عنوان ریشه‌های یکتای نام/نسخهٔ بسته در یک نصب تازه با اسکریپت‌های غیرفعال.

## خط زمانی ردپای نصب

**Monthly high** **۶۴۵ وابستگی** `2026.2.26` در این نمونه، بیشترین شمار وابستگی ماهانه را داشت. **Shrinkwrap introduced** **نصب ۱٬۰۲۰.۶MB** `2026.5.22` shrinkwrap ریشه را اضافه کرد و یک مشکل شکل بسته را آشکار کرد: ۹۱۱.۸MB زیر `openclaw/node_modules` تودرتو قرار گرفت. **Latest stable** **نصب ۳۶۱.۷MiB** `2026.5.28` اندازهٔ نصب تازه را نسبت به `2026.5.27` به میزان ۵۲.۸٪ کاهش می‌دهد، اما هنوز یک درخت تودرتوی OpenClaw به اندازهٔ ۲۵۹.۷MiB نصب می‌کند. **Dependency graph** **۳۰۰ ریشهٔ بسته** `2026.5.28` نسبت به `2026.5.27`، ۷۱ ریشهٔ یکتای نام/نسخهٔ بسته کمتر نصب می‌کند.

## چه چیزی در 5.28 تغییر کرد

پاک‌سازی بین `v2026.5.27` و `v2026.5.28` به‌جای حذف خود قابلیت‌ها، گراف نصب پیش‌فرض را کوچک‌تر کرد.

**گراف پیش‌فرض ریشه** ریشه‌های یکتای نام/نسخه بسته از **371** به **300** کاهش یافتند. نمونه‌های بسته از **372** به **301** رسیدند. **درخت تو در تو** `openclaw/node_modules` تو در تو در همان ممیزی نصب محلی از **656.1MiB** به **259.7MiB** کاهش یافت. **مخروط‌های اختیاری بومی** مخروط بسته بومی همه‌سکویی `@napi-rs/canvas` دیگر در نصب پیش‌فرض قرار نگرفت. **سطح زنجیره تأمین** بسته‌های پیش‌فرض کمتر یعنی آرشیوهای بسته، نگه‌دارندگان، باینری‌های بومی، رفتارهای زمان نصب، و مسیرهای به‌روزرسانی گذرای کمتری که به‌صورت پیش‌فرض باید به آن‌ها اعتماد کرد.
...
**پیش از shrinkwrap** `2026.5.20` نه shrinkwrap ریشه دارد و نه درخت بزرگ وابستگی OpenClaw تو در تو. **معرفی شد** `2026.5.22` shrinkwrap ریشه را اضافه می‌کند و 911.8MB را زیر `openclaw/node_modules` تو در تو نصب می‌کند. **آخرین پایدار** `2026.5.28` shrinkwrap را نگه می‌دارد و همچنان 259.7MiB را زیر `openclaw/node_modules` تو در تو نصب می‌کند. **fanout Canvas رفع شد** `2026.5.28` دیگر هیچ بسته `@napi-rs/canvas` را در ممیزی نصب تازه محلی نصب نمی‌کند.
...
برای توضیح ساده و انگلیسی درباره shrinkwrap و بررسی‌های بسته در سطح نگه‌دارنده، [shrinkwrap در npm](</fa/gateway/security/shrinkwrap>) را ببینید.
