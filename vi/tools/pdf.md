---
title: Công cụ PDF
source_url: https://docs.openclaw.ai/vi/tools/pdf
...
Ghi chú về tính khả dụng:
  ...
  * Chuỗi dự phòng có nhận biết xác thực. Một `provider/model` đã cấu hình chỉ được tính nếu OpenClaw thực sự có thể xác thực nhà cung cấp đó cho tác nhân.
...
Chế độ gốc được dùng cho nhà cung cấp `anthropic` và `google`. Công cụ gửi byte PDF thô trực tiếp tới API nhà cung cấp.

Giới hạn chế độ gốc:

  * `pages` không được hỗ trợ. Nếu được đặt, công cụ trả về lỗi.
  * Đầu vào nhiều PDF được hỗ trợ; mỗi PDF được gửi dưới dạng khối tài liệu gốc / phần PDF nội tuyến trước lời nhắc.
...
Chi tiết dự phòng:
  ...
  * Dự phòng trích xuất dùng Plugin `document-extract` được đóng gói. Plugin sở hữu `pdfjs-dist`; `@napi-rs/canvas` chỉ được dùng khi dự phòng kết xuất hình ảnh khả dụng.
