---
title: 频道消息 API
source_url: https://docs.openclaw.ai/zh-CN/plugins/sdk-channel-message
...
该桥接会将旧的出站发送结果转换为 `MessageReceipt` 值。新代码应端到端传递回执，并且只在兼容性边界使用 `listMessageReceiptPlatformIds(...)` 或 `resolveMessageReceiptPrimaryId(...)` 派生旧版 ID。 如果未提供接收策略，`createChannelMessageAdapterFromOutbound(...)` 会使用 `manual` 接收确认策略。这会明确插件拥有的平台确认责任，而不会改变那些在通用接收上下文之外确认 webhook、socket 或轮询偏移的渠道。
