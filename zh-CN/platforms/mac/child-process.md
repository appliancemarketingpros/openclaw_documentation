...
source_url: https://docs.openclaw.ai/zh-CN/platforms/mac/child-process
...
## 默认行为（launchd）
  ...
  * 应用会安装一个按用户划分的 LaunchAgent，标签为 `ai.openclaw.gateway` （使用 `--profile`/`OPENCLAW_PROFILE` 时为 `ai.openclaw.<profile>`；也支持旧版 `com.openclaw.*`）。
