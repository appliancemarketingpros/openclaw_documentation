...
source_url: https://docs.openclaw.ai/vi/gateway/background-process
...
Hành vi:
  ...
  * Với công việc chạy lâu bắt đầu ngay bây giờ, hãy khởi động một lần và dựa vào cơ chế đánh thức hoàn tất tự động khi nó được bật và lệnh phát ra đầu ra hoặc thất bại.
  * Nếu không có cơ chế đánh thức hoàn tất tự động, hoặc bạn cần xác nhận thành công im lặng cho một lệnh đã thoát sạch mà không có đầu ra, hãy dùng `process` để xác nhận hoàn tất.
  * Không mô phỏng nhắc việc hoặc theo dõi trễ bằng các vòng lặp `sleep` hay thăm dò lặp lại; dùng cron cho công việc trong tương lai.
...
Ghi chú:
  ...
  * Dùng `poll` / `log` cho trạng thái, nhật ký, xác nhận thành công im lặng, hoặc xác nhận hoàn tất khi cơ chế đánh thức hoàn tất tự động không có sẵn.
  ...
  * Dùng `write` / `send-keys` / `submit` / `paste` / `kill` khi bạn cần đầu vào hoặc can thiệp.
  * `process list` bao gồm `name` được suy ra (động từ lệnh + đích) để quét nhanh.
  * `process list`, `poll`, và `log` chỉ báo cáo `waitingForInput` khi phiên vẫn có stdin ghi được và đã nhàn rỗi lâu hơn ngưỡng chờ đầu vào.
  ...
  * Việc thăm dò dành cho trạng thái theo yêu cầu, không phải lập lịch vòng lặp chờ. Nếu công việc nên xảy ra sau, hãy dùng cron thay thế.
