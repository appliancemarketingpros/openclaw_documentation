---
title: Signal
source_url: https://docs.openclaw.ai/tr/channels/signal
...
JVM derlemesini (`signal-cli-${VERSION}.tar.gz`) kullanıyorsanız önce JRE 25+ kurun. `signal-cli` güncel tutun; upstream, Signal sunucu API'leri değiştikçe eski sürümlerin bozulabileceğini belirtir.
...
bashCopy code
[code]
    # Gateway'i kullanıcı systemd hizmeti olarak çalıştırıyorsanız:systemctl --user restart openclaw-gateway.service # Ardından doğrulayın:openclaw doctoropenclaw channels status --probe
[/code]
...
Sağlayıcı seçenekleri:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (varsayılan: auto). Bkz. Konteyner modu.
