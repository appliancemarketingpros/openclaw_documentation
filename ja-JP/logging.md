...
Control UI の **ログ** タブは、`logs.tail` を使って同じファイルを tail します。 開き方については [Control UI](</ja-JP/web/control-ui>) を参照してください。
...
隣接する面は 2 つあります。
  ...
  * **診断フラグ** — `logging.level` を上げずに追加ログを `logging.file` にルーティングする、対象を絞ったデバッグログフラグです。フラグは大文字小文字を区別せず、ワイルドカード (`telegram.*`、`*`) をサポートします。`diagnostics.flags` 配下、または `OPENCLAW_DIAGNOSTICS=...` 環境上書きで設定します。完全なガイド: [診断フラグ](</ja-JP/diagnostics/flags>)。
...
## 関連
  ...
  * [Gateway ログの内部](</ja-JP/gateway/logging>) — WS ログスタイル、サブシステムプレフィックス、コンソールキャプチャ
