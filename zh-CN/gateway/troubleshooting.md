---
title: 故障排除
source_url: https://docs.openclaw.ai/zh-CN/gateway/troubleshooting
...
OpenClaw 将每个 skill 根目录视为一个包含边界。位于 `~/.agents/skills`、`<workspace>/.agents/skills`、`<workspace>/skills` 或 `~/.openclaw/skills` 下的符号链接，如果其真实目标解析到该根目录之外， 除非该目标被显式信任，否则会被跳过。
...
然后启动新会话，或等待 skills 观察器刷新。如果正在运行的进程早于配置变更， 请重启 Gateway 网关。
