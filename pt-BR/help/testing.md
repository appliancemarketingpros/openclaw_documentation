---
title: Testes
source_url: https://docs.openclaw.ai/pt-BR/help/testing
scraped_at: 2026-05-25
---

OpenClaw tem três suítes Vitest (unitária/integração, e2e, live) e um pequeno conjunto de executores Docker. Este documento é um guia de "como testamos":
...
Quando `--credential-source convex` (ou `OPENCLAW_QA_CREDENTIAL_SOURCE=convex`) está habilitado para QA de transporte live, o QA lab adquire um lease exclusivo de um pool apoiado por Convex, envia Heartbeats desse lease enquanto a lane está rodando e libera o lease no encerramento. O nome da seção é anterior ao suporte a Discord, Slack e WhatsApp; o contrato de lease é compartilhado entre os tipos.
...
Comandos administrativos de mantenedor (adicionar/remover/listar pool) exigem especificamente `OPENCLAW_QA_CONVEX_SECRET_MAINTAINER`.
...
Use `doctor` antes de execuções live para verificar a URL do site Convex, os segredos do broker, o prefixo do endpoint, o tempo limite HTTP e a acessibilidade de admin/list sem imprimir valores secretos. Use `--json` para saída legível por máquina em scripts e utilitários de CI.
...
Projetos, shards e lanes com escopo
