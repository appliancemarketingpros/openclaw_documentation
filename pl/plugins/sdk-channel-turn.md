...
source_url: https://docs.openclaw.ai/pl/plugins/sdk-channel-turn
...
Użyj, gdy kanał rozwiązał już trasowanie, zbudował `FinalizedMsgContext` i potrzebuje tylko współdzielonej kolejności rejestrowania, potoku odpowiedzi, wysyłki oraz finalizacji. To preferowany kształt dla prostych dołączonych ścieżek przychodzących, które w przeciwnym razie powtarzałyby boilerplate `createChannelMessageReplyPipeline(...)` i `runPrepared(...)`.
...
Wybierz `runAssembled` zamiast `runPrepared`, gdy jedynym zachowaniem wysyłki należącym do kanału jest końcowe dostarczenie payloadu plus opcjonalne pisanie, opcje odpowiedzi, trwałe dostarczanie lub rejestrowanie błędów.
