---
title: OC Path 插件
source_url: https://docs.openclaw.ai/zh-CN/plugins/oc-path
...
`onStartup: false` 让该插件避开 Gateway 网关热路径。`onCommands: ["path"]` 告诉 CLI 在你第一次运行 `openclaw path …` 时懒加载该插件，因此从不使用该动词的安装不会付出成本。
