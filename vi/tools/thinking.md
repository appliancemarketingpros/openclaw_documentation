...
source_url: https://docs.openclaw.ai/vi/tools/thinking
...
## Chức năng
  ...
  * Ghi chú về nhà cung cấp: 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) chỉ hỗ trợ suy nghĩ nhị phân (`on`/`off`). Mọi mức không phải `off` được xem là `on` (ánh xạ tới `low`).
