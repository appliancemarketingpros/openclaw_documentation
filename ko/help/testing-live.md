---
title: 테스트: 라이브 스위트
source_url: https://docs.openclaw.ai/ko/help/testing-live
scraped_at: 2026-05-25
---

빠른 시작, QA 러너, 단위/통합 스위트, Docker 흐름은 [테스트](</ko/help/testing>)를 참고하세요. 이 페이지는 **라이브**(네트워크에 접속하는) 테스트 스위트를 다룹니다: 모델 매트릭스, CLI 백엔드, ACP, 미디어 제공자 라이브 테스트, 자격 증명 처리.

## 라이브: 로컬 프로필 스모크 명령

임시 라이브 점검 전에 `~/.profile`을 소싱하여 제공자 키와 로컬 도구 경로가 셸과 일치하게 하세요.
...
`voicecall smoke`는 `--yes`도 함께 있을 때를 제외하면 드라이런입니다. 실제 알림 전화를 걸 의도가 있을 때만 `--yes`를 사용하세요. Twilio, Telnyx, Plivo의 경우 성공적인 준비 상태 점검에는 공개 webhook URL이 필요합니다. 로컬 전용 loopback/비공개 폴백은 설계상 거부됩니다.
...
이는 Gemini에 응답 생성을 요청하지 않습니다. OpenClaw가 Gemini에 제공하는 동일한 시스템 설정을 쓴 다음, 저장된 `transport: "streamable-http"` 서버가 Gemini의 HTTP MCP 형태로 정규화되고 로컬 streamable-HTTP MCP 서버에 연결할 수 있음을 증명하기 위해 `gemini --debug mcp list`를 실행합니다.
...
도구 + 이미지로 gateway 스모크 실행: `OPENCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
이는 CLI 인수 파싱, 설정/기본 에이전트 해석, 번들 Plugin 활성화, 공유 이미지 생성 런타임, 라이브 제공자 요청을 다룹니다. Plugin 의존성은 런타임 로드 전에 존재해야 합니다.
