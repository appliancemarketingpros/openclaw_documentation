...
source_url: https://docs.openclaw.ai/es/pi
...
Los runtimes de acciones de mensajes específicas del canal ahora viven en los directorios de extensión propiedad del plugin en lugar de bajo `src/agents/tools`, por ejemplo:
...
La inyección de imágenes es local al prompt: OpenClaw carga las referencias de imagen del prompt actual y las pasa mediante `images` solo para ese turno. No vuelve a escanear turnos antiguos del historial para reinyectar cargas útiles de imagen.
...
Prompt del sistema | [AGENTS.md](<http://AGENTS.md>) \+ prompts | Dinámico por canal/contexto  
