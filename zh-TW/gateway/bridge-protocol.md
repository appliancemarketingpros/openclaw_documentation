---
title: 橋接協定
source_url: https://docs.openclaw.ai/zh-TW/gateway/bridge-protocol
...
## 傳輸
  ...
  * 歷史預設監聽器連接埠是 `18790`（目前建置不會啟動 TCP 橋接器）。


啟用 TLS 時，探索 TXT 記錄會包含 `bridgeTls=1`，以及作為非秘密提示的 `bridgeTlsSha256`。請注意，Bonjour/mDNS TXT 記錄未經驗證；用戶端不得在沒有明確使用者意圖或其他帶外驗證的情況下，將宣告的指紋視為具權威性的釘選。
...
過去，`hello-ok` 會傳回 `serverName`；託管的 Plugin 表面現在會透過 `pluginSurfaceUrls` 宣告。Canvas/A2UI 使用 `pluginSurfaceUrls.canvas`；已棄用的 `canvasHostUrl` 別名不屬於 重構後的通訊協定。
...
Gateway → 用戶端：
  ...
  * `invoke` / `invoke-res`：Node 命令（`canvas.*`、`camera.*`、`screen.record`、 `location.get`、`sms.send`）
  * `event`：已訂閱工作階段的聊天更新
  * `ping` / `pong`：保持連線
...
Node 可以發出 `exec.finished` 或 `exec.denied` 事件，以公開 system.run 活動。 這些事件會在 Gateway 中對應到系統事件。（舊版 Node 可能仍會發出 `exec.started`。）
...
## 歷史 tailnet 用法
  ...
  * 將橋接器繫結至 tailnet IP：在 `~/.openclaw/openclaw.json` 中設定 `bridge.bind: "tailnet"`（僅供歷史參考；`bridge.*` 不再有效）。
...
橋接器是**隱含 v1** （沒有最小/最大協商）。本節 僅供歷史參考；目前的 Node/操作員用戶端使用 WebSocket [Gateway Protocol](</zh-TW/gateway/protocol>)。
