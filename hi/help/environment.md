---
title: पर्यावरण चर
source_url: https://docs.openclaw.ai/hi/help/environment
scraped_at: 2026-06-29
---

HelpDiagnostics

OpenClaw कई स्रोतों से environment variables खींचता है। नियम है **मौजूदा मानों को कभी override न करें** । Workspace `.env` फ़ाइलें कम-विश्वसनीय स्रोत हैं: OpenClaw precedence लागू करने से पहले workspace `.env` से provider credentials और सुरक्षित runtime controls को अनदेखा करता है।
...
Config `env` block केवल literal string values स्वीकार करता है। यह `file:...` values expand नहीं करता; उदाहरण के लिए, `XAI_API_KEY: "file:secrets/xai-api-key.txt"` providers को उसी exact string के रूप में pass किया जाता है।

File-backed provider keys के लिए, credential field पर SecretRef का उपयोग करें जो इसे support करता है:
...
Supported fields के लिए [Secrets Management](</hi/gateway/secrets>) और [SecretRef credential surface](</hi/reference/secretref-credential-surface>) देखें।
...
Non-Windows Gateway hosts पर, bash और zsh `exec` commands default रूप से startup snapshot का उपयोग करते हैं। इस path को disable करने के लिए Gateway process environment में `OPENCLAW_EXEC_SHELL_SNAPSHOT=0` set करें। Values `false`, `no`, और `off` भी इसे disable करते हैं। Per-call `exec.env` values snapshots toggle नहीं कर सकते या snapshot cache redirect नहीं कर सकते।
...
ये runtime markers हैं (required user config नहीं)। इन्हें shell/profile logic में context-specific rules लागू करने के लिए उपयोग किया जा सकता है।
...
दोनों activation time पर process env से resolve होते हैं। SecretRef details [Secrets Management](</hi/gateway/secrets>) में documented हैं। Config `env` block स्वयं SecretRefs या `file:...` shorthand values resolve नहीं करता।
...
यदि Node.js **nvm** के माध्यम से installed था (system package manager नहीं), तो built-in `fetch()` nvm के bundled CA store का उपयोग करता है, जिसमें modern root CAs missing हो सकते हैं (Let’s Encrypt के लिए ISRG Root X1/X2, DigiCert Global Root G2, आदि)। इससे अधिकांश HTTPS sites पर `web_fetch` `"fetch failed"` के साथ fail होता है।
...
इस variable के लिए केवल `~/.openclaw/.env` में लिखने पर rely न करें; Node process startup पर `NODE_EXTRA_CA_CERTS` पढ़ता है।

## Legacy environment variables

OpenClaw केवल `OPENCLAW_*` environment variables पढ़ता है। Earlier releases से legacy `CLAWDBOT_*` और `MOLTBOT_*` prefixes silently ignored होते हैं।

यदि startup पर Gateway process पर इनमें से कोई भी अब भी set है, तो OpenClaw एक single Node deprecation warning (`OPENCLAW_LEGACY_ENV_VARS`) emit करता है, जिसमें detected prefixes और total count listed होते हैं। Legacy prefix को `OPENCLAW_` से replace करके प्रत्येक value rename करें (उदाहरण `CLAWDBOT_GATEWAY_TOKEN` → `OPENCLAW_GATEWAY_TOKEN`); old names का कोई effect नहीं होता।
