---
title: عیب‌یابی
source_url: https://docs.openclaw.ai/fa/gateway/troubleshooting
...
OpenClaw هر ریشهٔ مهارت را به‌عنوان مرز containment در نظر می‌گیرد. یک symlink زیر `~/.agents/skills`، `<workspace>/.agents/skills`، `<workspace>/skills` یا `~/.openclaw/skills` وقتی رد می‌شود که هدف واقعی آن بیرون از آن root resolve شود، مگر اینکه هدف به‌صراحت trusted باشد.
...
از هدف‌های گسترده مانند `~`، `/` یا کل یک پوشهٔ پروژهٔ sync‌شده استفاده نکنید. `allowSymlinkTargets` را به ریشهٔ واقعی مهارت که شامل دایرکتوری‌های trusted `SKILL.md` است محدود نگه دارید.
...
دنبال این موارد بگردید:
  ...
  * خطاهای `model_not_found` یا 404 با اینکه `/v1/chat/completions` مستقیم با همان bare model id کار می‌کند
...
زمانی از این استفاده کنید که راه‌اندازی Gateway با `Invalid config` شکست می‌خورد یا لاگ‌های بارگذاری مجدد داغ می‌گویند یک ویرایش نامعتبر را رد کرده است.
...
۱\. رفتار بازنویسی احراز هویت و URL تغییر کرده است bashCopy code
[code]
...
۲\. حفاظ‌های bind و احراز هویت سخت‌گیرانه‌تر شده‌اند bashCopy code
[code]
...
۳\. وضعیت جفت‌سازی و هویت دستگاه تغییر کرده است bashCopy code
[code]
