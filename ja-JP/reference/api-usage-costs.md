---
title: API の使用量とコスト
source_url: https://docs.openclaw.ai/ja-JP/reference/api-usage-costs
...
**CLI 使用量ウィンドウ（プロバイダークォータ）**
  ...
  * 現在の使用量ウィンドウプロバイダー: Anthropic、GitHub Copilot、Gemini CLI、OpenAI Codex、MiniMax、Xiaomi、[z.ai](<http://z.ai>)。
...
これには、**OpenAI Codex** 、**Alibaba Cloud Model Studio Coding Plan** 、**MiniMax Coding Plan** 、**[Z.AI](<http://Z.AI>) / GLM Coding Plan**、および **Extra Usage** が有効な Anthropic の OpenClaw Claude ログイン経路など、OpenClaw のローカル UI 外で引き続き課金されるサブスクリプション形式のホステッドプロバイダーも含まれます。
...
受信メディアは、返信の実行前に要約/文字起こしされることがあります。これはモデル/プロバイダー API を使用します。
  ...
  * 画像: OpenAI / OpenRouter / Anthropic / DeepInfra / Google / MiniMax / Moonshot / Qwen / [Z.AI](<http://Z.AI>)。
