...
Use `openclaw/plugin-sdk/channel-inbound` for the shared layer. Good fit for plugin-local logic:
...
OpenClaw loads this instead of the full entry when the channel is disabled or unconfigured. It avoids pulling in heavy runtime code during setup flows. See [Setup and Config](</plugins/sdk-setup#setup-entry>) for details.
...
[Getting Started](</plugins/building-plugins>)[Provider Plugins](</plugins/sdk-provider-plugins>)
