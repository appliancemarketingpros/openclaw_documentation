...
source_url: https://docs.openclaw.ai/fr/plugins/codex-native-plugins
...
**`app_inaccessible`, `app_disabled` ou `app_missing` :** la migration n’a pas installé le plugin parce que l’inventaire des applications Codex source n’a pas indiqué toutes les applications possédées comme présentes, activées et accessibles lorsque `--verify-plugin-apps` était défini. Réautorisez ou activez l’application dans Codex, puis relancez la migration avec `--verify-plugin-apps`.
