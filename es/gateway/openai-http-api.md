...
source_url: https://docs.openclaw.ai/es/gateway/openai-http-api
...
Rutas comunes de autenticación HTTP:
  ...
  * autenticación HTTP confiable con identidad (`gateway.auth.mode="trusted-proxy"`): enruta a través del proxy configurado con conocimiento de identidad y deja que inyecte los encabezados de identidad requeridos
...
Notas:
  ...
  * Cuando `gateway.auth.mode="trusted-proxy"`, la solicitud HTTP debe provenir de una fuente de proxy confiable configurada; los proxies de loopback del mismo host requieren `gateway.auth.trustedProxy.allowLoopback = true` explícito.
...
Matriz de autenticación:
  ...
  * `gateway.auth.mode="token"` o `"password"` \+ `Authorization: Bearer ...`
    ...
    * restaura el conjunto completo de alcances predeterminados de operador: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
Los ids devueltos son entradas `openclaw`, `openclaw/default` y `openclaw/<agentId>`. Úsalos directamente como valores `model` de OpenAI.
...
Ejemplos: `x-openclaw-model: openai/gpt-5.4` `x-openclaw-model: gpt-5.5`
...
Usa `model: "openclaw/default"` o `model: "openclaw/<agentId>"`. Cuando necesites un modelo de embeddings específico, envíalo en `x-openclaw-model`. Sin ese encabezado, la solicitud pasa a la configuración normal de embeddings del agente seleccionado.
