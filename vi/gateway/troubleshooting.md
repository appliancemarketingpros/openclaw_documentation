...
source_url: https://docs.openclaw.ai/vi/gateway/troubleshooting
...
OpenClaw xem mỗi thư mục gốc skill là một ranh giới chứa. Một symlink dưới `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, hoặc `~/.openclaw/skills` sẽ bị bỏ qua khi đích thật của nó phân giải ra ngoài thư mục gốc đó trừ khi đích được tin cậy rõ ràng.
...
Nếu đích là có chủ ý, hãy cấu hình cả thư mục gốc skill trực tiếp và đích symlink được cho phép:
...
Không dùng các đích quá rộng như `~`, `/`, hoặc toàn bộ thư mục dự án đã đồng bộ. Giữ `allowSymlinkTargets` được giới hạn trong thư mục gốc skill thật có chứa các thư mục `SKILL.md` đáng tin cậy.
...
Dùng mục này khi Gateway khởi động thất bại với `Invalid config` hoặc nhật ký tải lại nóng cho biết nó đã bỏ qua một chỉnh sửa không hợp lệ.
...
Dấu hiệu Chrome MCP / phiên hiện có
...
1\. Hành vi xác thực và ghi đè URL đã thay đổi bashCopy code
[code]
...
2\. Rào chắn bind và xác thực nghiêm ngặt hơn bashCopy code
[code]
...
3\. Trạng thái ghép đôi và danh tính thiết bị đã thay đổi bashCopy code
[code]
