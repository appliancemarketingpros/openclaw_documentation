---
title: 브리지 프로토콜
source_url: https://docs.openclaw.ai/ko/gateway/bridge-protocol
...
Node는 system.run 활동을 표시하기 위해 `exec.finished` 또는 `exec.denied` 이벤트를 내보낼 수 있습니다. 이 이벤트는 Gateway의 시스템 이벤트에 매핑됩니다. (레거시 Node는 여전히 `exec.started`를 내보낼 수 있습니다.)
...
## 역사적 tailnet 사용
  ...
  * 브리지를 tailnet IP에 바인딩: `~/.openclaw/openclaw.json`의 `bridge.bind: "tailnet"` (역사적 참고용만 해당, `bridge.*`는 더 이상 유효하지 않음).
