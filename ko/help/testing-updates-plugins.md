...
source_url: https://docs.openclaw.ai/ko/help/testing-updates-plugins
...
아티팩트 identity부터 시작하세요:
  ...
  * 패키지 승인 `resolve_package` 요약: 소스, 버전, SHA-256 및 아티팩트 이름.
  * Docker 아티팩트: `.artifacts/docker-tests/**/summary.json`, `failures.json`, 레인 로그 및 재실행 명령.
  * 업그레이드 생존자 요약: `.artifacts/upgrade-survivor/summary.json`, 기준 버전, 후보 버전, 시나리오, 단계별 타이밍 및 레시피 단계를 포함합니다.


전체 릴리스 우산을 다시 실행하기보다 동일한 패키지 아티팩트로 실패한 정확한 레인을 다시 실행하는 것을 선호합니다.
