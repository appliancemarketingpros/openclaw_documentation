---
title: 平台
source_url: https://docs.openclaw.ai/zh-CN/platforms
scraped_at: 2026-05-25
---

OpenClaw 核心使用 TypeScript 编写。**Node 是推荐的运行时** 。 不推荐将 Bun 用于 Gateway 网关 —— WhatsApp 和 Telegram 渠道存在已知问题；详情见 [Bun（实验性）](</zh-CN/install/bun>)。

macOS（菜单栏应用）和移动节点（iOS/Android）已有配套应用。Windows 和 Linux 配套应用已在计划中，但 Gateway 网关目前已完全支持。 Windows 原生配套应用也已在计划中；建议通过 WSL2 使用 Gateway 网关。
...
## VPS 和托管
  ...
  * [Fly.io](<http://Fly.io>): [Fly.io](</zh-CN/install/fly>)
