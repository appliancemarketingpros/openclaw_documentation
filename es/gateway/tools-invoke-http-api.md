...
source_url: https://docs.openclaw.ai/es/gateway/tools-invoke-http-api
...
Notas:
  ...
  * Cuando `gateway.auth.mode="trusted-proxy"`, la solicitud HTTP debe provenir de una fuente de proxy de confianza configurada; los proxies loopback en el mismo host requieren `gateway.auth.trustedProxy.allowLoopback = true` explícito.
...
Matriz de autenticación:
  ...
  * `gateway.auth.mode="token"` o `"password"` \+ `Authorization: Bearer ...`
    ...
    * restaura el conjunto completo predeterminado de alcances de operador: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
