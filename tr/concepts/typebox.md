---
title: TypeBox
source_url: https://docs.openclaw.ai/tr/concepts/typebox
scraped_at: 2026-05-25
---

TypeBox, TypeScript öncelikli bir şema kütüphanesidir. Bunu **Gateway WebSocket protokolünü** (el sıkışma, istek/yanıt, sunucu olayları) tanımlamak için kullanıyoruz. Bu şemalar, macOS uygulaması için **çalışma zamanı doğrulamasını** , **JSON Schema dışa aktarımını** ve **Swift kod üretimini** yönlendirir. Tek doğruluk kaynağı; geri kalan her şey üretilir.

Daha üst düzey protokol bağlamını istiyorsanız [Gateway mimarisi](</tr/concepts/architecture>) ile başlayın.
...
İlk çerçeve **mutlaka** bir `connect` isteği olmalıdır. Bundan sonra istemciler yöntemleri çağırabilir (örn. `health`, `send`, `chat.send`) ve olaylara abone olabilir (örn. `presence`, `tick`, `agent`).
...
Yetkili olarak duyurulan **keşif** envanteri `src/gateway/server-methods-list.ts` içinde yaşar (`listGatewayMethods`, `GATEWAY_EVENTS`).
...
## Şemaların çalışma zamanında nasıl kullanıldığı
  ...
  * **Özellik keşfi** : Gateway, `listGatewayMethods()` ve `GATEWAY_EVENTS` üzerinden `hello-ok` içinde korumacı bir `features.methods` ve `features.events` listesi gönderir.
...
Bunu `src/gateway/server-methods.ts` içinde kaydedin (zaten `systemHandlers` ile birleştirir), ardından `src/gateway/server-methods-list.ts` içindeki `listGatewayMethods` girdisine `"system.echo"` ekleyin.

Yöntem operatör veya node istemcileri tarafından çağrılabiliyorsa, kapsam zorlaması ve `hello-ok` özellik duyurusu hizalı kalsın diye bunu ayrıca `src/gateway/method-scopes.ts` içinde sınıflandırın.
...
## Sürümleme + uyumluluk
  ...
  * İstemciler `minProtocol` \+ `maxProtocol` gönderir; sunucu, mevcut protokolünü içermeyen aralıkları reddeder.
...
## Şema kalıpları ve kuralları
  ...
  * `agent`, çalışma zamanında üretilen orkestrasyon bağlamı için isteğe bağlı `internalEvents` kabul eder (örneğin alt ajan/cron görevi tamamlanma devri); bunu dahili API yüzeyi olarak ele alın.


## Canlı şema JSON'u

Üretilmiş JSON Schema depoda `dist/protocol.schema.json` konumundadır. Yayımlanan ham dosya genellikle şurada bulunur:
...
## Şemaları değiştirdiğinizde
  ...
  3. Yeni RPC operatör veya node kapsam sınıflandırmasına ihtiyaç duyduğunda `src/gateway/method-scopes.ts` dosyasını güncelleyin.
