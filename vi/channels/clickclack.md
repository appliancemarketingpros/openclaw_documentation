---
title: ClickClack
source_url: https://docs.openclaw.ai/vi/channels/clickclack
...
`replyMode: "model"` dùng trực tiếp `api.runtime.llm.complete` cho các phản hồi bot ngắn. Khi một tài khoản đặt `agentId`, OpenClaw yêu cầu bit tin cậy rõ ràng `plugins.entries.clickclack.llm.allowAgentIdOverride` để plugin có thể chạy hoàn tất cho agent bot đó. Hãy tắt tùy chọn này nếu bạn chỉ dùng tuyến agent mặc định.
