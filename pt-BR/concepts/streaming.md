...
source_url: https://docs.openclaw.ai/pt-BR/concepts/streaming
...
**Observação sobre canais:** o streaming de blocos fica **desativado a menos que** `*.blockStreaming` seja definido explicitamente como `true`. Canais podem transmitir uma prévia ao vivo (`channels.<channel>.streaming`) sem respostas em bloco.
...
`streaming.mode: "block"` é um modo de streaming de prévia para canais com capacidade de edição, como Discord e Telegram. Ele não habilita a entrega de blocos do canal nesses canais. Use `streaming.block.enabled` ou a chave legada de canal `blockStreaming` quando quiser respostas em bloco normais. Microsoft Teams é a exceção: ele não tem transporte de bloco de prévia de rascunho, então `streaming.mode: "block"` mapeia para entrega de blocos do Teams em vez de streaming parcial/progresso nativo.
