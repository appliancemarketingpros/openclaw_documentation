---
title: Signal
source_url: https://docs.openclaw.ai/nl/channels/signal
...
Als je de JVM-build (`signal-cli-${VERSION}.tar.gz`) gebruikt, installeer dan eerst JRE 25+. Houd `signal-cli` bijgewerkt; upstream vermeldt dat oude releases kunnen breken wanneer Signal-server-API's veranderen.
...
Provideropties:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (standaard: auto). Zie Containermodus.
