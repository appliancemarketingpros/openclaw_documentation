---
title: TypeBox
source_url: https://docs.openclaw.ai/zh-TW/concepts/typebox
scraped_at: 2026-05-25
---

TypeBox 是 TypeScript 優先的結構描述函式庫。我們使用它來定義 **Gateway WebSocket 協定** （交握、請求/回應、伺服器事件）。這些結構描述會驅動 **執行階段驗證** 、**JSON Schema 匯出** ，以及 macOS 應用程式的 **Swift 程式碼產生** 。單一真相來源；其他所有內容皆由此產生。

如果你想了解較高階的協定脈絡，請從 [Gateway 架構](</zh-TW/concepts/architecture>)開始。
...
第一個框架**必須** 是 `connect` 請求。之後，客戶端可以呼叫 方法（例如 `health`、`send`、`chat.send`）並訂閱事件（例如 `presence`、`tick`、`agent`）。
...
權威的已宣告 **探索** 清單位於 `src/gateway/server-methods-list.ts`（`listGatewayMethods`、`GATEWAY_EVENTS`）。

## 結構描述所在位置

  * 來源：`src/gateway/protocol/schema.ts`
  * 執行階段驗證器（AJV）：`src/gateway/protocol/index.ts`
  * 已宣告功能/探索登錄：`src/gateway/server-methods-list.ts`
...
## 結構描述在執行階段的使用方式
  ...
  * **伺服器端** ：每個傳入框架都會使用 AJV 驗證。交握只接受 params 符合 `ConnectParams` 的 `connect` 請求。
  * **客戶端** ：JS 客戶端會在使用事件與回應框架前先驗證它們。
  * **功能探索** ：Gateway 會在 `hello-ok` 中從 `listGatewayMethods()` 和 `GATEWAY_EVENTS` 傳送保守的 `features.methods` 與 `features.events` 清單。
  * 該探索清單不是 `coreGatewayHandlers` 中每個可呼叫 helper 的產生式傾印； 有些 helper RPC 實作於 `src/gateway/server-methods/*.ts`，但未列舉在已宣告的 功能清單中。
...
在 `src/gateway/server-methods/system.ts` 新增 handler：
...
在 `src/gateway/server-methods.ts` 中註冊它（已合併 `systemHandlers`）， 然後將 `"system.echo"` 加入 `src/gateway/server-methods-list.ts` 中的 `listGatewayMethods` 輸入。

如果該方法可由操作者或節點客戶端呼叫，也要在 `src/gateway/method-scopes.ts` 中分類，使範圍強制執行與 `hello-ok` 功能宣告 保持一致。
...
在 `src/gateway/server.*.test.ts` 新增伺服器測試，並在文件中註記該方法。

## Swift 程式碼產生行為

Swift 產生器會輸出：

  * 含 `req`、`res`、`event` 與 `unknown` case 的 `GatewayFrame` enum
  * 強型別 payload struct/enum
  * `ErrorCode` 值、`GATEWAY_PROTOCOL_VERSION` 與 `GATEWAY_MIN_PROTOCOL_VERSION`


未知框架類型會保留為原始 payload，以維持向前相容性。

## 版本控管 + 相容性

  * `PROTOCOL_VERSION` 位於 `src/gateway/protocol/version.ts`。
  * 客戶端傳送 `minProtocol` \+ `maxProtocol`；伺服器會拒絕不包含其目前協定版本的範圍。
  * Swift 模型會保留未知框架類型，以避免破壞較舊客戶端。


## 結構描述模式與慣例

  * 多數物件使用 `additionalProperties: false` 來保持 payload 嚴格。
  * `NonEmptyString` 是 ID 與方法/事件名稱的預設值。
  * 最上層的 `GatewayFrame` 在 `type` 上使用**鑑別器** 。
  * 有副作用的方法通常要求 params 中包含 `idempotencyKey` （範例：`send`、`poll`、`agent`、`chat.send`）。
  * `agent` 接受選用的 `internalEvents`，用於執行階段產生的協調脈絡 （例如子代理/Cron 任務完成交接）；請將此視為內部 API 介面。


## 即時結構描述 JSON

產生的 JSON Schema 位於 repo 的 `dist/protocol.schema.json`。已發布的原始檔案通常可在以下位置取得：
...
## 變更結構描述時
  ...
  3. 當新的 RPC 需要操作者或節點範圍分類時，更新 `src/gateway/method-scopes.ts`。
  4. 執行 `pnpm protocol:check`。
  5. 提交重新產生的 schema + Swift 模型。
