...
source_url: https://docs.openclaw.ai/tr/tools/searxng-search
scraped_at: 2026-05-25
---

OpenClaw, **kendi barındırdığınız, anahtarsız** bir `web_search` sağlayıcısı olarak [SearXNG](<https://docs.searxng.org/>) desteği sunar. SearXNG; Google, Bing, DuckDuckGo ve diğer kaynaklardan sonuçları bir araya getiren açık kaynaklı bir meta arama motorudur.
...
Ya da erişiminiz olan mevcut herhangi bir SearXNG dağıtımını kullanın. Üretim kurulumu için [SearXNG belgelerine](<https://docs.searxng.org/>) bakın.
...
## Notlar
  ...
  * **Temel URL doğrulaması** \-- `baseUrl` geçerli bir `http://` veya `https://` URL'si olmalıdır; genel ana makineler `https://` kullanmalıdır
  ...
  * **Kategori geri dönüşü** \-- `general` olmayan bir kategori isteği başarılı olur ancak sıfır sonuç döndürürse OpenClaw, boş bir sonuç kümesi döndürmeden önce aynı sorguyu bir kez `general` ile yeniden dener
