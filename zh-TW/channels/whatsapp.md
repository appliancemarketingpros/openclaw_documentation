---
title: WhatsApp
source_url: https://docs.openclaw.ai/zh-TW/channels/whatsapp
...
[**Pairing** 預設 DM 政策是對未知寄件者進行配對。 ](</zh-TW/channels/pairing>) [**Channel troubleshooting** 跨通道診斷與修復手冊。 ](</zh-TW/channels/troubleshooting>) [**Gateway configuration** 完整通道設定模式與範例。 ](</zh-TW/gateway/configuration>)
...
WhatsApp 傳入訊息可能包含個人訊息內容、電話號碼、 群組識別碼、寄件者名稱，以及工作階段關聯欄位。因此， 除非你明確選擇加入，WhatsApp 不會將傳入的 `message_received` hook payload 廣播給 Plugin：
...
可用時，也會填入回覆中繼資料欄位（`ReplyToId`、`ReplyToBody`、`ReplyToSender`、寄件者 JID/E.164）。 當引用回覆目標是可下載媒體時，OpenClaw 會透過 正常的傳入媒體儲存區儲存它，並將其公開為 `MediaPath`/`MediaType`，讓 代理程式可以檢查所參照的圖片，而不是只看到 `<media:image>`。
...
對已授權群組語音訊息，當 內文只有 `<media:audio>` 時，會在提及門檻判斷前先轉錄，因此在語音訊息中說出 bot 提及即可 觸發回覆。如果逐字稿仍未提及 bot， 則逐字稿會保留在待處理群組歷史中，而非原始預留位置。
...
WhatsApp 支援透過 `channels.whatsapp.ackReaction` 在收到傳入訊息時立即傳送確認反應。 確認反應受 `reactionLevel` 閘控，當 `reactionLevel` 為 `"off"` 時會被抑制。
