---
title: QA 채널
source_url: https://docs.openclaw.ai/ko/channels/qa-channel
...
계정 키:
  ...
  * `allowFrom` \- 발신자 허용 목록입니다(사용자 ID 또는 `"*"`). 직접 메시지와 허용 목록 기반 그룹 정책 모두 이 합성 발신자 ID를 사용합니다.
  * `groupPolicy` \- 공유 룸 정책: `"open"`(기본값), `"allowlist"` 또는 `"disabled"`입니다.
  * `groupAllowFrom` \- 선택 사항인 공유 룸 발신자 허용 목록입니다. `"allowlist"`에서 생략하면 QA Channel은 `allowFrom`으로 대체합니다.
  * `groups.<room>.requireMention` \- 특정 그룹/채널 룸에서 응답하기 전에 봇 멘션을 요구합니다. `groups."*"`는 기본값을 설정합니다.
