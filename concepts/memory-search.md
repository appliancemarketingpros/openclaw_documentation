...
With Gemini Embedding 2, you can index images and audio files alongside Markdown. Search queries remain text, but they match against visual and audio content. See the [Memory configuration reference](</reference/memory-config>) for setup.
...
You can optionally index session transcripts so `memory_search` can recall earlier conversations. This is opt-in via `memorySearch.experimental.sessionMemory`. See the [configuration reference](</reference/memory-config>) for details.
...
[Honcho memory](</concepts/memory-honcho>)[Active memory](</concepts/active-memory>)
