---
title: WhatsApp
source_url: https://docs.openclaw.ai/pt-BR/channels/whatsapp
...
## Instalação (sob demanda)
  ...
  * O onboarding (`openclaw onboard`) e `openclaw channels add --channel whatsapp` solicitam a instalação do plugin do WhatsApp na primeira vez que você o seleciona.
  ...
  * Estável/Beta: usa o pacote npm `@openclaw/whatsapp` na tag de lançamento oficial atual.
...
No Windows, o plugin do WhatsApp precisa do Git no `PATH` durante a instalação npm porque uma de suas dependências Baileys/libsignal é obtida de uma URL git. Instale o Git for Windows, depois reinicie o shell e execute novamente a instalação:
...
[**Pareamento** A política padrão de DM é pareamento para remetentes desconhecidos. ](</pt-BR/channels/pairing>) [**Solução de problemas de canais** Diagnósticos entre canais e playbooks de reparo. ](</pt-BR/channels/troubleshooting>) [**Configuração do Gateway** Padrões e exemplos completos de configuração de canais. ](</pt-BR/gateway/configuration>)

## Configuração rápida

* ### Configurar a política de acesso do WhatsApp
...
Mensagens recebidas do WhatsApp podem conter conteúdo pessoal de mensagens, números de telefone, identificadores de grupo, nomes de remetentes e campos de correlação de sessão. Por esse motivo, o WhatsApp não transmite payloads de hook `message_received` recebidos para plugins a menos que você aceite explicitamente:
...
Ative isso apenas para plugins nos quais você confia para receber conteúdo e identificadores de mensagens recebidas do WhatsApp.
...
### Política de grupos + listas de permissão
...
Campos de metadados de resposta também são preenchidos quando disponíveis (`ReplyToId`, `ReplyToBody`, `ReplyToSender`, JID/E.164 do remetente). Quando o destino da resposta citada é mídia baixável, a OpenClaw a salva pelo armazenamento normal de mídia recebida e a expõe como `MediaPath`/`MediaType` para que o agente possa inspecionar a imagem referenciada em vez de ver apenas `<media:image>`.
...
Notas de voz de grupos autorizados são transcritas antes do bloqueio por menção quando o corpo é apenas `<media:audio>`, então dizer a menção do bot na nota de voz pode acionar a resposta. Se a transcrição ainda não mencionar o bot, a transcrição é mantida no histórico pendente do grupo em vez do placeholder bruto.
...
O WhatsApp oferece suporte a reações de confirmação imediatas no recebimento de entrada via `channels.whatsapp.ackReaction`. Reações de confirmação são controladas por `reactionLevel` — elas são suprimidas quando `reactionLevel` é `"off"`.
...
Contas silenciosas podem permanecer conectadas além do tempo limite normal de mensagens; o watchdog reinicia quando a atividade de transporte do WhatsApp Web para, o socket fecha ou a atividade no nível da aplicação permanece silenciosa além da janela de segurança mais longa.

Se os logs mostrarem `status=408 Request Time-out Connection was lost` repetido, ajuste os tempos de socket do Baileys em `web.whatsapp`. Comece encurtando `keepAliveIntervalMs` para abaixo do tempo limite de inatividade da sua rede e aumentando `connectTimeoutMs` em links lentos ou com perda:
...
Se `~/.openclaw/logs/whatsapp-health.log` disser `Gateway inactive`, mas `openclaw gateway status` e `openclaw channels status --probe` mostrarem que o Gateway e o WhatsApp estão saudáveis, execute `openclaw doctor`. No Linux, o doctor avisa sobre entradas de crontab legadas que ainda invocam `~/.openclaw/bin/ensure-whatsapp.sh`; remova essas entradas obsoletas com `crontab -e`, porque o cron pode não ter o ambiente de barramento de usuário do systemd e fazer esse script antigo informar incorretamente a integridade do Gateway.
