...
Quick rules
  ...
  * Note: provider runtime `capabilities` is shared runner metadata (provider family, transcript/tooling quirks, transport/cache hints). It is not the same as the [public capability model](</plugins/architecture#public-capability-model>) which describes what a plugin registers (text inference, speech, etc.).
...
[Models CLI](</concepts/models>)[Model Failover](</concepts/model-failover>)
