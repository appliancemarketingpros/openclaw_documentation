...
With multi-agent routing, each agent can have its own sandbox + tool policy: use this to give **full access** , **read-only** , or **no access** per agent. See [Multi-Agent Sandbox & Tools](</tools/multi-agent-sandbox-tools>) for full details and precedence rules. Common use cases:
...
Copy
[code]pre-commit run --all-files detect-secrets
...
[Troubleshooting](</gateway/troubleshooting>)[Sandboxing](</gateway/sandboxing>)
