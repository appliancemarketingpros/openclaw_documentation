---
title: 線上狀態
source_url: https://docs.openclaw.ai/zh-TW/concepts/presence
...
[**輸入指示器** 何時傳送輸入指示器，以及如何調整它們。 ](</zh-TW/concepts/typing-indicators>) [**串流與分塊** 對外串流、分塊，以及每個通道的格式設定。 ](</zh-TW/concepts/streaming>) [**Gateway 架構** Gateway 元件，以及驅動存在狀態更新的 WebSocket 協定。 ](</zh-TW/concepts/architecture>) [**Gateway 協定** `connect`、`system-event` 和 `system-presence` 的線路協定。 ](</zh-TW/gateway/protocol>)
