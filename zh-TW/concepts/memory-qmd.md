---
title: QMD 記憶引擎
source_url: https://docs.openclaw.ai/zh-TW/concepts/memory-qmd
...
**找不到 QMD？** 請確認二進位檔位於 Gateway 的 `PATH` 中。如果 OpenClaw 以服務形式執行，請建立符號連結： `sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`。
...
**工作區可見的暫存 repos 導致`ENAMETOOLONG` 或索引損壞？** QMD traversal 目前遵循底層 QMD 掃描器行為，而不是 OpenClaw 的內建符號連結規則。在 QMD 公開 cycle-safe traversal 或明確排除控制之前，請將暫存 monorepo checkouts 放在 `.tmp/` 等隱藏目錄下，或放在已索引 QMD roots 之外。
