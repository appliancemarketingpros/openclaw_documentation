...
source_url: https://docs.openclaw.ai/fa/nodes/audio
...
اگر **مدل‌ها را پیکربندی نکنید** و `tools.media.audio.enabled` روی `false` تنظیم **نشده باشد** ، OpenClaw به این ترتیب شناسایی خودکار را انجام می‌دهد و در نخستین گزینه کارآمد متوقف می‌شود:
...
برای غیرفعال کردن شناسایی خودکار، `tools.media.audio.enabled: false` را تنظیم کنید. برای سفارشی‌سازی، `tools.media.audio.models` را تنظیم کنید. نکته: شناسایی باینری در macOS/Linux/Windows به‌صورت بهترین تلاش انجام می‌شود؛ مطمئن شوید CLI روی `PATH` قرار دارد (ما `~` را گسترش می‌دهیم)، یا یک مدل CLI صریح با مسیر کامل فرمان تنظیم کنید.
