---
title: 构建 CLI 后端插件
source_url: https://docs.openclaw.ai/zh-CN/plugins/cli-backend-plugins
...
只有当 CLI 实际能够使用该桥接时才启用它。如果 CLI 有自己的内置工具层且无法禁用，请设置 `nativeToolMode: "always-on"`，这样当调用方要求没有原生工具时，OpenClaw 可以失败关闭。
...
OPENCLAW_DOCS_MARKER:calloutOpen:Q2hlY2s `package.json` 具有 `openclaw.extensions`，并且已发布包具有构建后的运行时入口 OPENCLAW_DOCS_MARKER:calloutClose:

OPENCLAW_DOCS_MARKER:calloutOpen:Q2hlY2s `openclaw.plugin.json` 声明 `cliBackends` 和有意设置的 `activation.onStartup` OPENCLAW_DOCS_MARKER:calloutClose:

OPENCLAW_DOCS_MARKER:calloutOpen:Q2hlY2s 当设置/模型设备发现应在冷状态看到后端时，存在 `setup.cliBackends` OPENCLAW_DOCS_MARKER:calloutClose:

OPENCLAW_DOCS_MARKER:calloutOpen:Q2hlY2s `api.registerCliBackend(...)` 使用与清单相同的后端 id OPENCLAW_DOCS_MARKER:calloutClose:

OPENCLAW_DOCS_MARKER:calloutOpen:Q2hlY2s `agents.defaults.cliBackends.<id>` 下的用户覆盖仍然优先 OPENCLAW_DOCS_MARKER:calloutClose:
