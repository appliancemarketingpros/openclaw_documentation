...
バンドルされた `oc-path` Plugin は、`oc://` ワークスペースファイルアドレス指定スキーム用の [`openclaw path`](</ja-JP/cli/path>) CLI を追加します。これは OpenClaw リポジトリの `extensions/oc-path/` 配下に同梱されていますが、オプトインです。インストールやビルドをしても、有効化するまでは休止状態のままです。
