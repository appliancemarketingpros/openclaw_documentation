...
source_url: https://docs.openclaw.ai/zh-TW/providers/huggingface
...
設定：DeepSeek R1 搭配 Qwen 後備 json5Copy code
[code]
...
設定：Qwen 搭配 cheapest 與 fastest 變體 json5Copy code
[code]
...
設定：DeepSeek + Llama + GPT-OSS 搭配別名 json5Copy code
[code]
...
設定：多個 Qwen 和 DeepSeek 搭配策略尾碼 json5Copy code
[code]
...
[**模型選擇** 所有提供者、模型參照和容錯移轉行為的概觀。 ](</zh-TW/concepts/model-providers>) [**模型選擇** 如何選擇和設定模型。 ](</zh-TW/concepts/models>) [**Inference Providers 文件** Hugging Face Inference Providers 官方文件。 ](<https://huggingface.co/docs/inference-providers>) [**設定** 完整設定參考。 ](</zh-TW/gateway/configuration>)
