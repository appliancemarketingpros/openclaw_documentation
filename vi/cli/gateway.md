---
title: Gateway
source_url: https://docs.openclaw.ai/vi/cli/gateway
scraped_at: 2026-05-25
---

Gateway là máy chủ WebSocket của OpenClaw (kênh, node, phiên, hook). Các lệnh con trên trang này nằm dưới `openclaw gateway …`.

[**Bonjour discovery** Thiết lập mDNS cục bộ + DNS-SD diện rộng. ](</vi/gateway/bonjour>) [**Discovery overview** Cách OpenClaw quảng bá và tìm Gateway. ](</vi/gateway/discovery>) [**Configuration** Các khóa cấu hình Gateway cấp cao nhất. ](</vi/gateway/configuration>)
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tcG9ydCA8cG9ydA " type="number"> Cổng WebSocket (mặc định đến từ cấu hình/env; thường là `18789`).

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdG9rZW4gPHRva2Vu " type="string"> Ghi đè token (đồng thời đặt `OPENCLAW_GATEWAY_TOKEN` cho tiến trình).
...
Tạo cấu hình phát triển + workspace nếu còn thiếu (bỏ qua [BOOTSTRAP.md](<http://BOOTSTRAP.md>)).
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tbGltaXQgPGxpbWl0 " type="number" default="25"> Số sự kiện gần đây tối đa cần bao gồm (tối đa `1000`).

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdHlwZSA8dHlwZQ " type="string"> Lọc theo loại sự kiện chẩn đoán, chẳng hạn như `payload.large` hoặc `diagnostic.memory.pressure`.
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tb3V0cHV0IDxwYXRo " type="string"> Đường dẫn đầu ra cho `--export`.
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tc3NoIDx0YXJnZXQ " type="string"> `user@host` hoặc `user@host:port` (cổng mặc định là `22`).
...
Dùng `--wrapper` khi dịch vụ được quản lý phải khởi động thông qua một tệp thực thi khác, ví dụ một shim trình quản lý bí mật hoặc trình trợ giúp chạy dưới tư cách người dùng khác. Wrapper nhận các đối số Gateway thông thường và chịu trách nhiệm cuối cùng exec `openclaw` hoặc Node với các đối số đó.
...
Bạn cũng có thể đặt wrapper thông qua môi trường. `gateway install` xác thực rằng đường dẫn là một tệp thực thi, ghi wrapper vào `ProgramArguments` của dịch vụ, và lưu `OPENCLAW_WRAPPER` trong môi trường dịch vụ cho các lần cài đặt lại bắt buộc, cập nhật và sửa chữa bằng doctor sau này.
