...
If bridge mode reports zero exported artifacts, the active memory plugin is not currently exposing public bridge inputs yet. Run `openclaw wiki doctor` first, then confirm the active memory plugin supports public artifacts.
...
Use this when you want QMD for recall and `memory-wiki` for a maintained knowledge layer:
[code] 
    {
      memory: {
        backend: "qmd",
...
[Voice Call Plugin](</plugins/voice-call>)[Zalo Personal Plugin](</plugins/zalouser>)
