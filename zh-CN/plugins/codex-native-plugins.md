...
source_url: https://docs.openclaw.ai/zh-CN/plugins/codex-native-plugins
...
**`app_inaccessible`、`app_disabled` 或 `app_missing`：** 迁移没有安装该插件，因为在设置 `--verify-plugin-apps` 时，源 Codex 应用清单没有显示所有拥有的应用都存在、已启用且可访问。请在 Codex 中重新授权或启用该应用，然后使用 `--verify-plugin-apps` 重新运行迁移。
