...
source_url: https://docs.openclaw.ai/vi/install/migrating-claude
...
Trạng thái Claude nằm ngoài ~/.claude

Truyền `--from /actual/path` (CLI) hoặc `--import-source /actual/path` (thiết lập ban đầu).

Thiết lập ban đầu từ chối nhập trên bản thiết lập hiện có
...
Máy chủ MCP từ Claude Desktop không được nhập
...
Lệnh Claude trở thành Skills với lời gọi mô hình bị tắt
