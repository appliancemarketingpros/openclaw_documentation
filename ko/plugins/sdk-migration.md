...
source_url: https://docs.openclaw.ai/ko/plugins/sdk-migration
scraped_at: 2026-05-25
---

OpenClaw는 광범위한 이전 버전 호환성 계층에서 집중적이고 문서화된 import를 갖춘 현대적인 Plugin 아키텍처로 이동했습니다. 새 아키텍처 이전에 Plugin을 빌드했다면, 이 가이드가 마이그레이션을 도와줍니다.

## 변경되는 사항

이전 Plugin 시스템은 Plugin이 단일 진입점에서 필요한 모든 것을 import할 수 있게 하는 두 개의 넓게 열린 표면을 제공했습니다.
...
광범위한 import 표면은 이제 **deprecated** 되었습니다. 런타임에서는 아직 동작하지만, 새 Plugin은 이를 사용해서는 안 되며, 기존 Plugin은 다음 major release에서 제거되기 전에 마이그레이션해야 합니다. Pi 전용 embedded extension factory 등록 API는 제거되었습니다. 대신 tool-result middleware를 사용하세요.

OpenClaw는 대체 기능을 도입하는 같은 변경에서 문서화된 Plugin 동작을 제거하거나 재해석하지 않습니다. breaking contract 변경은 먼저 호환성 adapter, diagnostics, 문서, deprecation window를 거쳐야 합니다. 이는 SDK import, manifest field, setup API, hook, 런타임 등록 동작에 적용됩니다.
...
현대적인 Plugin SDK는 이를 해결합니다. 각 import path(`openclaw/plugin-sdk/\<subpath\>`)는 명확한 목적과 문서화된 contract를 가진 작고 독립적인 모듈입니다.

bundled channel을 위한 legacy provider convenience seam도 사라졌습니다. channel-branded helper seam은 private mono-repo shortcut이었고, 안정적인 Plugin contract가 아니었습니다. 대신 좁은 generic SDK subpath를 사용하세요. bundled Plugin workspace 안에서는 provider 소유 헬퍼를 해당 Plugin 자체의 `api.ts` 또는 `runtime-api.ts`에 유지하세요.

현재 bundled provider 예시:

  * Anthropic은 Claude별 stream helper를 자체 `api.ts` / `contract-api.ts` seam에 유지합니다
  * OpenAI는 provider builder, default-model helper, realtime provider builder를 자체 `api.ts`에 유지합니다
  * OpenRouter는 provider builder와 onboarding/config helper를 자체 `api.ts`에 유지합니다


## Talk 및 realtime voice 마이그레이션 계획

Realtime voice, telephony, meeting, browser Talk 코드는 surface-local turn bookkeeping에서 `openclaw/plugin-sdk/realtime-voice`가 export하는 공유 Talk session controller로 이동하고 있습니다. 새 controller는 공통 Talk event envelope, active turn state, capture state, output-audio state, recent event history, stale-turn rejection을 소유합니다. Provider Plugin은 vendor-specific realtime session을 계속 소유해야 하며, surface Plugin은 capture, playback, telephony, meeting 특수 사항을 계속 소유해야 합니다.

이 Talk 마이그레이션은 의도적으로 breaking-clean입니다.

  1. 공유 controller/runtime primitive를 `plugin-sdk/realtime-voice`에 유지합니다.
  ...
  4. Gateway `hello-ok.features.events`에서 하나의 live Talk event channel을 알립니다: `talk.event`.
  5. 기존 realtime HTTP endpoint와 request-time instruction override path를 삭제합니다.


새 코드는 low-level adapter나 test fixture를 구현하는 경우가 아니라면 `createTalkEventSequencer(...)`를 직접 호출해서는 안 됩니다. turn-scoped event가 turn id 없이 emit될 수 없고, stale `turnEnd` / `turnCancel` 호출이 더 새로운 active turn을 clear할 수 없으며, output-audio lifecycle event가 telephony, meeting, browser relay, managed-room handoff, native Talk client 전반에서 일관되게 유지되도록 공유 controller를 선호하세요.
...
브라우저 소유 WebRTC/provider-websocket session은 `talk.client.create`를 사용합니다. 브라우저가 provider negotiation과 media transport를 소유하고, Gateway가 credential, instruction, tool policy를 소유하기 때문입니다. `talk.session.*`는 gateway-relay realtime, gateway-relay transcription, managed-room native STT/TTS session을 위한 공통 Gateway-managed surface입니다.

`talk.provider` / `talk.providers` 옆에 realtime selector를 둔 legacy config는 `openclaw doctor --fix`로 수리해야 합니다. runtime Talk는 speech/TTS provider config를 realtime provider config로 재해석하지 않습니다.
...
핵심에서 이를 동작시키기 위해 provider 또는 플랫폼 특수 사례를 도입하지 마세요. 핵심은 Talk 세션 의미론을 소유합니다. Provider Plugin은 벤더 세션 설정을 소유합니다. Voice-call과 Google Meet은 전화/회의 어댑터를 소유합니다. 브라우저와 네이티브 앱은 디바이스 캡처/재생 UX를 소유합니다.
...
유지 관리자는 현재 마이그레이션 대기열을 `pnpm plugins:boundary-report`로 감사할 수 있습니다. 간결한 집계에는 `pnpm plugins:boundary-report:summary`를, 하나의 Plugin 또는 호환성 소유자에는 `--owner <id>`를, CI 게이트가 기한이 된 호환성 레코드, 소유자 간 예약 SDK import, 또는 사용되지 않는 예약 SDK 하위 경로에서 실패해야 할 때는 `pnpm plugins:boundary-report:ci`를 사용하세요. 보고서는 사용 중단된 호환성 레코드를 제거 날짜별로 그룹화하고, 로컬 코드/문서 참조 수를 집계하며, 소유자 간 예약 SDK import를 표시하고, 비공개 memory-host SDK 브리지를 요약해 임시 검색에 의존하지 않고 호환성 정리를 명시적으로 유지합니다. 예약 SDK 하위 경로에는 추적되는 소유자 사용이 있어야 합니다. 사용되지 않는 예약 helper export는 공개 SDK에서 제거해야 합니다.

manifest 필드가 여전히 허용되는 경우, Plugin 작성자는 문서와 진단이 달리 말할 때까지 계속 사용할 수 있습니다. 새 코드는 문서화된 대체 항목을 선호해야 하지만, 기존 Plugin이 일반적인 마이너 릴리스 중에 깨져서는 안 됩니다.

## 마이그레이션 방법

* ### 런타임 config load/write helper 마이그레이션

번들 Plugin은 `api.runtime.config.loadConfig()`와 `api.runtime.config.writeConfigFile(...)`를 직접 호출하지 않아야 합니다. 활성 호출 경로에 이미 전달된 config를 선호하세요. 현재 프로세스 스냅샷이 필요한 장기 실행 handler는 `api.runtime.config.current()`를 사용할 수 있습니다. 장기 실행 agent tool은 config 쓰기 전에 생성된 tool도 갱신된 런타임 config를 볼 수 있도록 `execute` 내부에서 tool 컨텍스트의 `ctx.getRuntimeConfig()`를 사용해야 합니다.
...
호출자가 변경에 깔끔한 Gateway 재시작이 필요하다는 것을 알고 있을 때는 `afterWrite: { mode: "restart", reason: "..." }`를 사용하고, 호출자가 후속 조치를 소유하며 reload planner를 의도적으로 억제하려는 경우에만 `afterWrite: { mode: "none", reason: "..." }`를 사용하세요. Mutation 결과에는 테스트와 로깅을 위한 typed `followUp` 요약이 포함됩니다. Gateway는 재시작을 적용하거나 예약할 책임을 계속 가집니다. `loadConfig`와 `writeConfigFile`은 마이그레이션 기간 동안 외부 Plugin을 위한 사용 중단된 호환성 helper로 남으며 `runtime-config-load-write` 호환성 코드로 한 번 경고합니다. 번들 Plugin과 repo 런타임 코드는 `pnpm check:deprecated-api-usage`와 `pnpm check:no-runtime-action-load-config`의 scanner guardrail로 보호됩니다. 새 production Plugin 사용은 즉시 실패하고, 직접 config 쓰기는 실패하며, Gateway 서버 메서드는 요청 런타임 스냅샷을 사용해야 하고, 런타임 channel send/action/client helper는 경계에서 config를 받아야 하며, 장기 실행 런타임 모듈에는 허용되는 ambient `loadConfig()` 호출이 0개입니다.

새 Plugin 코드도 광범위한 `openclaw/plugin-sdk/config-runtime` 호환성 barrel을 import하지 않아야 합니다. 작업에 맞는 좁은 SDK 하위 경로를 사용하세요.
...
번들 Plugin과 해당 테스트는 광범위한 barrel에 대해 scanner로 보호되므로 import와 mock이 필요한 동작에만 로컬로 유지됩니다. 광범위한 barrel은 외부 호환성을 위해 여전히 존재하지만, 새 코드는 이에 의존하지 않아야 합니다.

* ### Pi tool-result extension을 middleware로 마이그레이션

번들 Plugin은 Pi 전용 `api.registerEmbeddedExtensionFactory(...)` tool-result handler를 런타임 중립 middleware로 교체해야 합니다.
...
외부 Plugin은 tool-result middleware를 등록할 수 없습니다. 모델이 보기 전에 높은 신뢰도의 tool 출력을 다시 쓸 수 있기 때문입니다.

* ### 승인 네이티브 handler를 capability fact로 마이그레이션

승인 가능 channel Plugin은 이제 `approvalCapability.nativeRuntime`과 공유 runtime-context registry를 통해 네이티브 승인 동작을 노출합니다.
...
Plugin이 `openclaw/plugin-sdk/windows-spawn`을 사용하는 경우, 해결되지 않은 Windows `.cmd`/`.bat` wrapper는 이제 명시적으로 `allowShellFallback: true`를 전달하지 않으면 닫힌 상태로 실패합니다.
...
* ### 광범위한 infra-runtime import 교체

`openclaw/plugin-sdk/infra-runtime`은 외부 호환성을 위해 여전히 존재하지만, 새 코드는 실제로 필요한 집중된 helper surface를 import해야 합니다.
...
번들 Plugin은 `infra-runtime`에 대해 scanner로 보호되므로 repo 코드가 광범위한 barrel로 회귀할 수 없습니다.

* ### Channel route helper 마이그레이션

새 channel route 코드는 `openclaw/plugin-sdk/channel-route`를 사용해야 합니다. 이전 route-key 및 comparable-target 이름은 마이그레이션 기간 동안 호환성 alias로 남지만, 새 Plugin은 동작을 직접 설명하는 route 이름을 사용해야 합니다:
...
최신 경로 helper는 네이티브 승인, 답장 억제, 인바운드 중복 제거, Cron 전달, 세션 라우팅 전반에서 `{ channel, to, accountId, threadId }`를 일관되게 정규화합니다. Plugin이 사용자 지정 대상 문법을 소유하는 경우, `resolveChannelRouteTargetWithParser(...)`를 사용하여 해당 parser를 동일한 경로 대상 계약에 맞게 조정하세요.
...
Common import path table 가져오기 경로 | 목적 | 주요 내보내기  
---|---|---  
...
이 표는 의도적으로 전체 SDK 표면이 아니라 공통 마이그레이션 하위 집합입니다. 컴파일러 진입점 인벤터리는 `scripts/lib/plugin-sdk-entrypoints.json`에 있으며, 패키지 exports는 공개 하위 집합에서 생성됩니다.

예약된 번들 Plugin 헬퍼 연결부는 명시적으로 문서화된 호환성 파사드(예: 게시된 `@openclaw/discord@2026.3.13` 패키지를 위해 유지되는 deprecated `plugin-sdk/discord` shim)를 제외하고 공개 SDK export map에서 제거되었습니다. 소유자별 헬퍼는 해당 소유 Plugin 패키지 내부에 있습니다. 공유 호스트 동작은 `plugin-sdk/gateway-runtime`, `plugin-sdk/security-runtime`, `plugin-sdk/plugin-config-runtime` 같은 일반 SDK 계약을 통해 이동해야 합니다.
...
**이전 (`openclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`, `buildCommandsMessagePaginated`, `buildHelpMessage`.

**신규 (`openclaw/plugin-sdk/command-status`)**: 동일한 시그니처, 동일한 exports - 더 좁은 subpath에서 import하기만 하면 됩니다. `command-auth`는 compat stub으로 이를 re-export합니다.
...
**이전** : `openclaw/plugin-sdk/channel-inbound` 또는 `openclaw/plugin-sdk/channel-mention-gating`의 `resolveInboundMentionRequirement({ facts, policy })` 및 `shouldDropInboundForMention(...)`.
...
`openclaw/plugin-sdk/channel-runtime`은 이전 채널 plugins를 위한 호환성 shim입니다. 새 코드에서 import하지 마세요. 런타임 객체 등록에는 `openclaw/plugin-sdk/channel-runtime-context`를 사용하세요.
...
**신규** : 제공자 Plugin에 `createTool(...)`을 직접 구현하세요. OpenClaw는 더 이상 도구 wrapper를 등록하기 위해 SDK 헬퍼를 필요로 하지 않습니다.

Plaintext channel envelopes → BodyForAgent

**이전** : inbound 채널 메시지에서 평면 plaintext prompt envelope를 빌드하기 위한 `formatInboundEnvelope(...)` 및 `ChannelMessageForAgent.channelEnvelope`.

**신규** : `BodyForAgent`와 구조화된 사용자 컨텍스트 블록. 채널 plugins는 routing metadata(thread, topic, reply-to, reactions)를 prompt 문자열에 연결하지 않고 typed field로 첨부합니다. 합성된 assistant-facing envelopes에는 `formatAgentEnvelope(...)` 헬퍼가 계속 지원되지만, inbound plaintext envelopes는 제거되는 중입니다.
...
또한 legacy `ProviderCapabilities` static bag - 제공자 plugins는 static object 대신 `buildReplayPolicy`, `normalizeToolSchemas`, `wrapStreamFn` 같은 명시적 제공자 hook을 사용해야 합니다.

Thinking policy hooks → resolveThinkingProfile

**이전** (`ProviderThinkingPolicy`의 세 개 별도 hook): `isBinaryThinking(ctx)`, `supportsXHighThinking(ctx)`, 및 `resolveDefaultThinkingLevel(ctx)`.

**신규** : 표준 `id`, 선택적 `label`, 순위가 매겨진 level list를 포함하는 `ProviderThinkingProfile`을 반환하는 단일 `resolveThinkingProfile(ctx)`. OpenClaw는 오래된 저장 값을 profile rank에 따라 자동으로 downgrade합니다.
...
**이전** : Plugin manifest에 제공자를 선언하지 않고 `resolveExternalOAuthProfiles(...)`를 구현.

**신규** : Plugin manifest에 `contracts.externalAuthProviders`를 선언하고 **또한** `resolveExternalAuthProfiles(...)`를 구현하세요. 이전 "auth fallback" 경로는 런타임에 경고를 출력하며 제거될 예정입니다.
...
**이전** : 세 개의 별도 호출 - `api.registerMemoryPromptSection(...)`, `api.registerMemoryFlushPlan(...)`, `api.registerMemoryRuntime(...)`.

**신규** : memory-state API에서 한 번 호출 - `registerMemoryCapability(pluginId, { promptBuilder, flushPlanResolver, runtime })`.

동일한 slot, 단일 registration 호출입니다. Additive memory 헬퍼 (`registerMemoryPromptSupplement`, `registerMemoryCorpusSupplement`, `registerMemoryEmbeddingProvider`)는 영향받지 않습니다.
...
위의 "마이그레이션 방법 → Pi tool-result extensions를 middleware로 마이그레이션"에서 다룹니다. 완전성을 위해 여기에 포함합니다. 제거된 Pi 전용 `api.registerEmbeddedExtensionFactory(...)` 경로는 `contracts.agentToolResultMiddleware`의 명시적 runtime list와 함께 `api.registerAgentToolResultMiddleware(...)`로 대체됩니다.

OpenClawSchemaType alias → OpenClawConfig

`openclaw/plugin-sdk`에서 re-export되는 `OpenClawSchemaType`은 이제 `OpenClawConfig`의 한 줄 alias입니다. 표준 이름을 선호하세요.
