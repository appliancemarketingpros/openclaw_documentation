...
source_url: https://docs.openclaw.ai/id/gateway/openai-http-api
...
Jalur autentikasi HTTP umum:
  ...
  * autentikasi HTTP pembawa identitas tepercaya (`gateway.auth.mode="trusted-proxy"`): rutekan melalui proxy sadar-identitas yang dikonfigurasi dan biarkan ia menyisipkan header identitas yang diperlukan
...
Catatan:
  ...
  * Saat `gateway.auth.mode="trusted-proxy"`, permintaan HTTP harus berasal dari sumber proxy tepercaya yang dikonfigurasi; proxy loopback host yang sama memerlukan `gateway.auth.trustedProxy.allowLoopback = true` secara eksplisit.
...
Matriks autentikasi:
  ...
  * `gateway.auth.mode="token"` atau `"password"` \+ `Authorization: Bearer ...`
    ...
    * memulihkan kumpulan cakupan operator default penuh: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
Id yang dikembalikan adalah entri `openclaw`, `openclaw/default`, dan `openclaw/<agentId>`. Gunakan langsung sebagai nilai `model` OpenAI.
...
Contoh: `x-openclaw-model: openai/gpt-5.4` `x-openclaw-model: gpt-5.5`
...
Gunakan `model: "openclaw/default"` atau `model: "openclaw/<agentId>"`. Saat Anda memerlukan model embedding tertentu, kirimkan di `x-openclaw-model`. Tanpa header itu, permintaan diteruskan ke setup embedding normal agen yang dipilih.
