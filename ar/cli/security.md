---
title: الأمان
source_url: https://docs.openclaw.ai/ar/cli/security
...
يطبّق `--fix` معالجات آمنة وحتمية:
  ...
  * يشدد أذونات ملفات الحالة/الإعدادات والملفات الحساسة الشائعة (`credentials/*.json`، و`auth-profiles.json`، و`sessions.json`، وملفات الجلسات `*.jsonl`)
