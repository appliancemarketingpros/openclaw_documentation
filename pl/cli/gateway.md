---
title: Gateway
source_url: https://docs.openclaw.ai/pl/cli/gateway
...
[**Wykrywanie Bonjour** Konfiguracja lokalnego mDNS i szerokoobszarowego DNS-SD. ](</pl/gateway/bonjour>) [**Omówienie wykrywania** Jak OpenClaw ogłasza i znajduje gatewaye. ](</pl/gateway/discovery>) [**Konfiguracja** Najwyższego poziomu klucze konfiguracji gatewaya. ](</pl/gateway/configuration>)
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tcG9ydCA8cG9ydA " type="number"> Port WebSocket (wartość domyślna pochodzi z konfiguracji/środowiska; zwykle `18789`).

OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdG9rZW4gPHRva2Vu " type="string"> Nadpisanie tokenu (ustawia też `OPENCLAW_GATEWAY_TOKEN` dla procesu).
...
Utwórz konfigurację deweloperską i workspace, jeśli ich brakuje (pomija [BOOTSTRAP.md](<http://BOOTSTRAP.md>)).
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tbGltaXQgPGxpbWl0 " type="number" default="25"> Maksymalna liczba ostatnich zdarzeń do uwzględnienia (maks. `1000`).
...
Użyj `--wrapper`, gdy zarządzana usługa musi wystartować przez inny plik wykonywalny, na przykład shim menedżera sekretów lub pomocnik uruchamiania jako inny użytkownik. Wrapper otrzymuje normalne argumenty Gateway i jest odpowiedzialny za ostateczne wykonanie `openclaw` albo Node z tymi argumentami.
...
Możesz też ustawić wrapper przez środowisko. `gateway install` sprawdza, czy ścieżka jest plikiem wykonywalnym, zapisuje wrapper do `ProgramArguments` usługi i utrwala `OPENCLAW_WRAPPER` w środowisku usługi na potrzeby późniejszych wymuszonych reinstalacji, aktualizacji i napraw doctor.
