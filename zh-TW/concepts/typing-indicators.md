---
title: 輸入狀態指示器
source_url: https://docs.openclaw.ai/zh-TW/concepts/typing-indicators
scraped_at: 2026-05-25
---

正在輸入指示器會在執行處於作用中時傳送到聊天通道。使用 `agents.defaults.typingMode` 控制正在輸入**何時** 開始，並使用 `typingIntervalSeconds` 控制**多久** 重新整理一次。
...
## 備註
  ...
  * `thinking` 只會在執行串流推理（`reasoningLevel: "stream"`）時觸發。 如果模型未發出推理 delta，正在輸入不會開始。
  ...
  * `typingIntervalSeconds` 控制的是**重新整理節奏** ，不是開始時間。 預設值是 6 秒。


## 相關

[**狀態** Gateway 如何追蹤已連線的用戶端，並在 macOS Instances 分頁中顯示它們。 ](</zh-TW/concepts/presence>) [**串流與分塊** 對外串流行為、分塊邊界，以及通道特定的傳遞。 ](</zh-TW/concepts/streaming>)
