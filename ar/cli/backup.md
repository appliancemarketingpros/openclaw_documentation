...
source_url: https://docs.openclaw.ai/ar/cli/backup
...
ملفات تعريف مصادقة النماذج هي بالفعل جزء من دليل الحالة ضمن `agents/<agentId>/agent/auth-profiles.json`، لذلك تغطيها عادةً خانة النسخ الاحتياطي الخاصة بالحالة.
...
يوحّد OpenClaw المسارات قبل بناء الأرشيف. إذا كانت الإعدادات، أو دليل بيانات الاعتماد، أو مساحة عمل موجودة بالفعل داخل دليل الحالة، فلن تُكرّر كمصادر نسخ احتياطي علوية منفصلة. تُتخطى المسارات المفقودة.
...
تُضمّن ملفات مصدر وبيان Plugin المثبتة تحت شجرة `extensions/` في دليل الحالة، لكن تُتخطى أشجار تبعيات `node_modules/` المتداخلة الخاصة بها. هذه التبعيات عناصر تثبيت قابلة لإعادة البناء؛ بعد استعادة أرشيف، استخدم `openclaw plugins update <id>` أو أعد تثبيت Plugin باستخدام `openclaw plugins install <spec> --force` عندما يبلّغ Plugin مستعاد عن تبعيات مفقودة.
