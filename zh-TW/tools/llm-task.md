---
title: LLM 任務
source_url: https://docs.openclaw.ai/zh-TW/tools/llm-task
scraped_at: 2026-05-25
---

`llm-task` 是一個**選用的 Plugin 工具** ，會執行僅限 JSON 的 LLM 任務，並 傳回結構化輸出（可選擇依 JSON Schema 驗證）。

這很適合像 Lobster 這類工作流程引擎：你可以加入單一 LLM 步驟， 而不必為每個工作流程撰寫自訂 OpenClaw 程式碼。
...
`allowedModels` 是 `provider/model` 字串的允許清單。如果有設定，任何不在清單中的請求 都會被拒絕。
...
傳回包含已剖析 JSON 的 `details.json`（並在提供 `schema` 時依其 驗證）。
...
## 安全注意事項
  ...
  * 此工具**僅限 JSON** ，並指示模型只輸出 JSON（沒有 程式碼區塊，沒有評論）。
  * 此次執行不會向模型公開任何工具。
