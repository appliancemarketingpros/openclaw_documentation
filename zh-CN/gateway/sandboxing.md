---
title: 沙箱隔离
source_url: https://docs.openclaw.ai/zh-CN/gateway/sandboxing
scraped_at: 2026-05-25
---

OpenClaw 可以在**沙箱后端内运行工具** 以降低影响范围。这是**可选的** ，并由配置控制（`agents.defaults.sandbox` 或 `agents.list[].sandbox`）。如果沙箱隔离关闭，工具会在主机上运行。Gateway 网关保留在主机上；启用后，工具执行会在隔离的沙箱中运行。
...
不会被沙箱隔离：
  ...
  * 任何被显式允许在沙箱外运行的工具（例如 `tools.elevated`）。 
    ...
    * 如果沙箱隔离关闭，`tools.elevated` 不会改变执行方式（已经在主机上）。参见[提升模式](</zh-CN/tools/elevated>)。


## 模式

`agents.defaults.sandbox.mode` 控制沙箱隔离**何时** 使用：
...
仅沙箱隔离**非主** 会话（如果你希望普通聊天在主机上运行，这是默认选择）。
...
要将主机 GPU 暴露给 Docker 沙箱，请设置 `agents.defaults.sandbox.docker.gpus` 或每个智能体的 `agents.list[].sandbox.docker.gpus` 覆盖项。该值会作为单独参数传递给 Docker 的 `--gpus` 标志，例如 `"all"` 或 `"device=GPU-uuid"`，并且需要兼容的主机运行时，例如 NVIDIA Container Toolkit。
...
身份验证材料
  ...
  * `identityData`、`certificateData`、`knownHostsData`：使用内联字符串或 SecretRefs。OpenClaw 会通过正常的密钥运行时快照解析它们，将它们写入权限为 `0600` 的临时文件，并在 SSH 会话结束时删除它们。
  * 如果同一项同时设置了 `*File` 和 `*Data`，该 SSH 会话中 `*Data` 优先生效。

远程规范的影响

这是一个**远程规范** 模型。初始播种后，远程 SSH 工作区会成为真正的沙箱状态。
...
当你希望 OpenClaw 在 OpenShell 托管的远程环境中沙箱隔离工具时，请使用 `backend: "openshell"`。完整设置指南、配置参考和工作区模式比较，请参见专门的 [OpenShell 页面](</zh-CN/gateway/openshell>)。
...
当你希望**本地工作区保持规范状态** 时，请使用 `plugins.entries.openshell.config.mode: "mirror"`。
...
重要后果：
  ...
  * 使用 `scope: "agent"` 或 `scope: "shared"` 时，该远程工作区会在同一作用域内共享。
...
全局绑定和单 Agent 绑定会被**合并** （而不是替换）。在 `scope: "shared"` 下，单 Agent 绑定会被忽略。
...
如需带有常用工具（例如 `curl`、`jq`、`nodejs`、`python3`、`git`）且功能更完整的沙箱镜像：
...
对于 Docker Gateway 网关部署，`scripts/docker/setup.sh` 可以引导沙箱配置。设置 `OPENCLAW_SANDBOX=1`（或 `true`/`yes`/`on`）以启用该路径。你可以用 `OPENCLAW_DOCKER_SOCKET` 覆盖套接字位置。完整设置和环境参考：[Docker](</zh-CN/install/docker#agent-sandbox>)。
...
常见陷阱
  ...
  * 默认 `docker.network` 是 `"none"`（无出站流量），因此包安装会失败。
  ...
  * 沙箱 exec **不会** 继承主机 `process.env`。请使用 `agents.defaults.sandbox.docker.env`（或自定义镜像）来配置技能 API key。
...
`tools.elevated` 是一个显式逃生通道，会在沙箱外运行 `exec`（默认在 `gateway` 中，或当 exec 目标为 `node` 时在 `node` 中）。`/exec` 指令仅适用于已授权的发送方，并按会话持久化；要硬性禁用 `exec`，请使用工具策略拒绝（参见 [沙箱 vs 工具策略 vs Elevated](</zh-CN/gateway/sandbox-vs-tool-policy-vs-elevated>)）。

调试：

  * 使用 `openclaw sandbox explain` 检查实际生效的沙箱模式、工具策略以及修复用配置键。
  * 参见 [沙箱 vs 工具策略 vs Elevated](</zh-CN/gateway/sandbox-vs-tool-policy-vs-elevated>)，了解“为什么这被阻止了？”的思维模型。
...
每个智能体都可以覆盖沙箱 + 工具：`agents.list[].sandbox` 和 `agents.list[].tools`（以及用于沙箱工具策略的 `agents.list[].tools.sandbox.tools`）。优先级见 [多 Agent 沙盒和工具](</zh-CN/tools/multi-agent-sandbox-tools>)。
...
## 相关内容
  ...
  * [多 Agent 沙盒和工具](</zh-CN/tools/multi-agent-sandbox-tools>) — 按智能体覆盖项和优先级
