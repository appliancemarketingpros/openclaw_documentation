...
إذا كنت تستخدم إصدار JVM (`signal-cli-${VERSION}.tar.gz`)، فثبّت JRE 25+ أولا. حافظ على تحديث `signal-cli`؛ يذكر المنبع أن الإصدارات القديمة قد تتعطل مع تغير واجهات API لخادم Signal.
...
خيارات المزوّد:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (الافتراضي: auto). راجع وضع الحاوية.
