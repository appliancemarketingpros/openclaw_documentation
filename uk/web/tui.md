---
title: TUI
source_url: https://docs.openclaw.ai/uk/web/tui
...
Якщо `openclaw config validate` вже завершується помилкою, спочатку запустіть `openclaw configure` або `openclaw doctor --fix`. `openclaw chat` не обходить захист від недійсної конфігурації.
