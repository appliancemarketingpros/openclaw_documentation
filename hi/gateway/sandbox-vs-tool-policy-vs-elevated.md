...
source_url: https://docs.openclaw.ai/hi/gateway/sandbox-vs-tool-policy-vs-elevated
...
सामान्य नियम:
  ...
  * `/exec` केवल अधिकृत senders के लिए सेशन defaults बदलता है; यह टूल एक्सेस grant नहीं करता। प्रदाता टूल keys या तो `provider` (जैसे `google-antigravity`) या `provider/model` (जैसे `openai/gpt-5.4`) स्वीकार करती हैं।
...
उपलब्ध समूह:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` read-only एजेंटों के लिए, mutating filesystem tools के साथ-साथ `group:runtime` को भी deny करें, जब तक कि सैंडबॉक्स फाइलसिस्टम नीति या अलग होस्ट सीमा read-only constraint लागू न करे।
