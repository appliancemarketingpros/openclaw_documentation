...
source_url: https://docs.openclaw.ai/fr/platforms/linux
scraped_at: 2026-05-25
---

Le Gateway est entièrement pris en charge sur Linux. **Node est l’environnement d’exécution recommandé**. Bun n’est pas recommandé pour le Gateway (bogues WhatsApp/Telegram).
...
Pour les lancements de processus enfants Linux éligibles, OpenClaw démarre l’enfant au moyen d’un court wrapper `/bin/sh` qui augmente le `oom_score_adj` propre à l’enfant à `1000`, puis exécute la vraie commande avec `exec`. Il s’agit d’une opération non privilégiée, car l’enfant ne fait qu’augmenter sa propre probabilité d’arrêt OOM.
...
Cela ne remplace pas le réglage normal de la mémoire. Si un VPS ou un conteneur arrête régulièrement des enfants, augmentez la limite de mémoire, réduisez la concurrence ou ajoutez des contrôles de ressources plus stricts, comme `MemoryMax=` de systemd ou des limites de mémoire au niveau du conteneur.
