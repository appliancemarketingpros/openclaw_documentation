...
source_url: https://docs.openclaw.ai/es/gateway/openresponses-http-api
...
Si el agente decide llamar a una herramienta, la respuesta devuelve un elemento de salida `function_call`. Después, envías una solicitud de seguimiento con `function_call_output` para continuar el turno.
...
Tipos MIME permitidos (actuales): `image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/heic`, `image/heif`. Tamaño máximo (actual): 10MB.
...
Tipos MIME permitidos (actuales): `text/plain`, `text/markdown`, `text/html`, `text/csv`, `application/json`, `application/pdf`.

Tamaño máximo (actual): 5MB.

Comportamiento actual:

  * El contenido del archivo se decodifica y se agrega al **prompt del sistema** , no al mensaje del usuario, por lo que permanece efímero (no se conserva en el historial de sesión).
  * El texto decodificado del archivo se envuelve como **contenido externo no confiable** antes de agregarse, por lo que los bytes del archivo se tratan como datos, no como instrucciones de confianza.
  ...
  * Esta ruta de entrada de archivos omite intencionalmente el banner largo `SECURITY NOTICE:` para preservar el presupuesto de prompt; los marcadores de límite y los metadatos permanecen en su lugar.
  * Los PDF se analizan primero para extraer texto. Si se encuentra poco texto, las primeras páginas se rasterizan en imágenes y se pasan al modelo, y el bloque de archivo inyectado usa el marcador de posición `[PDF content rendered to images]`.
...
Nota de seguridad:
  ...
  * Para gateways expuestos a Internet, aplica controles de egreso de red además de las protecciones a nivel de aplicación. Consulta [Seguridad](</es/gateway/security>).
...
`usage` se rellena cuando el proveedor subyacente informa recuentos de tokens. OpenClaw normaliza aliases comunes de estilo OpenAI antes de que esos contadores lleguen a las superficies descendentes de estado/sesión, incluidos `input_tokens` / `output_tokens` y `prompt_tokens` / `completion_tokens`.
