---
title: 插件 SDK 子路径
source_url: https://docs.openclaw.ai/zh-CN/plugins/sdk-subpaths
...
`plugin-sdk/provider-zai-endpoint` | 已弃用的 [Z.AI](<http://Z.AI>) 提供商拥有的端点检测门面；请使用 [Z.AI](<http://Z.AI>) 插件公共 API  
...
目前没有保留的内置辅助工具 SDK 子路径。所有者特定的 辅助工具位于所属插件包内，而可复用的宿主契约 使用通用 SDK 子路径，例如 `plugin-sdk/gateway-runtime`、 `plugin-sdk/security-runtime` 和 `plugin-sdk/plugin-config-runtime`。
