---
title: Compaction
source_url: https://docs.openclaw.ai/it/concepts/compaction
...
Quando `agents.defaults.compaction.truncateAfterCompaction` è abilitato, OpenClaw non riscrive la trascrizione esistente sul posto. Crea una nuova trascrizione successiva attiva dal riepilogo della compattazione, dallo stato preservato e dalla coda non riassunta, quindi mantiene il JSONL precedente come origine del checkpoint archiviato. Le trascrizioni successive eliminano anche i turni utente lunghi esattamente duplicati che arrivano all'interno di una breve finestra di ripetizione, così le tempeste di retry del canale non vengono portate nella trascrizione attiva successiva dopo la compattazione.

I checkpoint pre-compattazione vengono conservati solo finché restano sotto il limite di dimensione dei checkpoint di OpenClaw; le trascrizioni attive sovradimensionate vengono comunque compattate, ma OpenClaw salta il grande snapshot di debug invece di raddoppiare l'uso del disco.
