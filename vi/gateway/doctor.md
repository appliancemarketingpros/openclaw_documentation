...
source_url: https://docs.openclaw.ai/vi/gateway/doctor
...
0\. Cập nhật tùy chọn (bản cài đặt git)
...
Doctor cũng cảnh báo khi `plugins.allow` không rỗng và chính sách công cụ dùng các mục công cụ wildcard hoặc thuộc sở hữu Plugin. `tools.allow: ["*"]` chỉ khớp các công cụ từ Plugin thực sự được tải; nó không bỏ qua danh sách cho phép Plugin độc quyền. Doctor ghi `plugins.bundledDiscovery: "compat"` cho các cấu hình danh sách cho phép legacy đã di chuyển để giữ nguyên hành vi provider bundled hiện có, và sau đó trỏ đến cài đặt `"allowlist"` chặt chẽ hơn.

2\. Di chuyển khóa cấu hình legacy
...
2c. Di chuyển trình duyệt và khả năng sẵn sàng Chrome MCP
...
2d. Điều kiện tiên quyết OAuth TLS
...
2e. Ghi đè nhà cung cấp Codex OAuth
...
3a. Di chuyển manifest Plugin cũ
...
12\. Kiểm tra xác thực Gateway (mã thông báo cục bộ)
...
12b. Sửa chữa chỉ đọc có nhận biết SecretRef
...
13b. Mức sẵn sàng của tìm kiếm bộ nhớ
...
15\. Kiểm tra + sửa chữa cấu hình supervisor
...
17\. Thực hành tốt nhất cho runtime Gateway
