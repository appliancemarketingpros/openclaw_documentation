---
title: 메모리 구성 참조
source_url: https://docs.openclaw.ai/ko/reference/memory-config
scraped_at: 2026-05-25
---

이 페이지는 OpenClaw 메모리 검색의 모든 구성 조정 항목을 나열합니다. 개념 개요는 다음을 참조하세요.

[**메모리 개요** 메모리 작동 방식. ](</ko/concepts/memory>) [**내장 엔진** 기본 SQLite 백엔드. ](</ko/concepts/memory-builtin>) [**QMD 엔진** 로컬 우선 사이드카. ](</ko/concepts/memory-qmd>) [**메모리 검색** 검색 파이프라인 및 튜닝. ](</ko/concepts/memory-search>) [**Active Memory** 대화형 세션용 메모리 하위 에이전트. ](</ko/concepts/active-memory>)
...
Local (GGUF + node-llama-cpp) 키 | 유형 | 기본값 | 설명  
---|---|---|---  
