...
source_url: https://docs.openclaw.ai/fa/reference/wizard
scraped_at: 2026-05-25
---

این مرجع کامل برای `openclaw onboard` است. برای مرور کلی سطح بالا، [Onboarding (CLI)](</fa/start/wizard>) را ببینید.
...
* ### Existing config detection
  ...
  * اجرای دوباره onboarding هیچ‌چیز را پاک **نمی‌کند** مگر اینکه صراحتا **Reset** را انتخاب کنید (یا `--reset` را پاس دهید).
  * `--reset` در CLI به‌طور پیش‌فرض روی `config+creds+sessions` است؛ از `--reset-scope full` استفاده کنید تا workspace هم حذف شود.
  * اگر config نامعتبر باشد یا کلیدهای قدیمی داشته باشد، wizard متوقف می‌شود و از شما می‌خواهد پیش از ادامه `openclaw doctor` را اجرا کنید.
...
* ### Model/Auth
  ...
  * **MiniMax** : config به‌طور خودکار نوشته می‌شود؛ پیش‌فرض میزبانی‌شده `MiniMax-M2.7` است. راه‌اندازی کلید API از `minimax/...` استفاده می‌کند، و راه‌اندازی OAuth از `minimax-portal/...` استفاده می‌کند.
...
نمونه‌های دستور مخصوص provider در [CLI Automation](</fa/start/wizard-cli-automation#provider-specific-examples>) قرار دارند. از این صفحه مرجع برای معنای flagها و ترتیب stepها استفاده کنید.
...
Gateway جریان onboarding را از طریق RPC (`wizard.start`، `wizard.next`، `wizard.cancel`، `wizard.status`) در معرض استفاده قرار می‌دهد. Clientها (app macOS، Control UI) می‌توانند stepها را بدون پیاده‌سازی دوباره منطق onboarding نمایش دهند.
...
برخی کانال‌ها به‌صورت Plugin ارائه می‌شوند. وقتی یکی را هنگام راه‌اندازی انتخاب کنید، فرایند راه‌اندازی پیش از امکان پیکربندی آن، برای نصب آن (npm یا یک مسیر محلی) از شما درخواست می‌کند.
