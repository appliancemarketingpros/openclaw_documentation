...
source_url: https://docs.openclaw.ai/ar/gateway/doctor
...
يحذر doctor أيضا عندما يكون `plugins.allow` غير فارغ وتستخدم سياسة الأدوات إدخالات wildcard أو أدوات مملوكة لـ Plugin. لا يطابق `tools.allow: ["*"]` إلا الأدوات من plugins التي تحمل فعليا؛ ولا يتجاوز قائمة سماح Plugin الحصرية. يكتب doctor القيمة `plugins.bundledDiscovery: "compat"` لإعدادات قائمة السماح القديمة المرحلة للحفاظ على سلوك الموفرات المضمنة الحالي، ثم يشير إلى إعداد `"allowlist"` الأكثر صرامة.
