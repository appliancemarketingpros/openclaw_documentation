---
title: عیب‌یاب
source_url: https://docs.openclaw.ai/fa/gateway/doctor
...
سلامت، رابط کاربری، و به‌روزرسانی‌ها
...
Gateway، سرویس‌ها، و supervisorها
...
0\. به‌روزرسانی اختیاری (نصب‌های git)
...
Doctor همچنین زمانی هشدار می‌دهد که `plugins.allow` خالی نیست و سیاست ابزار از entryهای wildcard یا ابزارهای متعلق به Plugin استفاده می‌کند. `tools.allow: ["*"]` فقط با ابزارهای Pluginهایی match می‌شود که واقعاً load می‌شوند؛ این مورد allowlist انحصاری Plugin را دور نمی‌زند. Doctor برای پیکربندی‌های allowlist قدیمی مهاجرت‌داده‌شده، `plugins.bundledDiscovery: "compat"` را می‌نویسد تا رفتار موجود ارائه‌دهنده bundled حفظ شود، و سپس به تنظیم سخت‌گیرانه‌تر `"allowlist"` اشاره می‌کند.

2\. مهاجرت‌های کلید پیکربندی قدیمی
...
2b. بازنویسی‌های ارائه‌دهنده OpenCode
...
2e. بازنویسی‌های ارائه‌دهنده Codex OAuth
...
3\. مهاجرت‌های وضعیت قدیمی (چیدمان دیسک)
...
3a. مهاجرت‌های manifest قدیمی Plugin
...
3b. مهاجرت‌های ذخیره‌گاه Cron قدیمی
...
4\. بررسی‌های یکپارچگی state (پایداری نشست، مسیریابی، و ایمنی)
...
8\. مهاجرت‌های سرویس Gateway و نکته‌های پاک‌سازی
...
8c. جفت‌سازی دستگاه و drift در auth
...
11\. وضعیت workspace (Skills، Pluginها، و دایرکتوری‌های legacy)
...
11d. پاک‌سازی Plugin کهنه channel
...
12\. بررسی‌های احراز هویت Gateway (توکن محلی)
...
12b. تعمیرهای فقط‌خواندنیِ آگاه از SecretRef
...
13\. بررسی سلامت Gateway + راه‌اندازی مجدد
...
16\. تشخیص‌های runtime و port در Gateway
...
17\. بهترین رویه‌های runtime در Gateway
...
19\. نکته‌های workspace (backup + سیستم حافظه)
