---
title: 에이전트 전송
source_url: https://docs.openclaw.ai/ko/tools/agent-send
...
[**에이전트 CLI 참조** 전체 `openclaw agent` 플래그 및 옵션 참조입니다. ](</ko/cli/agent>) [**하위 에이전트** 백그라운드 하위 에이전트 생성입니다. ](</ko/tools/subagents>) [**세션** 세션 키가 작동하는 방식과 `--to`, `--agent`, `--session-id`가 이를 해석하는 방식입니다. ](</ko/concepts/session>) [**슬래시 명령** 에이전트 세션 내에서 사용되는 네이티브 명령 카탈로그입니다. ](</ko/tools/slash-commands>)
