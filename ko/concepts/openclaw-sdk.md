...
source_url: https://docs.openclaw.ai/ko/concepts/openclaw-sdk
...
`new OpenClaw({ gateway: "ws://..." })`는 `url`과 동일합니다. `gateway: "auto"` 옵션은 생성자에서 허용되지만, 자동 Gateway 탐색은 아직 별도 SDK 기능이 아닙니다. 앱이 Gateway를 탐색하는 방법을 이미 알고 있지 않다면 `url`을 전달하세요.
...
아티팩트 헬퍼는 세션, 실행 또는 작업 컨텍스트에 대한 Gateway 아티팩트 프로젝션을 노출합니다. 각 호출에는 명시적인 `sessionKey`, `runId` 또는 `taskId` 범위가 하나 필요합니다.
