---
title: امنیت
source_url: https://docs.openclaw.ai/fa/gateway/security
...
کاتالوگ کامل را همراه با سطوح شدت، کلیدهای اصلاح، و پشتیبانی اصلاح خودکار در [بررسی‌های ممیزی امنیتی](</fa/gateway/security/audit-checks>) ببینید.
...
دستورهای اسلش و دستورالعمل‌ها فقط برای **فرستنده‌های مجاز** رعایت می‌شوند. مجوزدهی از allowlistها/جفت‌سازی کانال به‌علاوه `commands.useAccessGroups` مشتق می‌شود ([پیکربندی](</fa/gateway/configuration>) و [دستورهای اسلش](</fa/tools/slash-commands>) را ببینید). اگر allowlist یک کانال خالی باشد یا شامل `"*"` باشد، دستورها عملاً برای آن کانال باز هستند.
...
Pair کردن دستگاه محلی:
  ...
  * شواهد forwarded-header روی یک درخواست loopback، محلی بودن loopback را رد صلاحیت می‌کند. تأیید خودکار metadata-upgrade دامنهٔ محدودی دارد. برای هر دو قاعده، [pair کردن Gateway](</fa/gateway/pairing>) را ببینید.
...
**قاعدهٔ امنیتی:** این headerها را از reverse proxy خودتان forward نکنید. اگر TLS را terminate می‌کنید یا جلوی gateway پروکسی می‌گذارید، `gateway.auth.allowTailscale` را غیرفعال کنید و به‌جای آن از احراز هویت shared-secret (`gateway.auth.mode: "token"` یا `"password"`) یا [احراز هویت با پروکسی مورد اعتماد](</fa/gateway/trusted-proxy-auth>) استفاده کنید.
...
اگر Gateway شما راه دور است اما مرورگر روی ماشین دیگری اجرا می‌شود، یک **میزبان Node** روی ماشین مرورگر اجرا کنید و اجازه دهید Gateway کنش‌های مرورگر را پروکسی کند (ببینید [ابزار مرورگر](</fa/tools/browser>)). Pair کردن Node را مانند دسترسی admin در نظر بگیرید.
...
با routing چندعاملی، هر عامل می‌تواند sandbox و سیاست ابزار خودش را داشته باشد: از این برای دادن **دسترسی کامل** ، **فقط‌خواندنی** ، یا **بدون دسترسی** برای هر عامل استفاده کنید. برای جزئیات کامل و قوانین precedence، [Multi-Agent Sandbox & Tools](</fa/tools/multi-agent-sandbox-tools>) را ببینید.
