...
source_url: https://docs.openclaw.ai/ru/start/wizard-cli-automation
...
Используйте `--secret-input-mode ref`, чтобы хранить ссылки на env-backed значения в auth-профилях вместо plaintext-значений. Интерактивный выбор между env refs и настроенными provider refs (`file` или `exec`) доступен в процессе onboarding.

В неинтерактивном режиме `ref` переменные окружения провайдера должны быть заданы в окружении процесса. Передача inline key flags без соответствующей переменной окружения теперь быстро завершается ошибкой.
...
Пример с Anthropic API key bashCopy code
[code]
...
Пример Vercel AI Gateway bashCopy code
[code]
...
Пример Cloudflare AI Gateway bashCopy code
[code]
...
Пример Synthetic bashCopy code
[code]
...
Пример пользовательского провайдера bashCopy code
[code]
