---
title: Signal
source_url: https://docs.openclaw.ai/pl/channels/signal
...
Jeśli używasz kompilacji JVM (`signal-cli-${VERSION}.tar.gz`), najpierw zainstaluj JRE 25+. Utrzymuj `signal-cli` w aktualnej wersji; upstream zauważa, że stare wydania mogą przestać działać, gdy zmieniają się API serwera Signal.
...
Opcje dostawcy:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (domyślnie: auto). Zobacz Tryb kontenera.
