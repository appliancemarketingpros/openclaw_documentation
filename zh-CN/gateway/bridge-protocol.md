---
title: 桥接协议
source_url: https://docs.openclaw.ai/zh-CN/gateway/bridge-protocol
...
节点可以发出 `exec.finished` 或 `exec.denied` 事件来呈现 system.run 活动。 这些事件会在 Gateway 网关中映射为系统事件。（旧版节点仍可能发出 `exec.started`。）
