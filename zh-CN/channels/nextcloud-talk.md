...
source_url: https://docs.openclaw.ai/zh-CN/channels/nextcloud-talk
...
Nextcloud Talk 在当前 OpenClaw 版本中作为内置插件发布，因此 正常的打包构建不需要单独安装。

如果你使用较旧构建，或自定义安装中排除了 Nextcloud Talk， 请直接安装 npm 包：
...
使用裸包可跟随当前官方发布标签。只有在需要可复现安装时才固定精确 版本。
