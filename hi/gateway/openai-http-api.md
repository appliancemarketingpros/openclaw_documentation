...
source_url: https://docs.openclaw.ai/hi/gateway/openai-http-api
...
सामान्य HTTP auth पथ:
  ...
  * साझा-गुप्त auth (`gateway.auth.mode="token"` या `"password"`): `Authorization: Bearer <token-or-password>`
  * विश्वसनीय पहचान-युक्त HTTP auth (`gateway.auth.mode="trusted-proxy"`): कॉन्फ़िगर किए गए identity-aware proxy के माध्यम से रूट करें और उसे आवश्यक identity headers इंजेक्ट करने दें
  * private-ingress खुला auth (`gateway.auth.mode="none"`): कोई auth header आवश्यक नहीं
...
नोट्स:
  ...
  * जब `gateway.auth.mode="trusted-proxy"` हो, तो HTTP अनुरोध किसी कॉन्फ़िगर किए गए trusted proxy स्रोत से आना चाहिए; same-host loopback proxies के लिए स्पष्ट `gateway.auth.trustedProxy.allowLoopback = true` आवश्यक है.
  * Proxy को बायपास करने वाले आंतरिक same-host callers स्थानीय direct fallback के रूप में `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD` का उपयोग कर सकते हैं. कोई भी `Forwarded`, `X-Forwarded-*`, या `X-Real-IP` header evidence अनुरोध को इसके बजाय trusted-proxy पथ पर रखता है.
...
Auth matrix:
  ...
  * `gateway.auth.mode="token"` या `"password"` \+ `Authorization: Bearer ...`
    ...
    * पूर्ण default operator scope set को पुनर्स्थापित करता है: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
`/v1/models` क्या लौटाता है?

एक OpenClaw agent-target list.

लौटाए गए ids `openclaw`, `openclaw/default`, और `openclaw/<agentId>` entries हैं. इन्हें सीधे OpenAI `model` values के रूप में उपयोग करें.

क्या `/v1/models` agents या sub-agents सूचीबद्ध करता है?
...
`openclaw/default` क्यों शामिल है?
...
मैं backend model को कैसे override करूँ?
...
उदाहरण: `x-openclaw-model: openai/gpt-5.4` `x-openclaw-model: gpt-5.5`

यदि आप इसे छोड़ देते हैं, तो चुना गया agent अपने सामान्य configured model choice के साथ चलता है.

Embeddings इस contract में कैसे fit होते हैं?

`/v1/embeddings` वही agent-target `model` ids उपयोग करता है.

`model: "openclaw/default"` या `model: "openclaw/<agentId>"` का उपयोग करें. जब आपको कोई विशिष्ट embedding model चाहिए, तो उसे shared-secret caller या `operator.admin` वाले identity-bearing caller से `x-openclaw-model` में भेजें. उस header के बिना, अनुरोध चुने गए agent के सामान्य embedding setup से होकर गुजरता है.
