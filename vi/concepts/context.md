---
title: Bối cảnh
source_url: https://docs.openclaw.ai/vi/concepts/context
...
Theo mặc định, OpenClaw dùng context engine `legacy` tích hợp sẵn để lắp ráp và Compaction. Nếu bạn cài một plugin cung cấp `kind: "context-engine"` và chọn nó bằng `plugins.slots.contextEngine`, OpenClaw ủy quyền việc lắp ráp ngữ cảnh, `/compact`, và các hook vòng đời ngữ cảnh subagent liên quan cho engine đó. `ownsCompaction: false` không tự động fallback về engine `legacy`; engine đang hoạt động vẫn phải triển khai `compact()` đúng cách. Xem [Context Engine](</vi/concepts/context-engine>) để biết đầy đủ giao diện có thể cắm thêm, các hook vòng đời, và cấu hình.
...
[**Context engine** Chèn ngữ cảnh tùy chỉnh qua plugin. ](</vi/concepts/context-engine>) [**Compaction** Tóm tắt các cuộc hội thoại dài để giữ chúng trong cửa sổ mô hình. ](</vi/concepts/compaction>) [**System prompt** Cách system prompt được xây dựng và những gì nó chèn vào mỗi lượt. ](</vi/concepts/system-prompt>) [**Vòng lặp agent** Chu trình thực thi agent đầy đủ từ tin nhắn đến phản hồi cuối cùng. ](</vi/concepts/agent-loop>)
