...
OpenClaw 只读取 `OPENCLAW_*` 环境变量。早期版本中的旧版 `CLAWDBOT_*` 和 `MOLTBOT_*` 前缀会被静默 忽略。

如果在启动时 Gateway 网关进程上仍设置了其中任何变量，OpenClaw 会发出一条 Node 弃用警告（`OPENCLAW_LEGACY_ENV_VARS`），其中列出检测到的 前缀和总数量。请将每个值的旧版前缀替换为 `OPENCLAW_` 来重命名（例如 `CLAWDBOT_GATEWAY_TOKEN` → `OPENCLAW_GATEWAY_TOKEN`）；旧名称不会生效。
