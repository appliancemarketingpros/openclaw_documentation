---
title: 喀噠喀噠
source_url: https://docs.openclaw.ai/zh-TW/channels/clickclack
...
`replyMode: "model"` 會直接使用 `api.runtime.llm.complete` 來產生簡短的機器人回覆。 當帳號設定了 `agentId` 時，OpenClaw 需要明確的 `plugins.entries.clickclack.llm.allowAgentIdOverride` 信任位元，讓 Plugin 可以為該機器人代理程式執行 completions。如果你只使用預設 代理程式路由，請將其保持關閉。
