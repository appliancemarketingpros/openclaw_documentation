---
title: OAuth
source_url: https://docs.openclaw.ai/vi/concepts/oauth
scraped_at: 2026-05-25
---

OpenClaw hỗ trợ "xác thực bằng gói thuê bao" qua OAuth cho các nhà cung cấp có cung cấp tùy chọn này (đáng chú ý là **OpenAI Codex (ChatGPT OAuth)**). Với Anthropic, cách phân tách thực tế hiện là:
...
OpenAI Codex OAuth được hỗ trợ rõ ràng để sử dụng trong các công cụ bên ngoài như OpenClaw. Trang này giải thích:
...
OpenClaw cũng hỗ trợ **provider plugins** có các luồng OAuth hoặc API-key riêng. Chạy chúng bằng:
...
Để giảm tình trạng đó, OpenClaw xem `auth-profiles.json` là một **nơi gom token** :
  ...
  * việc tái sử dụng CLI bên ngoài là riêng theo nhà cung cấp: Codex CLI có thể khởi tạo một hồ sơ `openai-codex:default` trống, nhưng một khi OpenClaw có hồ sơ OAuth cục bộ, refresh token cục bộ sẽ là nguồn chuẩn; các tích hợp khác có thể vẫn được quản lý bên ngoài và đọc lại kho xác thực CLI của chúng
  * các đường dẫn trạng thái và khởi động đã biết tập nhà cung cấp được cấu hình sẽ giới hạn phạm vi phát hiện CLI bên ngoài vào tập đó, để kho đăng nhập CLI không liên quan không bị thăm dò trong thiết lập chỉ có một nhà cung cấp
...
Secret được lưu trong các kho xác thực của agent:
  ...
  * Tệp tương thích cũ: `~/.openclaw/agents/<agentId>/agent/auth.json` (các mục `api_key` tĩnh sẽ được xóa sạch khi được phát hiện)
...
Khi một agent phụ không có hồ sơ xác thực cục bộ, OpenClaw sử dụng kế thừa đọc xuyên từ kho của agent mặc định/chính. Nó không sao chép `auth-profiles.json` của agent chính khi đọc. OAuth refresh token đặc biệt nhạy cảm: các luồng sao chép thông thường bỏ qua chúng theo mặc định vì một số nhà cung cấp xoay vòng hoặc vô hiệu hóa refresh token sau khi sử dụng. Cấu hình một lần đăng nhập OAuth riêng cho agent khi agent đó cần một tài khoản độc lập.
...
OpenClaw lại hỗ trợ tái sử dụng Anthropic Claude CLI. Nếu bạn đã có đăng nhập Claude cục bộ trên máy chủ, onboarding/configure có thể tái sử dụng trực tiếp.
...
Ở runtime:
  ...
  * ngoại lệ: một số thông tin xác thực CLI bên ngoài vẫn được quản lý bên ngoài; OpenClaw đọc lại các kho xác thực CLI đó thay vì tiêu thụ các refresh token đã sao chép. Việc khởi tạo Codex CLI được cố ý thu hẹp hơn: nó gieo một hồ sơ `openai-codex:default` trống, sau đó các lần làm mới do OpenClaw sở hữu giữ hồ sơ cục bộ là nguồn chuẩn.
