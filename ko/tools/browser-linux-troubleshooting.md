---
title: 브라우저 문제 해결
source_url: https://docs.openclaw.ai/ko/tools/browser-linux-troubleshooting
...
기타 일반적인 Linux 실행 실패:
  ...
  * `The profile appears to be in use by another Chromium process`는 Chrome이 관리 프로필 디렉터리에서 오래된 `Singleton*` 잠금 파일을 찾았다는 뜻입니다. OpenClaw는 해당 잠금이 종료된 프로세스 또는 다른 호스트의 프로세스를 가리키는 경우 잠금을 제거하고 한 번 다시 시도합니다.
  * `Missing X server or $DISPLAY`는 데스크톱 세션이 없는 호스트에서 표시 가능한 브라우저가 명시적으로 요청되었다는 뜻입니다. 기본적으로 로컬 관리 프로필은 Linux에서 `DISPLAY`와 `WAYLAND_DISPLAY`가 모두 설정되지 않은 경우 이제 headless 모드로 대체됩니다. `OPENCLAW_BROWSER_HEADLESS=0`, `browser.headless: false` 또는 `browser.profiles.<name>.headless: false`를 설정했다면, 해당 headed 재정의를 제거하거나, `OPENCLAW_BROWSER_HEADLESS=1`을 설정하거나, `Xvfb`를 시작하거나, 일회성 관리 실행을 위해 `openclaw browser start --headless`를 실행하거나, 실제 데스크톱 세션에서 OpenClaw를 실행하세요.
...
snap Chromium을 반드시 사용해야 한다면, 수동으로 시작한 브라우저에 연결하도록 OpenClaw를 구성하세요.
...
[/code]
  ...
  3. 선택적으로 Chrome을 자동 시작하도록 systemd 사용자 서비스를 생성합니다.
...
`browser.executablePath` | Chromium 기반 브라우저 바이너리 경로(Chrome/Brave/Edge/Chromium) | 자동 감지됨(Chromium 기반인 경우 기본 브라우저를 선호)  
`browser.headless` | GUI 없이 실행 | `false`  
`OPENCLAW_BROWSER_HEADLESS` | 로컬 관리 브라우저 headless 모드에 대한 프로세스별 재정의 | 설정되지 않음  
...
`browser.localLaunchTimeoutMs` | 로컬 관리 Chrome 검색 제한 시간 | `15000`  
`browser.localCdpReadyTimeoutMs` | 로컬 관리 실행 후 CDP 준비 제한 시간 | `8000`  
  
Raspberry Pi, 오래된 VPS 호스트 또는 느린 스토리지에서는 Chrome이 CDP HTTP 엔드포인트를 노출하는 데 더 많은 시간이 필요할 때 `browser.localLaunchTimeoutMs`를 늘리세요. 실행은 성공하지만 `openclaw browser start`가 여전히 `not reachable after start`를 보고할 때는 `browser.localCdpReadyTimeoutMs`를 늘리세요. 값은 `120000`ms 이하의 양의 정수여야 하며, 유효하지 않은 설정 값은 거부됩니다.

### 문제: "profile="user"에 대한 Chrome 탭을 찾을 수 없음"

`existing-session` / Chrome MCP 프로필을 사용 중입니다. OpenClaw가 로컬 Chrome을 볼 수 있지만, 연결할 수 있는 열린 탭이 없습니다.

수정 옵션:

  1. **관리 브라우저 사용:** `openclaw browser start --browser-profile openclaw` 또는 `browser.defaultProfile: "openclaw"`를 설정합니다.
  2. **Chrome MCP 사용:** 로컬 Chrome이 최소 하나의 열린 탭으로 실행 중인지 확인한 다음 `--browser-profile user`로 다시 시도합니다.


참고:

  * `user`는 호스트 전용입니다. Linux 서버, 컨테이너 또는 원격 호스트의 경우 CDP 프로필을 선호하세요.
  * `user` / 기타 `existing-session` 프로필은 현재 Chrome MCP 제한을 유지합니다. ref 기반 작업, 단일 파일 업로드 훅, 대화 상자 제한 시간 재정의 없음, `wait --load networkidle` 없음, 그리고 `responsebody`, PDF 내보내기, 다운로드 가로채기 또는 배치 작업 없음.
  * 로컬 `openclaw` 프로필은 `cdpPort`/`cdpUrl`을 자동 할당합니다. 원격 CDP에만 이를 설정하세요.
  * 원격 CDP 프로필은 `http://`, `https://`, `ws://`, `wss://`를 허용합니다. `/json/version` 검색에는 HTTP(S)를 사용하고, 브라우저 서비스가 직접 DevTools 소켓 URL을 제공하는 경우 WS(S)를 사용하세요.
