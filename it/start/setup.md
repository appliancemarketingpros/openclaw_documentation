...
source_url: https://docs.openclaw.ai/it/start/setup
...
Usala quando esegui il debug dell’autenticazione o decidi cosa sottoporre a backup:
  ...
  * **Importazione OAuth legacy** : `~/.openclaw/credentials/oauth.json` Maggiori dettagli: [Sicurezza](</it/gateway/security#credential-storage-map>).
...
Per server always-on o multiutente, considera invece un servizio **system** anziché un servizio utente (lingering non necessario). Consulta il [runbook del Gateway](</it/gateway>) per le note su systemd.
