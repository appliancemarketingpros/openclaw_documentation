...
source_url: https://docs.openclaw.ai/uk/channels/msteams
scraped_at: 2026-05-25
---

Статус: підтримуються текст + вкладення в DM; надсилання файлів у каналах/групах потребує `sharePointSiteId` \+ дозволів Graph (див. Надсилання файлів у групових чатах). Опитування надсилаються через Adaptive Cards. Дії з повідомленнями надають явний `upload-file` для надсилань, де файл іде першим.
...
### Крок 3: Налаштуйте Messaging Endpoint
  ...
  2. Задайте **Messaging endpoint** як URL вашого webhook: 
     ...
     * Локальна розробка: використайте тунель (див. Локальна розробка нижче)
...
### Крок 5: Зберіть маніфест застосунку Teams
  ...
  * Додайте дозволи RSC (див. Дозволи RSC).
...
Докладніше про формат цілі див. у розділі Формати цілей нижче.
...
CodeCopy code
[code]
    https://teams.microsoft.com/l/team/19%3ABk4j...%40thread.tacv2/conversations?groupId=...                                    └────────────────────────────┘                                    ID розмови команди (URL-декодуйте це)
[/code]
...
CodeCopy code
[code]
    https://teams.microsoft.com/l/channel/19%3A15bc...%40thread.tacv2/ChannelName?groupId=...                                      └─────────────────────────┘                                      ID каналу (URL-декодуйте це)
[/code]
...
### Помилки завантаження manifest
  ...
  * **"[webApplicationInfo.Id](<http://webApplicationInfo.Id>) already in use":** застосунок усе ще встановлений в іншій команді/чаті. Спочатку знайдіть і видаліть його або зачекайте 5-10 хвилин для поширення змін.
