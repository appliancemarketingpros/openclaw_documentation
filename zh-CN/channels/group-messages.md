---
title: WhatsApp 群组消息
source_url: https://docs.openclaw.ai/zh-CN/channels/group-messages
scraped_at: 2026-05-25
---

对于跨渠道群组模型（Discord、iMessage、Matrix、Microsoft Teams、Signal、Slack、Telegram、WhatsApp、Zalo），请参阅[群组](</zh-CN/channels/groups>)。本页介绍该模型之上的 WhatsApp 专属行为：激活、群组允许列表、按群组划分的会话键，以及待处理消息上下文注入。
...
## 行为
  ...
  * 上下文注入：**仅待处理** 的群组消息（默认 50 条），即_未_触发运行的消息，会以 `[Chat messages since your last reply - for context]` 为前缀注入，触发消息行位于 `[Current message - respond to this]` 下。已经在会话中的消息不会被重新注入。
