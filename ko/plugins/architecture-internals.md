...
source_url: https://docs.openclaw.ai/ko/plugins/architecture-internals
...
선택적 매니페스트 `activation` 및 `setup` 블록은 제어 플레인에 남습니다. 이들은 활성화 계획 및 설정 발견을 위한 메타데이터 전용 설명자입니다. 런타임 등록, `register(...)`, 또는 `setupEntry`를 대체하지 않습니다. 첫 번째 라이브 활성화 소비자는 이제 더 넓은 레지스트리 구체화 전에 Plugin 로드를 좁히기 위해 매니페스트 명령, 채널, 공급자 힌트를 사용합니다.
...
OpenClaw는 Plugin 발견 결과나 직접 매니페스트 레지스트리 데이터를 벽시계 시간 창 뒤에 캐시하지 않습니다. 설치, 매니페스트 편집, 로드 경로 변경은 다음 명시적 메타데이터 읽기 또는 스냅샷 재구성에서 표시되어야 합니다. 매니페스트 파일 파서는 열린 매니페스트 경로, inode, 크기, 타임스탬프로 키가 지정된 제한된 파일 시그니처 캐시를 유지할 수 있습니다. 그 캐시는 변경되지 않은 바이트의 재파싱만 피하며, 발견, 레지스트리, 소유자, 정책 답변을 캐시해서는 안 됩니다.

안전한 메타데이터 빠른 경로는 숨겨진 캐시가 아니라 명시적 객체 소유권입니다. Gateway 시작 핫 경로는 현재 `PluginMetadataSnapshot`, 도출된 `PluginLookUpTable`, 또는 명시적 매니페스트 레지스트리를 호출 체인으로 전달해야 합니다. 구성 검증, 시작 자동 활성화, Plugin 부트스트랩, 공급자 선택은 해당 객체들이 현재 구성과 Plugin 인벤토리를 나타내는 동안 재사용할 수 있습니다. 설정 조회는 특정 설정 경로가 명시적 매니페스트 레지스트리를 받지 않는 한 여전히 필요 시 매니페스트 메타데이터를 재구성합니다. 숨겨진 조회 캐시를 추가하지 말고 이를 콜드 경로 폴백으로 유지하세요. 입력이 변경되면 스냅샷을 변경하거나 과거 사본을 유지하는 대신 다시 빌드하고 교체하세요. 활성 Plugin 레지스트리에 대한 뷰와 번들 채널 부트스트랩 헬퍼는 현재 레지스트리/루트에서 다시 계산해야 합니다. 작업 중복 제거 또는 재진입 방지를 위해 한 번의 호출 내부에서 짧게 유지되는 맵은 괜찮지만, 프로세스 메타데이터 캐시가 되어서는 안 됩니다.
...
모델/공급자 Plugin의 경우 OpenClaw는 대략 다음 순서로 훅을 호출합니다. "When to use" 열은 빠른 결정 가이드입니다. `ProviderPlugin.capabilities` 및 `suppressBuiltInModel`처럼 OpenClaw가 더 이상 호출하지 않는 호환성 전용 공급자 필드는 의도적으로 여기에 나열하지 않습니다.
...
`normalizeModelId`, `normalizeTransport`, `normalizeConfig`는 먼저 일치하는 프로바이더 Plugin을 확인한 다음, 다른 훅 지원 프로바이더 Plugin으로 순차적으로 넘어가며 실제로 모델 ID 또는 전송/구성을 변경하는 대상을 찾습니다. 이를 통해 호출자가 어떤 번들 Plugin이 재작성을 소유하는지 알 필요 없이 별칭/호환 프로바이더 shim이 계속 작동합니다. 지원되는 Google 계열 구성 항목을 재작성하는 프로바이더 훅이 없으면, 번들 Google 구성 정규화기가 여전히 해당 호환성 정리를 적용합니다.

프로바이더에 완전히 사용자 지정 wire protocol 또는 사용자 지정 request executor가 필요하다면, 그것은 다른 종류의 확장입니다. 이 훅들은 여전히 OpenClaw의 일반 추론 루프에서 실행되는 프로바이더 동작을 위한 것입니다.
...
번들 프로바이더 Plugin은 위의 훅들을 조합하여 각 벤더의 catalog, auth, thinking, replay, usage 요구사항에 맞춥니다. 권위 있는 훅 집합은 `extensions/` 아래의 각 Plugin에 있으며, 이 페이지는 목록을 그대로 반영하기보다는 형태를 설명합니다.

패스스루 catalog 프로바이더

OpenRouter, Kilocode, [Z.AI](<http://Z.AI>), xAI는 `catalog`와 `resolveDynamicModel` / `prepareDynamicModel`을 등록하여 OpenClaw의 정적 catalog보다 먼저 upstream 모델 ID를 노출할 수 있습니다.

OAuth 및 usage endpoint 프로바이더

GitHub Copilot, Gemini CLI, ChatGPT Codex, MiniMax, Xiaomi, z.ai는 `prepareRuntimeAuth` 또는 `formatApiKey`를 `resolveUsageAuth` \+ `fetchUsageSnapshot`과 함께 사용하여 토큰 교환 및 `/usage` 통합을 소유합니다.

Replay 및 transcript 정리 계열

공유 명명 계열(`google-gemini`, `passthrough-gemini`, `anthropic-by-model`, `hybrid-anthropic-openai`)을 통해 프로바이더는 각 Plugin이 정리를 다시 구현하는 대신 `buildReplayPolicy`를 통해 transcript 정책을 선택할 수 있습니다.

Catalog 전용 프로바이더

`byteplus`, `cloudflare-ai-gateway`, `huggingface`, `kimi-coding`, `nvidia`, `qianfan`, `synthetic`, `together`, `venice`, `vercel-ai-gateway`, `volcengine`은 `catalog`만 등록하고 공유 추론 루프를 사용합니다.

Anthropic 전용 stream helper

Beta headers, `/fast` / `serviceTier`, `context1m`은 일반 SDK가 아니라 Anthropic Plugin의 공개 `api.ts` / `contract-api.ts` seam (`wrapAnthropicProviderStream`, `resolveAnthropicBetas`, `resolveAnthropicFastMode`, `resolveAnthropicServiceTier`) 안에 있습니다.
...
이미지/오디오/비디오 이해를 위해 Plugin은 일반 key/value bag 대신 하나의 타입 지정된 media-understanding provider를 등록합니다.
...
Gateway가 수신을 시작하기 전에 반드시 존재해야 하는 시작 표면을 `setupEntry`가 완전히 포괄할 때만 이것을 사용하세요. 실제로는 setup entry가 시작 시 의존하는 모든 채널 소유 capability를 등록해야 한다는 뜻입니다. 예:
...
전체 entry가 필요한 시작 capability를 하나라도 계속 소유한다면 이 flag를 활성화하지 마세요. Plugin을 기본 동작에 두고 OpenClaw가 시작 중에 전체 entry를 로드하게 하세요.

번들 채널은 전체 채널 runtime이 로드되기 전에 core가 참조할 수 있는 setup 전용 contract-surface helper도 게시할 수 있습니다. 현재 setup promotion 표면은 다음과 같습니다.
...
Core는 전체 Plugin entry를 로드하지 않고 legacy single-account 채널 config를 `channels.<id>.accounts.*`로 승격해야 할 때 이 표면을 사용합니다. Matrix가 현재 번들 예시입니다. named account가 이미 존재하면 auth/bootstrap key만 이름이 지정된 승격 account로 이동하고, 항상 `accounts.default`를 생성하는 대신 구성된 비표준 default-account key를 보존할 수 있습니다.

이러한 setup patch adapter는 번들 contract-surface discovery를 lazy하게 유지합니다. Import 시점은 가볍게 유지되고, promotion 표면은 module import 시 번들 채널 시작에 다시 진입하는 대신 최초 사용 시에만 로드됩니다.

이러한 시작 표면에 Gateway RPC method가 포함될 때는 Plugin별 prefix에 유지하세요. Core admin namespace(`config.*`, `exec.approvals.*`, `wizard.*`, `update.*`)는 계속 예약되어 있으며, Plugin이 더 좁은 scope를 요청하더라도 항상 `operator.admin`으로 resolve됩니다.
...
채널 Plugin은 `openclaw.channel`을 통해 setup/discovery metadata를, `openclaw.install`을 통해 install hint를 알릴 수 있습니다. 이렇게 하면 core catalog가 data-free로 유지됩니다.
...
OpenClaw는 **external channel catalog** 도 병합할 수 있습니다(예: MPM registry export). 다음 위치 중 하나에 JSON file을 두세요.
...
또는 `OPENCLAW_PLUGIN_CATALOG_PATHS`(또는 `OPENCLAW_MPM_CATALOG_PATHS`)가 하나 이상의 JSON file을 가리키게 하세요(comma/semicolon/`PATH`로 구분). 각 file은 `{ "entries": [ { "name": "@scope/pkg", "openclaw": { "channel": {...}, "install": {...} } } ] }`를 포함해야 합니다. Parser는 `"entries"` key의 legacy alias로 `"packages"` 또는 `"plugins"`도 허용합니다.

생성된 채널 catalog entry와 provider install catalog entry는 raw `openclaw.install` block 옆에 normalized install-source fact를 노출합니다. Normalized fact는 npm spec이 정확한 version인지 floating selector인지, 예상 integrity metadata가 있는지, local source path도 사용할 수 있는지를 식별합니다. Catalog/package identity가 알려져 있으면 normalized fact는 파싱된 npm package name이 해당 identity에서 벗어날 때 경고합니다. 또한 `defaultChoice`가 유효하지 않거나 사용할 수 없는 source를 가리킬 때, 그리고 유효한 npm source 없이 npm integrity metadata가 있을 때도 경고합니다. Consumer는 hand-built entry와 catalog shim이 이를 합성하지 않아도 되도록 `installSource`를 additive optional field로 취급해야 합니다. 이를 통해 onboarding과 diagnostic은 Plugin runtime을 import하지 않고도 source-plane state를 설명할 수 있습니다.

공식 external npm entry는 정확한 `npmSpec`와 `expectedIntegrity`를 선호해야 합니다. Bare package name과 dist-tag도 호환성을 위해 계속 동작하지만 source-plane warning을 표시하므로, catalog는 기존 Plugin을 깨뜨리지 않고 pinned, integrity-checked install로 이동할 수 있습니다. Onboarding이 local catalog path에서 install할 때는 가능한 경우 `source: "path"`와 workspace-relative `sourcePath`가 있는 managed Plugin Plugin index entry를 기록합니다. 절대 operational load path는 `plugins.load.paths`에 유지되고, install record는 local workstation path를 장기 config에 중복하지 않습니다. 이렇게 하면 local development install이 source-plane diagnostic에 표시되면서 두 번째 raw filesystem-path disclosure 표면을 추가하지 않습니다. Persisted `plugins/installs.json` Plugin index는 install source of truth이며 Plugin runtime module을 로드하지 않고도 refresh할 수 있습니다. 해당 `installRecords` map은 Plugin manifest가 없거나 유효하지 않아도 durable하고, `plugins` array는 재빌드 가능한 manifest view입니다.

## 컨텍스트 엔진 Plugin

컨텍스트 엔진 Plugin은 ingest, assembly, Compaction을 위한 session context orchestration을 소유합니다. Plugin에서 `api.registerContextEngine(id, factory)`로 등록한 다음 `plugins.slots.contextEngine`으로 활성 engine을 선택하세요.

Plugin이 memory search나 hook을 추가하는 수준을 넘어 기본 context pipeline을 교체하거나 확장해야 할 때 이것을 사용하세요.
...
Factory `ctx`는 construction-time initialization을 위한 optional `config`, `agentDir`, `workspaceDir` 값을 노출합니다.

Engine이 Compaction algorithm을 소유하지 **않는다면** , `compact()`를 구현한 상태로 유지하고 명시적으로 delegate하세요.
...
Plugin에 현재 API에 맞지 않는 동작이 필요하다면 private reach-in으로 Plugin system을 우회하지 마세요. 누락된 capability를 추가하세요.
...
권장 순서:
  ...
  2. typed Plugin registration/runtime 표면 추가 가장 작고 유용한 typed capability 표면으로 `OpenClawPluginApi` 및/또는 `api.runtime`을 확장하세요.
...
이것이 OpenClaw가 한 provider의 worldview에 hardcode되지 않으면서도 opinionated하게 유지되는 방식입니다. 구체적인 file checklist와 worked example은 [Capability Cookbook](</ko/plugins/adding-capabilities>)을 참조하세요.
