...
source_url: https://docs.openclaw.ai/zh-TW/plugins/sdk-subpaths
...
`plugin-sdk/provider-zai-endpoint` | 已棄用的 [Z.AI](<http://Z.AI>) 供應商擁有端點偵測外觀介面；請使用 [Z.AI](<http://Z.AI>) Plugin 公用 API  
...
目前沒有保留的隨附輔助工具 SDK 子路徑。擁有者專屬 輔助工具位於其擁有的 Plugin 套件內，而可重用的主機契約 使用通用 SDK 子路徑，例如 `plugin-sdk/gateway-runtime`、 `plugin-sdk/security-runtime` 與 `plugin-sdk/plugin-config-runtime`。
