...
OpenClaw, zaten kullandığınız herhangi bir sohbet uygulamasında sizinle konuşabilir. Her kanal Gateway üzerinden bağlanır. Metin her yerde desteklenir; medya ve tepkiler kanala göre değişir.

## Teslim notları

  * `![alt](url)` gibi markdown görsel söz dizimi içeren Telegram yanıtları, mümkün olduğunda son giden yolda medya yanıtlarına dönüştürülür.
  * Slack çok kişili DM'leri grup sohbetleri olarak yönlendirilir; bu nedenle grup ilkesi, bahsetme davranışı ve grup oturumu kuralları MPIM konuşmaları için geçerlidir.
  * WhatsApp kurulumu isteğe bağlı yüklemedir: katılım akışı, Plugin paketi yüklenmeden önce kurulum akışını gösterebilir ve Gateway, WhatsApp runtime'ını yalnızca kanal gerçekten etkin olduğunda yükler.
...
## Notlar
  ...
  * En hızlı kurulum genellikle **Telegram** 'dır (basit bot token'ı). WhatsApp QR eşleştirmesi gerektirir ve diskte daha fazla durum saklar.
