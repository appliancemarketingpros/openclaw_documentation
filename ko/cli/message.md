---
title: 메시지
source_url: https://docs.openclaw.ai/ko/cli/message
scraped_at: 2026-05-25
---

# `openclaw message`

메시지와 채널 작업을 보내기 위한 단일 아웃바운드 명령 (Discord/Google Chat/iMessage/Matrix/Mattermost (Plugin)/Microsoft Teams/Signal/Slack/Telegram/WhatsApp).
...
### 핵심
  ...
  * `read`
    ...
    * 필수: `--target`
    * 선택 사항: `--limit`, `--message-id`, `--before`, `--after`
    * Slack 전용: `--message-id`는 특정 Slack 메시지 타임스탬프를 읽습니다. 정확한 스레드 답글을 읽으려면 `--thread-id`와 함께 사용하세요.
    * Discord 전용: `--around`
  * `edit`
