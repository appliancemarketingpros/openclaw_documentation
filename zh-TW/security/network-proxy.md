---
title: 網路代理
source_url: https://docs.openclaw.ai/zh-TW/security/network-proxy
...
## 限制
  ...
  * Gateway 回環控制平面流量預設會透過 `proxy.loopbackMode: "gateway-only"` 直接本機旁路。OpenClaw 會在受管理的 `global-agent` `NO_PROXY` 控制器中註冊作用中的 Gateway 回環 authority，以實作該旁路。操作員可以設定 `proxy.loopbackMode: "proxy"`，將 Gateway 回環流量透過受管理代理傳送，或設定 `proxy.loopbackMode: "block"`，拒絕回環 Gateway 連線。請參閱 Gateway 回環模式 了解遠端代理注意事項。
