---
title: 程式碼執行
source_url: https://docs.openclaw.ai/zh-TW/tools/code-execution
...
針對 `code_execution` 和 `x_search` 執行 `openclaw onboard --auth-choice xai-api-key`，或設定 `XAI_API_KEY`／在 xAI Plugin 下設定金鑰，讓 Grok 網頁搜尋也使用同一組憑證：
...
[**Exec 工具** 在你的機器或配對節點上執行本機 shell。 ](</zh-TW/tools/exec>) [**Exec 核准** shell 執行的允許／拒絕政策。 ](</zh-TW/tools/exec-approvals>) [**網頁工具** `web_search`、`x_search` 和 `web_fetch`。 ](</zh-TW/tools/web>) [**xAI 供應商** Grok 模型、web/x 搜尋，以及程式碼執行設定。 ](</zh-TW/providers/xai>)
