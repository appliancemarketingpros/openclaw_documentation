...
source_url: https://docs.openclaw.ai/de/nodes/audio
...
Um die automatische Erkennung zu deaktivieren, setzen Sie `tools.media.audio.enabled: false`. Zum Anpassen setzen Sie `tools.media.audio.models`. Hinweis: Die Binärerkennung erfolgt bestmöglich über macOS/Linux/Windows hinweg; stellen Sie sicher, dass die CLI in `PATH` liegt (wir erweitern `~`), oder legen Sie ein explizites CLI-Modell mit einem vollständigen Befehlspfad fest.
