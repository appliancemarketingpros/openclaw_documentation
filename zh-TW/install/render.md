---
title: 渲染
source_url: https://docs.openclaw.ai/zh-TW/install/render
...
Render Blueprints 是用來定義基礎架構的 YAML 檔案。此儲存庫中的 `render.yaml` 會設定執行 OpenClaw 所需的一切：
...
Blueprint 預設為 `starter`。若要使用免費方案，請在你的 fork 的 `render.yaml` 中改成 `plan: free`（但請注意：沒有持久化磁碟表示 OpenClaw 狀態 會在每次部署時重設）。
...
使用已設定的共用密鑰連線。此部署範本會自動產生 `OPENCLAW_GATEWAY_TOKEN`（可在 **Dashboard → 你的服務 → Environment** 中找到）；如果你改用密碼驗證，請改用該密碼。
...
這會建立一個可攜式備份封存檔，內含 OpenClaw 狀態以及任何已設定的工作區。 詳情請參閱[備份](</zh-TW/cli/backup>)。
...
這會發生在免費方案（沒有持久化磁碟）。請升級到付費方案，或 定期在 Render Shell 中透過 `openclaw backup create` 匯出完整備份。
