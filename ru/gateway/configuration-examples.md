...
source_url: https://docs.openclaw.ai/ru/gateway/configuration-examples
...
Оставляйте сопряжение устройств ручным, если вы не контролируете сетевой путь. Для выделенной лаборатории или подсети tailnet можно включить автоодобрение устройства узла при первом подключении с точными CIDR или IP:
...
Если параметр не задан, эта функция остается выключенной. Она применяется только к новому сопряжению `role: node` без запрошенных областей доступа. Клиенты оператора/браузера, а также обновления роли, области доступа, метаданных или открытого ключа по-прежнему требуют ручного одобрения.
...
Для Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC авторизация отправителя по умолчанию сначала проверяет ID. Включайте прямое сопоставление по изменяемому имени, адресу электронной почты или никнейму с помощью `dangerouslyAllowNameMatching: true` в соответствующем канале только если вы явно принимаете этот риск.
