---
title: Plugin 測試
source_url: https://docs.openclaw.ai/zh-TW/plugins/sdk-testing
...
這些 test-helper 子路徑是 OpenClaw 自身 內建 Plugin 測試的 repo-local 原始碼進入點。它們不是第三方 Plugin 的套件匯出。
...
新的 Plugin 測試請優先使用下方聚焦的子路徑。較廣泛的 `openclaw/plugin-sdk/testing` barrel 僅供舊版相容性使用。 Repo guardrails 會拒絕從 `plugin-sdk/testing` 和 `plugin-sdk/test-utils` 新增真正匯入；這些名稱只保留作為相容性記錄測試的已棄用相容性介面。
...
內建 Plugin 合約套件也會使用 SDK 測試子路徑，作為僅供測試的 登錄、資訊清單、公開成品與執行階段 fixture 輔助工具。依賴內建 OpenClaw 清單的 僅核心套件會保留在 `src/plugins/contracts` 下。請將新的 extension 測試放在已文件化且聚焦的 SDK 子路徑上，例如 `plugin-sdk/plugin-test-api`、`plugin-sdk/channel-contract-testing`、 `plugin-sdk/agent-runtime-test-contracts`、`plugin-sdk/channel-test-helpers`、 `plugin-sdk/plugin-test-contracts`、`plugin-sdk/plugin-test-runtime`、 `plugin-sdk/provider-test-contracts`、`plugin-sdk/provider-http-test-mocks`、 `plugin-sdk/test-env` 或 `plugin-sdk/test-fixtures`，而不是直接匯入範圍較廣的 `plugin-sdk/testing` 相容性 barrel、儲存庫 `src/**` 檔案，或儲存庫 `test/helpers/*` 橋接。
...
將手寫 `api` mock 傳給 `register(api)` 的單元測試，不會實際測到 OpenClaw 的載入器接受門檻。請為你的 Plugin 依賴的每個註冊介面至少加入一個由載入器支援的煙霧測試，尤其是 hook 與記憶體等 獨佔能力。

真正的載入器會在必要中繼資料缺失，或 Plugin 呼叫未擁有的能力 API 時，使 Plugin 註冊失敗。例如， `api.registerHook(...)` 需要 hook 名稱，而 `api.registerMemoryCapability(...)` 需要 Plugin 資訊清單或匯出的 entry 宣告 `kind: "memory"`。

### 測試執行階段設定存取

測試內建通道 Plugin 時，優先使用 `openclaw/plugin-sdk/channel-test-helpers` 提供的共用 Plugin 執行階段 mock。其已棄用的 `runtime.config.loadConfig()` 與 `runtime.config.writeConfigFile(...)` mock 預設會拋出錯誤，讓測試能抓到相容性 API 的新用法。只有在測試明確涵蓋舊版相容性行為時，才覆寫這些 mock。
