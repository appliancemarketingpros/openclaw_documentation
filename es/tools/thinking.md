...
source_url: https://docs.openclaw.ai/es/tools/thinking
...
## Qué hace
  ...
  * Notas de proveedor: 
    ...
    * [Z.AI](<http://Z.AI>) (`zai/*`) solo admite razonamiento binario (`on`/`off`). Cualquier nivel que no sea `off` se trata como `on` (asignado a `low`).
