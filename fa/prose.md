---
title: OpenProse
source_url: https://docs.openclaw.ai/fa/prose
...
`/prose run <handle/slug>` به `https://p.prose.md/<handle>/<slug>` resolve می‌شود. URLهای مستقیم همان‌طور که هستند واکشی می‌شوند. این کار از ابزار `web_fetch` (یا `exec` برای POST) استفاده می‌کند.
