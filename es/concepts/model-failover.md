...
source_url: https://docs.openclaw.ai/es/concepts/model-failover
...
* ### Construir la cadena de candidatos
...
* ### Revertir de forma acotada ante fallo
...
* ### Lanzar FallbackSummaryError si se agotan
...
Para los modelos de agente de OpenAI, la autenticación y el runtime están separados. `openai/gpt-*` permanece en el arnés de Codex mientras la autenticación puede rotar entre un perfil de suscripción de Codex y un respaldo de clave de API de OpenAI.
...
Los perfiles de suscripción de Codex existentes todavía pueden usar el id de perfil heredado `openai-codex:*`. El respaldo de clave de API ordenado puede ser un perfil de clave de API `openai:*` normal. Cuando la suscripción alcanza un límite de uso de Codex, OpenClaw registra la hora exacta de restablecimiento cuando Codex proporciona una, prueba el siguiente perfil de autenticación ordenado y mantiene la ejecución dentro del arnés de Codex. Una vez que pasa la hora de restablecimiento, el perfil de suscripción vuelve a ser elegible y la siguiente selección automática puede volver a él.
...
Qué entra en el grupo de límite de tasa / tiempo de espera
