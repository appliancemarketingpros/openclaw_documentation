---
title: 形式化驗證（安全模型）
source_url: https://docs.openclaw.ai/zh-TW/security/formal-verification
...
bashCopy code
[code]
    git clone https://github.com/vignesh07/openclaw-formal-modelscd openclaw-formal-models # 需要 Java 11+（TLC 在 JVM 上執行）。# 此 repo 隨附固定版本的 `tla2tools.jar`（TLA+ 工具），並提供 `bin/tlc` + Make targets。 make <target>
[/code]
