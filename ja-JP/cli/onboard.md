---
title: オンボード
source_url: https://docs.openclaw.ai/ja-JP/cli/onboard
...
[**CLI オンボーディングハブ** インタラクティブな CLI フローのウォークスルー。 ](</ja-JP/start/wizard>) [**オンボーディング概要** OpenClaw のオンボーディング全体の構成。 ](</ja-JP/start/onboarding-overview>) [**CLI セットアップリファレンス** 出力、内部構造、ステップごとの動作。 ](</ja-JP/start/wizard-cli-reference>) [**CLI 自動化** 非インタラクティブフラグとスクリプト化されたセットアップ。 ](</ja-JP/start/wizard-cli-automation>) [**macOS アプリのオンボーディング** macOS メニューバーアプリのオンボーディングフロー。 ](</ja-JP/start/onboarding>)
...
`--modern` は Crestodian の対話型オンボーディングプレビューを開始します。 `--modern` なしの場合、`openclaw onboard` は従来のオンボーディングフローを維持します。

平文のプライベートネットワーク `ws://` ターゲット（信頼済みネットワークのみ）では、オンボーディングプロセス環境で `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` を設定します。 このクライアント側トランスポートの緊急回避策に対応する `openclaw.json` 設定はありません。
...
`--custom-api-key` は非インタラクティブモードでは任意です。省略した場合、オンボーディングは `CUSTOM_API_KEY` を確認します。 OpenClaw は一般的なビジョンモデル ID を画像対応として自動的にマークします。不明なカスタムビジョン ID には `--custom-image-input` を渡し、テキスト専用メタデータを強制するには `--custom-text-input` を渡します。
...
`--secret-input-mode ref` を指定すると、オンボーディングは平文のキー値ではなく環境変数ベースの参照を書き込みます。 認証プロファイルに基づくプロバイダーでは `keyRef` エントリを書き込みます。カスタムプロバイダーでは `models.providers.<id>.apiKey` を環境変数参照として書き込みます（例: `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`）。
...
### 非インタラクティブ [Z.AI](<http://Z.AI>) エンドポイントの選択
