...
source_url: https://docs.openclaw.ai/es/gateway/sandbox-vs-tool-policy-vs-elevated
...
Reglas prácticas:
  ...
  * `/exec` solo cambia los valores predeterminados de sesión para remitentes autorizados; no concede acceso a herramientas. Las claves de herramientas de proveedor aceptan `provider` (por ejemplo, `google-antigravity`) o `provider/model` (por ejemplo, `openai/gpt-5.4`).
...
Grupos disponibles:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` Para agentes de solo lectura, deniega `group:runtime` además de las herramientas mutadoras del sistema de archivos, a menos que la política del sistema de archivos del sandbox o un límite de host separado aplique la restricción de solo lectura.
