...
source_url: https://docs.openclaw.ai/it/channels
scraped_at: 2026-05-25
---

OpenClaw può parlarti su qualsiasi app di chat che usi già. Ogni canale si connette tramite il Gateway. Il testo è supportato ovunque; media e reazioni variano in base al canale.

## Note di recapito

  * Le risposte Telegram che contengono sintassi markdown per immagini, come `![alt](url)`, vengono convertite in risposte multimediali nel percorso finale in uscita quando possibile.
  * I DM Slack con più persone vengono instradati come chat di gruppo, quindi le policy di gruppo, il comportamento delle menzioni e le regole delle sessioni di gruppo si applicano alle conversazioni MPIM.
  * La configurazione di WhatsApp è install-on-demand: l'onboarding può mostrare il flusso di configurazione prima che il pacchetto plugin sia installato, e il Gateway carica il runtime WhatsApp solo quando il canale è effettivamente attivo.
...
## Note
  ...
  * La configurazione più rapida di solito è **Telegram** (semplice token del bot). WhatsApp richiede l'associazione tramite QR e memorizza più stato su disco.
