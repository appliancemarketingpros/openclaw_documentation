---
title: 기본 AGENTS.md
source_url: https://docs.openclaw.ai/ko/reference/AGENTS.default
scraped_at: 2026-05-25
---

## 첫 실행(권장)

OpenClaw는 에이전트용 전용 작업 영역 디렉터리를 사용합니다. 기본값: `~/.openclaw/workspace` (`agents.defaults.workspace`로 구성 가능).

  1. 작업 영역을 만듭니다(아직 없는 경우).
...
[/code]
  ...
  4. 선택 사항: `agents.defaults.workspace`를 설정해 다른 작업 영역을 선택합니다(`~` 지원).
...
## 안전 기본값
  ...
  * 외부 메시징 표면에 부분/스트리밍 답장을 보내지 마세요(최종 답장만).
...
## 메모리 시스템(권장)
  ...
  * 장기 메모리: 지속적인 사실, 선호, 결정에는 `MEMORY.md`.
  * 소문자 `memory.md`는 레거시 복구 입력 전용입니다. 루트 파일 둘을 의도적으로 함께 유지하지 마세요.
  * 세션 시작 시 있으면 오늘 + 어제 + `MEMORY.md`를 읽으세요.
  * 기록 대상: 결정, 선호, 제약, 열린 작업.
  * 명시적으로 요청받지 않는 한 비밀 정보는 피하세요.


## 도구와 Skills

  * 도구는 Skills에 있습니다. 필요할 때 각 skill의 `SKILL.md`를 따르세요.
  * 환경별 메모는 `TOOLS.md`(Skills용 메모)에 보관하세요.


## 백업 팁(권장)

이 작업 영역을 Clawd의 "메모리"로 취급한다면, `AGENTS.md`와 메모리 파일이 백업되도록 git 저장소(이상적으로는 비공개)로 만드세요.
...
## OpenClaw가 하는 일
  ...
  * macOS 앱은 권한(화면 기록, 알림, 마이크)을 관리하고 번들된 바이너리를 통해 `openclaw` CLI를 노출합니다.
  * 직접 채팅은 기본적으로 에이전트의 `main` 세션으로 접히며, 그룹은 `agent:<agentId>:<channel>:group:<id>`로 격리됩니다(방/채널: `agent:<agentId>:<channel>:channel:<id>`). Heartbeat는 백그라운드 작업을 계속 유지합니다.
...
## 핵심 Skills(설정 → Skills에서 활성화)
  ...
  * **oracle** \- 세션 리플레이와 브라우저 제어를 지원하는 OpenAI 준비 완료 에이전트 CLI.
  ...
  * **Gemini CLI** \- 빠른 Q&A를 위해 터미널에서 사용하는 Google Gemini 모델.
...
## 사용 참고 사항
  ...
  * 비서가 알림을 예약하고, 받은 편지함을 모니터링하고, 카메라 캡처를 트리거할 수 있도록 Heartbeat를 활성화해 두세요.
  * Canvas UI는 네이티브 오버레이와 함께 전체 화면으로 실행됩니다. 중요한 컨트롤을 왼쪽 위/오른쪽 위/아래쪽 가장자리에 배치하지 마세요. 레이아웃에 명시적 거터를 추가하고 안전 영역 인셋에 의존하지 마세요.
  * 브라우저 기반 검증에는 OpenClaw가 관리하는 Chrome 프로필과 함께 `openclaw browser`(탭/상태/스크린샷)를 사용하세요.
  * DOM 검사에는 `openclaw browser eval|query|dom|snapshot`를 사용하세요(머신 출력이 필요하면 `--json`/`--out` 사용).
  * 상호작용에는 `openclaw browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`을 사용하세요(click/type은 스냅샷 참조가 필요합니다. CSS 선택자에는 `evaluate` 사용).
