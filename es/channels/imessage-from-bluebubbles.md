...
source_url: https://docs.openclaw.ai/es/channels/imessage-from-bluebubbles
...
`channels status --probe` solo prueba cuentas configuradas y habilitadas. No reinicies el Gateway con BlueBubbles e iMessage habilitados a la vez, salvo que quieras intencionadamente que ambos monitores de canal se ejecuten. Si no vas a hacer la transición de inmediato, vuelve a establecer `channels.imessage.enabled` en `false` antes de reiniciar el Gateway. Usa los comandos directos de `imsg` en Antes de empezar para validar el Mac antes de habilitar el tráfico de OpenClaw.
