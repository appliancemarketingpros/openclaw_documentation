...
source_url: https://docs.openclaw.ai/fr/automation/hooks
...
### Format de [HOOK.md](<http://HOOK.md>)
...
Les plugins peuvent enregistrer des points d’accroche typés via le SDK de Plugin pour une intégration plus profonde : interception des appels d’outils, modification des prompts, contrôle du flux des messages, et plus encore. Utilisez les points d’accroche de Plugin lorsque vous avez besoin de `before_tool_call`, `before_agent_reply`, `before_install` ou d’autres points d’accroche de cycle de vie dans le processus.
