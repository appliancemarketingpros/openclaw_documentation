---
title: OpenProse
source_url: https://docs.openclaw.ai/vi/prose
...
`/prose run <handle/slug>` phân giải thành `https://p.prose.md/<handle>/<slug>`. URL trực tiếp được tải nguyên trạng. Việc này dùng công cụ `web_fetch` (hoặc `exec` cho POST).
