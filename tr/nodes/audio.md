...
source_url: https://docs.openclaw.ai/tr/nodes/audio
...
**Model yapılandırmazsanız** ve `tools.media.audio.enabled` **`false` olarak ayarlanmamışsa**, OpenClaw bu sırayla otomatik algılama yapar ve çalışan ilk seçenekte durur:
...
Otomatik algılamayı devre dışı bırakmak için `tools.media.audio.enabled: false` ayarlayın. Özelleştirmek için `tools.media.audio.models` ayarlayın. Not: İkili dosya algılama macOS/Linux/Windows genelinde en iyi çaba esasına dayanır; CLI'ın `PATH` üzerinde olduğundan emin olun (`~` genişletilir) veya tam komut yoluyla açık bir CLI modeli ayarlayın.
