...
source_url: https://docs.openclaw.ai/vi/automation/cron-jobs
...
* ### Kiểm tra các công việc của bạn
...
Phiên chính so với cô lập so với tùy chỉnh
...
'Phiên mới' nghĩa là gì đối với công việc cô lập
...
OPENCLAW_DOCS_MARKER:accordionOpen:IHRpdGxlPSJIb29rIMSRxrDhu6NjIMOhbmggeOG6oSAoUE9TVCAvaG9va3MvPG5hbWU )"> Tên hook tùy chỉnh được phân giải qua `hooks.mappings` trong cấu hình. Ánh xạ có thể chuyển đổi payload tùy ý thành hành động `wake` hoặc `agent` bằng template hoặc biến đổi mã.
...
* ### Tạo topic và cấp quyền truy cập push cho Gmail
