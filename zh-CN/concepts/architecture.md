---
title: Gateway 网关架构
source_url: https://docs.openclaw.ai/zh-CN/concepts/architecture
scraped_at: 2026-05-25
---

## 概览

  * 单个长期运行的 **Gateway 网关** 拥有所有消息表面（通过 Baileys 的 WhatsApp、通过 grammY 的 Telegram、Slack、Discord、Signal、iMessage、WebChat）。
  * 控制平面客户端（macOS 应用、CLI、Web UI、自动化）通过配置的绑定主机上的 **WebSocket** 连接到 Gateway 网关（默认 `127.0.0.1:18789`）。
  ...
  * **canvas host** 由 Gateway 网关 HTTP 服务器在以下路径提供： 
    ...
    * `/__openclaw__/a2ui/`（A2UI host） 它使用与 Gateway 网关相同的端口（默认 `18789`）。
...
## 线缆协议（摘要）
  ...
  * 共享密钥认证使用 `connect.params.auth.token` 或 `connect.params.auth.password`，具体取决于配置的 Gateway 网关认证模式。
  * 携带身份的模式，例如 Tailscale Serve (`gateway.auth.allowTailscale: true`) 或非回环 `gateway.auth.mode: "trusted-proxy"`，会从请求标头满足认证，而不是使用 `connect.params.auth.*`。
