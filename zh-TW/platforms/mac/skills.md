---
title: Skills (macOS)
source_url: https://docs.openclaw.ai/zh-TW/platforms/mac/skills
...
## 資料來源
  ...
  * `skills.status`（Gateway）會回傳所有 Skills，以及資格與缺少的需求 （包含內建 Skills 的允許清單封鎖）。
...
## 安裝動作
  ...
  * 如果每個安裝選項都是 `download`，Gateway 會呈現所有下載 選擇。
  * 否則，Gateway 會使用目前的 安裝偏好設定與主機二進位檔，挑選一個偏好的安裝程式：當 `skills.install.preferBrew` 已啟用且 `brew` 存在時，Homebrew 優先，接著是 `uv`，再來是 `skills.install.nodeManager` 中設定的 Node 管理器，然後才是後續 備援，例如 `go` 或 `download`。
