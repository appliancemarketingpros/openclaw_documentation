---
title: 社区插件
source_url: https://docs.openclaw.ai/zh-CN/plugins/community
...
你的插件必须能通过 `openclaw plugins install \<package-name\>` 安装。 除非你明确需要仅 npm 分发，否则请发布到 [ClawHub](</zh-CN/clawhub>)。 完整指南见 [构建插件](</zh-CN/plugins/building-plugins>)。
