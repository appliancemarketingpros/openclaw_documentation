---
title: PixVerse
source_url: https://docs.openclaw.ai/zh-CN/providers/pixverse
...
向导会先询问是否使用国际版端点 (`https://app-api.pixverse.ai/openapi/v2`) 或 CN 端点 (`https://app-api.pixverseai.cn/openapi/v2`)，然后将 `region` 和 `baseUrl` 写入提供商配置。

* ### 将 PixVerse 设为默认视频提供商
...
OpenClaw 默认使用国际版 PixVerse API。当你的 key 属于特定 PixVerse 平台区域时，可以手动设置 `models.providers.pixverse.region`，或使用 `openclaw onboard --auth-choice pixverse-api-key` 在设置向导中选择：
...
仅当通过受信任的兼容代理路由时，才设置 `models.providers.pixverse.baseUrl`。 `baseUrl` 优先于 `region`。
...
PixVerse 会从生成请求返回 `video_id`。OpenClaw 会轮询 `/openapi/v2/video/result/{video_id}`，直到任务成功、失败或超时。
