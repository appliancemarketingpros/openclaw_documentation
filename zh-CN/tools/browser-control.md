---
title: 浏览器控制 API
source_url: https://docs.openclaw.ai/zh-CN/tools/browser-control
scraped_at: 2026-05-25
---

有关设置、配置和故障排除，请参阅[浏览器](</zh-CN/tools/browser>)。 本页是本地控制 HTTP API、`openclaw browser` CLI 和脚本模式（快照、引用、等待、调试流程）的参考。
...
如果你的 Gateway 网关在 Docker 中运行，请避免使用 `npx playwright`（会与 npm 覆盖项冲突）。 对于自定义镜像，请将 Chromium 烘焙进镜像：
...
基础：状态、标签页、打开/聚焦/关闭 bashCopy code
[code]
...
检查：截图、快照、控制台、错误、请求 bashCopy code
[code]
...
操作：导航、点击、输入、拖拽、等待、求值 bashCopy code
[code]
...
状态：Cookie、存储、离线、标头、地理位置、设备 bashCopy code
[code]
