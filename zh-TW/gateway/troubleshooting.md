---
title: 疑難排解
source_url: https://docs.openclaw.ai/zh-TW/gateway/troubleshooting
...
OpenClaw 會將每個 skill 根目錄視為包含邊界。位於 `~/.agents/skills`、`<workspace>/.agents/skills`、`<workspace>/skills` 或 `~/.openclaw/skills` 底下的符號連結，在其真實目標解析到該根目錄外時會被略過， 除非該目標已被明確信任。
...
如果該目標是有意設定，請同時設定直接 skill 根目錄與 允許的符號連結目標：
...
然後啟動新的工作階段，或等待 skills 監看器重新整理。如果執行中的程序早於設定變更，請重新啟動 Gateway。

請勿使用範圍過廣的目標，例如 `~`、`/` 或整個同步專案資料夾。 請將 `allowSymlinkTargets` 限定在包含受信任 `SKILL.md` 目錄的真實 skill 根目錄。
...
bashCopy code
[code]
    openclaw gateway statusopenclaw statusopenclaw logs --followopenclaw doctoropenclaw gateway status --deep   # 也掃描系統層級服務
[/code]
...
當 Gateway 啟動因 `Invalid config` 失敗，或熱重新載入記錄表示 已略過無效編輯時使用此項。
...
Chrome MCP / existing-session 特徵
...
1\. 驗證與 URL 覆寫行為已變更 bashCopy code
[code]
