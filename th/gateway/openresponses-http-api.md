...
source_url: https://docs.openclaw.ai/th/gateway/openresponses-http-api
...
ภายใน คำขอจะถูกดำเนินการเหมือนการรันเอเจนต์ Gateway ตามปกติ (เส้นทางโค้ดเดียวกับ `openclaw agent`) ดังนั้นการกำหนดเส้นทาง/สิทธิ์/config จะตรงกับ Gateway ของคุณ
...
ตารางการยืนยันตัวตน:
  ...
  * `gateway.auth.mode="token"` หรือ `"password"` \+ `Authorization: Bearer ...`
    ...
    * คืนค่าชุด scope เริ่มต้นเต็มรูปแบบของผู้ปฏิบัติการ: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
หากคำขอมีสตริง `user` ของ OpenResponses Gateway จะสร้างคีย์เซสชันที่เสถียร จากค่านั้น เพื่อให้การเรียกซ้ำสามารถใช้เซสชันเอเจนต์ร่วมกันได้
...
หากเอเจนต์ตัดสินใจเรียกเครื่องมือ คำตอบจะส่งคืนรายการเอาต์พุต `function_call` จากนั้นคุณส่งคำขอติดตามผลด้วย `function_call_output` เพื่อดำเนิน turn ต่อ
...
ชนิด MIME ที่อนุญาต (ปัจจุบัน): `image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/heic`, `image/heif` ขนาดสูงสุด (ปัจจุบัน): 10MB
...
ชนิด MIME ที่อนุญาต (ปัจจุบัน): `text/plain`, `text/markdown`, `text/html`, `text/csv`, `application/json`, `application/pdf`

ขนาดสูงสุด (ปัจจุบัน): 5MB

ลักษณะการทำงานปัจจุบัน:

  * เนื้อหาไฟล์จะถูกถอดรหัสและเพิ่มเข้าใน**พรอมป์ระบบ** ไม่ใช่ข้อความผู้ใช้ ดังนั้นจึงคงเป็นแบบชั่วคราว (ไม่ถูกเก็บถาวรในประวัติเซสชัน)
  * ข้อความไฟล์ที่ถอดรหัสแล้วจะถูกห่อเป็น**เนื้อหาภายนอกที่ไม่น่าเชื่อถือ** ก่อนเพิ่มเข้าไป ดังนั้นไบต์ของไฟล์จะถูกถือเป็นข้อมูล ไม่ใช่คำสั่งที่เชื่อถือได้
  * บล็อกที่แทรกใช้เครื่องหมายขอบเขตที่ชัดเจน เช่น `<<&lt;EXTERNAL_UNTRUSTED_CONTENT id=&quot;...&quot;&gt;>>` / `<<&lt;END_EXTERNAL_UNTRUSTED_CONTENT id=&quot;...&quot;&gt;>>` และมีบรรทัดเมทาดาทา `Source: External`
  * เส้นทางอินพุตไฟล์นี้จงใจละเว้นแบนเนอร์ `SECURITY NOTICE:` แบบยาวเพื่อ รักษางบประมาณพรอมป์ เครื่องหมายขอบเขตและเมทาดาทายังคงอยู่ครบ
  * PDF จะถูกแยกข้อความก่อน หากพบข้อความน้อย หน้าแรก ๆ จะถูก แปลงเป็นภาพแรสเตอร์และส่งต่อให้โมเดล และบล็อกไฟล์ที่แทรกจะใช้ ตัวแทนข้อความ `[PDF content rendered to images]`
...
หมายเหตุด้านความปลอดภัย:
  ...
  * สำหรับ Gateway ที่เปิดสู่อินเทอร์เน็ต ให้ใช้การควบคุมการออกเครือข่ายเพิ่มเติมจากการป้องกันระดับแอป ดู [ความปลอดภัย](</th/gateway/security>)
...
`usage` จะถูกเติมค่าเมื่อผู้ให้บริการเบื้องหลังรายงานจำนวนโทเค็น OpenClaw ทำให้นามแฝงทั่วไปแบบ OpenAI อยู่ในรูปแบบมาตรฐานก่อนที่ตัวนับเหล่านั้นจะไปถึง พื้นผิวสถานะ/เซสชันปลายทาง รวมถึง `input_tokens` / `output_tokens` และ `prompt_tokens` / `completion_tokens`
