...
source_url: https://docs.openclaw.ai/es/platforms/mac/remote
...
En el modo de túnel SSH, los nombres de host LAN/tailnet descubiertos se guardan como `gateway.remote.sshTarget`. La app mantiene `gateway.remote.url` en el endpoint del túnel local, por ejemplo `ws://127.0.0.1:18789`, para que la CLI, Web Chat y el servicio de host Node local usen todos el mismo transporte local loopback seguro.

La automatización del navegador en modo remoto pertenece al host Node de la CLI, no al Node nativo de la app de macOS. La app inicia el servicio de host Node instalado cuando es posible; si necesitas control del navegador desde ese Mac, instálalo/inícialo con `openclaw node install ...` y `openclaw node start` (o ejecuta `openclaw node run ...` en primer plano), y luego apunta a ese Node con capacidad de navegador.
