...
**Teams + Kanal-Allowlist**
  ...
  * Beim Start löst OpenClaw Team-/Kanal- und Benutzer-Allowlist-Namen in IDs auf (wenn Graph-Berechtigungen dies zulassen) und protokolliert die Zuordnung; nicht aufgelöste Team-/Kanalnamen werden wie eingegeben beibehalten, aber standardmäßig für das Routing ignoriert, sofern `channels.msteams.dangerouslyAllowNameMatching: true` nicht aktiviert ist.
...
**Manuelle Einrichtung (ohne Teams CLI)**
...
Wichtige Einstellungen (gemeinsame Kanalmuster siehe `/gateway/configuration`):
  ...
  * `toolsBySender`-Schlüssel sollten explizite Präfixe verwenden: `channel:`, `id:`, `e164:`, `username:`, `name:` (alte Schlüssel ohne Präfix werden weiterhin nur `id:` zugeordnet).
...
### Fehler beim Manifest-Upload
  ...
  * **"[webApplicationInfo.Id](<http://webApplicationInfo.Id>) already in use":** Die App ist noch in einem anderen Team/Chat installiert. Suchen und deinstallieren Sie sie zuerst, oder warten Sie 5-10 Minuten auf die Verteilung.
