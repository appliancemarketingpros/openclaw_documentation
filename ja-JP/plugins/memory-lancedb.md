...
Ollama 埋め込みでは、同梱の Ollama 埋め込みプロバイダーを優先してください。これはネイティブの Ollama `/api/embed` エンドポイントを使用し、[Ollama](</ja-JP/providers/ollama>) に記載されている Ollama プロバイダーと同じ認証およびベース URL ルールに従います。
