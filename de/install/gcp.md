---
title: GCP
source_url: https://docs.openclaw.ai/de/install/gcp
...
Dieser eingehängte Zustand `~/.openclaw` enthält `openclaw.json`, agentenspezifische `agents/<agentId>/agent/auth-profiles.json` und `.env`.
...
Diese Anleitung verwendet Debian auf GCP Compute Engine. Ubuntu funktioniert ebenfalls; ordnen Sie die Pakete entsprechend zu. Den generischen Docker-Ablauf finden Sie unter [Docker](</de/install/docker>).
...
Diese `.env`-Datei ist für Container-/Laufzeitumgebungsvariablen wie `OPENCLAW_GATEWAY_TOKEN`. Gespeicherte Provider-OAuth-/API-Schlüssel-Authentifizierung liegt in der eingehängten Datei `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`.
...
Benötigen Sie die Referenz zu gemeinsamer Persistenz und Updates erneut? Siehe [Docker-VM-Laufzeit](</de/install/docker-vm-runtime#what-persists-where>) und [Docker-VM-Laufzeit-Updates](</de/install/docker-vm-runtime#updates>).
