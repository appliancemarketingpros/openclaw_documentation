...
source_url: https://docs.openclaw.ai/ko/gateway/openresponses-http-api
...
인증 매트릭스:
  ...
  * `gateway.auth.mode="token"` 또는 `"password"` \+ `Authorization: Bearer ...`
    ...
    * 전체 기본 운영자 범위 세트를 복원합니다. `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
...
에이전트가 도구 호출을 결정하면 응답은 `function_call` 출력 항목을 반환합니다. 그런 다음 `function_call_output`이 포함된 후속 요청을 보내 턴을 계속합니다.
...
허용 MIME 유형(현재): `image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/heic`, `image/heif`. 최대 크기(현재): 10MB.
...
허용 MIME 유형(현재): `text/plain`, `text/markdown`, `text/html`, `text/csv`, `application/json`, `application/pdf`.
...
현재 동작:
  ...
  * 삽입된 블록은 `<<&lt;EXTERNAL_UNTRUSTED_CONTENT id=&quot;...&quot;&gt;>>` / `<<&lt;END_EXTERNAL_UNTRUSTED_CONTENT id=&quot;...&quot;&gt;>>` 같은 명시적 경계 마커를 사용하고 `Source: External` 메타데이터 줄을 포함합니다.
...
보안 참고:
  ...
  * 인터넷에 노출된 Gateway의 경우 앱 수준 보호 장치에 더해 네트워크 이그레스 제어를 적용하세요. [보안](</ko/gateway/security>)을 참조하세요.
...
기본 제공자가 토큰 수를 보고하면 `usage`가 채워집니다. OpenClaw는 해당 카운터가 다운스트림 상태/세션 표면에 도달하기 전에 일반적인 OpenAI 스타일 별칭을 정규화합니다. 여기에는 `input_tokens` / `output_tokens` 및 `prompt_tokens` / `completion_tokens`가 포함됩니다.
