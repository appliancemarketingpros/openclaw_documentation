...
source_url: https://docs.openclaw.ai/fr/tools/brave-search
...
Les paramètres de recherche Brave propres au fournisseur résident désormais sous `plugins.entries.brave.config.webSearch.*`. L’ancien `tools.web.search.apiKey` se charge toujours via la couche de compatibilité, mais ce n’est plus le chemin de configuration canonique.
...
## Notes
  ...
  * Les valeurs personnalisées de `webSearch.baseUrl` sont incluses dans l’identité de cache Brave, de sorte que les réponses propres au proxy n’entrent pas en collision.
