---
title: Matrix
source_url: https://docs.openclaw.ai/de/channels/matrix
scraped_at: 2026-05-25
---

Matrix ist ein herunterladbares Kanal-Plugin für OpenClaw. Es verwendet das offizielle `matrix-js-sdk` und unterstützt Direktnachrichten, Räume, Threads, Medien, Reaktionen, Umfragen, Standort und E2EE.
...
## Einrichtung
  ...
  4. Starten Sie eine Direktnachricht mit dem Bot oder laden Sie ihn in einen Raum ein (siehe automatischer Beitritt \- neue Einladungen landen nur, wenn `autoJoin` sie zulässt).
...
Verifizierungsbenachrichtigungen
...
Diese Einstellung beeinflusst die Sichtbarkeit von ergänzendem Kontext, nicht, ob die eingehende Nachricht selbst eine Antwort auslösen kann. Die Auslöserautorisierung kommt weiterhin aus `groupPolicy`, `groups`, `groupAllowFrom` und den DM-Richtlinieneinstellungen.
...
Standardmäßig blockiert OpenClaw private/interne Matrix-Homeserver zum SSRF-Schutz, sofern Sie dies nicht explizit pro Konto erlauben.
...
Benannte Konten können die Standardeinstellung auf oberster Ebene mit `channels.matrix.accounts.<id>.proxy` überschreiben. OpenClaw verwendet dieselbe Proxy-Einstellung für Matrix-Datenverkehr zur Laufzeit und Kontostatusprüfungen.
...
Matrix-Raum-IDs beachten Groß- und Kleinschreibung. Verwenden Sie die exakte Schreibweise der Raum-ID aus Matrix, wenn Sie explizite Zustellziele, Cron-Jobs, Bindings oder Allowlists konfigurieren. OpenClaw hält interne Sitzungsschlüssel für die Speicherung kanonisch, daher sind diese kleingeschriebenen Schlüssel keine verlässliche Quelle für Matrix-Zustell-IDs.
