---
title: أمثلة التكوين
source_url: https://docs.openclaw.ai/ar/gateway/configuration-examples
...
أبقِ إقران الأجهزة يدويًا ما لم تكن تتحكم في مسار الشبكة. بالنسبة إلى مختبر مخصص أو شبكة فرعية ضمن tailnet، يمكنك تفعيل الموافقة التلقائية على جهاز Node عند أول استخدام باستخدام CIDR أو عناوين IP دقيقة:
...
يبقى هذا معطلاً عند عدم ضبطه. لا ينطبق إلا على إقران جديد لـ `role: node` دون نطاقات مطلوبة. لا يزال عملاء المشغّل/المتصفح وترقيات الدور أو النطاق أو البيانات الوصفية أو المفتاح العام تتطلب موافقة يدوية.
...
بالنسبة إلى Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC، يكون تفويض المرسل قائمًا على المعرّف أولاً افتراضيًا. لا تفعّل المطابقة المباشرة القابلة للتغيير للاسم/البريد الإلكتروني/اللقب باستخدام `dangerouslyAllowNameMatching: true` الخاص بكل قناة إلا إذا كنت تقبل هذا الخطر صراحةً.
