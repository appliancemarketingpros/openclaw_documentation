---
title: 一般疑難排解
source_url: https://docs.openclaw.ai/zh-TW/help/troubleshooting
...
良好輸出的一行摘要：
  ...
  * `openclaw channels status --probe` → 可連線的 Gateway 會回傳即時的逐帳號 傳輸狀態，以及像是 `works` 或 `audit ok` 這類探測/稽核結果；如果 Gateway 無法連線，該命令會退回到僅設定摘要。
  * `openclaw logs --follow` → 活動穩定，沒有重複的嚴重錯誤。


## Anthropic 長上下文 429

如果你看到： `HTTP 429: rate_limit_error: Extra usage is required for long context requests`， 請前往 [/gateway/troubleshooting#anthropic-429-extra-usage-required-for-long-context](</zh-TW/gateway/troubleshooting#anthropic-429-extra-usage-required-for-long-context>)。

## 本機 OpenAI 相容後端可直接運作，但在 OpenClaw 中失敗

如果你的本機或自架 `/v1` 後端能回應小型直接 `/v1/chat/completions` 探測，但在 `openclaw infer model run` 或一般 代理輪次中失敗：

  1. 如果錯誤提到 `messages[].content` 預期為字串，請設定 `models.providers.<provider>.models[].compat.requiresStringContent: true`。
  2. 如果後端仍然只在 OpenClaw 代理輪次中失敗，請設定 `models.providers.<provider>.models[].compat.supportsTools: false` 並重試。
  3. 如果極小的直接呼叫仍可運作，但較大的 OpenClaw 提示會讓 後端崩潰，請將剩餘問題視為上游模型/伺服器限制，並 繼續閱讀深入執行手冊： [/gateway/troubleshooting#local-openai-compatible-backend-passes-direct-probes-but-agent-runs-fail](</zh-TW/gateway/troubleshooting#local-openai-compatible-backend-passes-direct-probes-but-agent-runs-fail>)


## Plugin 安裝因缺少 openclaw extensions 而失敗

如果安裝因 `package.json missing openclaw.extensions` 失敗，表示該 Plugin 套件 正在使用 OpenClaw 不再接受的舊格式。
...
表示 Plugin 檔案的擁有者與載入它們的程序所屬 Unix 使用者不同。 請不要移除 Plugin 設定。請修正檔案擁有權，或以擁有狀態目錄的 同一位使用者執行 OpenClaw。

Docker 安裝通常會以 `node`（uid `1000`）執行。對於預設 Docker 設定，請修復主機繫結掛載：
...
如果你刻意以 root 執行 OpenClaw，請改為將受管理的 Plugin 根目錄修復為 root 擁有權：
...
常見日誌特徵：
  ...
  * `origin not allowed` → 瀏覽器 `Origin` 不允許用於控制 UI Gateway 目標。
  * `AUTH_TOKEN_MISMATCH` 搭配重試提示（`canRetryWithDeviceToken=true`）→ 可能會自動發生一次受信任裝置權杖重試。
  * 該快取權杖重試會重用與已配對 裝置權杖一起儲存的快取範圍集合。明確的 `deviceToken` / 明確的 `scopes` 呼叫者會保留 其要求的範圍集合。
  * 在非同步 Tailscale Serve 控制 UI 路徑上，同一個 `{scope, ip}` 的失敗嘗試會在限制器記錄失敗前被序列化，因此 第二個並行的錯誤重試可能已經顯示 `retry later`。
  * 來自 localhost 瀏覽器來源的 `too many failed authentication attempts (retry later)` → 來自同一個 `Origin` 的重複失敗會暫時 被鎖定；另一個 localhost 來源會使用不同的桶。
...
Gateway 無法啟動，或服務已安裝但未執行 bashCopy code
[code]
    openclaw statusopenclaw gateway statusopenclaw logs --followopenclaw doctoropenclaw channels status --probe
[/code]
...
Cron 或 Heartbeat 未觸發或未送達 bashCopy code
[code]
...
Node 已配對但相機、畫布、螢幕或 exec 工具失敗 bashCopy code
[code]
