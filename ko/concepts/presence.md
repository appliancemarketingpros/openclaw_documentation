---
title: 프레즌스
source_url: https://docs.openclaw.ai/ko/concepts/presence
...
모든 WS 클라이언트는 `connect` 요청으로 시작합니다. 핸드셰이크에 성공하면 Gateway는 해당 연결에 대한 프레즌스 항목을 upsert합니다.
...
[**입력 표시기** 입력 표시기가 전송되는 시점과 이를 조정하는 방법입니다. ](</ko/concepts/typing-indicators>) [**스트리밍 및 청킹** 아웃바운드 스트리밍, 청킹, 채널별 포맷 지정입니다. ](</ko/concepts/streaming>) [**Gateway 아키텍처** 프레즌스 업데이트를 구동하는 Gateway 구성 요소와 WebSocket 프로토콜입니다. ](</ko/concepts/architecture>) [**Gateway 프로토콜** `connect`, `system-event`, `system-presence`의 와이어 프로토콜입니다. ](</ko/gateway/protocol>)
