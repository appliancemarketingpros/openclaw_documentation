...
source_url: https://docs.openclaw.ai/pl/gateway/sandbox-vs-tool-policy-vs-elevated
...
Praktyczne reguły:
  ...
  * `/exec` zmienia tylko domyślne ustawienia sesji dla autoryzowanych nadawców; nie przyznaje dostępu do narzędzi. Klucze narzędzi dostawcy akceptują `provider` (np. `google-antigravity`) albo `provider/model` (np. `openai/gpt-5.4`).
...
Dostępne grupy:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` W przypadku agentów tylko do odczytu odmawiaj `group:runtime` oraz narzędzi mutujących system plików, chyba że zasady systemu plików piaskownicy albo osobna granica hosta wymuszają ograniczenie tylko do odczytu.
