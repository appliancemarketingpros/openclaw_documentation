...
source_url: https://docs.openclaw.ai/vi/security/THREAT-MODEL-ATLAS
scraped_at: 2026-05-25
---

## Khung MITRE ATLAS

**Phiên bản:** 1.0-draft **Cập nhật lần cuối:** 2026-02-04 **Phương pháp luận:** MITRE ATLAS + Sơ đồ luồng dữ liệu **Khung:** [MITRE ATLAS](<https://atlas.mitre.org/>) (Bối cảnh mối đe dọa đối nghịch cho hệ thống AI)

### Ghi nhận khung

Mô hình mối đe dọa này được xây dựng trên [MITRE ATLAS](<https://atlas.mitre.org/>), khung tiêu chuẩn ngành để ghi tài liệu các mối đe dọa đối nghịch đối với hệ thống AI/ML. ATLAS được [MITRE](<https://www.mitre.org/>) duy trì với sự cộng tác của cộng đồng bảo mật AI.
...
---|---|---  
Thời gian chạy tác nhân OpenClaw | Có | Thực thi tác nhân lõi, lệnh gọi công cụ, phiên  
...
F2 | Gateway | Tác nhân | Tin nhắn được định tuyến | Cô lập phiên  
...
---|---|---  
Tuổi tài khoản GitHub | `requireGitHubAccountAge()` | Trung bình - Nâng rào cản đối với kẻ tấn công mới  
Làm sạch đường dẫn | `sanitizePath()` | Cao - Ngăn chặn duyệt vượt đường dẫn  
...
[SKILL.md](<http://SKILL.md>) bắt buộc | Tệp đọc bắt buộc | Giá trị bảo mật thấp - Chỉ mang tính thông tin  
Kiểm duyệt mẫu | FLAG_RULES trong moderation.ts | Thấp - Dễ dàng bị vượt qua  
...
javascriptCopy code
[code]
    // Known-bad identifiers/(keepcold131\/ClawdAuthenticatorTool|ClawdAuthenticatorTool)/i // Suspicious keywords/(malware|stealer|phish|phishing|keylogger)/i/(api[-_ ]?key|token|password|private key|secret)/i/(wallet|seed phrase|mnemonic|crypto)/i/(discord\.gg|webhook|hooks\.slack)/i/(curl[^\n]+\|\s*(sh|bash))/i/(bit\.ly|tinyurl\.com|t\.co|goo\.gl|is\.gd)/i
[/code]
...
---|---|---  
Tích hợp VirusTotal | Đang thực hiện | Cao - Phân tích hành vi Code Insight  
Báo cáo cộng đồng | Một phần (bảng `skillReports` tồn tại) | Trung bình  
Ghi nhật ký kiểm toán | Một phần (bảng `auditLogs` tồn tại) | Trung bình  
...
ID | Khuyến nghị | Xử lý  
---|---|---  
R-001 | Hoàn thiện tích hợp VirusTotal | T-PERSIST-001, T-EVADE-001  
R-002 | Triển khai cơ chế sandbox cho Skill | T-PERSIST-001, T-EXFIL-003  
R-003 | Thêm kiểm tra đầu ra cho các hành động nhạy cảm | T-EXEC-001, T-EXEC-002  
...
R-006 | Cải thiện UX phê duyệt exec và kiểm tra | T-EXEC-004  
...
ID ATLAS | Tên kỹ thuật | Các mối đe dọa OpenClaw  
---|---|---  
...
`src/routing/resolve-route.ts` | Cô lập phiên | **Trung bình**  
...
_Mô hình mối đe dọa này là một tài liệu sống. Báo cáo vấn đề bảo mật tới[security@openclaw.ai](<mailto:security@openclaw.ai>)_
