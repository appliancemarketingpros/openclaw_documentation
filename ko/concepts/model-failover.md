---
title: 모델 장애 조치
source_url: https://docs.openclaw.ai/ko/concepts/model-failover
...
* ### 모두 소진되면 FallbackSummaryError 발생
...
OpenAI 에이전트 모델의 경우 인증과 런타임은 별개입니다. `openai/gpt-*`는 Codex 하네스에 유지되며, 인증은 Codex 구독 프로필과 OpenAI API 키 백업 사이에서 순환할 수 있습니다.
...
기존 Codex 구독 프로필은 여전히 레거시 `openai-codex:*` 프로필 ID를 사용할 수 있습니다. 순서가 지정된 API 키 백업은 일반 `openai:*` API 키 프로필일 수 있습니다. 구독이 Codex 사용량 제한에 도달하면, OpenClaw는 Codex가 재설정 시간을 제공하는 경우 정확한 재설정 시간을 기록하고, 다음 순서의 인증 프로필을 시도하며, 실행을 Codex 하네스 안에 유지합니다. 재설정 시간이 지나면 구독 프로필은 다시 사용할 수 있게 되며 다음 자동 선택에서 해당 프로필로 돌아갈 수 있습니다.

해당 세션에 대해 하나의 계정/키를 강제하려는 경우에만 사용자가 고정한 프로필을 사용하세요. 사용자가 고정한 프로필은 의도적으로 엄격하며 다른 프로필로 조용히 이동하지 않습니다.

## 쿨다운

프로필이 인증/속도 제한 오류(또는 속도 제한처럼 보이는 시간 초과)로 실패하면 OpenClaw는 해당 프로필을 쿨다운 상태로 표시하고 다음 프로필로 이동합니다.

속도 제한 / 시간 초과 버킷에 들어가는 항목
