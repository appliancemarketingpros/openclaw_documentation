...
See [Plugin shapes](</plugins/architecture#plugin-shapes>) for more on the capability model. The `--json` flag outputs a machine-readable report suitable for scripting and auditing. `info` is an alias for `inspect`.

[webhooks](</cli/webhooks>)[skills](</cli/skills>)
