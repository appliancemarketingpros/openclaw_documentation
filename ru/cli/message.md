---
title: Сообщение
source_url: https://docs.openclaw.ai/ru/cli/message
...
Кнопки веб-приложений Telegram поддерживаются только в приватных чатах между пользователем и ботом. Старые payload JSON с `web_app` по-прежнему разбираются, но `webApp` является каноническим полем представления.
