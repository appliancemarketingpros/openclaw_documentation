---
title: 測試：更新與 Plugin
source_url: https://docs.openclaw.ai/zh-TW/help/testing-updates-plugins
...
從 artifact identity 開始：
  ...
  * 套件驗收 `resolve_package` 摘要：來源、版本、SHA-256 和 成品名稱。
  * Docker 成品：`.artifacts/docker-tests/**/summary.json`、 `failures.json`、lane 記錄，以及重新執行命令。
  * 升級存續摘要：`.artifacts/upgrade-survivor/summary.json`， 包含基準版本、候選版本、情境、階段時間，以及 配方步驟。


優先使用相同的套件成品重新執行失敗的精確 lane，而不是 重新執行整個 release umbrella。
