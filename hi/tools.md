---
title: अवलोकन
source_url: https://docs.openclaw.ai/hi/tools
...
यह एक अवलोकन और रूटिंग पृष्ठ है। विस्तृत टूल नीति, डिफॉल्ट, ग्रुप सदस्यता, प्रोवाइडर प्रतिबंध, और कॉन्फिगरेशन फील्ड के लिए, [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>) का उपयोग करें।
...
नियंत्रित करें कि एजेंट क्या कॉल कर सकता है | टूल नीति | [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>)  
एजेंट को वर्कफ़्लो सिखाएं | Skills | [Skills](</hi/tools/skills>), [Skills बनाना](</hi/tools/creating-skills>), और [Skill Workshop](</hi/tools/skill-workshop>)  
नया इंटीग्रेशन या रनटाइम सतह जोड़ें | Plugin | [Plugin](</hi/tools/plugin>) और [Plugin बनाएं](</hi/plugins/building-plugins>)  
...
टूल एक टाइप्ड फ़ंक्शन है जिसे एजेंट कॉल कर सकता है, जैसे `exec`, `browser`, `web_search`, `message`, या `image_generate`। जब एजेंट को डेटा पढ़ना, फाइलें बदलना, संदेश भेजना, प्रोवाइडर कॉल करना, या किसी दूसरी प्रणाली को संचालित करना हो, तब टूल का उपयोग करें। दिखने वाले टूल मॉडल को संरचित फ़ंक्शन परिभाषाओं के रूप में भेजे जाते हैं।
...
Skill एक `SKILL.md` निर्देश पैक है जो एजेंट प्रॉम्प्ट में लोड होता है। Skill का उपयोग तब करें जब एजेंट के पास आवश्यक टूल पहले से हों, लेकिन उसे दोहराए जा सकने वाला वर्कफ़्लो, समीक्षा रूब्रिक, कमांड क्रम, या संचालन संबंधी बाधा चाहिए हो।
...
तालिका प्रतिनिधि टूल सूचीबद्ध करती है ताकि आप सतह को पहचान सकें। यह पूर्ण नीति संदर्भ नहीं है। सटीक ग्रुप, डिफॉल्ट, और allow/deny सेमांटिक्स के लिए, [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>) का उपयोग करें।
...
Plugin अतिरिक्त टूल रजिस्टर कर सकते हैं। Plugin लेखक टूल को `api.registerTool(...)` और मैनिफेस्ट के `contracts.tools` के माध्यम से वायर करते हैं; कॉन्ट्रैक्ट विवरण के लिए [Plugin SDK](</hi/plugins/sdk-overview>) और [Plugin मैनिफेस्ट](</hi/plugins/manifest>) का उपयोग करें।
...
सामान्य Plugin-प्रदत्त टूल में शामिल हैं:
  ...
  * शोर वाले `exec` और `bash` टूल आउटपुट को संक्षिप्त करने के लिए [Tokenjuice](</hi/tools/tokenjuice>)
  * हर स्कीमा को प्रॉम्प्ट में डाले बिना बड़े टूल कैटलॉग खोजने और कॉल करने के लिए [टूल खोज](</hi/tools/tool-search>)
  * नोड Canvas नियंत्रण और A2UI रेंडरिंग के लिए [Canvas](</hi/plugins/reference/canvas>)
...
टूल नीति मॉडल कॉल से पहले लागू की जाती है। यदि नीति कोई टूल हटाती है, तो मॉडल को उस टर्न के लिए उस टूल का स्कीमा नहीं मिलता। कोई रन टूल खो सकता है क्योंकि ग्लोबल कॉन्फिग, प्रति-एजेंट कॉन्फिग, चैनल नीति, प्रोवाइडर प्रतिबंध, सैंडबॉक्स नियम, चैनल/रनटाइम नीति, या Plugin उपलब्धता लागू हो सकती है।
  ...
  * [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>) टूल प्रोफ़ाइल, allow/deny सूचियां, प्रोवाइडर-विशिष्ट प्रतिबंध, लूप डिटेक्शन, और प्रोवाइडर-समर्थित टूल सेटिंग्स दस्तावेजित करता है।
  * [Exec स्वीकृतियां](</hi/tools/exec-approvals>) होस्ट कमांड स्वीकृति नीति दस्तावेजित करता है।
  * [Elevated exec](</hi/tools/elevated>) सैंडबॉक्स के बाहर नियंत्रित निष्पादन दस्तावेजित करता है।
  ...
  * [प्रति-एजेंट सैंडबॉक्स और टूल प्रतिबंध](</hi/tools/multi-agent-sandbox-tools>) डेलिगेटेड रन के लिए एजेंट-विशिष्ट प्रतिबंध दस्तावेजित करता है।
...
जिस काम के लिए आपको OpenClaw चाहिए, उसके आधार पर एक्सटेंशन पथ चुनें:
  ...
  * नया इंटीग्रेशन, प्रोवाइडर, चैनल, टूल, या हुक [Plugin बनाएं](</hi/plugins/building-plugins>) के साथ बनाएं।
  * [Skills](</hi/tools/skills>) और [Skills बनाना](</hi/tools/creating-skills>) के साथ पुन: उपयोग योग्य एजेंट निर्देश जोड़ें या ट्यून करें।
...
यदि मॉडल कोई टूल देख या कॉल नहीं कर सकता, तो मौजूदा टर्न की प्रभावी नीति से शुरू करें:
  ...
  1. सक्रिय प्रोफ़ाइल, `tools.allow`, और `tools.deny` को [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>) में जांचें।
  2. [टूल और कस्टम प्रोवाइडर](</hi/gateway/config-tools>) में प्रोवाइडर-विशिष्ट प्रतिबंध जांचें और पुष्टि करें कि चुना गया [मॉडल प्रोवाइडर](</hi/concepts/model-providers>) टूल आकार का समर्थन करता है।
  3. चैनल अनुमतियां, सैंडबॉक्स स्थिति, और elevated एक्सेस को [सैंडबॉक्स बनाम टूल नीति बनाम elevated](</hi/gateway/sandbox-vs-tool-policy-vs-elevated>) और [Elevated exec](</hi/tools/elevated>) के साथ जांचें।
  4. जांचें कि मालिक Plugin इंस्टॉल और सक्षम है या नहीं [Plugin](</hi/tools/plugin>) में।
  5. डेलिगेटेड रन के लिए, [प्रति-एजेंट सैंडबॉक्स और टूल प्रतिबंध](</hi/tools/multi-agent-sandbox-tools>) में प्रति-एजेंट प्रतिबंध जांचें।
  6. बड़े OpenClaw कैटलॉग के लिए, पुष्टि करें कि रन सीधे टूल एक्सपोजर का उपयोग करता है या [टूल खोज](</hi/tools/tool-search>) का।
