...
source_url: https://docs.openclaw.ai/id/gateway/sandbox-vs-tool-policy-vs-elevated
...
Aturan praktis:
  ...
  * `/exec` hanya mengubah default sesi untuk pengirim yang berwenang; ini tidak memberikan akses tool. Kunci tool provider menerima `provider` (mis. `google-antigravity`) atau `provider/model` (mis. `openai/gpt-5.4`).
...
Grup yang tersedia:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` Untuk agent read-only, tolak `group:runtime` serta tool filesystem yang memutasi kecuali kebijakan filesystem sandbox atau batas host terpisah memberlakukan batasan read-only.
