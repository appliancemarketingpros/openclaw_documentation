...
source_url: https://docs.openclaw.ai/ru/concepts/agent
...
Если `agents.defaults.sandbox` включен, неосновные сеансы могут переопределить это рабочими пространствами для отдельных сеансов в `agents.defaults.sandbox.workspaceRoot` (см. [Конфигурация Gateway](</ru/gateway/configuration>)).
...
Основные инструменты (read/exec/edit/write и связанные системные инструменты) всегда доступны, с учетом политики инструментов. `apply_patch` является необязательным и управляется `tools.exec.applyPatch`. `TOOLS.md` **не** управляет тем, какие инструменты существуют; это руководство о том, как _вы_ хотите, чтобы они использовались.
...
Корни Skills могут содержать сгруппированные папки, такие как `<workspace>/skills/personal/foo/SKILL.md`; Skill по-прежнему предоставляется по своему плоскому имени из frontmatter, например `foo`.
