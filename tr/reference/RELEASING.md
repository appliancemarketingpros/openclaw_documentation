...
source_url: https://docs.openclaw.ai/tr/reference/RELEASING
...
Yardımcı `release-ci/<sha>-...` dalını gönderir, bu daldan `ref=<sha>` ile `Full Release Validation` başlatır, her alt workflow `headSha` değerinin hedefle eşleştiğini doğrular, ardından geçici dalı siler. Bu, yanlışlıkla daha yeni bir `main` alt çalıştırmasını kanıtlamayı önler.

Sürüm dalı veya etiket doğrulaması için, bunu güvenilen `main` workflow ref'inden çalıştırın ve sürüm dalını ya da etiketini `ref` olarak geçirin:
...
Bu kutuyu "kaynak ağacı tam normal test suite'ini geçti mi?" sorusunu yanıtlamak için kullanın. Bu, sürüm yolu ürün doğrulamasıyla aynı değildir. Saklanacak kanıtlar:
...
Docker kutusu, `openclaw-live-and-e2e-checks-reusable.yml` üzerinden `OpenClaw Release Checks` içinde ve sürüm modu `install-smoke` workflow'unda yer alır. Sürüm adayını yalnızca kaynak düzeyi testler yerine paketlenmiş Docker ortamları üzerinden doğrular.
...
QA Lab kutusu da `OpenClaw Release Checks` parçasıdır. Vitest ve Docker paket mekaniklerinden ayrı olarak ajan davranışı ve kanal düzeyi sürüm kapısıdır.
...
Maintainer'lar gerçek runbook için özel sürüm dokümanlarını [`openclaw/maintainers/release/README.md`](<https://github.com/openclaw/maintainers/blob/main/release/README.md>) konumunda kullanır.
