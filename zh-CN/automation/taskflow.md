---
title: 任务流程
source_url: https://docs.openclaw.ai/zh-CN/automation/taskflow
...
每个流程都会持久化自身状态并跟踪修订，因此进度可以在 Gateway 网关重启后保留。修订跟踪支持在多个来源尝试并发推进同一流程时检测冲突。 流程注册表使用 SQLite，并带有有界的预写日志维护，包括 周期性检查点和关机检查点，因此长时间运行的 Gateway 网关不会保留 无界的 `registry.sqlite-wal` 辅助文件。
