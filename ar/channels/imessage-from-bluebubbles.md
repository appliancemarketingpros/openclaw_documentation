...
source_url: https://docs.openclaw.ai/ar/channels/imessage-from-bluebubbles
...
لا يفحص `channels status --probe` إلا الحسابات المكوّنة والمفعّلة. لا تعِد تشغيل Gateway مع تفعيل كل من BlueBubbles وiMessage إلا إذا كنت تريد عمدًا تشغيل مراقبي القناتين معًا. إذا لم تكن ستنتقل فورًا، فأعد ضبط `channels.imessage.enabled` إلى `false` قبل إعادة تشغيل Gateway. استخدم أوامر `imsg` المباشرة في قبل أن تبدأ للتحقق من جهاز Mac قبل تفعيل حركة OpenClaw.
