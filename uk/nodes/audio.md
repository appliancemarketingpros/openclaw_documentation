...
source_url: https://docs.openclaw.ai/uk/nodes/audio
...
Якщо ви **не налаштовуєте моделі** , а `tools.media.audio.enabled` **не** встановлено в `false`, OpenClaw виконує автоматичне виявлення в такому порядку й зупиняється на першому робочому варіанті:
...
Щоб вимкнути автоматичне виявлення, встановіть `tools.media.audio.enabled: false`. Щоб налаштувати вручну, встановіть `tools.media.audio.models`. Примітка: Виявлення бінарних файлів виконується за принципом найкращої спроби в macOS/Linux/Windows; переконайтеся, що CLI є в `PATH` (ми розгортаємо `~`), або задайте явну CLI-модель із повним шляхом до команди.
