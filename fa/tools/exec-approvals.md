---
title: تأییدهای اجرا
source_url: https://docs.openclaw.ai/fa/tools/exec-approvals
...
برای binهای امن (مسیر سریع فقط stdin)، جزئیات اتصال مفسر، و نحوه هدایت promptهای تأیید به Slack/Discord/Telegram (یا اجرای آن‌ها به‌عنوان کلاینت‌های تأیید بومی)، ببینید [تأییدهای exec - پیشرفته](</fa/tools/exec-approvals-advanced>).
...
CLI: `openclaw approvals` از ویرایش gateway یا node پشتیبانی می‌کند - ببینید [CLI تأییدها](</fa/cli/approvals>).
...
[**تأییدهای exec - پیشرفته** binهای امن، اتصال مفسر، و هدایت تأیید به chat. ](</fa/tools/exec-approvals-advanced>) [**ابزار exec** ابزار اجرای فرمان shell. ](</fa/tools/exec>) [**حالت elevated** مسیر break-glass که تأییدها را نیز رد می‌کند. ](</fa/tools/elevated>) [**Sandboxing** حالت‌های sandbox و دسترسی workspace. ](</fa/gateway/sandboxing>) [**امنیت** مدل امنیتی و سخت‌سازی. ](</fa/gateway/security>) [**sandbox در برابر tool policy در برابر elevated** چه زمانی سراغ هر کنترل بروید. ](</fa/gateway/sandbox-vs-tool-policy-vs-elevated>) [**Skills** رفتار مجازسازی خودکار با پشتوانه Skills. ](</fa/tools/skills>)
