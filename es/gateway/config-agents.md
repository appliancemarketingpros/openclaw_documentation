...
source_url: https://docs.openclaw.ai/es/gateway/config-agents
scraped_at: 2026-05-25
---

Claves de configuración con alcance de agente bajo `agents.*`, `multiAgent.*`, `session.*`, `messages.*` y `talk.*`. Para canales, herramientas, runtime del Gateway y otras claves de nivel superior, consulta [Referencia de configuración](</es/gateway/configuration-reference>).
...
[/code]
  ...
  * `postCompactionMaxChars`: límite del extracto de [AGENTS.md](<http://AGENTS.md>) usado durante la inyección de actualización posterior a la Compaction.
...
Tamaño máximo en píxeles del lado más largo de la imagen en bloques de imagen de transcript/herramienta antes de las llamadas al proveedor. Valor predeterminado: `1200`.

Los valores más bajos suelen reducir el uso de tokens de visión y el tamaño de la carga útil de la solicitud en ejecuciones con muchas capturas de pantalla. Los valores más altos conservan más detalle visual.
...
[/code]
  ...
  * `params.preserveThinking`: activación opcional solo para [Z.AI](<http://Z.AI>) de pensamiento preservado. Cuando se habilita y el pensamiento está activado, OpenClaw envía `thinking.clear_thinking: false` y reproduce `reasoning_content` anterior; consulta [pensamiento y pensamiento preservado de Z.AI](</es/providers/zai#thinking-and-preserved-thinking>).
...
[/code]
  ...
  * `provider`: id de un Plugin de proveedor de compaction registrado. Cuando está definido, se llama a `summarize()` del proveedor en lugar del resumen LLM integrado. Recurre al integrado si falla. Configurar un proveedor fuerza `mode: "safeguard"`. Consulta [Compaction](</es/concepts/compaction>).
...
Consulta [Poda de sesiones](</es/concepts/session-pruning>) para ver los detalles de comportamiento.
...
Consulta [Transmisión](</es/concepts/streaming>) para ver detalles de comportamiento y fragmentación.
...
Aislamiento opcional para el agente incrustado. Consulta [Aislamiento](</es/gateway/sandboxing>) para ver la guía completa.
...
Cuando se selecciona `backend: "openshell"`, la configuración específica del entorno de ejecución pasa a `plugins.entries.openshell.config`.
...
Acceso completo (sin sandbox) json5Copy code
[code]
    {agents: {  list: [    {      id: "personal",      workspace: "~/.openclaw/workspace-personal",      sandbox: { mode: "off" },    },  ],},}
[/code]

Herramientas de solo lectura + espacio de trabajo json5Copy code
[code]
...
Sin acceso al sistema de archivos (solo mensajería) json5Copy code
[code]
