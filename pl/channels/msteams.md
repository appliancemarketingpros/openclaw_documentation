...
source_url: https://docs.openclaw.ai/pl/channels/msteams
scraped_at: 2026-05-25
---

Stan: obsługiwane są tekst + załączniki DM; wysyłanie plików w kanale/grupie wymaga `sharePointSiteId` \+ uprawnień Graph (zobacz Wysyłanie plików w czatach grupowych). Ankiety są wysyłane przez Adaptive Cards. Akcje wiadomości udostępniają jawne `upload-file` do wysyłek, w których plik jest pierwszy.
...
**Allowlist Teams + kanałów**
  ...
  * Podczas uruchamiania OpenClaw rozwiązuje nazwy zespołu/kanału i użytkowników z allowlist na identyfikatory (gdy pozwalają na to uprawnienia Graph) i zapisuje mapowanie w logach; nierozwiązane nazwy zespołu/kanału są zachowywane tak, jak wpisano, ale domyślnie ignorowane na potrzeby routingu, chyba że włączono `channels.msteams.dangerouslyAllowNameMatching: true`.
...
### Krok 3: Skonfiguruj endpoint wiadomości
  ...
  2. Ustaw **Messaging endpoint** na URL Webhook: 
     ...
     * Lokalne programowanie: użyj tunelu (zobacz Lokalne programowanie poniżej)
...
### Krok 5: Zbuduj manifest aplikacji Teams
  ...
  * Dodaj uprawnienia RSC (zobacz Uprawnienia RSC).
...
bashCopy code
[code]
    # Jednorazowa konfiguracja:devtunnel create my-openclaw-bot --allow-anonymousdevtunnel port create my-openclaw-bot -p 3978 --protocol auto # Każda sesja deweloperska:devtunnel host my-openclaw-bot
[/code]
...
bashCopy code
[code]
    # Pobierz, edytuj i ponownie prześlij manifestteams app manifest download <teamsAppId> manifest.json# Edytuj manifest.json lokalnie...teams app manifest upload manifest.json <teamsAppId># Wersja jest automatycznie podbijana, jeśli treść się zmieniła
[/code]
...
Kluczowe ustawienia (zobacz `/gateway/configuration`, aby poznać wspólne wzorce kanałów):
  ...
  * `channels.msteams.replyStyle`: `thread | top-level` (zobacz Styl odpowiedzi).
  ...
  * Klucze `toolsBySender` powinny używać jawnych prefiksów: `channel:`, `id:`, `e164:`, `username:`, `name:` (starsze klucze bez prefiksu nadal mapują tylko na `id:`).
  ...
  * `channels.msteams.sharePointSiteId`: identyfikator witryny SharePoint do przesyłania plików w czatach grupowych/kanałach (zobacz Wysyłanie plików w czatach grupowych).
...
Bez uprawnień Graph wiadomości kanału z obrazami będą odbierane tylko jako tekst (zawartość obrazu nie jest dostępna dla bota). Domyślnie OpenClaw pobiera multimedia tylko z nazw hostów Microsoft/Teams. Nadpisz to za pomocą `channels.msteams.mediaAllowHosts` (użyj `["*"]`, aby zezwolić na dowolny host). Nagłówki Authorization są dołączane tylko dla hostów z `channels.msteams.mediaAuthAllowHosts` (domyślnie hosty Graph + Bot Framework). Utrzymuj tę listę ścisłą (unikaj sufiksów wielodzierżawnych).
...
### Błędy przesyłania manifestu
  ...
  * **"[webApplicationInfo.Id](<http://webApplicationInfo.Id>) already in use":** aplikacja jest nadal zainstalowana w innym zespole/czacie. Najpierw ją znajdź i odinstaluj albo odczekaj 5-10 minut na propagację.
  * **"Something went wrong" podczas przesyłania:** zamiast tego prześlij przez <https://admin.teams.microsoft.com>, otwórz narzędzia deweloperskie przeglądarki (F12) → karta Network i sprawdź treść odpowiedzi pod kątem rzeczywistego błędu.
