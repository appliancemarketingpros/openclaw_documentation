...
source_url: https://docs.openclaw.ai/id/channels/imessage-from-bluebubbles
...
`channels status --probe` hanya mem-probe akun yang dikonfigurasi dan diaktifkan. Jangan mulai ulang Gateway dengan BlueBubbles dan iMessage sama-sama aktif kecuali Anda memang ingin kedua pemantau channel berjalan. Jika Anda tidak langsung melakukan cutover, setel kembali `channels.imessage.enabled` ke `false` sebelum memulai ulang Gateway. Gunakan perintah langsung `imsg` di Sebelum memulai untuk memvalidasi Mac sebelum mengaktifkan trafik OpenClaw.
