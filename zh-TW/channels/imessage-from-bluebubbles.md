...
source_url: https://docs.openclaw.ai/zh-TW/channels/imessage-from-bluebubbles
...
`channels status --probe` 只會探測已設定且已啟用的帳號。除非你有意同時執行兩個頻道監視器，否則不要在 BlueBubbles 與 iMessage 都啟用的狀態下重新啟動 Gateway。如果你不是要立即切換，請在重新啟動 Gateway 前將 `channels.imessage.enabled` 設回 `false`。在啟用 OpenClaw 流量前，請使用 開始之前 中的直接 `imsg` 指令驗證 Mac。
