---
title: Matrix 마이그레이션
source_url: https://docs.openclaw.ai/ko/channels/matrix-migration
...
Gateway가 시작될 때와 [`openclaw doctor --fix`](</ko/gateway/doctor>)를 실행할 때, OpenClaw는 오래된 Matrix 상태를 자동으로 복구하려고 시도합니다. 실행 가능한 Matrix 마이그레이션 단계가 디스크상의 상태를 변경하기 전에, OpenClaw는 집중 복구 스냅샷을 생성하거나 재사용합니다.
...
## 권장 업그레이드 흐름
  ...
  1. OpenClaw와 Matrix Plugin을 일반적인 방식으로 업데이트합니다. Matrix 마이그레이션을 시작 시 즉시 완료할 수 있도록 `--no-restart` 없이 일반 `openclaw update`를 사용하는 것이 좋습니다.
...
복구 키가 승인되고 백업을 사용할 수 있지만 `Cross-signing verified` 가 여전히 `no`이면, 다른 Matrix 클라이언트에서 자체 인증을 완료합니다.

bashCopy code
[code]openclaw matrix verify self
[/code]

다른 Matrix 클라이언트에서 요청을 수락하고, 이모지 또는 숫자를 비교한 뒤, 일치할 때만 `yes`를 입력합니다. 이 명령은 `Cross-signing verified`가 `yes`가 된 후에만 성공적으로 종료됩니다.
