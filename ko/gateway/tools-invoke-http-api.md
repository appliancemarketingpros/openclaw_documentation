---
title: 도구 호출 API
source_url: https://docs.openclaw.ai/ko/gateway/tools-invoke-http-api
...
일반적인 HTTP 인증 경로:
  ...
  * 신뢰된 ID 보유 HTTP 인증(`gateway.auth.mode="trusted-proxy"`): 구성된 ID 인식 프록시를 통해 라우팅하고 필요한 ID 헤더를 주입하게 합니다
  * private-ingress 공개 인증(`gateway.auth.mode="none"`): 인증 헤더가 필요하지 않습니다
...
참고:
  ...
  * `gateway.auth.mode="trusted-proxy"`일 때는 HTTP 요청이 구성된 신뢰된 프록시 소스에서 와야 합니다. 같은 호스트의 loopback 프록시는 명시적으로 `gateway.auth.trustedProxy.allowLoopback = true`가 필요합니다.
