...
source_url: https://docs.openclaw.ai/it/help/testing-updates-plugins
...
Per la mappa più ampia del test runner, consulta [Test](</it/help/testing>). Per le chiavi dei provider live e le suite che toccano la rete, consulta [Test live](</it/help/testing-live>).
...
Full Release Validation usa `source=artifact` per impostazione predefinita, costruito dallo SHA di release risolto. Per la prova post-pubblicazione, passa `package_acceptance_package_spec=openclaw@YYYY.M.D` così la stessa matrice di upgrade punta invece al pacchetto npm distribuito.
