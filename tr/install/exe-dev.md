---
title: exe.dev
source_url: https://docs.openclaw.ai/tr/install/exe-dev
...
Shelley, [exe.dev](<https://exe.dev>)'in ajanı, istemimizle OpenClaw'ı anında kurabilir. Kullanılan istem aşağıdaki gibidir:
...
İstemci tarafından sağlanan zincirleri korumak yerine yönlendirme başlıklarının üzerine yazın. OpenClaw, yönlendirilen IP meta verilerine yalnızca açıkça yapılandırılmış proxy'lerden geldiğinde güvenir ve ekleme tarzı `X-Forwarded-For` zincirleri sertleştirme riski olarak değerlendirilir.

## 5) OpenClaw'a erişin ve ayrıcalıkları verin

`https://<vm-name>.exe.xyz/` adresine erişin (onboarding çıktısındaki Kontrol UI çıktısına bakın). Kimlik doğrulaması isterse, VM'den yapılandırılmış paylaşılan gizli değeri yapıştırın. Bu kılavuz token kimlik doğrulamasını kullanır, bu yüzden `gateway.auth.token` değerini `openclaw config get gateway.auth.token` ile alın (veya `openclaw doctor --generate-gateway-token` ile bir tane oluşturun). Gateway'i parola kimlik doğrulamasına değiştirdiyseniz bunun yerine `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD` kullanın. Cihazları `openclaw devices list` ve `openclaw devices approve <requestId>` ile onaylayın. Emin değilseniz, tarayıcınızdan Shelley'yi kullanın!
