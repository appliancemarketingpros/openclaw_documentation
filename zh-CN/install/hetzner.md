---
title: Hetzner
source_url: https://docs.openclaw.ai/zh-CN/install/hetzner
...
如果你使用的是其他 Linux VPS，请相应映射软件包。 通用 Docker 流程请参阅 [Docker](</zh-CN/install/docker>)。
...
此 `.env` 文件用于容器/运行时环境变量，例如 `OPENCLAW_GATEWAY_TOKEN`。 存储的提供商 OAuth/API-key 身份验证位于挂载的 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`。
