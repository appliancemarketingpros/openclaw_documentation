...
See [Full release validation](</fa/reference/full-release-validation>) for the stage matrix, exact workflow job names, profile differences, artifacts, and focused rerun handles.

`OpenClaw Release Publish` is the manual mutating release workflow. Dispatch it from `release/YYYY.M.D` or `main` after the release tag exists and after the OpenClaw npm preflight has succeeded. It verifies `pnpm plugins:sync:check`, dispatches `Plugin NPM Release` for all publishable plugin packages, dispatches `Plugin ClawHub Release` for the same release SHA, and only then dispatches `OpenClaw NPM Release` with the saved `preflight_run_id`.
...
برای اثبات commit پین‌شده روی یک شاخه با تغییرات سریع، به‌جای `gh workflow run ... --ref main -f ref=<sha>` از helper استفاده کنید:
...
رفرنس‌های dispatch در workflowهای GitHub باید شاخه یا tag باشند، نه SHA خام commit. این helper یک شاخه موقت `release-ci/<sha>-...` در SHA هدف push می‌کند، `Full Release Validation` را از همان ref پین‌شده dispatch می‌کند، بررسی می‌کند که `headSha` هر workflow فرزند با هدف یکی باشد، و پس از تکمیل اجرا شاخه موقت را حذف می‌کند. راستی‌آزمای umbrella همچنین اگر هر workflow فرزند روی SHA متفاوتی اجرا شده باشد fail می‌شود.

`release_profile` گستره live/provider پاس‌داده‌شده به release checkها را کنترل می‌کند. workflowهای release دستی به‌صورت پیش‌فرض `stable` هستند؛ فقط وقتی از `full` استفاده کنید که عمدا ماتریس گسترده advisory provider/media را می‌خواهید. `run_release_soak` کنترل می‌کند آیا release checkهای stable/default، soak کامل live/E2E و Docker مسیر release را اجرا کنند یا نه؛ `full` soak را اجباری فعال می‌کند.
...
اجراهای تکراری `Full Release Validation` برای `ref=main` و `rerun_group=all` umbrella قدیمی‌تر را supersede می‌کنند. monitor والد هر workflow فرزندی را که قبلا dispatch کرده باشد، هنگام cancel شدن والد cancel می‌کند؛ بنابراین validation جدید main پشت یک اجرای release-check دو ساعته قدیمی نمی‌ماند. validation شاخه/tagهای release و گروه‌های rerun متمرکز `cancel-in-progress: false` را نگه می‌دارند.
...
برای policy اختصاصی مربوط به تست update و Plugin، شامل دستورهای محلی، laneهای Docker، ورودی‌های پذیرش بسته، پیش‌فرض‌های release، و triage شکست، [تست updateها و Pluginها](</fa/help/testing-updates-plugins>) را ببینید.
