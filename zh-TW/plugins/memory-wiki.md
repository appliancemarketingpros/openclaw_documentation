---
title: 記憶維基
source_url: https://docs.openclaw.ai/zh-TW/plugins/memory-wiki
...
當橋接模式啟用且 `bridge.readMemoryArtifacts` 已啟用時，`openclaw wiki status`、`openclaw wiki doctor` 與 `openclaw wiki bridge import` 會透過執行中的 Gateway 讀取。這會讓 CLI 橋接檢查與 runtime 記憶 Plugin 上下文保持一致。如果橋接已停用或成品讀取已關閉，這些命令會保留其本機/離線行為。
