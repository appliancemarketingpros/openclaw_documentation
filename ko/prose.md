---
title: OpenProse
source_url: https://docs.openclaw.ai/ko/prose
...
`/prose run <handle/slug>`는 `https://p.prose.md/<handle>/<slug>`로 확인됩니다. 직접 URL은 있는 그대로 fetch됩니다. 이는 `web_fetch` 도구(또는 POST용 `exec`)를 사용합니다.
