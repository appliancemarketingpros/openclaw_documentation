...
source_url: https://docs.openclaw.ai/vi/plugins/codex-native-plugins
...
**`app_inaccessible`, `app_disabled`, hoặc `app_missing`:** quá trình di chuyển không cài đặt plugin vì kho app Codex nguồn không hiển thị tất cả app được sở hữu là hiện diện, được bật và có thể truy cập trong khi `--verify-plugin-apps` được đặt. Ủy quyền lại hoặc bật app trong Codex, rồi chạy lại di chuyển với `--verify-plugin-apps`.
