---
title: Yuanbao
source_url: https://docs.openclaw.ai/ko/channels/yuanbao
...
> **OpenClaw 2026.4.10 이상이 필요합니다.** `openclaw --version`을 실행해 확인하세요. `openclaw update`로 업그레이드하세요.
...
`dmPolicy`를 구성하여 봇에게 DM을 보낼 수 있는 사용자를 제어하세요.
...
### 봇이 빈 응답이나 fallback 응답을 보냄
  ...
  3. `channels.yuanbao.fallbackReply`로 사용자 지정하세요.
...
`bindings`를 사용하여 Yuanbao DM 또는 그룹을 다른 에이전트로 라우팅합니다.
