---
title: Raft
source_url: https://docs.openclaw.ai/zh-TW/channels/raft
...
當代理未叫用 Raft 命令列介面時，這是預期行為。喚醒橋接器不會攜帶訊息本文或自動最終回覆。請檢查代理的工具政策，並確保它可以執行 `raft --profile <profile> message check` 和 `message send`。
