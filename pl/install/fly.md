---
title: Fly.io
source_url: https://docs.openclaw.ai/pl/install/fly
...
## Czego potrzebujesz
  ...
  * Konto [Fly.io](<http://Fly.io>) (wystarczy darmowa warstwa)
...
**Uwaga dotycząca bezpieczeństwa:** Domyślna konfiguracja wystawia publiczny URL. Aby uzyskać wzmocnione wdrożenie bez publicznego IP, zobacz Wdrożenie prywatne albo użyj `deploy/fly.private.toml`.
...
**Uwagi:**
  ...
  * Powiązania inne niż local loopback (`--bind lan`) wymagają prawidłowej ścieżki uwierzytelniania Gateway. Ten przykład [Fly.io](<http://Fly.io>) używa `OPENCLAW_GATEWAY_TOKEN`, ale `gateway.auth.password` albo poprawnie skonfigurowane wdrożenie `trusted-proxy` inne niż local loopback również spełniają to wymaganie.
...
Jeśli po restarcie tracisz profile uwierzytelniania, stan kanału/dostawcy albo sesje, katalog stanu jest zapisywany w systemie plików kontenera.
...
## Uwagi
  ...
  * [Fly.io](<http://Fly.io>) używa **architektury x86** (nie ARM)
