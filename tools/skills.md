...
Plugins can ship their own skills by listing `skills` directories in `openclaw.plugin.json` (paths relative to the plugin root). Plugin skills load when the plugin is enabled and participate in the normal skill precedence rules. You can gate them via `metadata.openclaw.requires.config` on the plugin’s config entry. See [Plugins](</tools/plugin>) for discovery/config and [Tools](</tools>) for the tool surface those skills teach.
...
ClawHub is the public skills registry for OpenClaw. Browse at <https://clawhub.com>. Use it to discover, install, update, and back up skills. Full guide: [ClawHub](</tools/clawhub>). Common flows:
...
[Internals](</plugins/architecture>)[Creating Skills](</tools/creating-skills>)
