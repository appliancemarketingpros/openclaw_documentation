---
title: iMessage
source_url: https://docs.openclaw.ai/fa/channels/imessage
...
**کنش‌های API خصوصی** پاسخ‌ها، tapbackها، افکت‌ها، پیوست‌ها و مدیریت گروه. [**جفت‌سازی** پیام‌های مستقیم iMessage به‌صورت پیش‌فرض در حالت جفت‌سازی هستند. ](</fa/channels/pairing>) **Mac راه‌دور** وقتی Gateway روی Mac مربوط به Messages اجرا نمی‌شود، از یک پوشش SSH استفاده کنید. [**مرجع پیکربندی** مرجع کامل فیلدهای iMessage. ](</fa/gateway/config-channels#imessage>)
...
## نیازمندی‌ها و مجوزها (macOS)
  ...
  * برای کنش‌های پیشرفته (واکنش / ویرایش / لغو ارسال / پاسخ رشته‌ای / افکت‌ها / عملیات گروهی)، System Integrity Protection باید غیرفعال باشد — بخش فعال‌سازی API خصوصی imsg را در پایین ببینید. ارسال/دریافت متن و رسانه پایه بدون آن کار می‌کند.
...
اگر غیرفعال بودن SIP برای مدل تهدید شما قابل قبول نیست:
  ...
  * می‌توانید برای بار کاری iMessage یک Mac جداگانه غیر Apple-Silicon (یا یک Mac اختصاصی bot) با SIP خاموش اجرا کنید، در حالی که SIP را روی دستگاه‌های اصلی خود فعال نگه می‌دارید. بخش کاربر macOS اختصاصی bot (هویت iMessage جداگانه) را در پایین ببینید.
...
fallback زمان اجرا: اگر `groupAllowFrom` تنظیم نشده باشد، بررسی‌های فرستنده گروه iMessage از `allowFrom` استفاده می‌کنند؛ وقتی پذیرش پیام مستقیم و گروه باید متفاوت باشد، `groupAllowFrom` را تنظیم کنید. یادداشت زمان اجرا: اگر `channels.imessage` کاملاً وجود نداشته باشد، runtime به `groupPolicy="allowlist"` برمی‌گردد و یک هشدار ثبت می‌کند (حتی اگر `channels.defaults.groupPolicy` تنظیم شده باشد).
...
برخی رشته‌گفتگوهای iMessage با چند شرکت‌کننده ممکن است با `is_group=false` وارد شوند. اگر آن `chat_id` به‌طور صریح زیر `channels.imessage.groups` پیکربندی شده باشد، OpenClaw آن را به‌عنوان ترافیک گروهی در نظر می‌گیرد (کنترل گروه + جداسازی جلسهٔ گروهی).
...
از کلیدهای SSH استفاده کنید تا هم SSH و هم SCP غیرتعاملی باشند. ابتدا مطمئن شوید کلید میزبان مورد اعتماد است (برای مثال `ssh bot@mac-mini.tailnet-1234.ts.net`) تا `known_hosts` پر شود.
