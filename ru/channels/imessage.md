---
title: iMessage
source_url: https://docs.openclaw.ai/ru/channels/imessage
...
**Действия private API** Ответы, tapback-реакции, эффекты, вложения и управление группами. [**Сопряжение** Личные сообщения iMessage по умолчанию используют режим сопряжения. ](</ru/channels/pairing>) **Удаленный Mac** Используйте SSH-обертку, когда Gateway не запущен на Mac с Messages. [**Справочник по конфигурации** Полный справочник по полям iMessage. ](</ru/gateway/config-channels#imessage>)

## Быстрая настройка

### Локальный Mac (быстрый путь)

* ### Установите и проверьте imsg
...
* ### Одобрите первое сопряжение личного сообщения (dmPolicy по умолчанию)
...
Если `remoteHost` не задан, OpenClaw пытается автоматически определить его, разбирая SSH-скрипт-обертку. `remoteHost` должен быть `host` или `user@host` (без пробелов и SSH-опций). OpenClaw использует строгую проверку ключей хоста для SCP, поэтому ключ хоста ретрансляции уже должен существовать в `~/.ssh/known_hosts`. Пути вложений проверяются относительно разрешенных корней (`attachmentRoots` / `remoteAttachmentRoots`).
...
Резервное поведение во время выполнения: если `groupAllowFrom` не задан, проверки отправителей групп iMessage используют `allowFrom`; задайте `groupAllowFrom`, когда правила допуска для личных сообщений и групп должны различаться. Примечание о времени выполнения: если `channels.imessage` полностью отсутствует, среда выполнения возвращается к `groupPolicy="allowlist"` и записывает предупреждение в журнал (даже если задан `channels.defaults.groupPolicy`).
...
Некоторые многопользовательские потоки iMessage могут приходить с `is_group=false`. Если этот `chat_id` явно настроен в `channels.imessage.groups`, OpenClaw обрабатывает его как групповой трафик (групповая проверка + изоляция групповой сессии).
...
Используйте SSH-ключи, чтобы и SSH, и SCP были неинтерактивными. Сначала убедитесь, что ключ хоста доверенный (например, `ssh bot@mac-mini.tailnet-1234.ts.net`), чтобы `known_hosts` был заполнен.
...
Отчеты о прочтении и набор текста
