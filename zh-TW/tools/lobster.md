---
title: 龍蝦
source_url: https://docs.openclaw.ai/zh-TW/tools/lobster
...
OpenClaw 使用嵌入式執行器**在程序內** 執行 Lobster 工作流程。不會產生外部 CLI 子程序；工作流程引擎會在 gateway 程序內執行，並直接回傳 JSON 封套。 如果管線因核准而暫停，工具會回傳 `resumeToken`，讓你稍後繼續。
...
對於需要**結構化 LLM 步驟** 的工作流程，請啟用選用的 `llm-task` plugin 工具，並從 Lobster 呼叫它。這可讓工作流程保持 確定性，同時仍能使用模型進行分類、摘要與草擬。
