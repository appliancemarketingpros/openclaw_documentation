...
source_url: https://docs.openclaw.ai/pt-BR/tools/browser-control
scraped_at: 2026-05-25
---

Para instalação, configuração e solução de problemas, consulte [Browser](</pt-BR/tools/browser>). Esta página é a referência para a API HTTP de controle local, a CLI `openclaw browser` e padrões de script (snapshots, refs, esperas, fluxos de depuração).
...
Todos os endpoints aceitam `?profile=<name>`. `POST /start?headless=true` solicita uma inicialização headless única para perfis locais gerenciados sem alterar a configuração persistida do navegador; perfis attach-only, CDP remoto e existing-session rejeitam essa substituição porque o OpenClaw não inicia esses processos de navegador.
...
Observações:
  ...
  * Esta API autônoma de navegador por loopback **não** consome cabeçalhos de identidade de trusted-proxy ou Tailscale Serve.
  * Se `gateway.auth.mode` for `none` ou `trusted-proxy`, essas rotas de navegador por loopback não herdam esses modos com identidade; mantenha-as somente em loopback.


### Contrato de erro de `/act`

`POST /act` usa uma resposta de erro estruturada para validação em nível de rota e falhas de política:
...
Alguns recursos (navigate/act/snapshot de IA/snapshot de função, capturas de tela de elementos, PDF) exigem Playwright. Se o Playwright não estiver instalado, esses endpoints retornam um erro 501 claro.

O que ainda funciona sem o Playwright:

  * Snapshots ARIA
  * Snapshots de acessibilidade em estilo de função (`--interactive`, `--compact`, `--depth`, `--efficient`) quando um WebSocket CDP por aba está disponível. Este é um fallback para inspeção e descoberta de refs; o Playwright continua sendo o mecanismo principal de ações.
  * Capturas de tela da página para o navegador gerenciado `openclaw` quando um WebSocket CDP por aba está disponível
...
Se você vir `Playwright is not available in this gateway build`, o Gateway empacotado não inclui a dependência principal de runtime do navegador. Reinstale ou atualize o OpenClaw e reinicie o gateway. Para Docker, instale também os binários do navegador Chromium conforme mostrado abaixo.

#### Instalação do Playwright no Docker

Se o seu Gateway roda no Docker, evite `npx playwright` (conflitos de substituição do npm). Para imagens personalizadas, inclua o Chromium na imagem:
...
Para persistir downloads do navegador, defina `PLAYWRIGHT_BROWSERS_PATH` (por exemplo, `/home/node/.cache/ms-playwright`) e garanta que `/home/node` seja persistido por meio de `OPENCLAW_HOME_VOLUME` ou um bind mount. O OpenClaw detecta automaticamente o Chromium persistido no Linux. Consulte [Docker](</pt-BR/install/docker>).
...
Básico: status, abas, abrir/focar/fechar bashCopy code
[code]
...
Inspeção: captura de tela, snapshot, console, erros, solicitações bashCopy code
[code]
...
Ações: navigate, click, type, drag, wait, evaluate bashCopy code
[code]
...
Estado: cookies, armazenamento, offline, cabeçalhos, geo, dispositivo bashCopy code
[code]
...
IDs e rótulos estáveis de abas sobrevivem à substituição de alvo bruto do Chromium quando o OpenClaw consegue comprovar a aba substituta, como a mesma URL ou uma única aba antiga se tornando uma única nova aba após envio de formulário. IDs brutos de alvo ainda são voláteis; prefira `suggestedTargetId` de `tabs` em scripts.
...
O OpenClaw oferece suporte a dois estilos de "snapshot":
  ...
  * **Snapshot de função (refs de função como`e12`)**: `openclaw browser snapshot --interactive` (ou `--compact`, `--depth`, `--selector`, `--frame`)
    ...
    * Adicione `--urls` quando o texto do link for ambíguo e o agente precisar de alvos concretos de navegação.
  * **Snapshot ARIA (refs ARIA como`ax12`)**: `openclaw browser snapshot --format aria`
    ...
    * Ações: `openclaw browser click ax12` funciona quando o caminho do snapshot consegue vincular a ref por meio do Playwright e dos IDs DOM do backend do Chrome.
  * Se o Playwright não estiver disponível, snapshots ARIA ainda podem ser úteis para inspeção, mas as refs podem não ser acionáveis. Gere um novo snapshot com `--format ai` ou `--interactive` quando precisar de refs de ação.

  * Prova do Docker para o caminho de fallback CDP bruto: `pnpm test:docker:browser-cdp-snapshot` inicia o Chromium com CDP, executa `browser doctor --deep` e verifica se snapshots de papéis incluem URLs de links, elementos clicáveis promovidos por cursor e metadados de iframe.
...
Comportamento das refs:
  ...
  * `/act` retorna o `targetId` bruto atual após substituição acionada por ação quando consegue comprovar a aba substituta. Continue usando IDs/rótulos estáveis de aba para comandos de acompanhamento.
...
## Segurança e privacidade
  ...
  * `browser act kind=evaluate` / `openclaw browser evaluate` e `wait --fn` executam JavaScript arbitrário no contexto da página. Injeção de prompt pode direcionar isso. Desative com `browser.evaluateEnabled=false` se você não precisar.
