...
source_url: https://docs.openclaw.ai/hi/plugins/sdk-runtime
...
[**Channel Plugin** Channel Plugin के संदर्भ में इन helpers का उपयोग करने वाली चरण-दर-चरण मार्गदर्शिका। ](</hi/plugins/sdk-channel-plugins>) [**Provider Plugin** Provider Plugin के संदर्भ में इन helpers का उपयोग करने वाली चरण-दर-चरण मार्गदर्शिका। ](</hi/plugins/sdk-provider-plugins>)
...
`openclaw/plugin-sdk/pair-loop-guard-runtime` का सीधे उपयोग केवल custom two-party event loops के लिए करें जो shared inbound reply runner से होकर नहीं गुजरते।
...
Task Flow टिकाऊ बहु-चरणीय workflow state को ट्रैक करता है। यह scheduler नहीं है: भविष्य के wakeups के लिए Cron या `api.session.workflow.scheduleSessionTurn(...)` का उपयोग करें, फिर scheduled turn से `managedFlows` का उपयोग करें जब उस काम को flow state, child tasks, waits, या cancellation की आवश्यकता हो।
...
मौजूदा runtime config snapshot और transactional config writes। सक्रिय call path में पहले से पास किए गए config को प्राथमिकता दें; `current()` का उपयोग केवल तब करें जब handler को सीधे process snapshot चाहिए।
...
`mutateConfigFile(...)` और `replaceConfigFile(...)` एक `followUp` value लौटाते हैं, उदाहरण के लिए `{ mode: "restart", requiresRestart: true, reason }`, जो gateway से restart control छीने बिना writer intent रिकॉर्ड करता है।
...
`runCommandWithTimeout(...)` captured `stdout` और `stderr`, optional truncation counts, `code`, `signal`, `killed`, `termination`, और `noOutputTimedOut` लौटाता है। Timeout और no-output-timeout results `code: 124` रिपोर्ट करते हैं जब child process non-zero exit code प्रदान नहीं करता। Non-timeout signal exits अब भी `code: null` लौटा सकते हैं, इसलिए timeout reasons अलग करने के लिए `termination` और `noOutputTimedOut` का उपयोग करें।
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9ImFwaS5wbHVnaW5Db25maWciIHR5cGU9IlJlY29yZDxzdHJpbmcsIHVua25vd24 "> `plugins.entries.<id>.config` से Plugin-specific config।
