---
title: Signal
source_url: https://docs.openclaw.ai/pt-BR/channels/signal
...
Se você usar a build JVM (`signal-cli-${VERSION}.tar.gz`), instale primeiro o JRE 25+. Mantenha `signal-cli` atualizado; o upstream observa que versões antigas podem quebrar conforme as APIs do servidor Signal mudam.
...
Opções do provedor:
  ...
  * `channels.signal.apiMode`: `auto | native | container` (padrão: auto). Consulte Modo container.
