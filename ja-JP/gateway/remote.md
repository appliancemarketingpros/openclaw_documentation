...
注記:
  ...
  * **ノードは gateway サービスを実行しません。** 意図的に分離プロファイルを実行する場合を除き、ホストごとに 1 つの gateway のみを実行してください（[複数の gateway](</ja-JP/gateway/multiple-gateways>) を参照）。
...
## 関連
  ...
  * [リモート gateway 構成](</ja-JP/gateway/remote-gateway-readme>)
