...
source_url: https://docs.openclaw.ai/vi/platforms/mac/remote
...
Ở chế độ đường hầm SSH, các tên máy chủ LAN/tailnet được phát hiện sẽ được lưu thành `gateway.remote.sshTarget`. Ứng dụng giữ `gateway.remote.url` trên endpoint đường hầm cục bộ, ví dụ `ws://127.0.0.1:18789`, để CLI, Web Chat và dịch vụ node-host cục bộ đều dùng cùng phương thức truyền tải loopback an toàn.

Tự động hóa trình duyệt ở chế độ từ xa do CLI node host sở hữu, không phải do node của ứng dụng macOS native. Ứng dụng khởi động dịch vụ node host đã cài đặt khi có thể; nếu bạn cần điều khiển trình duyệt từ máy Mac đó, hãy cài đặt/khởi động bằng `openclaw node install ...` và `openclaw node start` (hoặc chạy `openclaw node run ...` ở foreground), rồi nhắm tới node có khả năng trình duyệt đó.
