...
source_url: https://docs.openclaw.ai/zh-CN/reference/openclaw-sdk-api-design
scraped_at: 2026-05-25
---

此页面是公共 [OpenClaw 应用 SDK](</zh-CN/concepts/openclaw-sdk>) 的详细 API 参考设计。它有意与 [插件 SDK](</zh-CN/plugins/sdk-overview>) 分开。

公共应用 SDK 应构建为两层：

  1. 低层生成式 Gateway 网关客户端。
  2. 高层易用封装，包含 `OpenClaw`、`Agent`、`Session`、`Run`、 `Task`、`Artifact`、`Approval` 和 `Environment` 对象。
...
分层：
  ...
  4. 高级 `OpenClaw`、`Agent`、`Session`、`Run`、`Task` 和 `Artifact` 封装。
  5. Cookbook 示例和集成测试。
