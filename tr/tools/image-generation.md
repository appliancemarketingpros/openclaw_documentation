...
source_url: https://docs.openclaw.ai/tr/tools/image-generation
...
Ajan `image_generate` çağrısını otomatik olarak yapar. Araç izin listesine gerek yoktur; bir sağlayıcı kullanılabilir olduğunda varsayılan olarak etkindir.
...
`openai/gpt-image-1.5`, `openai/gpt-image-1` ve `openai/gpt-image-1-mini` modelleri hâlâ açıkça seçilebilir. Şeffaf arka planlı PNG/WebP çıktısı için `gpt-image-1.5` kullanın; geçerli `gpt-image-2` API'si `background: "transparent"` değerini reddeder.

`gpt-image-2`, aynı `image_generate` aracı üzerinden hem metinden görsel üretimini hem de referans görsel düzenlemeyi destekler. OpenClaw, `prompt`, `count`, `size`, `quality`, `outputFormat` ve referans görselleri OpenAI'a iletir. OpenAI, `aspectRatio` veya `resolution` değerlerini doğrudan almaz; mümkün olduğunda OpenClaw bunları desteklenen bir `size` değerine eşler, aksi takdirde araç bunları yok sayılan geçersiz kılmalar olarak bildirir.
...
OpenAI görsel üretimini `api.openai.com` yerine bir Azure OpenAI dağıtımı üzerinden yönlendirmek için bkz. [Azure OpenAI uç noktaları](</tr/providers/openai#azure-openai-endpoints>).
