...
source_url: https://docs.openclaw.ai/fa/concepts/qa-matrix
...
Flag | پیش‌فرض | توضیح  
---|---|---  
`--profile <profile>` | `all` | profile سناریو. Profileها را ببینید.  
`--fail-fast` | خاموش | پس از اولین check یا سناریوی ناموفق متوقف شود.  
`--scenario <id>` | - | فقط این سناریو را اجرا کند. تکرارپذیر است. سناریوها را ببینید.  
