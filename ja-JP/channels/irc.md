---
title: IRC
source_url: https://docs.openclaw.ai/ja-JP/channels/irc
scraped_at: 2026-05-25
---

IRC は、クラシックなチャンネル（`#room`）とダイレクトメッセージで OpenClaw を使いたい場合に使用します。 IRC はバンドルされた Plugin として同梱されていますが、メイン設定の `channels.irc` で構成します。
...
許可リストのエントリには、安定した送信者 ID（`nick!user@host`）を使用してください。 bare nick の照合は変更可能であり、`channels.irc.dangerouslyAllowNameMatching: true` の場合にのみ有効になります。
...
公開チャンネルで `allowFrom: ["*"]` を許可すると、誰でもボットにプロンプトを送信できます。 リスクを減らすには、そのチャンネルのツールを制限してください。
...
メモ:
  ...
  * `toolsBySender` キーでは、IRC 送信者 ID 値に `id:` を使用してください。 より強い照合には `id:eigen` または `id:eigen!~eigen@174.127.248.171` を使用します。
