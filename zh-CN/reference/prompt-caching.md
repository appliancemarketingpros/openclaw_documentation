---
title: 提示词缓存
source_url: https://docs.openclaw.ai/zh-CN/reference/prompt-caching
...
基线文件保存最近观察到的实时数值，以及测试所使用的按提供商划分的回归下限。 运行器还会使用每次运行全新的会话 ID 和提示词命名空间，这样之前的缓存状态就不会污染当前的回归样本。
...
yamlCopy code
[code]
    diagnostics:  cacheTrace:    enabled: true    filePath: "~/.openclaw/logs/cache-trace.jsonl" # 可选    includeMessages: false # 默认 true    includePrompt: false # 默认 true    includeSystem: false # 默认 true
[/code]
