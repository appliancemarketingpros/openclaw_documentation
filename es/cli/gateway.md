---
title: Gateway
source_url: https://docs.openclaw.ai/es/cli/gateway
...
[**Descubrimiento Bonjour** Configuración de mDNS local + DNS-SD de área amplia. ](</es/gateway/bonjour>) [**Descripción general del descubrimiento** Cómo OpenClaw anuncia y encuentra gateways. ](</es/gateway/discovery>) [**Configuración** Claves de configuración de gateway de nivel superior. ](</es/gateway/configuration>)
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tcG9ydCA8cG9ydA " type="number"> Puerto WebSocket (el valor predeterminado proviene de config/env; normalmente `18789`).
...
Crear una configuración de desarrollo + workspace si falta (omite [BOOTSTRAP.md](<http://BOOTSTRAP.md>)).
...
OPENCLAW_DOCS_MARKER:paramOpen:IHBhdGg9Ii0tdHlwZSA8dHlwZQ " type="string"> Filtrar por tipo de evento diagnóstico, como `payload.large` o `diagnostic.memory.pressure`.
...
Usa `--wrapper` cuando el servicio gestionado deba iniciarse mediante otro ejecutable, por ejemplo un shim de gestor de secretos o un ayudante run-as. El wrapper recibe los argumentos normales de Gateway y es responsable de finalmente hacer exec de `openclaw` o Node con esos argumentos.
