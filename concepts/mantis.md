...
Required inputs for `--credential-source env`:
  ...
  * `OPENCLAW_LIVE_OPENAI_KEY` for the remote model lane. If only `OPENAI_API_KEY` is set locally, Mantis maps it to `OPENCLAW_LIVE_OPENAI_KEY` before invoking Crabbox so Crabbox's `OPENCLAW_*` env forwarding can carry it into the VM.
...
Mantis should build on the existing private QA stack instead of starting from zero:
  ...
  * `pnpm openclaw qa discord` already runs a live Discord lane with driver and SUT bots.
  * The live transport runner already writes reports and observed-message artifacts under `.artifacts/qa-e2e/`.
  * Convex credential leases already provide exclusive access to shared live transport credentials.
...
The browser lane has two modes:
  ...
  * **Headless automation** : default for CI. Chrome runs with CDP enabled, and Playwright or OpenClaw browser control captures screenshots.
