...
source_url: https://docs.openclaw.ai/es/plugins/skill-workshop
...
Las propuestas pendientes y en cuarentena se deduplican por nombre de habilidad y carga útil de cambio. El almacén conserva las propuestas pendientes/en cuarentena más recientes hasta `maxPending`.
...
Usa esto cuando la captura automática parezca no hacer nada y los registros mencionen `skill-workshop: quarantined <skill>`.
...
Solicitar escritura inmediata en modo automático (apply: true) jsonCopy code
[code]
...
Con `approvalPolicy: "pending"`, `apply: true` sigue poniendo la propuesta en cola. Revísala y luego usa la acción `apply` después de aprobarla.

Forzar pendiente con política automática (apply: false) jsonCopy code
[code]
...
Anexar a una sección con nombre jsonCopy code
[code]
...
Reemplazar texto exacto jsonCopy code
[code]
...
Con `approvalPolicy: "pending"`, esta acción solicita aprobación del operador antes de escribir la habilidad del espacio de trabajo.
...
Los archivos de soporte tienen alcance de espacio de trabajo, se comprueban por ruta, están limitados en bytes por `maxSkillBytes`, se escanean y se escriben de forma atómica.
...
Todas las escrituras son atómicas y actualizan inmediatamente la instantánea de Skills en memoria, de modo que la Skill nueva o actualizada puede hacerse visible sin reiniciar el Gateway.
...
Para recuperarte de una propuesta en cuarentena, crea una nueva propuesta segura con el contenido inseguro eliminado. No edites manualmente el JSON del almacén.

## Guía de prompt

Cuando está habilitado, Skill Workshop inyecta una breve sección de prompt que indica al agente que use `skill_workshop` para memoria procedimental duradera.
...
La revisión con LLM usa una ejecución incrustada en el modelo activo/predeterminado del agente. Está basada en umbrales, por lo que no se ejecuta en cada turno de forma predeterminada.
...
Si el revisor falla, agota el tiempo de espera o devuelve JSON no válido, el Plugin registra un mensaje de advertencia/depuración y omite esa pasada de revisión.
...
El escenario del revisor está separado intencionadamente porque habilita `reviewMode: "llm"` y ejercita la pasada del revisor incrustado.
...
Usa primero el modo pendiente. Cambia al modo automático solo después de revisar el tipo de Skills que propone el agente en ese espacio de trabajo.
