...
Agent-scoped configuration keys under `agents.*`, `multiAgent.*`, `session.*`, `messages.*`, and `talk.*`. For channels, tools, gateway runtime, and other top-level keys, see [Configuration reference](</gateway/configuration-reference>).
...
Controls the agent-visible system-prompt notice when bootstrap context is truncated. Default: `"once"`.
...
[/code]
  ...
  * `memoryGetDefaultLines`: default `memory_get` line window when `lines` is omitted.
  * `toolResultMaxChars`: live tool-result cap used for persisted results and overflow recovery.
...
Ack reaction
  ...
  * `messages.statusReactions.enabled`: enables lifecycle status reactions on Slack, Discord, and Telegram. On Slack and Discord, unset keeps status reactions enabled when ack reactions are active. On Telegram, set it explicitly to `true` to enable lifecycle status reactions.
...
[Configuration reference](</gateway/configuration-reference>)[Configuration — channels](</gateway/config-channels>)
