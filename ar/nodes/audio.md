...
source_url: https://docs.openclaw.ai/ar/nodes/audio
...
إذا **لم تضبط النماذج** ولم يتم تعيين `tools.media.audio.enabled` إلى `false`، فإن OpenClaw يكتشف تلقائيًا بهذا الترتيب ويتوقف عند أول خيار يعمل:
...
لتعطيل الاكتشاف التلقائي، عيّن `tools.media.audio.enabled: false`. للتخصيص، عيّن `tools.media.audio.models`. ملاحظة: اكتشاف الملفات الثنائية يتم بأفضل جهد عبر macOS/Linux/Windows؛ تأكد من أن CLI موجودة على `PATH` (نوسّع `~`)، أو عيّن نموذج CLI صريحًا بمسار أمر كامل.
