---
title: Raft
source_url: https://docs.openclaw.ai/zh-CN/channels/raft
...
当智能体未调用 Raft CLI 时，这是预期行为。唤醒桥接器不携带消息正文，也不会自动发送最终回复。检查智能体的工具策略，并确保它可以运行 `raft --profile <profile> message check` 和 `message send`。
