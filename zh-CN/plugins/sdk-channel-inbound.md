---
title: 频道入站 API
source_url: https://docs.openclaw.ai/zh-CN/plugins/sdk-channel-inbound
...
使用 `openclaw/plugin-sdk/channel-inbound` 进行入站事件规范化、格式化、根目录和编排。使用 `openclaw/plugin-sdk/channel-outbound` 处理原生发送、回执、持久交付和实时预览行为。
...
注入的插件运行时会在 `runtime.channel.inbound.*` 下暴露相同的高级辅助函数，供已经接收运行时对象的内置/原生渠道使用。
