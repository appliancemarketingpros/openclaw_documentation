...
source_url: https://docs.openclaw.ai/nl/gateway/troubleshooting
...
OpenClaw behandelt elke Skill-root als een afgebakende containmentgrens. Een symlink onder `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills` of `~/.openclaw/skills` wordt overgeslagen wanneer het echte doel buiten die root uitkomt, tenzij het doel expliciet vertrouwd is.
...
Als het doel bewust is gekozen, configureer dan zowel de directe Skill-root als het toegestane symlinkdoel:
...
Start daarna een nieuwe sessie of wacht tot de Skills-watcher is vernieuwd. Herstart de Gateway als het lopende proces ouder is dan de configuratiewijziging.

Gebruik geen brede doelen zoals `~`, `/` of een hele gesynchroniseerde projectmap. Houd `allowSymlinkTargets` beperkt tot de echte Skill-root die vertrouwde `SKILL.md`-mappen bevat.
...
* ### Een in aanmerking komende referentie gebruiken
...
Let op:
  ...
  * `model_not_found`\- of 404-fouten, ook al werkt directe `/v1/chat/completions` met dezelfde kale model-id
...
Gebruik dit wanneer het starten van de Gateway mislukt met `Invalid config` of hot-reloadlogs zeggen dat een ongeldige bewerking is overgeslagen.
...
Inspecteer en herstel bashCopy code
[code]
...
1\. Gedrag voor auth- en URL-overrides is gewijzigd bashCopy code
[code]
...
2\. Bind- en auth-vangrails zijn strenger bashCopy code
[code]
...
3\. Status van koppeling en apparaatidentiteit is gewijzigd bashCopy code
[code]
