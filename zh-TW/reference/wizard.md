---
title: 入門參考
source_url: https://docs.openclaw.ai/zh-TW/reference/wizard
scraped_at: 2026-05-25
---

這是 `openclaw onboard` 的完整參考。 如需高階概觀，請參閱[入門設定（CLI）](</zh-TW/start/wizard>)。
...
* ### 現有設定偵測
  ...
  * 如果設定無效或包含舊版金鑰，精靈會停止並要求 你先執行 `openclaw doctor` 再繼續。
...
* ### 模型/驗證
  ...
  * **MiniMax** ：設定會自動寫入；託管預設值為 `MiniMax-M2.7`。 API 金鑰設定使用 `minimax/...`，OAuth 設定使用 `minimax-portal/...`。
...
提供者特定的命令範例位於 [CLI 自動化](</zh-TW/start/wizard-cli-automation#provider-specific-examples>)。 使用此參考頁了解旗標語意和步驟順序。
...
Gateway 透過 RPC（`wizard.start`、`wizard.next`、`wizard.cancel`、`wizard.status`）公開入門設定流程。 用戶端（macOS app、Control UI）可以呈現步驟，而不需要重新實作入門設定邏輯。
...
WhatsApp 憑證會放在 `~/.openclaw/credentials/whatsapp/<accountId>/` 下。 工作階段會儲存在 `~/.openclaw/agents/<agentId>/sessions/` 下。
