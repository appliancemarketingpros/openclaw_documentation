...
[**Skills** スキルとは何か、そしてどのように読み込まれるか。 ](</ja-JP/tools/skills>) [**スキルの作成** カスタムスキルパックの作成。 ](</ja-JP/tools/creating-skills>) [**スラッシュコマンド** ネイティブコマンドカタログとチャットディレクティブ。 ](</ja-JP/tools/slash-commands>) [**設定リファレンス** 完全な `skills` と `agents.skills` のスキーマ。 ](</ja-JP/gateway/configuration-reference>)
