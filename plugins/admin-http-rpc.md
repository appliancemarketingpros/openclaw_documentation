...
The bundled `admin-http-rpc` plugin exposes selected Gateway control-plane methods over HTTP for trusted host automation that cannot use the normal Gateway WebSocket RPC client. The plugin is included with OpenClaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
The route is registered during plugin startup. Restart the Gateway after changing plugin config. Disable it when you no longer need the HTTP surface:
[code] 
...
[Webhooks plugin](</plugins/webhooks>)[Voice call](</plugins/voice-call>)
