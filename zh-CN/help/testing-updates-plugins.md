---
title: 更新和插件测试
source_url: https://docs.openclaw.ai/zh-CN/help/testing-updates-plugins
...
从制品身份开始：
  ...
  * 包验收 `resolve_package` 摘要：来源、版本、SHA-256 和 构件名称。
  * Docker 构件：`.artifacts/docker-tests/**/summary.json`、 `failures.json`、lane 日志，以及重新运行命令。
  * 升级保留项摘要：`.artifacts/upgrade-survivor/summary.json`， 包括基线版本、候选版本、场景、阶段耗时，以及 recipe 步骤。


优先使用同一个包构件重新运行失败的精确 lane，而不是 重新运行整个发布总流程。
