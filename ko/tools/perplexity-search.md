---
title: Perplexity 검색
source_url: https://docs.openclaw.ai/ko/tools/perplexity-search
scraped_at: 2026-05-25
---

OpenClaw은 Perplexity Search API를 `web_search` 제공자로 지원합니다. 이 API는 `title`, `url`, `snippet` 필드가 포함된 구조화된 결과를 반환합니다.

호환성을 위해 OpenClaw은 기존 Perplexity Sonar/OpenRouter 설정도 지원합니다. `OPENROUTER_API_KEY`, `plugins.entries.perplexity.config.webSearch.apiKey`의 `sk-or-...` 키를 사용하거나 `plugins.entries.perplexity.config.webSearch.baseUrl` / `model`을 설정하면, 제공자는 chat completions 경로로 전환되어 구조화된 Search API 결과 대신 인용이 포함된 AI 합성 답변을 반환합니다.
...
**Config 사용:** `openclaw configure --section web`을 실행하세요. 이 명령은 키를 `plugins.entries.perplexity.config.webSearch.apiKey` 아래의 `~/.openclaw/openclaw.json`에 저장합니다. 해당 필드는 SecretRef 객체도 허용합니다.

**환경 사용:** Gateway 프로세스 환경에서 `PERPLEXITY_API_KEY` 또는 `OPENROUTER_API_KEY`를 설정하세요. gateway 설치의 경우 `~/.openclaw/.env`(또는 서비스 환경)에 넣으세요. [환경 변수](</ko/help/faq#env-vars-and-env-loading>)를 참고하세요.
...
[**Web search overview** 모든 제공자와 자동 감지 규칙입니다. ](</ko/tools/web>) [**Brave search** 국가 및 언어 필터가 있는 구조화된 결과입니다. ](</ko/tools/brave-search>) [**Exa search** 콘텐츠 추출을 포함한 신경망 검색입니다. ](</ko/tools/exa-search>) [**Perplexity Search API docs** 공식 Perplexity Search API 빠른 시작 및 참조입니다. ](<https://docs.perplexity.ai/docs/search/quickstart>)
