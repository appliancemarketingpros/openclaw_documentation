---
title: TUI
source_url: https://docs.openclaw.ai/ar/web/tui
...
إذا كان `openclaw config validate` يفشل بالفعل، فابدأ أولًا بـ `openclaw configure` أو `openclaw doctor --fix`. لا يتجاوز `openclaw chat` حارس الإعداد غير الصالح.
