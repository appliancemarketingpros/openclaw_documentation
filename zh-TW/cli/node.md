---
title: Node
source_url: https://docs.openclaw.ai/zh-TW/cli/node
scraped_at: 2026-05-25
---

# `openclaw node`

執行一個連線到 Gateway WebSocket 的**無頭 Node 主機** ，並在這台機器上公開 `system.run` / `system.which`。
...
預設情況下，Proxy 會公開 Node 的一般瀏覽器設定檔介面。如果你設定 `nodeHost.browserProxy.allowProfiles`，Proxy 會變成限制模式： 非允許清單中的設定檔目標會遭拒，且持久設定檔的建立/刪除路由會透過 Proxy 被封鎖。
...
若 Node 要連線到受信任私人網路上非迴圈位址的 `ws://` Gateway，請設定 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`。若未設定，Node 啟動會失敗關閉，並要求你使用 `wss://`、SSH 通道或 Tailscale。 這是程序環境的選擇加入，不是 `openclaw.json` 設定鍵。 當安裝命令環境中存在此值時，`openclaw node install` 會將它保存到受監督的 Node 服務中。
...
第一次連線會在 Gateway 上建立待處理的裝置配對請求（`role: node`）。 透過以下方式核准：
...
如果 Node 以變更後的驗證詳細資訊（角色/範圍/公開金鑰）重試配對，先前待處理的請求會被取代，並建立新的 `requestId`。 核准前請再次執行 `openclaw devices list`。

Node 主機會將其 Node id、token、顯示名稱與 gateway 連線資訊儲存在 `~/.openclaw/node.json`。
...
對於已核准的非同步 Node exec，OpenClaw 會在提示前準備標準的 `systemRunPlan`。 稍後已核准的 `system.run` 轉送會重用該已儲存的計畫，因此在核准請求建立後對 command/cwd/session 欄位所做的編輯會被拒絕，而不是改變 Node 執行的內容。
