...
source_url: https://docs.openclaw.ai/pl/nodes/media-understanding
...
Podstawowe/zapasowe referencje `agents.defaults.imageModel` (tylko obraz). Preferuj referencje `provider/model`. Niekwalifikowane referencje są kwalifikowane z wpisów modeli dostawcy skonfigurowanych z obsługą obrazów tylko wtedy, gdy dopasowanie jest unikalne.
...
Dołączona kolejność zapasowa:
  ...
  * Obraz: OpenAI → Anthropic → Google → MiniMax → MiniMax Portal → [Z.AI](<http://Z.AI>)
