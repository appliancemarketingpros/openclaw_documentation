...
注意事项：
  ...
  * `dashboard` 遵循 `gateway.tls.enabled`：启用 TLS 的 Gateway 网关会打印/打开 `https://` 控制 UI URL，并通过 `wss://` 连接。
  * 如果带令牌认证的仪表板 URL 的剪贴板/浏览器传递失败， `dashboard` 会记录一条安全的手动认证提示，提到 `OPENCLAW_GATEWAY_TOKEN`、 `gateway.auth.token` 和片段键 `token`，但不会打印令牌 值。
