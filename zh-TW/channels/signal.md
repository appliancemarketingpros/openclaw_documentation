---
title: Signal
source_url: https://docs.openclaw.ai/zh-TW/channels/signal
...
如果使用 JVM build（`signal-cli-${VERSION}.tar.gz`），請先安裝 JRE 25+。 請持續更新 `signal-cli`；上游指出，舊版本可能會因 Signal server API 變更而中斷。
...
提供者選項：
  ...
  * `channels.signal.apiMode`：`auto | native | container`（預設：auto）。請參閱容器模式。
