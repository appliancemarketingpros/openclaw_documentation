---
title: ACP
source_url: https://docs.openclaw.ai/zh-TW/cli/acp
...
使用內建 ACP 用戶端可在沒有 IDE 的情況下對橋接器進行基本檢查。 它會產生 ACP 橋接器，並讓你以互動方式輸入提示。
...
每個 ACP 工作階段對應到單一 Gateway 工作階段金鑰。一個代理可以有多個 工作階段；除非你覆寫金鑰或標籤，否則 ACP 預設使用隔離的 `acp:<uuid>` 工作階段。

橋接模式不支援每個工作階段個別設定 `mcpServers`。如果 ACP 用戶端在 `newSession` 或 `loadSession` 期間傳送它們，橋接器會回傳清楚的錯誤， 而不是默默忽略。

如果你想讓 ACPX 後端支援的工作階段看見 OpenClaw Plugin 工具，或所選的 內建工具例如 `cron`，請改為啟用 Gateway 端的 ACPX MCP 橋接器， 而不是嘗試傳遞每個工作階段的 `mcpServers`。請參閱 [ACP 代理](</zh-TW/tools/acp-agents-setup#plugin-tools-mcp-bridge>)和 [OpenClaw 工具 MCP 橋接器](</zh-TW/tools/acp-agents-setup#openclaw-tools-mcp-bridge>)。

## 從 `acpx` 使用（Codex、Claude、其他 ACP 用戶端）

如果你想讓 Codex 或 Claude Code 等程式碼代理透過 ACP 與你的 OpenClaw bot 溝通，請使用 `acpx` 及其內建的 `openclaw` 目標。
...
如果你想讓 `acpx openclaw` 每次都指向特定的 Gateway 和工作階段金鑰， 請在 `~/.acpx/config.json` 中覆寫 `openclaw` 代理命令：
...
對於 repo 本機的 OpenClaw checkout，請使用直接的 CLI 進入點， 而不是開發執行器，讓 ACP 串流保持乾淨。例如：
...
這是讓 Codex、Claude Code 或其他支援 ACP 的用戶端從 OpenClaw 代理 擷取情境資訊的最簡單方式，不需要擷取終端機內容。
...
預設情況下，ACP 工作階段會取得帶有 `acp:` 前綴的隔離 Gateway 工作階段金鑰。 若要重用已知工作階段，請傳入工作階段金鑰或標籤：
