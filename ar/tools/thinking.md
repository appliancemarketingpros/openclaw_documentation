...
source_url: https://docs.openclaw.ai/ar/tools/thinking
...
## ما الذي يفعله
  ...
  * ملاحظات المزوّد: 
    ...
    * يدعم [Z.AI](<http://Z.AI>) (`zai/*`) التفكير الثنائي فقط (`on`/`off`). يُعامَل أي مستوى غير `off` على أنه `on` (ويُعيَّن إلى `low`).
