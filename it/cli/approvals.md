---
title: Approvazioni
source_url: https://docs.openclaw.ai/it/cli/approvals
scraped_at: 2026-05-25
---

# `openclaw approvals`

Gestisci le approvazioni exec per l'**host locale** , l'**host Gateway** o un **host Node**. Per impostazione predefinita, i comandi puntano al file delle approvazioni locale sul disco. Usa `--gateway` per puntare al Gateway, oppure `--node` per puntare a un Node specifico.
...
`openclaw exec-policy` è il comando locale di utilità per mantenere allineati in un solo passaggio la configurazione richiesta `tools.exec.*` e il file delle approvazioni dell'host locale.
...
Se devi modificare direttamente le approvazioni di host remoti, continua a usare `openclaw approvals set --gateway` oppure `openclaw approvals set --node <id|name|ip>`.
...
Questa scorciatoia locale aggiorna insieme sia la configurazione locale richiesta `tools.exec.*` sia i valori predefiniti delle approvazioni locali. È equivalente nell'intento alla configurazione manuale in due passaggi qui sopra, ma solo per la macchina locale.
