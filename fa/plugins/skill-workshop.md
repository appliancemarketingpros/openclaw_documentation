...
source_url: https://docs.openclaw.ai/fa/plugins/skill-workshop
...
json5Copy code
[code]
    // محافظه‌کارانه: فقط استفاده صریح از ابزار، بدون ثبت خودکار.{  autoCapture: false,  approvalPolicy: "pending",  reviewMode: "off",}
[/code]

json5Copy code
[code]
    // بازبینی‌محور: ثبت خودکار، اما با نیاز به تأیید.{  autoCapture: true,  approvalPolicy: "pending",  reviewMode: "hybrid",}
[/code]

json5Copy code
[code]
    // خودکارسازی مورد اعتماد: proposalهای ایمن را فوراً بنویس.{  autoCapture: true,  approvalPolicy: "auto",  reviewMode: "hybrid",}
[/code]

json5Copy code
[code]
    // کم‌هزینه: بدون فراخوانی LLM بازبین، فقط عبارت‌های اصلاح صریح.{  autoCapture: true,  approvalPolicy: "pending",  reviewMode: "heuristic",}
[/code]
...
پیشنهادهای در انتظار و قرنطینه‌شده بر اساس نام مهارت و بار دادهٔ تغییر تکرارزدایی می‌شوند. ذخیره‌ساز جدیدترین پیشنهادهای در انتظار/قرنطینه‌شده را تا سقف `maxPending` نگه می‌دارد.
...
وقتی ثبت خودکار ظاهراً هیچ کاری انجام نمی‌دهد و لاگ‌ها به `skill-workshop: quarantined <skill>` اشاره می‌کنند، از این استفاده کنید.
...
با `approvalPolicy: "pending"`، حتی `apply: true` نیز پیشنهاد را در صف قرار می‌دهد. آن را بازبینی کنید، سپس پس از تأیید از کنش `apply` استفاده کنید.
...
فایل‌های پشتیبان محدود به workspace هستند، مسیرشان بررسی می‌شود، اندازه‌شان با `maxSkillBytes` محدود می‌شود، اسکن می‌شوند و به‌صورت اتمیک نوشته می‌شوند.
...
همهٔ نوشتن‌ها اتمیک هستند و snapshot درون‌حافظه‌ای مهارت‌ها را بلافاصله تازه‌سازی می‌کنند، بنابراین مهارت جدید یا به‌روزشده می‌تواند بدون راه‌اندازی دوبارهٔ Gateway قابل مشاهده شود.
...
برای بازیابی از یک پیشنهاد قرنطینه‌شده، یک پیشنهاد امن جدید ایجاد کنید که محتوای ناامن از آن حذف شده باشد. JSON ذخیره‌گاه را دستی ویرایش نکنید.

## راهنمایی پرامپت

وقتی فعال باشد، Skill Workshop یک بخش کوتاه پرامپت تزریق می‌کند که به عامل می‌گوید برای حافظهٔ رویه‌ای پایدار از `skill_workshop` استفاده کند.
...
بازبینی LLM از یک اجرای embedded روی مدل فعال/پیش‌فرض عامل استفاده می‌کند. این بازبینی آستانه‌محور است، بنابراین به‌طور پیش‌فرض در هر نوبت اجرا نمی‌شود.
...
بازبین:
  ...
  * فقط می‌تواند پیشنهادی emit کند که از مسیر معمول اسکنر و تأیید/قرنطینه عبور می‌کند


اگر بازبین شکست بخورد، timeout شود، یا JSON نامعتبر برگرداند، Plugin یک پیام هشدار/debug ثبت می‌کند و آن گذر بازبینی را رد می‌کند.
...
سناریوی بازبین عمداً جداست، چون `reviewMode: "llm"` را فعال می‌کند و گذر بازبین embedded را تمرین می‌دهد.
...
ابتدا از حالت pending استفاده کنید. فقط پس از بازبینی نوع مهارت‌هایی که عامل در آن workspace پیشنهاد می‌دهد، به حالت auto بروید.
