---
title: โมเดล
source_url: https://docs.openclaw.ai/th/cli/models
...
`models scan` อ่านแค็ตตาล็อก `:free` สาธารณะของ OpenRouter และจัดอันดับ candidate สำหรับ การใช้เป็น fallback ตัวแค็ตตาล็อกเป็นสาธารณะ ดังนั้นการสแกนเฉพาะเมทาดาทาจึงไม่ต้องมี key ของ OpenRouter

โดยค่าเริ่มต้น OpenClaw จะพยายาม probe การรองรับเครื่องมือและรูปภาพด้วยการเรียกโมเดลแบบสด หากไม่ได้กำหนดค่า key ของ OpenRouter คำสั่งจะถอยกลับไปเป็นเอาต์พุตเฉพาะเมทาดาทา และอธิบายว่าโมเดล `:free` ยังต้องใช้ `OPENROUTER_API_KEY` สำหรับ probe และ inference
...
`--json` กัน stdout ไว้สำหรับ payload JSON diagnostics ของโปรไฟล์การยืนยันตัวตน ผู้ให้บริการ และ startup จะถูกส่งไปยัง stderr เพื่อให้สคริปต์สามารถ pipe stdout โดยตรง เข้าสู่เครื่องมือ เช่น `jq`
...
`models auth add` คือ helper การยืนยันตัวตนแบบโต้ตอบ สามารถเปิด flow การยืนยันตัวตนของผู้ให้บริการ (OAuth/API key) หรือนำคุณไปสู่การวาง token ด้วยตนเอง ขึ้นอยู่กับ ผู้ให้บริการที่คุณเลือก

`models auth list` แสดงรายการโปรไฟล์การยืนยันตัวตนที่บันทึกไว้สำหรับ agent ที่เลือกโดยไม่ พิมพ์ token, API-key หรือ secret material ของ OAuth ใช้ `--provider <id>` เพื่อ กรองเป็นผู้ให้บริการหนึ่งราย เช่น `openai-codex` และใช้ `--json` สำหรับ scripting
...
สำหรับโมเดล OpenAI, `--provider openai` ใช้การเข้าสู่ระบบบัญชี ChatGPT/Codex เป็นค่าเริ่มต้น ใช้ `--method api-key` เฉพาะเมื่อคุณต้องการเพิ่มโปรไฟล์ API-key ของ OpenAI โดยปกติเป็นตัวสำรองสำหรับขีดจำกัด subscription ของ Codex การสะกดแบบ legacy `--provider openai-codex` ยังใช้งานได้กับสคริปต์ที่มีอยู่
