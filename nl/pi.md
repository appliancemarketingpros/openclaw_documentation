...
source_url: https://docs.openclaw.ai/nl/pi
scraped_at: 2026-05-25
---

OpenClaw integreert met [pi-coding-agent](<https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent>) en de bijbehorende siblingpakketten (`pi-ai`, `pi-agent-core`, `pi-tui`) om de mogelijkheden van zijn AI-agent aan te drijven.
...
Kanaalspecifieke runtimes voor berichtacties staan nu in de extensionmappen die eigendom zijn van de Plugin, in plaats van onder `src/agents/tools`, bijvoorbeeld:
...
Afbeeldingsinjectie is prompt-lokaal: OpenClaw laadt afbeeldingsreferenties uit de huidige prompt en geeft ze alleen voor die beurt door via `images`. Oudere history turns worden niet opnieuw gescand om afbeeldingspayloads opnieuw te injecteren.
...
### Toolpipeline
  ...
  6. **Schemanormalisatie** : Schema's opgeschoond voor eigenaardigheden van Gemini/OpenAI
...
Automatische Compaction wordt geactiveerd bij contextoverflow. Veelvoorkomende overflow-signaturen zijn `request_too_large`, `context length exceeded`, `input exceeds the maximum number of tokens`, `input token count exceeds the maximum number of input tokens`, `input is too long for the model` en `ollama error: context length exceeded`. `compactEmbeddedPiSessionDirect()` verwerkt handmatige Compaction:
...
Systeemprompt | [AGENTS.md](<http://AGENTS.md>) \+ prompts | Dynamisch per kanaal/context  
...
Gebieden voor mogelijke herziening:
  ...
  1. **Afstemming van toolsignatures** : Past momenteel aan tussen pi-agent-core- en pi-coding-agent-signatures
  2. **Omwikkeling van sessiebeheerder** : `guardSessionManager` voegt veiligheid toe, maar verhoogt de complexiteit
...
Zie [Pi-ontwikkelworkflow](</nl/pi-dev>) voor huidige uitvoeropdrachten.
