...
source_url: https://docs.openclaw.ai/th/reference/release-performance-sweep
...
**Stable agent turn** **cold turn เร็วขึ้น 5.1 เท่า**

  * `v2026.4.14`: 9.8 วินาที
  * `v2026.5.28`: 1.9 วินาที

**Published package** **tarball 17.9MB** แพ็กเกจ stable ล่าสุด ลดลงจากจุดสูงสุดของขนาดแพ็กเกจเดือนมีนาคมที่ 43.3MB **Latest stable install** **การติดตั้งใหม่ 361.7MiB** `v2026.5.28` ลด dependency tree ของ OpenClaw ที่ซ้อนอยู่ลงอย่างมาก แต่ยังมี tree ที่ซ้อนอยู่ขนาดเล็กกว่า 259.7MiB เหลืออยู่ในการตรวจสอบการติดตั้งภายในเครื่อง **Dependency graph** **แพ็กเกจที่ติดตั้ง 300 รายการ** รีลีส stable ล่าสุด วัดเป็น root ของชื่อ/เวอร์ชันแพ็กเกจที่ไม่ซ้ำกันในการติดตั้งใหม่ โดยปิดใช้งานสคริปต์

## ไทม์ไลน์พื้นที่ติดตั้ง

**Monthly high** **dependency 645 รายการ** `2026.2.26` เป็นจุดสูงสุดของจำนวน dependency รายเดือนในตัวอย่างนี้ **Shrinkwrap introduced** **การติดตั้ง 1,020.6MB** `2026.5.22` เพิ่ม shrinkwrap ที่ root และเผยให้เห็นปัญหารูปร่างแพ็กเกจ: มี 911.8MB ไปอยู่ใต้ `openclaw/node_modules` ที่ซ้อนอยู่ **Latest stable** **การติดตั้ง 361.7MiB** `2026.5.28` ลดขนาดการติดตั้งใหม่ลง 52.8% จาก `2026.5.27` แต่ยังคง ติดตั้ง tree ของ OpenClaw ที่ซ้อนอยู่ขนาด 259.7MiB **Dependency graph** **root แพ็กเกจ 300 รายการ** `2026.5.28` ติดตั้ง root ของชื่อ/เวอร์ชันแพ็กเกจที่ไม่ซ้ำกันน้อยกว่า `2026.5.27` อยู่ 71 รายการ
...
**กราฟค่าเริ่มต้นระดับราก** รากชื่อแพ็กเกจ/เวอร์ชันที่ไม่ซ้ำลดจาก **371** เหลือ **300** อินสแตนซ์ของแพ็กเกจ ลดจาก **372** เหลือ **301** **ทรีที่ซ้อนอยู่** `openclaw/node_modules` ที่ซ้อนอยู่ลดจาก **656.1MiB** เหลือ **259.7MiB** ใน การตรวจสอบการติดตั้งภายในเครื่องเดียวกัน **กรวย native แบบ optional** กรวยแพ็กเกจ native สำหรับทุกแพลตฟอร์มของ `@napi-rs/canvas` หยุดถูกติดตั้งใน การติดตั้งค่าเริ่มต้น **พื้นที่ผิวของห่วงโซ่อุปทาน** แพ็กเกจค่าเริ่มต้นที่น้อยลงหมายถึง tarball, ผู้ดูแล, ไบนารี native, พฤติกรรมระหว่างติดตั้ง และเส้นทางอัปเดตแบบ transitive ที่ต้องเชื่อถือโดยค่าเริ่มต้นน้อยลง
...
**ก่อนมี shrinkwrap** `2026.5.20` ไม่มี shrinkwrap ระดับรูท และไม่มี dependency tree ของ OpenClaw แบบซ้อนขนาดใหญ่ **เริ่มนำมาใช้** `2026.5.22` เพิ่ม shrinkwrap ระดับรูท และติดตั้ง 911.8MB ภายใต้ `openclaw/node_modules` แบบซ้อน **stable ล่าสุด** `2026.5.28` ยังคงมี shrinkwrap และยังติดตั้ง 259.7MiB ภายใต้ `openclaw/node_modules` แบบซ้อน **แก้ไข fanout ของ Canvas แล้ว** `2026.5.28` ไม่ติดตั้งแพ็กเกจ `@napi-rs/canvas` ใด ๆ อีกแล้วในการตรวจสอบ fresh install ในเครื่อง
...
สำหรับคำอธิบาย shrinkwrap แบบภาษาทั่วไปและการตรวจสอบแพ็กเกจระดับ maintainer ดู [npm shrinkwrap](</th/gateway/security/shrinkwrap>)
