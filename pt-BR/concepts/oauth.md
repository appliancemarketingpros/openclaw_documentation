---
title: OAuth
source_url: https://docs.openclaw.ai/pt-BR/concepts/oauth
scraped_at: 2026-05-25
---

OpenClaw é compatível com "autenticação de assinatura" via OAuth para provedores que a oferecem (especialmente **OpenAI Codex (ChatGPT OAuth)**). Para Anthropic, a divisão prática agora é:
...
O OpenAI Codex OAuth é explicitamente compatível para uso em ferramentas externas como OpenClaw. Esta página explica:
...
O OpenClaw também é compatível com **plugins de provedor** que trazem seus próprios fluxos de OAuth ou chave de API. Execute-os via:
...
Para reduzir isso, o OpenClaw trata `auth-profiles.json` como um **coletor de tokens** :
  ...
  * a reutilização de CLI externa é específica do provedor: o Codex CLI pode inicializar um perfil `openai-codex:default` vazio, mas, quando o OpenClaw tem um perfil OAuth local, o token de atualização local é canônico; outras integrações podem permanecer gerenciadas externamente e reler o armazenamento de autenticação da CLI delas
  * caminhos de status e inicialização que já conhecem o conjunto configurado de provedores limitam a descoberta de CLI externa a esse conjunto, para que um armazenamento de login de CLI não relacionado não seja consultado em uma configuração de provedor único
...
Segredos são armazenados nos armazenamentos de autenticação do agente:
  ...
  * Arquivo de compatibilidade legado: `~/.openclaw/agents/<agentId>/agent/auth.json` (entradas estáticas de `api_key` são limpas quando descobertas)
...
Quando um agente secundário não tem perfil de autenticação local, o OpenClaw usa herança com leitura direta do armazenamento do agente padrão/principal. Ele não clona o `auth-profiles.json` do agente principal na leitura. Tokens de atualização OAuth são especialmente sensíveis: fluxos normais de cópia os ignoram por padrão porque alguns provedores rotacionam ou invalidam tokens de atualização após o uso. Configure um login OAuth separado para um agente quando ele precisar de uma conta independente.
...
O OpenClaw voltou a ser compatível com a reutilização da Anthropic Claude CLI. Se você já tiver um login local do Claude no host, o onboarding/configuração pode reutilizá-lo diretamente.
...
Em runtime:
  ...
  * exceção: algumas credenciais de CLI externa permanecem gerenciadas externamente; o OpenClaw relê esses armazenamentos de autenticação de CLI em vez de gastar tokens de atualização copiados. A inicialização do Codex CLI é intencionalmente mais restrita: ela semeia um perfil `openai-codex:default` vazio, então as atualizações de propriedade do OpenClaw mantêm o perfil local como canônico.
