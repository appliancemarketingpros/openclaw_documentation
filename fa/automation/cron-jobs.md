...
source_url: https://docs.openclaw.ai/fa/automation/cron-jobs
...
نشست اصلی در برابر جداافتاده در برابر سفارشی
...
معنای «نشست تازه» برای کارهای جداافتاده
...
OPENCLAW_DOCS_MARKER:accordionOpen:IHRpdGxlPSJob29r2YfYp9uMINmG2q_Yp9i02KrigIzYtNiv2YcgKFBPU1QgL2hvb2tzLzxuYW1l )"> نام‌های hook سفارشی از طریق `hooks.mappings` در پیکربندی حل می‌شوند. نگاشت‌ها می‌توانند payloadهای دلخواه را با قالب‌ها یا تبدیل‌های کد به کنش‌های `wake` یا `agent` تبدیل کنند.
...
به نظر می‌رسد Cron یا Heartbeat مانع چرخش به سبک /new می‌شود
