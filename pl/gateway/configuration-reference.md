...
source_url: https://docs.openclaw.ai/pl/gateway/configuration-reference
...
Uwierzytelnianie: `Authorization: Bearer <token>` lub `x-openclaw-token: <token>`. Tokeny hooków w query string są odrzucane.
...
Starsza konfiguracja mostu (odniesienie historyczne) jsonCopy code
[code]
