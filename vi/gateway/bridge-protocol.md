...
source_url: https://docs.openclaw.ai/vi/gateway/bridge-protocol
...
## Truyền tải
  ...
  * Cổng lắng nghe mặc định trước đây là `18790` (các bản dựng hiện tại không khởi động cầu nối TCP).


Khi TLS được bật, các bản ghi TXT khám phá bao gồm `bridgeTls=1` cùng với `bridgeTlsSha256` như một gợi ý không bí mật. Lưu ý rằng các bản ghi TXT Bonjour/mDNS không được xác thực; máy khách không được xem fingerprint được quảng bá là một pin có thẩm quyền nếu không có chủ ý rõ ràng của người dùng hoặc xác minh ngoài băng tần khác.
...
Trước đây, `hello-ok` trả về `serverName`; các bề mặt Plugin được lưu trữ hiện nay được quảng bá qua `pluginSurfaceUrls`. Canvas/A2UI sử dụng `pluginSurfaceUrls.canvas`; bí danh `canvasHostUrl` đã ngừng dùng không thuộc giao thức đã tái cấu trúc.
...
Các node có thể phát sự kiện `exec.finished` hoặc `exec.denied` để hiển thị hoạt động system.run. Các sự kiện này được ánh xạ thành sự kiện hệ thống trong Gateway. (Các node legacy vẫn có thể phát `exec.started`.)
...
## Cách dùng tailnet trước đây
  ...
  * Gắn cầu nối vào một IP tailnet: `bridge.bind: "tailnet"` trong `~/.openclaw/openclaw.json` (chỉ là lịch sử; `bridge.*` không còn hợp lệ).
...
Cầu nối là **v1 ngầm định** (không thương lượng min/max). Phần này chỉ để tham khảo lịch sử; các máy khách node/operator hiện tại sử dụng WebSocket [Gateway Protocol](</vi/gateway/protocol>).
