...
source_url: https://docs.openclaw.ai/nl/automation
scraped_at: 2026-05-25
---

OpenClaw voert werk op de achtergrond uit via taken, geplande jobs, afgeleide toezeggingen, event hooks en vaste instructies. Deze pagina helpt je het juiste mechanisme te kiezen en te begrijpen hoe ze samenhangen.
...
Toezeggingen zijn opt-in, kortlevende opvolgingsherinneringen. OpenClaw leidt ze af uit normale gesprekken, beperkt ze tot dezelfde agent en hetzelfde kanaal, en levert verschuldigde check-ins via Heartbeat. Exacte door de gebruiker gevraagde herinneringen horen nog steeds bij cron.
...
Interne hooks zijn eventgestuurde scripts die worden geactiveerd door levenscyclusgebeurtenissen van agents (`/new`, `/reset`, `/stop`), sessie-Compaction, Gateway-start en berichtstroom. Ze worden automatisch uit mappen ontdekt en kunnen worden beheerd met `openclaw hooks`. Gebruik voor in-process onderschepping van toolaanroepen [Plugin hooks](</nl/plugins/hooks>).
