---
title: OC 路徑 Plugin
source_url: https://docs.openclaw.ai/zh-TW/plugins/oc-path
...
`onStartup: false` 會讓此 Plugin 離開 Gateway hot path。`onCommands: ["path"]` 會告訴 CLI 在你第一次執行 `openclaw path …` 時才延遲載入此 Plugin，因此從未使用該 verb 的安裝不會付出任何成本。
