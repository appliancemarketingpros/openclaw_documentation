...
source_url: https://docs.openclaw.ai/tr/gateway/sandbox-vs-tool-policy-vs-elevated
...
Pratik kurallar:
  ...
  * `/exec` yalnızca yetkili göndericiler için oturum varsayılanlarını değiştirir; araç erişimi vermez. Sağlayıcı araç anahtarları `provider` (örn. `google-antigravity`) veya `provider/model` (örn. `openai/gpt-5.4`) kabul eder.
...
Kullanılabilir gruplar:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` Salt okunur agent'lar için, sandbox dosya sistemi politikası veya ayrı bir host sınırı salt okunurluk kısıtını zorunlu kılmıyorsa değiştiren dosya sistemi araçlarının yanı sıra `group:runtime` öğesini de reddedin.
