...
source_url: https://docs.openclaw.ai/th/gateway/sandbox-vs-tool-policy-vs-elevated
...
หลักปฏิบัติทั่วไป:
  ...
  * `/exec` เปลี่ยนเฉพาะค่าเริ่มต้นของเซสชันสำหรับผู้ส่งที่ได้รับอนุญาต ไม่ได้ให้สิทธิ์เข้าถึงเครื่องมือ คีย์เครื่องมือของผู้ให้บริการรับได้ทั้ง `provider` (เช่น `google-antigravity`) หรือ `provider/model` (เช่น `openai/gpt-5.4`)
...
กลุ่มที่พร้อมใช้งาน:
  ...
  * `group:fs`: `read`, `write`, `edit`, `apply_patch` สำหรับ agent แบบอ่านอย่างเดียว ให้ปฏิเสธ `group:runtime` รวมถึงเครื่องมือระบบไฟล์ที่แก้ไขได้ เว้นแต่นโยบายระบบไฟล์ของแซนด์บ็อกซ์หรือขอบเขตโฮสต์แยกต่างหากจะบังคับข้อจำกัดอ่านอย่างเดียว
