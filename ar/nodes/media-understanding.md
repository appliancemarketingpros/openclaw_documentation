---
title: فهم الوسائط
source_url: https://docs.openclaw.ai/ar/nodes/media-understanding
...
مراجع `agents.defaults.imageModel` الأساسية/الاحتياطية (للصور فقط). فضّل مراجع `provider/model`. تُؤهَّل المراجع المجردة من إدخالات نماذج المزوّدين القادرة على الصور والمكوَّنة فقط عندما تكون المطابقة فريدة.

* ### واجهات CLI المحلية (للصوت فقط)
...
ترتيب الرجوع الاحتياطي المضمّن:
  ...
  * الصورة: OpenAI → Anthropic → Google → MiniMax → MiniMax Portal → [Z.AI](<http://Z.AI>)
