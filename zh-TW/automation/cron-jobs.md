---
title: 排程任務
source_url: https://docs.openclaw.ai/zh-TW/automation/cron-jobs
...
OPENCLAW_DOCS_MARKER:accordionOpen:IHRpdGxlPSJNYXBwZWQgaG9va3MgKFBPU1QgL2hvb2tzLzxuYW1l )"> 自訂 hook 名稱會透過 config 中的 `hooks.mappings` 解析。Mapping 可以使用 template 或 code transform，將任意 payload 轉換為 `wake` 或 `agent` action。
...
Cron 或 Heartbeat 似乎阻止 /new 樣式輪替
