---
title: Google Chat
source_url: https://docs.openclaw.ai/de/channels/googlechat
...
bashCopy code
[code]# If bound to localhost (127.0.0.1 or 0.0.0.0):tailscale serve --bg --https 8443 http://127.0.0.1:18789 # If bound to Tailscale IP only (e.g., 100.106.161.80):tailscale serve --bg --https 8443 http://100.106.161.80:18789
[/code]

  3. **Machen Sie nur den Webhook-Pfad öffentlich verfügbar:**

bashCopy code
[code]# If bound to localhost (127.0.0.1 or 0.0.0.0):tailscale funnel --bg --set-path /googlechat http://127.0.0.1:18789/googlechat # If bound to Tailscale IP only (e.g., 100.106.161.80):tailscale funnel --bg --set-path /googlechat http://100.106.161.80:18789/googlechat
[/code]

  4. **Autorisieren Sie die Node für Funnel-Zugriff:** Wenn Sie dazu aufgefordert werden, rufen Sie die in der Ausgabe angezeigte Autorisierungs-URL auf, um Funnel für diese Node in Ihrer Tailnet-Richtlinie zu aktivieren.

  5. **Überprüfen Sie die Konfiguration:**

bashCopy code
[code]tailscale serve statustailscale funnel status
[/code]


Ihre öffentliche Webhook-URL lautet: `https://<node-name>.<tailnet>.ts.net/googlechat`

Ihr privates Dashboard bleibt ausschließlich im Tailnet erreichbar: `https://<node-name>.<tailnet>.ts.net:8443/`
...
bashCopy code
[code]openclaw config get channels.googlechat
[/code]

Wenn `"Config path not found"` zurückgegeben wird, fügen Sie die Konfiguration hinzu (siehe Konfigurations-Highlights).

  2. **Plugin nicht aktiviert** : Prüfen Sie den Plugin-Status:

bashCopy code
[code]openclaw plugins list | grep googlechat
[/code]
