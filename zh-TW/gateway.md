---
title: Gateway 作業手冊
source_url: https://docs.openclaw.ai/zh-TW/gateway
scraped_at: 2026-05-25
---

使用此頁面進行 Gateway 服務的第 1 天啟動與第 2 天維運。

[**Deep troubleshooting** 以症狀優先的診斷，提供精確的命令階梯與日誌特徵。 ](</zh-TW/gateway/troubleshooting>) [**Configuration** 任務導向的設定指南 + 完整設定參考。 ](</zh-TW/gateway/configuration>) [**Secrets management** SecretRef 合約、執行階段快照行為，以及遷移/重新載入操作。 ](</zh-TW/gateway/secrets>) [**Secrets plan contract** 精確的 `secrets apply` 目標/路徑規則，以及僅 ref 的 auth-profile 行為。 ](</zh-TW/gateway/secrets-plan-contract>)
...
在可連線的 gateway 下，這會針對每個帳號執行即時頻道探測與選用稽核。 如果 gateway 無法連線，CLI 會退回到僅設定的頻道摘要，而不是即時探測輸出。
...
## 執行階段模型
  ...
  * 單一多工連接埠用於： 
    ...
    * 控制 UI 與 hook
  * 預設繫結模式：`loopback`。
  * 預設需要驗證。共享密鑰設定使用 `gateway.auth.token` / `gateway.auth.password`（或 `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`），非 loopback 的反向代理設定可以使用 `gateway.auth.mode: "trusted-proxy"`。
...
Gateway 啟動會在為非 loopback 繫結播種本機 控制 UI origin 時，使用相同的有效連接埠與繫結。例如，`--bind lan --port 3000` 會在執行階段驗證執行前，播種 `http://localhost:3000` 與 `http://127.0.0.1:3000`。 請將任何遠端瀏覽器 origin，例如 HTTPS proxy URL，明確加入 `gateway.controlUi.allowedOrigins`。
...
`gateway status --deep` 用於額外的服務探索（LaunchDaemons/systemd 系統 unit/schtasks），不是更深入的 RPC 健康探測。

## 多個 gateway（同一主機）

多數安裝應該在每台機器執行一個 gateway。單一 gateway 可以承載多個 agent 與頻道。
...
預期結果：
  ...
  * `gateway status --deep` 可以回報 `Other gateway-like services detected (best effort)` 並在過期的 launchd/systemd/schtasks 安裝仍存在時列印清理提示。
...
首選：Tailscale/VPN。 備援：SSH tunnel。
...
原生 Windows 受管理啟動使用名為 `OpenClaw Gateway` （或具名設定檔的 `OpenClaw Gateway (<profile>)`）的 Scheduled Task。如果 Scheduled Task 建立遭拒，OpenClaw 會退回到每位使用者的 Startup 資料夾 launcher， 指向狀態目錄內的 `gateway.cmd`。
...
使用與 user unit 相同的服務主體，但將它安裝在 `/etc/systemd/system/openclaw-gateway[-<profile>].service` 下，並在你的 `openclaw` binary 位於其他位置時調整 `ExecStart=`。
...
## 通訊協定快速參考（operator 視角）
  ...
  * 常見事件包含 `connect.challenge`、`agent`、`chat`、 `session.message`、`session.tool`、`sessions.changed`、`presence`、`tick`、 `health`、`heartbeat`、配對/核准生命週期事件，以及 `shutdown`。
