...
source_url: https://docs.openclaw.ai/ko/tools/multi-agent-sandbox-tools
scraped_at: 2026-05-25
---

다중 에이전트 설정의 각 에이전트는 전역 샌드박스 및 도구 정책을 재정의할 수 있습니다. 이 페이지에서는 에이전트별 구성, 우선순위 규칙, 예시를 다룹니다.

[**샌드박싱** 백엔드와 모드 — 전체 샌드박스 참조입니다. ](</ko/gateway/sandboxing>) [**샌드박스 vs 도구 정책 vs 승격** "왜 이것이 차단되나요?"를 디버그합니다. ](</ko/gateway/sandbox-vs-tool-policy-vs-elevated>) [**승격 모드** 신뢰할 수 있는 발신자를 위한 승격된 exec입니다. ](</ko/tools/elevated>)

* * *

## 구성 예시

예시 1: 개인용 + 제한된 가족 에이전트 jsonCopy code
[code]
...
예시 2: 공유 샌드박스를 사용하는 업무 에이전트 jsonCopy code
[code]
...
예시 2b: 전역 코딩 프로필 + 메시징 전용 에이전트 jsonCopy code
[code]
...
예시 3: 에이전트별 서로 다른 샌드박스 모드 jsonCopy code
[code]
