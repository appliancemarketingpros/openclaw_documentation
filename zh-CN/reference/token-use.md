---
title: Token 用量和费用
source_url: https://docs.openclaw.ai/zh-CN/reference/token-use
...
其他表面：
  ...
  * **CLI：**`openclaw status --usage` 和 `openclaw channels list` 会显示 规范化的提供商配额窗口（`X% left`，不是逐响应成本）。 当前用量窗口提供商：Anthropic、GitHub Copilot、Gemini CLI、 OpenAI Codex、MiniMax、Xiaomi 和 [z.ai](<http://z.ai>)。
