...
If Caddy terminates TLS for the Gateway, configure trusted proxy settings for the exact proxy path rather than disabling auth checks globally. See [Trusted proxy auth](</gateway/trusted-proxy-auth>).
...
## Updates and backups
  ...
  * Run `openclaw doctor` after major updates to catch config migrations and service warnings.
...
## Troubleshooting
  ...
  * Files are root-owned after restore: repair the mounted volumes so the container user can write `/home/node/.openclaw` and `/workspace`.
