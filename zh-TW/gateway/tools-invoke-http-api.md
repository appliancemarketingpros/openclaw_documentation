---
title: 工具呼叫 API
source_url: https://docs.openclaw.ai/zh-TW/gateway/tools-invoke-http-api
...
注意事項：
  ...
  * 當 `gateway.auth.mode="trusted-proxy"` 時，HTTP 請求必須來自 已設定的受信任 Proxy 來源；同主機 loopback Proxy 需要明確設定 `gateway.auth.trustedProxy.allowLoopback = true`。
...
驗證矩陣：
  ...
  * `gateway.auth.mode="token"` 或 `"password"` \+ `Authorization: Bearer ...`
    ...
    * 還原完整的預設操作者範圍集合： `operator.admin`、`operator.approvals`、`operator.pairing`、 `operator.read`、`operator.talk.secrets`、`operator.write`
