...
source_url: https://docs.openclaw.ai/ar/gateway/troubleshooting
...
يعامل OpenClaw كل جذر Skills كحد احتواء. يتم تخطي الرابط الرمزي ضمن `~/.agents/skills` أو `<workspace>/.agents/skills` أو `<workspace>/skills` أو `~/.openclaw/skills` عندما يُحل هدفه الحقيقي خارج ذلك الجذر ما لم يكن الهدف موثوقًا به صراحة.
...
إذا كان الهدف مقصودًا، فاضبط كلًا من جذر Skill المباشر وهدف الرابط الرمزي المسموح به:
...
ثم ابدأ جلسة جديدة أو انتظر محدّث Skills حتى ينعشها. أعد تشغيل Gateway إذا كانت العملية الجارية أسبق من تغيير الإعدادات.

لا تستخدم أهدافًا واسعة مثل `~` أو `/` أو مجلد مشروع كامل متزامن. أبقِ `allowSymlinkTargets` محدودًا بجذر Skill الحقيقي الذي يحتوي على أدلة `SKILL.md` موثوقة.
...
استخدم هذا عندما يفشل بدء Gateway مع `Invalid config` أو عندما تقول سجلات إعادة التحميل الساخن إنها تخطت تعديلا غير صالح.
...
1\. تغيّر سلوك المصادقة وتجاوز عنوان URL bashCopy code
[code]
...
2\. حواجز الربط والمصادقة أكثر صرامة bashCopy code
[code]
...
3\. تغيّرت حالة الاقتران وهوية الجهاز bashCopy code
[code]
