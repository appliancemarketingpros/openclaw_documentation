---
title: 구성 — 채널
source_url: https://docs.openclaw.ai/ko/gateway/config-channels
scraped_at: 2026-05-25
---

`channels.*` 아래의 채널별 구성 키입니다. DM 및 그룹 접근, 다중 계정 설정, 멘션 게이팅, Slack, Discord, Telegram, WhatsApp, Matrix, iMessage 및 기타 번들 채널 Plugin의 채널별 키를 다룹니다.

에이전트, 도구, Gateway 런타임 및 기타 최상위 키는 [구성 참조](</ko/gateway/configuration-reference>)를 참조하세요.
...
특정 채널 ID를 모델에 고정하려면 `channels.modelByChannel`을 사용하세요. 값은 `provider/model` 또는 구성된 모델 별칭을 허용합니다. 채널 매핑은 세션에 이미 모델 재정의가 없을 때 적용됩니다(예: `/model`로 설정한 경우).
...
제공자 전반에 걸친 공유 그룹 정책 및 Heartbeat 동작에는 `channels.defaults`를 사용하세요.
...
[/code]
  ...
  * `channels.defaults.groupPolicy`: 제공자 수준 `groupPolicy`가 설정되지 않았을 때의 대체 그룹 정책입니다.
  * `channels.defaults.contextVisibility`: 모든 채널의 기본 보조 컨텍스트 표시 모드입니다. 값: `all`(기본값, 인용/스레드/기록 컨텍스트 모두 포함), `allowlist`(허용 목록에 있는 발신자의 컨텍스트만 포함), `allowlist_quote`(허용 목록과 같지만 명시적 인용/답장 컨텍스트 유지). 채널별 재정의: `channels.<channel>.contextVisibility`.
...
다중 계정 WhatsApp json5Copy code
[code]
    {channels: {  whatsapp: {    accounts: {      default: {},      personal: {},      biz: {        // authDir: "~/.openclaw/credentials/whatsapp/biz",      },    },  },},}
[/code]

  * 아웃바운드 명령은 계정 `default`가 있으면 기본적으로 해당 계정을 사용하고, 없으면 처음 구성된 계정 ID(정렬 기준)를 사용합니다.
...
[/code]
  ...
  * 봇 토큰: `channels.telegram.botToken` 또는 `channels.telegram.tokenFile`(일반 파일만 가능, 심볼릭 링크는 거부됨)이며, 기본 계정의 대체값으로 `TELEGRAM_BOT_TOKEN`을 사용합니다.
  ...
  * 다중 계정 설정(계정 ID 2개 이상)에서는 대체 라우팅을 피하려면 명시적 기본값(`channels.telegram.defaultAccount` 또는 `channels.telegram.accounts.default`)을 설정하세요. 이것이 없거나 유효하지 않으면 `openclaw doctor`가 경고합니다.
  * `configWrites: false`는 Telegram에서 시작된 구성 쓰기(슈퍼그룹 ID 마이그레이션, `/config set|unset`)를 차단합니다.
  * `type: "acp"`가 있는 최상위 `bindings[]` 항목은 포럼 주제의 영구 ACP 바인딩을 구성합니다(`match.peer.id`에서 표준 `chatId:topic:topicId` 사용). 필드 의미는 [ACP 에이전트](</ko/tools/acp-agents#persistent-channel-bindings>)에서 공유됩니다.
...
[/code]
  ...
  * 토큰: 기본 계정의 fallback으로 `DISCORD_BOT_TOKEN`을 사용하는 `channels.discord.token`.
  * 명시적 Discord `token`을 제공하는 직접 outbound 호출은 해당 호출에 그 토큰을 사용하며, 계정 재시도/정책 설정은 여전히 활성 런타임 snapshot에서 선택된 계정에서 가져옵니다.
  * 선택 사항인 `channels.discord.defaultAccount`는 구성된 계정 id와 일치할 때 기본 계정 선택을 재정의합니다.
  * 전달 대상에는 `user:<id>`(DM) 또는 `channel:<id>`(guild channel)를 사용하세요. 단독 숫자 ID는 거부됩니다.
  * Guild slug는 소문자이며 공백은 `-`로 대체됩니다. channel key는 slug 처리된 이름을 사용합니다(`#` 없음). guild ID를 권장합니다.
  * 봇이 작성한 메시지는 기본적으로 무시됩니다. `allowBots: true`는 이를 활성화합니다. 봇을 mention하는 봇 메시지만 허용하려면 `allowBots: "mentions"`를 사용하세요(자신의 메시지는 계속 필터링됨).
  * `channels.discord.guilds.<id>.ignoreOtherMentions`(및 channel override)는 봇이 아닌 다른 사용자나 role을 mention하는 메시지를 드롭합니다(@everyone/@here 제외).
  * `channels.discord.mentionAliases`는 전송 전에 안정적인 outbound `@handle` 텍스트를 Discord 사용자 ID에 매핑하므로, 일시적 디렉터리 캐시가 비어 있어도 알려진 팀원을 결정론적으로 mention할 수 있습니다. 계정별 override는 `channels.discord.accounts.<accountId>.mentionAliases` 아래에 있습니다.
  * `maxLinesPerMessage`(기본값 17)는 2000자 미만이어도 긴 메시지를 분할합니다.
  * `channels.discord.threadBindings`는 Discord thread-bound 라우팅을 제어합니다. 
    * `enabled`: thread-bound 세션 기능(`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`, bound 전달/라우팅)에 대한 Discord override
    * `idleHours`: 비활성 auto-unfocus 시간 단위 Discord override(`0`은 비활성화)
    * `maxAgeHours`: hard max age 시간 단위 Discord override(`0`은 비활성화)
    * `spawnSessions`: `sessions_spawn({ thread: true })` 및 ACP thread-spawn 자동 thread 생성/binding을 위한 switch(기본값: `true`)
    * `defaultSpawnContext`: thread-bound spawn을 위한 native subagent context(기본값 `"fork"`)
  * `type: "acp"`가 있는 최상위 `bindings[]` 항목은 channel 및 thread용 persistent ACP binding을 구성합니다(`match.peer.id`에 channel/thread id 사용). 필드 의미 체계는 [ACP Agents](</ko/tools/acp-agents#persistent-channel-bindings>)에서 공유됩니다.
  * `channels.discord.ui.components.accentColor`는 Discord components v2 container의 accent color를 설정합니다.
  * `channels.discord.voice`는 Discord voice channel 대화와 선택적 auto-join + LLM + TTS override를 활성화합니다. 텍스트 전용 Discord config는 기본적으로 voice가 꺼져 있습니다. 사용하려면 `channels.discord.voice.enabled=true`를 설정하세요.
  * `channels.discord.voice.model`은 Discord voice channel 응답에 사용되는 LLM model을 선택적으로 재정의합니다.
  * `channels.discord.voice.daveEncryption` 및 `channels.discord.voice.decryptionFailureTolerance`는 `@discordjs/voice` DAVE option으로 전달됩니다(기본값은 `true` 및 `24`).
  * `channels.discord.voice.connectTimeoutMs`는 `/vc join` 및 auto-join 시도의 초기 `@discordjs/voice` Ready 대기를 제어합니다(기본값 `30000`).
  * `channels.discord.voice.reconnectGraceMs`는 연결이 끊긴 voice session이 OpenClaw가 이를 제거하기 전에 reconnect signalling에 진입할 수 있는 시간을 제어합니다(기본값 `15000`).
  * Discord voice playback은 다른 사용자의 speaking-start event로 중단되지 않습니다. feedback loop를 피하기 위해 OpenClaw는 TTS 재생 중 새 voice capture를 무시합니다.
  * OpenClaw는 반복되는 decrypt failure 후 voice session을 나갔다가 다시 참여하여 voice receive recovery도 시도합니다.
  * `channels.discord.streaming`은 canonical stream mode key입니다. Discord의 기본값은 `streaming.mode: "progress"`이므로 tool/work 진행 상황이 하나의 편집된 preview message에 표시됩니다. 비활성화하려면 `streaming.mode: "off"`를 설정하세요. 기존 `streamMode` 및 boolean `streaming` 값은 runtime alias로 남아 있습니다. 저장된 config를 다시 쓰려면 `openclaw doctor --fix`를 실행하세요.
  * `channels.discord.autoPresence`는 runtime availability를 bot presence에 매핑하고(healthy => online, degraded => idle, exhausted => dnd) 선택적 status text override를 허용합니다.
  * `channels.discord.dangerouslyAllowNameMatching`은 mutable name/tag matching을 다시 활성화합니다(break-glass compatibility mode).
  * `channels.discord.execApprovals`: Discord-native exec approval 전달 및 approver authorization. 
    * `enabled`: `true`, `false` 또는 `"auto"`(기본값). auto mode에서는 `approvers` 또는 `commands.ownerAllowFrom`에서 approver를 확인할 수 있을 때 exec approval이 활성화됩니다.
    * `approvers`: exec request를 승인할 수 있는 Discord user ID. 생략하면 `commands.ownerAllowFrom`으로 fallback합니다.
    * `agentFilter`: 선택적 agent ID allowlist. 모든 agent에 대한 approval을 forward하려면 생략하세요.
    * `sessionFilter`: 선택적 session key pattern(substring 또는 regex).
    * `target`: approval prompt를 보낼 위치. `"dm"`(기본값)은 approver DM으로 보내고, `"channel"`은 원본 channel로 보내며, `"both"`는 둘 다로 보냅니다. target에 `"channel"`이 포함되면 button은 확인된 approver만 사용할 수 있습니다.
    * `cleanupAfterResolve`: `true`이면 approval, denial 또는 timeout 후 approval DM을 삭제합니다.


**Reaction notification mode:** `off`(없음), `own`(bot의 메시지, 기본값), `all`(모든 메시지), `allowlist`(모든 메시지에서 `guilds.<id>.users` 기준).
...
[/code]
  ...
  * 서비스 계정 JSON: inline(`serviceAccount`) 또는 file-based(`serviceAccountFile`).
  ...
  * `channels.googlechat.dangerouslyAllowNameMatching`은 mutable email principal matching을 다시 활성화합니다(break-glass compatibility mode).
...
[/code]
  ...
  * **Socket mode** 에는 `botToken`과 `appToken`이 모두 필요합니다(기본 계정 env fallback은 `SLACK_BOT_TOKEN` \+ `SLACK_APP_TOKEN`).
  * **HTTP mode** 에는 `botToken`과 `signingSecret`(root 또는 per-account)이 필요합니다.
  * `socketMode`는 Slack SDK Socket Mode transport tuning을 public Bolt receiver API로 전달합니다. ping/pong timeout 또는 stale websocket 동작을 조사할 때만 사용하세요.
  * `botToken`, `appToken`, `signingSecret`, `userToken`은 plaintext string 또는 SecretRef object를 허용합니다.
  * Slack account snapshot은 `botTokenSource`, `botTokenStatus`, `appTokenStatus`, HTTP mode의 `signingSecretStatus` 같은 credential별 source/status field를 노출합니다. `configured_unavailable`은 계정이 SecretRef를 통해 구성되었지만 현재 command/runtime path가 secret value를 resolve할 수 없었음을 의미합니다.
  * `configWrites: false`는 Slack에서 시작된 config write를 차단합니다.
  * 선택 사항인 `channels.slack.defaultAccount`는 구성된 계정 id와 일치할 때 기본 계정 선택을 재정의합니다.
  * `channels.slack.streaming.mode`는 canonical Slack stream mode key입니다. `channels.slack.streaming.nativeTransport`는 Slack의 native streaming transport를 제어합니다. 기존 `streamMode`, boolean `streaming`, `nativeStreaming` 값은 runtime alias로 남아 있습니다. 저장된 config를 다시 쓰려면 `openclaw doctor --fix`를 실행하세요.
  * `unfurlLinks`와 `unfurlMedia`는 bot reply를 위해 Slack의 `chat.postMessage` link 및 media unfurl boolean을 전달합니다. Slack의 기본 동작을 유지하려면 생략하세요. 한 계정에 대해 최상위 기본값을 재정의하려면 `channels.slack.accounts.<accountId>`에 설정하세요.
  * 전달 대상에는 `user:<id>`(DM) 또는 `channel:<id>`를 사용하세요.


**Reaction notification mode:** `off`, `own`(기본값), `all`, `allowlist`(`reactionAllowlist` 기준).

**Thread session isolation:** `thread.historyScope`는 thread별(기본값)이거나 channel 전체에서 공유됩니다. `thread.inheritParent`는 parent channel transcript를 새 thread로 복사합니다.

  * Slack native streaming과 Slack assistant-style "is typing..." thread status에는 reply thread target이 필요합니다. 최상위 DM은 기본적으로 off-thread이므로 thread-style native stream/status preview를 표시하는 대신 Slack draft post-and-edit preview를 통해 계속 stream할 수 있습니다.
  * `typingReaction`은 reply가 실행되는 동안 inbound Slack message에 임시 reaction을 추가한 다음 완료 시 제거합니다. `"hourglass_flowing_sand"` 같은 Slack emoji shortcode를 사용하세요.
  * `channels.slack.execApprovals`: Slack-native exec approval 전달 및 approver authorization. Discord와 같은 schema입니다: `enabled`(`true`/`false`/`"auto"`), `approvers`(Slack user ID), `agentFilter`, `sessionFilter`, `target`(`"dm"`, `"channel"` 또는 `"both"`).

Action group | 기본값 | 참고  
---|---|---  
reactions | 활성화됨 | reaction 추가 + reaction 목록  
messages | 활성화됨 | 읽기/보내기/편집/삭제  
pins | 활성화됨 | 고정/고정 해제/목록  
memberInfo | 활성화됨 | member 정보  
emojiList | 활성화됨 | custom emoji 목록  
  
### Mattermost

Mattermost는 현재 OpenClaw 릴리스에서 bundled Plugin으로 제공됩니다. 이전 또는 custom build는 `openclaw plugins install @openclaw/mattermost`로 현재 npm package를 설치할 수 있습니다. version을 pin하기 전에 [npmjs.com/package/@openclaw/mattermost](<https://www.npmjs.com/package/@openclaw/mattermost>)에서 현재 dist-tag를 확인하세요.
...
Mattermost 네이티브 명령이 활성화된 경우:
  ...
  * `channels.mattermost.configWrites`: Mattermost에서 시작한 config 쓰기를 허용하거나 거부합니다.
...
json5Copy code
[code]
    {  channels: {    signal: {      enabled: true,      account: "+15555550123", // optional account binding      dmPolicy: "pairing",      allowFrom: ["+15551234567", "uuid:123e4567-e89b-12d3-a456-426614174000"],      configWrites: true,      reactionNotifications: "own", // off | own | all | allowlist      reactionAllowlist: ["+15551234567", "uuid:123e4567-e89b-12d3-a456-426614174000"],      historyLimit: 50,    },  },}
[/code]

**반응 알림 모드:** `off`, `own`(기본값), `all`, `allowlist`(`reactionAllowlist`에서).

  * `channels.signal.account`: 채널 시작을 특정 Signal 계정 ID에 고정합니다.
  * `channels.signal.configWrites`: Signal에서 시작한 config 쓰기를 허용하거나 거부합니다.
  * 선택적 `channels.signal.defaultAccount`는 구성된 계정 id와 일치할 때 기본 계정 선택을 재정의합니다.


### iMessage

OpenClaw는 `imsg rpc`(stdio를 통한 JSON-RPC)를 생성합니다. 데몬이나 포트는 필요 없습니다. 호스트가 Messages 데이터베이스와 자동화 권한을 부여할 수 있는 경우, 새 OpenClaw iMessage 설정에 선호되는 경로입니다.

BlueBubbles 지원은 제거되었습니다. `channels.bluebubbles`는 현재 OpenClaw에서 지원되는 런타임 config 표면이 아닙니다. 이전 config를 `channels.imessage`로 마이그레이션하세요. 짧은 버전은 [BlueBubbles 제거 및 imsg iMessage 경로](</ko/announcements/bluebubbles-imessage>)를, 전체 번역 표는 [BlueBubbles에서 이전하기](</ko/channels/imessage-from-bluebubbles>)를 사용하세요.

Gateway가 로그인된 Messages Mac에서 실행 중이 아니라면 `channels.imessage.enabled=true`를 유지하고 `channels.imessage.cliPath`를 해당 Mac에서 `imsg "$@"`를 실행하는 SSH 래퍼로 설정하세요. 기본 로컬 `imsg` 경로는 macOS 전용입니다.
...
[/code]
  ...
  * SCP는 엄격한 호스트 키 검사를 사용하므로 릴레이 호스트 키가 `~/.ssh/known_hosts`에 이미 있는지 확인하세요.

  * `channels.imessage.configWrites`: iMessage에서 시작한 config 쓰기를 허용하거나 거부합니다.

  * `channels.imessage.actions.*`: `imsg status` / `openclaw channels status --probe`로도 게이트되는 비공개 API 작업을 활성화합니다.

  * `channels.imessage.includeAttachments`는 기본적으로 꺼져 있습니다. 에이전트 턴에서 인바운드 미디어를 기대하기 전에 `true`로 설정하세요.

  * `channels.imessage.catchup.enabled`: Gateway가 다운된 동안 도착한 인바운드 메시지 재생에 옵트인합니다.

  * `channels.imessage.groups`: 그룹 레지스트리 및 그룹별 설정입니다. `groupPolicy: "allowlist"`를 사용하는 경우 그룹 메시지가 레지스트리 게이트를 통과할 수 있도록 명시적 `chat_id` 키 또는 `"*"` 와일드카드 항목을 구성하세요.

  * `type: "acp"`가 있는 최상위 `bindings[]` 항목은 iMessage 대화를 지속 ACP 세션에 바인딩할 수 있습니다. `match.peer.id`에 정규화된 핸들 또는 명시적 채팅 대상(`chat_id:*`, `chat_guid:*`, `chat_identifier:*`)을 사용하세요. 공유 필드 의미: [ACP 에이전트](</ko/tools/acp-agents#persistent-channel-bindings>).


iMessage SSH 래퍼 예시 bashCopy code
[code]
...
[/code]
  ...
  * `channels.matrix.proxy`는 명시적 HTTP(S) 프록시를 통해 Matrix HTTP 트래픽을 라우팅합니다. 이름이 지정된 계정은 `channels.matrix.accounts.<id>.proxy`로 이를 재정의할 수 있습니다.
  ...
  * `channels.matrix.dm.sessionScope`는 Matrix DM이 세션으로 그룹화되는 방식을 제어합니다. `per-user`(기본값)는 라우팅된 피어별로 공유하고, `per-room`은 각 DM 방을 분리합니다.
...
json5Copy code
[code]
    {  channels: {    msteams: {      enabled: true,      configWrites: true,      // appId, appPassword, tenantId, webhook, team/channel policies:      // see /channels/msteams    },  },}
[/code]
...
[/code]
  ...
  * 단일 계정 최상위 채널 config를 유지한 상태에서 `openclaw channels add`(또는 채널 온보딩)를 통해 기본이 아닌 계정을 추가하면, OpenClaw는 원래 계정이 계속 작동하도록 계정 범위의 최상위 단일 계정 값을 먼저 채널 계정 맵으로 승격합니다. 대부분의 채널은 이를 `channels.<channel>.accounts.default`로 이동합니다. Matrix는 대신 기존의 일치하는 이름 지정/기본 대상을 보존할 수 있습니다.
  * 기존 채널 전용 바인딩(`accountId` 없음)은 기본 계정과 계속 일치하며, 계정 범위 바인딩은 선택 사항으로 유지됩니다.
  * `openclaw doctor --fix`도 계정 범위의 최상위 단일 계정 값을 해당 채널에 대해 선택된 승격 계정으로 이동하여 혼합 형태를 복구합니다. 대부분의 채널은 `accounts.default`를 사용합니다. Matrix는 대신 기존의 일치하는 이름 지정/기본 대상을 보존할 수 있습니다.


### 기타 Plugin 채널

많은 Plugin 채널은 `channels.<id>`로 구성되며 전용 채널 페이지에 문서화되어 있습니다(예: Feishu, Matrix, LINE, Nostr, Zalo, Nextcloud Talk, Synology Chat, Twitch). 전체 채널 색인을 참조하세요: [채널](</ko/channels>).

### 그룹 채팅 멘션 게이트

그룹 메시지는 기본적으로 **멘션 필요**(메타데이터 멘션 또는 안전한 정규식 패턴)입니다. WhatsApp, Telegram, Discord, Google Chat, iMessage 그룹 채팅에 적용됩니다.

표시되는 응답은 별도로 제어됩니다. 그룹/채널 방은 기본적으로 `messages.groupChat.visibleReplies: "message_tool"`을 사용합니다. OpenClaw는 여전히 해당 턴을 처리하지만, 일반 최종 응답은 비공개로 유지되며 방에 표시되는 출력에는 `message(action=send)`가 필요합니다. 일반 응답이 방에 다시 게시되는 레거시 동작을 원할 때만 `"automatic"`으로 설정하세요. 동일한 도구 전용 표시 응답 동작을 다이렉트 채팅에도 적용하려면 `messages.visibleReplies: "message_tool"`을 설정하세요. Codex 하네스도 설정되지 않은 다이렉트 채팅 기본값으로 이 도구 전용 동작을 사용합니다.

도구 전용 표시 응답에는 도구를 안정적으로 호출하는 모델/런타임이 필요합니다. 세션 로그에 `didSendViaMessagingTool: false`인 어시스턴트 텍스트가 표시되면, 모델이 메시지 도구를 호출하는 대신 비공개 최종 답변을 생성한 것입니다. 해당 채널에 더 강력한 도구 호출 모델로 전환하거나, 레거시 표시 최종 응답을 복원하려면 `messages.groupChat.visibleReplies: "automatic"`을 설정하세요.

활성 도구 정책에서 메시지 도구를 사용할 수 없는 경우, OpenClaw는 응답을 조용히 억제하는 대신 자동 표시 응답으로 폴백합니다. `openclaw doctor`는 이 불일치에 대해 경고합니다.
...
json5Copy code
[code]
    {  messages: {    visibleReplies: "automatic", // global default for direct/source chats; Codex harness defaults unset direct chats to message_tool    groupChat: {      historyLimit: 50,      visibleReplies: "message_tool", // default; use "automatic" for legacy final replies    },  },  agents: {    list: [{ id: "main", groupChat: { mentionPatterns: ["@openclaw", "openclaw"] } }],  },}
[/code]
...
`messages.visibleReplies`는 전역 소스 턴 기본값입니다. `messages.groupChat.visibleReplies`는 그룹/채널 소스 턴에서 이를 재정의합니다. `messages.visibleReplies`가 설정되지 않은 경우, 하네스는 자체 다이렉트/소스 기본값을 제공할 수 있습니다. Codex 하네스는 기본값으로 `message_tool`을 사용합니다. 채널 허용 목록과 멘션 게이팅은 여전히 턴을 처리할지 결정합니다.
...
Command details
  ...
  * `channels.<provider>.commands.nativeSkills`로 채널별 네이티브 skill 등록을 재정의합니다.
  * `channels.telegram.customCommands`는 추가 Telegram 봇 메뉴 항목을 추가합니다.
  * `bash: true`는 호스트 셸에 대해 `! <cmd>`를 활성화합니다. `tools.elevated.enabled`와 `tools.elevated.allowFrom.<channel>`에 있는 발신자가 필요합니다.
  ...
  * `allowFrom`은 제공자별입니다. 설정된 경우 이것이 **유일한** 권한 부여 소스입니다(채널 허용 목록/페어링과 `useAccessGroups`는 무시됨).
