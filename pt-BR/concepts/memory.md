...
source_url: https://docs.openclaw.ai/pt-BR/concepts/memory
...
[**Integrado (padrão)** Baseado em SQLite. Funciona imediatamente com busca por palavra-chave, similaridade vetorial e busca híbrida. Sem dependências extras. ](</pt-BR/concepts/memory-builtin>) [**QMD** Sidecar local-first com reranking, expansão de consulta e capacidade de indexar diretórios fora do workspace. ](</pt-BR/concepts/memory-qmd>) [**Honcho** Memória cross-session nativa de IA com modelagem de usuário, busca semântica e consciência multiagente. Instalação de Plugin. ](</pt-BR/concepts/memory-honcho>) [**LanceDB** Memória integrada baseada em LanceDB com embeddings compatíveis com OpenAI, recuperação automática, captura automática e suporte a embeddings locais do Ollama. ](</pt-BR/plugins/memory-lancedb>)

## Camada de wiki de conhecimento

[**Memory Wiki** Compila memória durável em um cofre wiki rico em proveniência com afirmações, dashboards, modo ponte e fluxos de trabalho compatíveis com Obsidian. ](</pt-BR/plugins/memory-wiki>)
...
bashCopy code
[code]
    openclaw memory status          # Verificar status do índice e provedoropenclaw memory search "query"  # Buscar pela linha de comandoopenclaw memory index --force   # Reconstruir o índice
[/code]
