---
title: Netwerkproxy
source_url: https://docs.openclaw.ai/nl/security/network-proxy
...
## Limieten
  ...
  * Gateway-loopbackverkeer voor het control-plane gebruikt standaard een directe lokale bypass via `proxy.loopbackMode: "gateway-only"`. OpenClaw implementeert die bypass door de actieve Gateway-loopbackautoriteit te registreren in de beheerde `global-agent`-`NO_PROXY`-controller. Operators kunnen `proxy.loopbackMode: "proxy"` instellen om Gateway-loopbackverkeer via de beheerde proxy te verzenden, of `proxy.loopbackMode: "block"` om loopback-Gateway-verbindingen te weigeren. Zie Gateway-loopbackmodus voor de kanttekening bij externe proxy's.
