---
title: Anthropic
source_url: https://docs.openclaw.ai/vi/providers/anthropic
...
Các tham chiếu mô hình cũ `claude-cli/claude-opus-4-7` vẫn hoạt động để tương thích, nhưng cấu hình mới nên giữ phần chọn nhà cung cấp/mô hình là `anthropic/*` và đặt backend thực thi trong chính sách runtime của nhà cung cấp/mô hình.
...
Plugin Anthropic đi kèm đăng ký khả năng hiểu hình ảnh và PDF. OpenClaw tự động phân giải năng lực media từ xác thực Anthropic đã cấu hình — không cần cấu hình bổ sung.
...
Khi hình ảnh hoặc PDF được đính kèm vào một cuộc trò chuyện, OpenClaw tự động định tuyến nó qua nhà cung cấp hiểu media của Anthropic.
...
`params.context1m: true` cũng áp dụng cho backend Claude CLI (`claude-cli/*`) với các mô hình Opus và Sonnet đủ điều kiện, mở rộng cửa sổ ngữ cảnh runtime cho các phiên CLI đó để khớp hành vi API trực tiếp.
...
[**Model selection** Chọn nhà cung cấp, tham chiếu mô hình và hành vi chuyển dự phòng. ](</vi/concepts/model-providers>) [**CLI backends** Thiết lập backend Claude CLI và chi tiết runtime. ](</vi/gateway/cli-backends>) [**Prompt caching** Cách lưu bộ nhớ đệm prompt hoạt động trên các nhà cung cấp. ](</vi/reference/prompt-caching>) [**OAuth and auth** Chi tiết xác thực và quy tắc tái sử dụng thông tin đăng nhập. ](</vi/gateway/authentication>)
