...
JS が多用されているサイトやログイン保護されたページには、代わりに [Webブラウザー](</ja-JP/tools/browser>) を使用してください。
...
Readability 抽出が失敗した場合、`web_fetch` は bot 回避とより優れた抽出のために [Firecrawl](</ja-JP/tools/firecrawl>) へフォールバックできます。
