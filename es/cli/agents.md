---
title: Agentes
source_url: https://docs.openclaw.ai/es/cli/agents
...
Notas:
  ...
  * En modo interactivo, la siembra de autenticación copia solo perfiles estáticos portables (`api_key` y `token` estático de forma predeterminada). Los perfiles de token de actualización OAuth siguen disponibles solo mediante herencia de lectura desde el almacén real del agente `main`. Si el agente predeterminado configurado no es `main`, inicia sesión por separado para los perfiles OAuth en el nuevo agente.
