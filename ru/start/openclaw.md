...
source_url: https://docs.openclaw.ai/ru/start/openclaw
...
Полная схема рабочего пространства + руководство по резервному копированию: [Рабочее пространство агента](</ru/concepts/agent-workspace>) Рабочий процесс памяти: [Память](</ru/concepts/memory>)
...
По умолчанию OpenClaw запускает Heartbeat каждые 30 минут с промптом: `Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.` Задайте `agents.defaults.heartbeat.every: "0m"`, чтобы отключить.
