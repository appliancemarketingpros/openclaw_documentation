---
title: Diagnostic
source_url: https://docs.openclaw.ai/fr/gateway/doctor
...
Doctor avertit aussi lorsque `plugins.allow` n’est pas vide et que la politique d’outils utilise des entrées d’outils génériques ou appartenant à un plugin. `tools.allow: ["*"]` ne correspond qu’aux outils provenant des plugins qui se chargent réellement ; il ne contourne pas la liste d’autorisation exclusive des plugins. Doctor écrit `plugins.bundledDiscovery: "compat"` pour les configurations héritées de liste d’autorisation migrées afin de préserver le comportement existant des fournisseurs groupés, puis pointe vers le réglage plus strict `"allowlist"`.
...
2c. Migration du navigateur et préparation à Chrome MCP
...
3\. Migrations d’état héritées (agencement disque)
...
3a. Migrations d’anciens manifests de Plugins
...
3b. Migrations d’anciens magasins cron
...
3d. Réparation de branche de transcription de session
...
4\. Contrôles d’intégrité d’état (persistance de session, routage et sécurité)
...
7b. Nettoyage d’installation de Plugin
...
8c. Appairage d’appareils et dérive d’authentification
...
12b. Réparations en lecture seule compatibles avec SecretRef
...
13\. Vérification d’intégrité du Gateway + redémarrage
...
16\. Diagnostics d’exécution + port du Gateway
