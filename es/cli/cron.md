---
title: Cron
source_url: https://docs.openclaw.ai/es/cli/cron
...
Nota: las ejecuciones Cron aisladas tratan los fallos de agente a nivel de ejecución como errores del trabajo incluso cuando no se produce una carga útil de respuesta, por lo que los fallos de modelo/proveedor siguen incrementando los contadores de error y activan notificaciones de fallo.

Si una ejecución aislada agota el tiempo antes de la primera solicitud al modelo, `openclaw cron show` y `openclaw cron runs` incluyen un error específico de fase como `setup timed out before runner start` o `stalled before first model call (last phase: context-engine)`. Para proveedores respaldados por CLI, el watchdog previo al modelo permanece activo hasta que comienza el turno de la CLI externa, por lo que los bloqueos en búsqueda de sesión, hook, autenticación, prompt y configuración de CLI se informan como fallos Cron previos al modelo.
