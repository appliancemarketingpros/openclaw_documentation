...
Message presentation is OpenClaw’s shared contract for rich outbound chat UI. It lets agents, CLI commands, approval flows, and plugins describe the message intent once, while each channel plugin renders the best native shape it can. Use presentation for portable message UI:
...
Button semantics:
  ...
  * `value` is an application action value routed back through the channel’s existing interaction path when the channel supports clickable controls.
...
When a `ReplyPayload` or message action includes `presentation`, core:
  ...
  7. Applies delivery metadata such as `delivery.pin` after the first successful sent message.
...
Pinning is delivery behavior, not presentation. Use `delivery.pin` instead of provider-native fields such as `channelData.telegram.pin`. Semantics:
...
Plugin author checklist
  ...
  * Add fallback tests for unsupported buttons, selects, URL buttons, title/text duplication, and mixed `message` plus `presentation` sends.
...
[Memory wiki](</plugins/memory-wiki>)[Skill workshop plugin](</plugins/skill-workshop>)
