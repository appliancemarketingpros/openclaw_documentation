...
bashCopy code
[code]
    ollama list  # インストール済みのものを確認ollama pull gemma4ollama pull gpt-oss:20bollama pull llama3.3     # または別のモデル
[/code]
...
bashCopy code
[code]
    # Ollama が実行中か確認ps aux | grep ollama # または Ollama を再起動ollama serve
[/code]
...
[**モデルプロバイダー** すべてのプロバイダー、モデル参照、フェイルオーバー動作の概要。 ](</ja-JP/concepts/model-providers>) [**モデル選択** モデルの選択と設定方法。 ](</ja-JP/concepts/models>) [**Ollama Web Search** Ollama を利用した Web 検索の完全なセットアップと動作詳細。 ](</ja-JP/tools/ollama-search>) [**設定** 完全な設定リファレンス。 ](</ja-JP/gateway/configuration>)
