---
title: Node
source_url: https://docs.openclaw.ai/id/cli/nodes
...
Catatan persetujuan:
  ...
  * `openclaw nodes approve <requestId>` mewarisi persyaratan cakupan tambahan dari permintaan tertunda: 
...
Untuk eksekusi shell pada node, gunakan alat `exec` dengan `host=node`, bukan `openclaw nodes run`. CLI `nodes` sekarang berfokus pada kapabilitas: RPC langsung melalui `nodes invoke`, ditambah pemasangan, kamera, layar, lokasi, Canvas, dan notifikasi. Perintah Canvas diimplementasikan oleh plugin Canvas eksperimental yang dibundel; inti mempertahankan hook kompatibilitas agar perintah tersebut tetap berada di bawah `openclaw nodes canvas`.
