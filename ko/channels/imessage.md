---
title: iMessage
source_url: https://docs.openclaw.ai/ko/channels/imessage
...
**Private API actions** 답장, tapback, 효과, 첨부 파일, 그룹 관리입니다. [**Pairing** iMessage DM은 기본적으로 페어링 모드를 사용합니다. ](</ko/channels/pairing>) **Remote Mac** Gateway가 Messages Mac에서 실행되지 않을 때 SSH 래퍼를 사용하세요. [**Configuration reference** 전체 iMessage 필드 참조입니다. ](</ko/gateway/config-channels#imessage>)
...
`remoteHost`가 설정되지 않은 경우 OpenClaw는 SSH 래퍼 스크립트를 파싱하여 자동 감지를 시도합니다. `remoteHost`는 `host` 또는 `user@host`여야 합니다(공백 또는 SSH 옵션 없음). OpenClaw는 SCP에 엄격한 호스트 키 검사를 사용하므로, 릴레이 호스트 키가 이미 `~/.ssh/known_hosts`에 있어야 합니다. 첨부 파일 경로는 허용된 루트(`attachmentRoots` / `remoteAttachmentRoots`)를 기준으로 검증됩니다.
...
Runtime fallback: `groupAllowFrom`이 설정되지 않은 경우 iMessage 그룹 발신자 검사는 `allowFrom`을 사용합니다. DM과 그룹 허용 기준이 달라야 하면 `groupAllowFrom`을 설정하세요. Runtime 참고: `channels.imessage`가 완전히 없으면 runtime은 `groupPolicy="allowlist"`로 대체하고 경고를 기록합니다(`channels.defaults.groupPolicy`가 설정되어 있더라도).
...
일부 다중 참가자 iMessage 스레드는 `is_group=false`로 도착할 수 있습니다. 해당 `chat_id`가 `channels.imessage.groups` 아래에 명시적으로 구성되어 있으면 OpenClaw는 이를 그룹 트래픽으로 처리합니다(그룹 게이팅 + 그룹 세션 격리).
...
SSH와 SCP가 모두 비대화식으로 동작하도록 SSH 키를 사용하세요. 먼저 호스트 키를 신뢰하도록 설정해(예: `ssh bot@mac-mini.tailnet-1234.ts.net`) `known_hosts`가 채워지게 하세요.
