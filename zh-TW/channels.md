---
title: 聊天頻道
source_url: https://docs.openclaw.ai/zh-TW/channels
scraped_at: 2026-05-25
---

OpenClaw 可以在你已經使用的任何聊天應用程式上與你對話。每個頻道都透過 Gateway 連線。 所有地方都支援文字；媒體與反應則依頻道而異。

## 傳送注意事項

  * 包含 Markdown 圖片語法的 Telegram 回覆，例如 `![alt](url)`， 會在可行時於最終對外傳送路徑上轉換成媒體回覆。
  * Slack 多人私訊會以群組聊天方式路由，因此群組政策、提及 行為與群組工作階段規則都適用於 MPIM 對話。
  * WhatsApp 設定採隨需安裝：入門流程可以在 Plugin 套件安裝前顯示設定流程， 而 Gateway 只有在頻道實際啟用時才會載入 WhatsApp 執行階段。
...
## 支援的頻道
  ...
  * [iMessage](</zh-TW/channels/imessage>) \- 在已登入的 Mac 上透過 `imsg` 橋接器進行原生 macOS 整合（或在 Gateway 於其他位置執行時使用 SSH 包裝器），包含用於回覆、Tapback、效果、附件與群組管理的私有 API 動作。當主機權限與 Messages 存取權符合需求時，建議新的 OpenClaw iMessage 設定使用此方式。
  * [IRC](</zh-TW/channels/irc>) \- 傳統 IRC 伺服器；頻道 + 具配對/允許清單控制的私訊。
  * [LINE](</zh-TW/channels/line>) \- LINE Messaging API 機器人（可下載 Plugin）。
  * [Matrix](</zh-TW/channels/matrix>) \- Matrix 通訊協定（可下載 Plugin）。
  ...
  * [Synology Chat](</zh-TW/channels/synology-chat>) \- 透過傳出+傳入 Webhook 使用 Synology NAS Chat（內建 Plugin）。
  * [Telegram](</zh-TW/channels/telegram>) \- 透過 grammY 使用 Bot API；支援群組。
  * [Tlon](</zh-TW/channels/tlon>) \- 以 Urbit 為基礎的即時通訊工具（內建 Plugin）。
  * [Twitch](</zh-TW/channels/twitch>) \- 透過 IRC 連線使用 Twitch 聊天（內建 Plugin）。
  * [Voice Call](</zh-TW/plugins/voice-call>) \- 透過 Plivo 或 Twilio 的電話語音（Plugin，需另行安裝）。
  ...
  * [Zalo](</zh-TW/channels/zalo>) \- Zalo Bot API；越南熱門即時通訊工具（內建 Plugin）。
...
## 注意事項
  ...
  * 最快的設定通常是 **Telegram** （簡單的機器人權杖）。WhatsApp 需要 QR 配對，並且 會在磁碟上儲存更多狀態。
