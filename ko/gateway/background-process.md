...
source_url: https://docs.openclaw.ai/ko/gateway/background-process
...
주요 매개변수:
  ...
  * `timeout`(초, 기본값 `tools.exec.timeoutSec`): 이 시간 초과 후 프로세스를 종료합니다. 해당 호출에서 exec 프로세스 시간 초과만 비활성화하려면 `timeout: 0`을 설정하세요.
  * `elevated`(bool): elevated 모드가 활성화/허용된 경우 샌드박스 외부에서 실행합니다(기본값은 `gateway`, exec 대상이 `node`이면 `node`).
...
동작:
  ...
  * 백그라운드로 전환되면(명시적 전환 또는 시간 초과) 도구가 `status: "running"` \+ `sessionId`와 짧은 tail을 반환합니다.
  ...
  * 지금 시작되는 장기 실행 작업의 경우 한 번만 시작하고, 활성화되어 있으며 명령이 출력을 내보내거나 실패할 때 자동 완료 깨우기에 의존하세요.
  * 자동 완료 깨우기를 사용할 수 없거나, 출력 없이 정상 종료된 명령에 대해 조용한 성공 확인이 필요하면 `process`를 사용해 완료를 확인하세요.
  * `sleep` 루프나 반복 폴링으로 리마인더 또는 지연된 후속 작업을 흉내 내지 마세요. 향후 작업에는 Cron을 사용하세요.


## 자식 프로세스 브리징

exec/process 도구 외부에서 장기 실행 자식 프로세스를 생성할 때(예: CLI 재생성 또는 Gateway 헬퍼) 자식 프로세스 브리지 헬퍼를 연결하여 종료 신호가 전달되고 종료/오류 시 리스너가 분리되도록 하세요. 이렇게 하면 systemd에서 고아 프로세스를 방지하고 플랫폼 전반의 종료 동작을 일관되게 유지할 수 있습니다.

환경 재정의:

  * `PI_BASH_YIELD_MS`: 기본 yield(ms)
  * `PI_BASH_MAX_OUTPUT_CHARS`: 메모리 내 출력 한도(문자)
  * `OPENCLAW_BASH_PENDING_MAX_OUTPUT_CHARS`: 스트림별 대기 중 stdout/stderr 한도(문자)
  * `PI_BASH_JOB_TTL_MS`: 완료된 세션의 TTL(ms, 1분~3시간으로 제한)
  * `OPENCLAW_PROCESS_INPUT_WAIT_IDLE_MS`: 쓰기 가능한 백그라운드 세션이 입력을 기다리는 것으로 표시되기 전의 유휴 출력 임계값(기본값 15000 ms)
...
구성(권장):
  ...
  * `tools.exec.notifyOnExit`(기본값 true): 백그라운드로 전환된 exec가 종료되면 시스템 이벤트를 큐에 넣고 Heartbeat를 요청합니다.
  * `tools.exec.notifyOnExitEmptySuccess`(기본값 false): true이면 출력이 없는 성공적인 백그라운드 실행에 대해서도 완료 이벤트를 큐에 넣습니다.
...
참고:
  ...
  * 프로세스가 다시 시작되면 세션은 손실됩니다(디스크 영속성 없음).
  ...
  * 대화형 CLI를 복구하기 전에 `log`를 사용해 현재 transcript, stdin 상태, 입력 대기 힌트를 함께 볼 수 있게 하세요.
  * 입력 또는 개입이 필요할 때는 `write` / `send-keys` / `submit` / `paste` / `kill`을 사용하세요.
  * `process list`에는 빠른 확인을 위한 파생 `name`(명령 동사 + 대상)이 포함됩니다.
  * `process list`, `poll`, `log`는 세션에 아직 쓰기 가능한 stdin이 있고 입력 대기 임계값보다 오래 유휴 상태인 경우에만 `waitingForInput`을 보고합니다.
  ...
  * 폴링은 주문형 상태 확인용이지 wait-loop 예약용이 아닙니다. 작업을 나중에 수행해야 한다면 대신 Cron을 사용하세요.


## 예시

장기 작업을 실행하고 나중에 폴링하기:
