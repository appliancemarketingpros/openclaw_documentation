...
[**Web Search の概要** すべての provider と自動検出ルール。 ](</ja-JP/tools/web>) [**Firecrawl** 検索に加えてコンテンツ抽出付きのスクレイピング。 ](</ja-JP/tools/firecrawl>) [**Exa Search** コンテンツ抽出付きのニューラル検索。 ](</ja-JP/tools/exa-search>) [**設定** Plugin エントリとツールルーティングの完全な config スキーマ。 ](</ja-JP/gateway/configuration>)
