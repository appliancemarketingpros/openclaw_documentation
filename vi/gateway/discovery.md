...
source_url: https://docs.openclaw.ai/vi/gateway/discovery
...
## Thuật ngữ
  ...
  * **Cầu nối TCP cũ (đã gỡ bỏ)** : truyền tải Node cũ hơn (xem [Giao thức cầu nối](</vi/gateway/bridge-protocol>)); không còn được quảng bá cho khám phá và không còn là một phần của các bản dựng hiện tại.
...
Hướng mục tiêu:
  ...
  * **Gateway** quảng bá endpoint WS của nó qua Bonjour khi plugin `bonjour` đi kèm được bật. Plugin tự động khởi động trên máy chủ macOS và là tùy chọn bật ở nơi khác.
...
Bật/tắt/ghi đè:
  ...
  * Khi plugin Bonjour được bật và `OPENCLAW_DISABLE_BONJOUR` chưa được đặt, Bonjour quảng bá trên các máy chủ thông thường và tự động tắt bên trong container được phát hiện. Khởi động Gateway macOS với cấu hình rỗng sẽ tự động bật plugin; các triển khai Linux, Windows và container cần bật rõ ràng. Chỉ dùng `0` trên host, macvlan, hoặc mạng khác có khả năng mDNS; dùng `1` để buộc tắt.
...
Hành vi client được khuyến nghị:
  ...
  3. Nếu không, nếu DNS/IP tailnet được cấu hình, hãy thử trực tiếp. Với Node di động trên route tailnet/public, trực tiếp nghĩa là endpoint an toàn, không phải `ws://` từ xa dạng plaintext.
