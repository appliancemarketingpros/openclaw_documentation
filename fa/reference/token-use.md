...
source_url: https://docs.openclaw.ai/fa/reference/token-use
...
در گفتگو از این‌ها استفاده کنید:
  ...
  * `/status` → **کارت وضعیت سرشار از ایموجی** با مدل نشست، مصرف زمینه، توکن‌های ورودی/خروجی آخرین پاسخ، و **هزینه‌ی تخمینی** (فقط کلید API).
...
سطح‌های دیگر:
  ...
  * **CLI:** `openclaw status --usage` و `openclaw channels list` پنجره‌های سهمیه‌ی نرمال‌شده‌ی ارائه‌دهنده را نشان می‌دهند (`X% left`، نه هزینه‌های به‌ازای هر پاسخ). ارائه‌دهندگان فعلی پنجره‌ی مصرف: Anthropic، GitHub Copilot، Gemini CLI، OpenAI Codex، MiniMax، Xiaomi و [z.ai](<http://z.ai>).
...
آن را در [پیکربندی Gateway](</fa/gateway/configuration>) تنظیم کنید و جزئیات رفتار را در [هرس نشست](</fa/concepts/session-pruning>) ببینید.
