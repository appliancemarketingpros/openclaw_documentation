---
title: 記憶概覽
source_url: https://docs.openclaw.ai/zh-TW/concepts/memory
...
[**內建（預設）** 以 SQLite 為基礎。可立即使用關鍵字搜尋、向量相似度和混合搜尋。沒有額外相依性。 ](</zh-TW/concepts/memory-builtin>) [**QMD** Local-first sidecar，具備重新排序、查詢擴展，以及索引工作區外部目錄的能力。 ](</zh-TW/concepts/memory-qmd>) [**Honcho** AI 原生跨工作階段記憶，具備使用者建模、語意搜尋和多代理感知。Plugin 安裝。 ](</zh-TW/concepts/memory-honcho>) [**LanceDB** 隨附的 LanceDB 支援記憶，具備 OpenAI 相容嵌入、自動回憶、自動擷取，以及本機 Ollama 嵌入支援。 ](</zh-TW/plugins/memory-lancedb>)

## 知識 wiki 層

[**Memory Wiki** 將持久記憶編譯成富含來源脈絡的 wiki vault，具備主張、儀表板、橋接模式，以及適合 Obsidian 的工作流程。 ](</zh-TW/plugins/memory-wiki>)
