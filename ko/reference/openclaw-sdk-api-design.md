...
source_url: https://docs.openclaw.ai/ko/reference/openclaw-sdk-api-design
scraped_at: 2026-05-25
---

이 페이지는 공개 [OpenClaw 앱 SDK](</ko/concepts/openclaw-sdk>)의 상세 API 참조 설계입니다. 의도적으로 [Plugin SDK](</ko/plugins/sdk-overview>)와 분리되어 있습니다.

공개 앱 SDK는 두 계층으로 빌드해야 합니다.

  1. 저수준 생성 Gateway 클라이언트.
  2. `OpenClaw`, `Agent`, `Session`, `Run`, `Task`, `Artifact`, `Approval`, `Environment` 객체를 제공하는 고수준 인체공학적 래퍼.
...
`id`는 재생 커서입니다. 소비자는 `events({ after: id })`로 다시 연결하고, 보존 정책이 허용하는 경우 놓친 이벤트를 받을 수 있어야 합니다.
...
런타임 네이티브 페이로드는 `raw`를 통해 사용할 수 있어야 하지만, 앱이 일반 UI를 위해 `raw`를 파싱해야 해서는 안 됩니다.
...
결과는 단순하고 안정적이어야 합니다. 타임스탬프 값은 Gateway 형태를 보존하므로, 현재 라이프사이클 기반 실행은 일반적으로 epoch 밀리초 숫자를 보고하지만 어댑터는 여전히 ISO 문자열을 노출할 수 있습니다. 풍부한 UI, 도구 추적, 런타임 네이티브 세부 정보는 이벤트와 아티팩트에 속합니다.

`accepted`는 비종단 대기 결과입니다. 이는 실행이 라이프사이클 종료/오류를 생성하기 전에 Gateway 대기 기한이 만료되었음을 의미합니다. 이를 `timed_out`으로 처리해서는 안 됩니다. `timed_out`은 실행이 자체 런타임 제한 시간을 초과한 경우에만 사용됩니다.
...
첫 구현은 호스팅 SaaS일 필요가 없습니다. 기존 노드 호스트, 임시 워크스페이스, CI 스타일 러너 또는 Testbox 스타일 환경을 대상으로 할 수 있습니다. 중요한 계약은 다음과 같습니다.
...
저장소에는 이미 Plugin용 `openclaw/plugin-sdk/*`가 있습니다. Plugin 작성자와 앱 개발자가 혼동하지 않도록 이 네임스페이스를 분리해 두세요.

## 생성된 클라이언트 전략

저수준 클라이언트는 버전이 지정된 Gateway 프로토콜 스키마에서 생성한 다음, 손으로 작성한 인체공학적 클래스로 감싸야 합니다.
...
계층화:
  ...
  4. 고수준 `OpenClaw`, `Agent`, `Session`, `Run`, `Task`, `Artifact` 래퍼.
  5. 쿡북 예제 및 통합 테스트.
