...
source_url: https://docs.openclaw.ai/fr/automation/tasks
...
bashCopy code
[code]
    # Lister toutes les tâches (les plus récentes d’abord)openclaw tasks list # Filtrer par environnement d’exécution ou statutopenclaw tasks list --runtime acpopenclaw tasks list --status running
[/code]
...
bashCopy code
[code]
    # Afficher les détails d’une tâche précise (par ID, ID d’exécution ou clé de session)openclaw tasks show <lookup>
[/code]
...
bashCopy code
[code]
    # Annuler une tâche en cours d’exécution (tue la session enfant)openclaw tasks cancel <lookup> # Modifier la politique de notification d’une tâcheopenclaw tasks notify <lookup> state_changes
[/code]
...
bashCopy code
[code]
    # Exécuter un audit de santéopenclaw tasks audit # Prévisualiser ou appliquer la maintenanceopenclaw tasks maintenanceopenclaw tasks maintenance --apply
[/code]
...
bashCopy code
[code]
    # Inspecter l’état TaskFlowopenclaw tasks flow listopenclaw tasks flow show <lookup>openclaw tasks flow cancel <lookup>
[/code]
...
Valeurs par défaut de notification pour cron et les médias
...
Garde-fou video_generate concurrent
...
maintenance des tâches bashCopy code
[code]
...
Le registre est chargé en mémoire au démarrage du Gateway et synchronise les écritures vers SQLite pour garantir la durabilité entre redémarrages. Le Gateway garde le journal d’écriture anticipée de SQLite borné en utilisant le seuil d’autocheckpoint par défaut de SQLite, ainsi que des checkpoints `TRUNCATE` périodiques et à l’arrêt.
