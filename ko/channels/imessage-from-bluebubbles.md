...
source_url: https://docs.openclaw.ai/ko/channels/imessage-from-bluebubbles
...
`channels status --probe`는 구성되고 활성화된 계정만 프로브합니다. 두 채널 모니터를 모두 실행하려는 의도가 아니라면 BlueBubbles와 iMessage를 둘 다 활성화한 상태로 Gateway를 다시 시작하지 마세요. 즉시 전환하지 않을 경우 Gateway를 다시 시작하기 전에 `channels.imessage.enabled`를 다시 `false`로 설정하세요. OpenClaw 트래픽을 활성화하기 전에 Mac을 검증하려면 시작하기 전에의 직접 `imsg` 명령을 사용하세요.
