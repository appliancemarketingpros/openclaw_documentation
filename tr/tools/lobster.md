---
title: Istakoz
source_url: https://docs.openclaw.ai/tr/tools/lobster
...
OpenClaw, Lobster iş akışlarını gömülü bir çalıştırıcı kullanarak **işlem içinde** çalıştırır. Harici CLI alt süreci başlatılmaz; iş akışı motoru gateway sürecinin içinde yürütülür ve doğrudan bir JSON zarfı döndürür. İşlem hattı onay için duraklarsa, araç daha sonra devam edebilmeniz için bir `resumeToken` döndürür.
...
**Yapılandırılmış LLM adımı** gerektiren iş akışları için isteğe bağlı `llm-task` Plugin aracını etkinleştirin ve Lobster'dan çağırın. Bu, bir modelle sınıflandırma/özetleme/taslak oluşturma yapmanıza izin verirken iş akışını belirlenimci tutar.
