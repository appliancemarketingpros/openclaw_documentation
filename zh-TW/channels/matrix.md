---
title: 矩陣
source_url: https://docs.openclaw.ai/zh-TW/channels/matrix
scraped_at: 2026-05-25
---

Matrix 是 OpenClaw 的可下載 channel Plugin。 它使用官方的 `matrix-js-sdk`，並支援 DM、房間、執行緒、媒體、回應、投票、位置與 E2EE。
...
如果相符的 `MATRIX_*` 環境變數已存在，且所選帳號沒有已儲存的驗證資料，精靈會提供環境變數捷徑。若要在儲存 allowlist 前解析房間名稱，請執行 `openclaw channels resolve --channel matrix "Project Room"`。啟用 E2EE 時，精靈會寫入設定，並執行與 `openclaw matrix encryption setup` 相同的 bootstrap。
...
若要保留即時回答預覽，但隱藏中途的工具/進度行，請使用物件 形式：
...
此設定影響補充上下文可見性，而不是傳入訊息本身是否可以觸發回覆。 觸發授權仍來自 `groupPolicy`、`groups`、`groupAllowFrom`，以及 DM 政策設定。
...
預設情況下，OpenClaw 會阻擋私有/內部 Matrix homeserver 以提供 SSRF 保護，除非你 明確按帳號選擇加入。

如果你的 homeserver 執行於 localhost、LAN/Tailscale IP，或內部主機名稱，請為該 Matrix 帳號啟用 `network.dangerouslyAllowPrivateNetwork`：
...
此選擇加入只允許受信任的私人／內部目標。公開的明文 homeserver，例如 `http://matrix.example.org:8008` 仍會被封鎖。請盡可能偏好使用 `https://`。
...
具名帳戶可以使用 `channels.matrix.accounts.<id>.proxy` 覆寫頂層預設值。 OpenClaw 會對執行階段 Matrix 流量和帳戶狀態探測使用相同的 Proxy 設定。
...
Matrix 房間 ID 區分大小寫。在設定明確的傳遞目標、Cron 工作、繫結或允許清單時，請使用 Matrix 中完全相同大小寫的房間 ID。 OpenClaw 會讓內部工作階段金鑰保持標準化以供儲存，因此這些小寫金鑰不能作為 Matrix 傳遞 ID 的可靠來源。
