...
source_url: https://docs.openclaw.ai/vi/pi
...
Runtime hành động tin nhắn theo kênh hiện nằm trong các thư mục extension do Plugin sở hữu thay vì dưới `src/agents/tools`, ví dụ:
...
Việc chèn ảnh là cục bộ theo prompt: OpenClaw tải các tham chiếu ảnh từ prompt hiện tại và truyền chúng qua `images` chỉ cho lượt đó. Nó không quét lại các lượt lịch sử cũ hơn để chèn lại payload ảnh.
...
System prompt | [AGENTS.md](<http://AGENTS.md>) \+ prompts | Động theo kênh/ngữ cảnh  
