...
レガシートランスポート: [Bridge プロトコル](</ja-JP/gateway/bridge-protocol>)（TCP JSONL。 現在のノードでは履歴用途のみ）。
...
注記:
  ...
  * exec 承認は `~/.openclaw/exec-approvals.json` によってローカルで適用されます （[Exec approvals](</ja-JP/tools/exec-approvals>) を参照）。
