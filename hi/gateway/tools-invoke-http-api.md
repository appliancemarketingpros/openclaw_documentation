...
source_url: https://docs.openclaw.ai/hi/gateway/tools-invoke-http-api
...
सामान्य HTTP auth paths:
  ...
  * trusted identity-bearing HTTP auth (`gateway.auth.mode="trusted-proxy"`): configured identity-aware proxy के माध्यम से route करें और उसे आवश्यक identity headers inject करने दें
  * private-ingress open auth (`gateway.auth.mode="none"`): कोई auth header आवश्यक नहीं
...
नोट्स:
  ...
  * जब `gateway.auth.mode="trusted-proxy"` हो, तो HTTP request को configured trusted proxy source से आना चाहिए; same-host loopback proxies के लिए explicit `gateway.auth.trustedProxy.allowLoopback = true` आवश्यक है।
  * Internal same-host callers जो proxy को bypass करते हैं, वे local direct fallback के रूप में `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD` का उपयोग कर सकते हैं। कोई भी `Forwarded`, `X-Forwarded-*`, या `X-Real-IP` header evidence request को इसके बजाय trusted-proxy path पर रखता है।
...
Auth matrix:
  ...
  * `gateway.auth.mode="token"` या `"password"` \+ `Authorization: Bearer ...`
    ...
    * full default operator scope set restore करता है: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`
