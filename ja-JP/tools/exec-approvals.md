...
安全な bin（stdin のみの高速パス）、インタープリターのバインディング詳細、承認プロンプトを Slack/Discord/Telegram に転送する方法（またはネイティブ承認クライアントとして実行する方法）については、[Exec 承認 - 高度な設定](</ja-JP/tools/exec-approvals-advanced>)を参照してください。
...
CLI: `openclaw approvals` は Gateway または Node の編集に対応しています。[Approvals CLI](</ja-JP/cli/approvals>)を参照してください。
...
[**Exec 承認 - 高度な設定** 安全な bin、インタープリターのバインディング、チャットへの承認転送。 ](</ja-JP/tools/exec-approvals-advanced>) [**Exec ツール** シェルコマンド実行ツール。 ](</ja-JP/tools/exec>) [**昇格モード** 承認もスキップする緊急時用パス。 ](</ja-JP/tools/elevated>) [**サンドボックス化** サンドボックスモードとワークスペースアクセス。 ](</ja-JP/gateway/sandboxing>) [**セキュリティ** セキュリティモデルと強化。 ](</ja-JP/gateway/security>) [**サンドボックス vs ツールポリシー vs 昇格** 各制御をいつ使うべきか。 ](</ja-JP/gateway/sandbox-vs-tool-policy-vs-elevated>) [**Skills** Skills を基盤とした自動許可の動作。 ](</ja-JP/tools/skills>)
