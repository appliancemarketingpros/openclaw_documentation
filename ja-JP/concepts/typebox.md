---
title: TypeBox
source_url: https://docs.openclaw.ai/ja-JP/concepts/typebox
scraped_at: 2026-05-25
---

TypeBox は TypeScript ファーストのスキーマライブラリです。これを使って **Gateway WebSocket プロトコル** （ハンドシェイク、リクエスト/レスポンス、サーバーイベント）を定義しています。これらのスキーマは、macOS アプリ向けの **ランタイム検証** 、**JSON Schema エクスポート** 、**Swift コード生成** を駆動します。信頼できる情報源は 1 つで、それ以外はすべて生成されます。

より上位のプロトコルコンテキストを知りたい場合は、 [Gatewayアーキテクチャ](</ja-JP/concepts/architecture>) から始めてください。
...
最初のフレームは **必ず** `connect` リクエストでなければなりません。その後、クライアントはメソッド（例: `health`、`send`、`chat.send`）を呼び出し、イベント（例: `presence`、`tick`、`agent`）を購読できます。
...
信頼できる、公開される **ディスカバリー** インベントリは `src/gateway/server-methods-list.ts`（`listGatewayMethods`、`GATEWAY_EVENTS`）にあります。
...
`src/gateway/server-methods.ts`（すでに `systemHandlers` をマージしています）で登録し、 `src/gateway/server-methods-list.ts` の `listGatewayMethods` 入力に `"system.echo"` を追加します。
