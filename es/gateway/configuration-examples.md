...
source_url: https://docs.openclaw.ai/es/gateway/configuration-examples
...
Mantén el emparejamiento de dispositivos como manual a menos que controles la ruta de red. Para un laboratorio dedicado o una subred tailnet, puedes optar por la aprobación automática de dispositivos de nodo en el primer uso con CIDR o IP exactas:
...
Esto permanece desactivado cuando no se configura. Solo se aplica al emparejamiento nuevo con `role: node` sin ámbitos solicitados. Los clientes de operador/navegador y las actualizaciones de rol, ámbito, metadatos o clave pública siguen requiriendo aprobación manual.
...
Para Discord/Slack/Google Chat/Microsoft Teams/Mattermost/IRC, la autorización del remitente se basa primero en el ID de forma predeterminada. Habilita la coincidencia directa mutable por nombre/correo electrónico/apodo con `dangerouslyAllowNameMatching: true` de cada canal solo si aceptas explícitamente ese riesgo.
