---
title: Podman
source_url: https://docs.openclaw.ai/ar/install/podman
...
افتراضيًا، هذه أدلة على المضيف وليست حالة حاوية مجهولة، لذلك تبقى `openclaw.json` و`auth-profiles.json` لكل وكيل وحالة القنوات/المزوّدين والجلسات ومساحة العمل بعد استبدال الحاوية. يزرع إعداد Podman أيضًا `gateway.controlUi.allowedOrigins` لـ `127.0.0.1` و`localhost` على منفذ Gateway المنشور لكي تعمل لوحة التحكم المحلية مع ربط الحاوية غير المعتمد على local loopback.
...
## أوامر مفيدة
  ...
  * **الصحة/الحالة عبر CLI على المضيف:** `openclaw gateway status --deep` (فحص RPC + فحص خدمة إضافي)
