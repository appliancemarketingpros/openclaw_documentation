...
[**Exec ツール** エージェントからのシェルコマンド実行。 ](</ja-JP/tools/exec>) [**Exec 承認** `exec` の承認および許可リストシステム。 ](</ja-JP/tools/exec-approvals>) [**サンドボックス化** Gateway レベルのサンドボックス設定。 ](</ja-JP/gateway/sandboxing>) [**サンドボックス vs ツールポリシー vs 昇格** ツール呼び出し中に 3 つのゲートがどのように合成されるか。 ](</ja-JP/gateway/sandbox-vs-tool-policy-vs-elevated>)
