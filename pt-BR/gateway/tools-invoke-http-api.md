...
source_url: https://docs.openclaw.ai/pt-BR/gateway/tools-invoke-http-api
...
Caminhos comuns de autenticação HTTP:
  ...
  * autenticação HTTP confiável com identidade (`gateway.auth.mode="trusted-proxy"`): roteie pelo proxy configurado com reconhecimento de identidade e deixe que ele injete os cabeçalhos de identidade necessários
...
Observações:
  ...
  * Quando `gateway.auth.mode="trusted-proxy"`, a solicitação HTTP deve vir de uma origem de proxy confiável configurada; proxies loopback no mesmo host exigem `gateway.auth.trustedProxy.allowLoopback = true` explícito.
