...
source_url: https://docs.openclaw.ai/tr/gateway/background-process
...
Davranış:
  ...
  * Otomatik tamamlama uyandırması kullanılamıyorsa ya da çıktı olmadan temiz şekilde sonlanan bir komut için sessiz başarı onayına ihtiyacınız varsa, tamamlanmayı onaylamak için `process` kullanın.
  * Hatırlatıcıları veya gecikmeli takipleri `sleep` döngüleri ya da tekrarlı sorgulamalarla taklit etmeyin; gelecekteki işler için Cron kullanın.
...
Notlar:
  ...
  * Etkileşimli bir CLI'yı kurtarmadan önce `log` kullanın; böylece mevcut transkript, stdin durumu ve girdi bekleme ipucu birlikte görünür.
  ...
  * `process list`, `poll` ve `log`, `waitingForInput` değerini yalnızca oturumun hâlâ yazılabilir stdin'i olduğunda ve girdi bekleme eşiğinden daha uzun süre boşta kaldığında bildirir.
  ...
  * Sorgulama, isteğe bağlı durum içindir; bekleme döngüsü zamanlaması için değildir. İş daha sonra yapılacaksa bunun yerine Cron kullanın.
